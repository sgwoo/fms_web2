...
<!--
	function AccidentDisp(car_no){
		var fm = document.form1;
		fm.s_st.value = '3';
		fm.s_kd.value = '3';
		fm.t_wd.value = car_no;
		fm.sort.value = '4';
		fm.asc.value = 'desc';		
		fm.cmd.value = "u";		
		fm.submit();
...
<input type='hidden' name='go_url' value='../accid_stat/accid_s3_frame.jsp'>
<table border="0" cellspacing="0" cellpadding="0" width=100%>
    ...
    <tr> 
        <td colspan="2"><iframe src="accid_s3_sc_in.jsp?auth_rw=<%=auth_rw%>&br_id=<%=br_id%>&user_id=<%=user_id%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&gubun5=<%=gubun5%>&gubun6=<%=gubun6%>&brch_id=<%=brch_id%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort=<%=sort%>&asc=<%=asc%>&s_st=<%=s_st%>&go_url=<%=go_url%>#<%=idx%>" name="i_no" width="100%" height="<%=height - 30%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 > 
        </iframe></td>
    </tr>
</table>
</form>
