...
<!--
	...
	function cng_input1(){
		...
		if(fm.gubun2.options[fm.gubun2.selectedIndex].value == '3'){ //월별
			...
			fm.st_dt.value = '<%=st_dt%>';
			fm.end_dt.value = '<%=end_dt%>';			
			fm.st_dt.focus();		
		}else{
			td_gubun2_1.style.display	= 'none';
			td_gubun2_2.style.display	= 'none';
    ...
    <tr>
        ...
        <td width=13%><img src=/acar/images/center/arrow_ggjh.gif align=absmiddle>&nbsp;  
            <select name="gubun2" onChange="javascript:cng_input1()">
              ...
              <option value="6" <%if(gubun2.equals("6")){%>selected<%}%>>전체</option>
            </select>
        </td>
        <td width=15%> 
        ...
        <td width=17%><img src=/acar/images/center/arrow_jr.gif align=absmiddle>&nbsp;  
            <select name="sort">
              <option value="1" <%if(sort.equals("1")){%> selected <%}%>>상호</option>
              <option value="2" <%if(sort.equals("2")){%> selected <%}%>>총건수</option>
              <option value="3" <%if(sort.equals("3")){%> selected <%}%>>피해</option>
              <option value="4" <%if(sort.equals("4")){%> selected <%}%>>가해</option>
              <option value="5" <%if(sort.equals("5")){%> selected <%}%>>쌍방</option>
              <option value="6" <%if(sort.equals("6")){%> selected <%}%>>단독</option>
              <option value="7" <%if(sort.equals("7")){%> selected <%}%>>자차</option>
              <option value="8" <%if(sort.equals("8")){%> selected <%}%>>인사</option>
              <option value="9" <%if(sort.equals("9")){%> selected <%}%>>대물</option>
              <option value="10" <%if(sort.equals("10")){%> selected <%}%>>병합</option>
              <option value="11" <%if(sort.equals("11")){%> selected <%}%>>차대차</option>
              <option value="12" <%if(sort.equals("12")){%> selected <%}%>>차대인</option>
              <option value="13" <%if(sort.equals("13")){%> selected <%}%>>차량단독</option>
              <option value="14" <%if(sort.equals("14")){%> selected <%}%>>차대열차</option>
            </select>
            <select name="asc">
              <option value="asc" <%if(asc.equals("asc")){%> selected <%}%>>오름</option>
              <option value="desc" <%if(asc.equals("desc")){%> selected <%}%>>내림</option>
            </select>
        </td>
