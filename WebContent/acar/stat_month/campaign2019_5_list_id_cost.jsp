...
<%@ page import="java.util.*, acar.util.*, acar.common.*, acar.stat_bus.*, acar.car_office.*, acar.user_mng.*"%>
...
<%
	...
	Hashtable ht2 = cmp_db.getCampaignCase_2019_05(save_dt, bus_id);
	...
	//2019년2,3,4월 누락분 5월캠페인 반영
	if(cs_dt.equals("20190501") || cs_dt.equals("2019-05-01")){
		vt2 = cmp_db.getStatBusCmpBaseBusCostListRmRetroact(bus_id, "c", bs_dt, be_dt, "20190201", "20190430", avg_cost_cnt);
	}	
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr> 
    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2><%=c_db.getNameById(bus_id,"USER")%></span></td>
  </tr>
  <tr>
    <td class=line2></td>
  </tr>
  <tr> 
    <td width="100%" class="line">
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr> 
          ...
          <td class="title" width="150">차명</td>		  		  		  
          ...
          <td class="title" width="90">영업효율대수<br>(=a/b)</td>		  		  
	  	  ...
	  	  <td class="title" width="150">영업사원</td>
	...
	<%		for(int i=0; i<vt.size(); i++){
				...
				CarOffEmpBean coe_bean = new CarOffEmpBean();
				CarOffBean co_bean = new CarOffBean();
				
				if(!String.valueOf(ht.get("BUS_EMP_ID")).equals("")){
					coe_bean = cod.getCarOffEmpBean(String.valueOf(ht.get("BUS_EMP_ID")));					
	...
	%>
        <tr> 
          ...
          <td align="center">
          	<%if(!String.valueOf(ht.get("BUS_EMP_ID")).equals("")){%>
          		<%=co_bean.getCar_off_nm() %> <%= coe_bean.getEmp_nm() %>
          	<%}%>
          </td>		
          <td align="center">
          	<%if(nm_db.getWorkAuthUser("전산팀",ck_acar_id) || nm_db.getWorkAuthUser("임원",ck_acar_id)){%>
          	<font color=grey><%= ht.get("ETC") %></font>
          	<%} %>
          </td>  		  	  
        </tr>
        <% 			
	...
	%>
        ...
        <%	}else{	 %>
        <tr> 
          <td colspan="12"><div align="center">해당 데이터가 없습니다.</div></td>
        </tr>
        <% 	} %>
            </table>
        </td>
...
<p>
  <font color="#999999" style="font-size : 9pt;">
    ...
    <br>
  ♣ 한 명의 영업사원(타사 에이전트 포함)을 통해서 한 캠페인동안 6대 이상 대여개시할 경우 6대째부터 영업효율금액 50% 인정
  <br>     
  </font>    
</p>
</body>
