...
<%
	...
	//권한
	auth_rw = rs_db.getAuthRw(user_id, "09", "05", "13");
	...
	Vector vt = cmp_db.getCampaignList_2014_05_sc1(save_dt, "", "", "");//20140501 1군 전체에서 서울과 지점으로 분리
	...
	Hashtable ht3 = cmp_db.getCampaignVar(v_year, v_tm, "1_1"); //1군 변수
...
<!--
	...
	//프린트화면보기	
	function cmp_print(){
		window.open("campaign2014_5_print.jsp?loan_st=1_1&save_dt=<%=save_dt%>&auth_rw=<%=auth_rw%>","print","left=30,top=50,width=1050,height=850,scrollbars=yes");	
	}

	//계산식보기
	function view_sik(){
		window.open("campaign2014_5_sik.jsp?loan_st=1_1&save_dt=<%=save_dt%>&bus_down_per=<%=down_per%>&mng_down_per=<%=down_per%>&cs_dt=<%=cs_dt%>&ce_dt=<%=ce_dt%>&bs_dt=<%=bs_dt%>&be_dt=<%=be_dt%>&bs_dt2=<%=bs_dt%>&be_dt2=<%=be_dt%>&enter_dt=<%=ns_dt1%>","sik","left=30,top=50,width=1050,height=600,scrollbars=yes");
	}
	...
	function updateVar(){
		...
		fm.action = "campaign2014_5_Var1_iu.jsp";
//		fm.target = 'i_no';
		fm.target = '_blank';	
		fm.submit();
	}
...
<body>
<form name="form1" method="post" action="/acar/stat_month/campaign2014_5_sc1.jsp">
...
<input type="hidden" name="from_page" 	value="/acar/stat_month/campaign2014_5_sc1.jsp">
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>경영정보 > 캠페인관리 > <span class=style5>영업캠페인(1군 서울.인천)</span></span></td>
        ...
        <td class="line">
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                ...
                <tr> 
                    ...
                    <td align=""><img src=../../images/result1.gif width=<%= graph %> height=10><%if(String.valueOf(ht2.get("DALSUNG")).equals("")){%>미적용 (편입기준 불충족)<%}%>
...
<table width="1000" border="0" cellspacing="0" cellpadding="0">
    <tr> 
		...
		~
		<input type="text" name="be_dt" size="11" value="<%= AddUtil.ChangeDate2(be_dt) %>" class=text onBlur='javascript:this.value=ChangeDate(this.value)'>
		</td>
    </tr>
    ...
    <tr id=tr_f1 style='display:none'> 
        ...
        , 1군(관리) 기준실적 
        <input name="nm_cnt1" type="text" class=num id="n_cnt1" onBlur='javascript:this.value=parseDecimal(this.value)' value="<%= n_cnt1 %>" size="3">
    ...
    <tr id=tr_f2 style='display:none'> 
        ...
        , 1군(관리) 기준실적 
        <input name="nm_cnt2" type="text" class=num id="n_cnt2" onBlur='javascript:this.value=parseDecimal(this.value)' value="<%= n_cnt2 %>" size="3">
        대 </td>
    </tr>
    <tr id=tr_f3 style='display:none'> 
        ...
        , 1군(관리) 기준실적 
        <input name="nm_cnt3" type="text" class=num id="n_cnt3" onBlur='javascript:this.value=parseDecimal(this.value)' value="<%= n_cnt3 %>" size="3">
       대</td>
    </tr>
    <tr id=tr_f4 style='display:none'> 
        ...
        , 1군(관리) 기준실적 
        <input name="nm_cnt4" type="text" class=num id="n_cnt4" onBlur='javascript:this.value=parseDecimal(this.value)' value="<%= n_cnt4 %>" size="3">
       대</td>
    </tr>    	
</table>
<table width="1000" border="0" cellspacing="0" cellpadding="0">
