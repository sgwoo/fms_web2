...
<%
	...
	Vector vt = cmp_db.getCampaignList_2014_05_sc2(save_dt, "", "", "");//20140501 1군 전체에서 서울과 지점으로 분리
	...
	Hashtable ht3 = cmp_db.getCampaignVar(v_year, v_tm, "1_2"); //1군 변수
...
<!--
	...
	//프린트화면보기	
	function cmp_print(){
		window.open("campaign2014_5_print.jsp?loan_st=1_2&save_dt=<%=save_dt%>&auth_rw=<%=auth_rw%>","print","left=30,top=50,width=1050,height=850,scrollbars=yes");	
	}

	//계산식보기
	function view_sik(){
		window.open("campaign2014_5_sik.jsp?loan_st=1_2&save_dt=<%=save_dt%>&bus_down_per=<%=down_per%>&mng_down_per=<%=down_per%>&cs_dt=<%=cs_dt%>&ce_dt=<%=ce_dt%>&bs_dt=<%=bs_dt%>&be_dt=<%=be_dt%>&bs_dt2=<%=bs_dt%>&be_dt2=<%=be_dt%>&enter_dt=<%=ns_dt1%>","sik","left=30,top=50,width=1050,height=600,scrollbars=yes");
	}
	...
	function updateVar(){
		...
		fm.action = "campaign2014_5_Var2_iu.jsp";
//		fm.target = 'i_no';
		fm.target = '_blank';	
		fm.submit();
	}
...
<body>
<form name="form1" method="post" action="/acar/stat_month/campaign2014_5_sc2.jsp">
...
<input type="hidden" name="from_page" 	value="/acar/stat_month/campaign2014_5_sc2.jsp">
