...
<%
	...
	Vector vt = cmp_db.getCampaignList_2014_05_sc4(save_dt, "", "", "");//20140201 영업대수,유효실적,적용실적 적용, 영업대리인/영업담당자 실적배분 변경
	...
	Hashtable ht3 = cmp_db.getCampaignVar(v_year, v_tm, "2_2");
...
<!--
	...
	//프린트화면보기
	function cmp_print(){
		window.open("campaign2014_5_print.jsp?loan_st=2_2&save_dt=<%=save_dt%>&auth_rw=<%=auth_rw%>","print","left=30,top=50,width=1050,height=850,scrollbars=yes");	
	}
	
	//계산식보기 --2군
	function view_sik(){
		window.open("campaign2014_5_sik.jsp?loan_st=2_2&save_dt=<%=save_dt%>&bus_down_per=<%=down_per%>&mng_down_per=<%=down_per%>&cs_dt=<%=cs_dt%>&ce_dt=<%=ce_dt%>&bs_dt=<%=bs_dt%>&be_dt=<%=be_dt%>&bs_dt2=<%=bs_dt%>&be_dt2=<%=be_dt%>&enter_dt=<%=ns_dt1%>","sik","left=30,top=50,width=1050,height=600,scrollbars=yes");
	}
	...
	function updateVar(){
		...
		fm.action = "campaign2014_5_Var4_iu.jsp";
		fm.target = 'i_no';
//		fm.target = '_blank';	
		fm.submit();
	}	
...
<body>
<form name="form1" method="post" action="/acar/stat_month/campaign2014_5_sc4.jsp">
...
<input type="hidden" name="from_page" 	value="/acar/stat_month/campaign2014_5_sc4.jsp">
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>경영정보 > 캠페인관리 > <span class=style5>영업캠페인(2군 지점)</span></span></td>
