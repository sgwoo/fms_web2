...
<%
	...
	//권한
	if(auth_rw.equals(""))	auth_rw = rs_db.getXmlAuthRw(user_id, "09", "08", "22");
	...
	Vector vt = cmp_db.getCampaignList_2019_05_sc(save_dt, "1");
...
<!--
	...
	//프린트화면보기	
	function cmp_print(){
		window.open("campaign2019_5_print.jsp?loan_st=1&save_dt=<%=save_dt%>&auth_rw=<%=auth_rw%>","print","left=30,top=50,width=1050,height=850,scrollbars=yes");	
	}

	//계산식보기
	function view_sik(){
		window.open("campaign2019_5_sik.jsp?loan_st=1&save_dt=<%=save_dt%>&bus_down_per=<%=down_per%>&mng_down_per=<%=down_per%>&cs_dt=<%=cs_dt%>&ce_dt=<%=ce_dt%>&bs_dt=<%=bs_dt%>&be_dt=<%=be_dt%>&bs_dt2=<%=bs_dt%>&be_dt2=<%=be_dt%>&enter_dt=<%=ns_dt1%>","sik","left=30,top=50,width=1050,height=600,scrollbars=yes");
	}
	...
	function updateVar(){
		...
		fm.action = "campaign2019_5_Var1_iu.jsp";
		fm.target = '_blank';	
		fm.submit();
	}
	...
	//변수수정 -캠페인포상건만 저장 ( 20200213 ) 
	function updateVarS(){
		var fm = document.form1;
		if(!confirm("캠페인 포상값을 저장하시겠습니까?"))	return;	
		fm.action = "campaign2019_magam_i.jsp";
		fm.target = 'i_no';	
	//	fm.target = '_blank';	
		fm.submit();
	}
...
<body>
<form name="form1" method="post" action="/acar/stat_month/campaign2019_5_sc1.jsp">
...
<input type="hidden" name="from_page" 	value="/acar/stat_month/campaign2019_5_sc1.jsp">
    ...
    <tr>
        <td>
          <p><font color="#999999">
            ...
            <br>
            ♣ 기본 평가기준 : 캠페인 마감일 기준 입사 48개월 이상인 직원 전체의 현재 캠페인 실적 평균
            <br>
            ♣ 할인율 : 캠페인 마감일 기준 입사 48개월 미만자는 1개월당 할인율를 입사개월수로 반영
            <br>
            ♣ 평가기준 : 기본평가기준에 할인율 반영
            <br>
...
<table width="1000" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
		...
		% 
		신입사원 1개월 할인치 <input type="text" name="new_mon_ga" size="3" value="<%= new_mon_ga %>" class=num >
		% 반영
		</td>
    </tr>       
    <tr> 
...
<table width="1000" border="0" cellspacing="0" cellpadding="0">
    <tr> 
        <td align="right"><input type="button" class="button" id="updateCmpVar" value='변수 수정' onclick="javascript:updateVar();">
