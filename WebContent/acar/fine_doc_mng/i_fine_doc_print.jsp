...
<%
	...
	//첫번째 과태료건에 따른 과태료공문 종류 분기(20190621)
	String vio_st =	"";
	if(FineList.size()>0){
...
<object id=factory style="display:none" classid="clsid:1663ed61-23eb-11d2-b92f-008048fdd814" codebase="/smsx.cab#Version=6,3,439,30">
            ...
            <table width='<%=img_width%>' height="<%=table1_h%>" border="0" cellpadding="0" cellspacing="0">
            	<tr> 
            		<td colspan="2" height="40" align="center" style="font-size : 18pt;"><b><font face="바탕">Pick amazoncar! We'll pick you up.</font></b></td>
            	</tr>
            	<tr> 
            		...
            		<td height="125" colspan="2" align='center'> 
            			<table width="100%" border="0" cellspacing="0" cellpadding="0">
            				<tr> 
            					...
            					<td height="25" width="87%" style="font-size : 10pt;"><font face="바탕"><%=FineDocBn.getDoc_id()%></font></td>
            				</tr>
            				<tr> 
            				...
            				<tr> 
            					...
            					<%if(vio_st.equals("2")){	//유료도로법	%>
            						유료도로법위반 통행료 부과 처분에 대한 <%if(exp.equals("Y")){%>이의 제기 (비송사건절차법에 따른 처리 요청)<%}else{%>의견 제출 (과태료 납부의무자 변경 요청)<%}%>
            					<%}else if(vio_st.equals("3")){	//주차장법 %>
            						주차장법위반 주차요금 부과 처분에 대한 <%if(exp.equals("Y")){%>이의 제기 (비송사건절차법에 따른 처리 요청)<%}else{%>의견 제출 (과태료 납부의무자 변경 요청)<%}%>
            					<%}else if(vio_st.equals("4")){ 	//장애인전용주차구역%>
            						장애인전용주차구역위반 과태료 부과 처분에 대한 <%if(exp.equals("Y")){%>이의 제기 (비송사건절차법에 따른 처리 요청)<%}else{%>의견 제출 (과태료 납부의무자 변경 요청)<%}%>
            					<%}else if(vio_st.equals("5")){  //폐기물관리법%>
            						폐기물관리법위반 과태료 부과 처분에 대한 <%if(exp.equals("Y")){%>이의 제기 (비송사건절차법에 따른 처리 요청)<%}else{%>의견 제출 (과태료 납부의무자 변경 요청)<%}%>
            					<%}else{	//도로교통법%>
            						도로교통법위반 과태료 부과 처분에 대한 <%if(exp.equals("Y")){%>이의 제기 (비송사건절차법에 따른 처리 요청)<%}else{%>의견 제출 (과태료 납부의무자 변경 요청)<%}%>
            					<%} %>
            					</font></td>
            				</tr>
            			</table>
            	...
            	<tr> 
            		...
            		<td width="*" height="30" style="font-size : 10pt;"><font face="바탕">
            			1. 귀 <%=FineDocBn.getGov_st()%>의 무궁한 발전을 기원합니다.
            		</font></td>
            	</tr>
            	<tr> 
            		<td align='center' height="30" style="font-size : 10pt;"><font face="바탕">&nbsp;</font></td>
            		<td height="30" style="font-size : 10pt;"><font face="바탕">
            		<%if(vio_st.equals("2")){	//유료도로법	%>
            			2. 당사에 부과(부과예정)한 유료도로법위반 통행료 처분에 대하여 아래와 같이
  					<%}else if(vio_st.equals("3")){	//주차장법 %>
  						2. 당사에 부과(부과예정)한 주차장법위반 주차요금 처분에 대하여 아래와 같이
  					<%}else if(vio_st.equals("4")){ 	//장애인전용주차구역%>
  						2. 당사에 부과(부과예정)한 장애인전용주차구역위반 과태료 처분에 대하여 아래와 같이
  					<%}else if(vio_st.equals("5")){  //폐기물관리법%>
  						2. 당사에 부과(부과예정)한 폐기물관리법위반 범칙금 또는 과태료 처분에 대하여 아래와 같이
  					<%}else{	//도로교통법%>
            			2. 당사에 부과(부과예정)한 도로교통법위반 범칙금 또는 과태료 처분에 대하여 아래와 같이 
  					<%} %>	
            		</font></td>
            	</tr>
            	<tr>
            		...
            		<%if(vio_st.equals("2")){	//유료도로법	%>
            			의견을 진술하오니 (질서 위반 행위 규제법 제20조)
  					<%}else if(vio_st.equals("3")){	//주차장법 %>
  						의견을 진술하오니 (주차장법 제9조 1항, 질서 위반 행위 규제법 제20조)
  					<%}else if(vio_st.equals("4")){ 	//장애인전용주차구역%>
  						의견을 진술하오니 (질서 위반 행위 규제법 제20조)
  					<%}else if(vio_st.equals("5")){  //폐기물관리법%>
  						의견을 진술하오니 (관련근거 질서 위반 행위 규제법 제20조)
  					<%}else{	//도로교통법%>
            			의견을 진술하오니 (관련근거 도로교통법 제160조 4항)
  					<%} %>
            		</font></td>
            	</tr>
            	<tr> 
            		...
            		<td height="30" colspan="2" style="font-size : 10pt;"><font face="바탕">
            		<%if(vio_st.equals("2")){	//유료도로법	%>
            			<%if(exp.equals("Y")){%>3. 질서위반행위규제법 제21조 1항 및 제28조에 근거하여 관할법원에 이 사실을 통보하여 정당한 처분을<%}else{%>3. 납부의무자를 당사에서 임차인으로 변경 처분하여 주시거나, 질서 위반 행위 규제법 제21조 1항 및 제28조<%}%>	
  					<%}else if(vio_st.equals("3")){	//주차장법 %>
  						<%if(exp.equals("Y")){%>3. 질서위반행위규제법 제21조 1항 및 제28조에 근거하여 관할법원에 이 사실을 통보하여 정당한 처분을<%}else{%>3. 납부의무자를 당사에서 임차인으로 변경 처분하여 주시거나, 질서 위반 행위 규제법 제21조 1항 및 제28조<%}%>
  					<%}else if(vio_st.equals("4")){ 	//장애인전용주차구역%>
  						<%if(exp.equals("Y")){%>3. 질서위반행위규제법 제21조 1항 및 제28조에 근거하여 관할법원에 이 사실을 통보하여 정당한 처분을<%}else{%>3. 납부의무자를 당사에서 임차인으로 변경 처분하여 주시거나, 질서 위반 행위 규제법 제21조 1항 및 제28조<%}%>
  					<%}else if(vio_st.equals("5")){  //폐기물관리법%>
  						<%if(exp.equals("Y")){%>3. 질서위반행위규제법 제21조 1항 및 제28조에 근거하여 관할법원에 이 사실을 통보하여 정당한 처분을<%}else{%>3. 납부의무자를 당사에서 임차인으로 변경 처분하여 주시거나, 질서 위반 행위 규제법 제21조 1항 및 제28조<%}%>
  					<%}else{	//도로교통법%>
	            		<%if(exp.equals("Y")){%>3. 질서위반행위규제법 제21조 1항 및 제28조에 근거하여 관할법원에 이 사실을 통보하여 정당한 처분을<%}else{%>3. 납부의무자를 당사에서 임차인으로 변경 처분하여 주시거나, 질서위반행위규제법 제16조 제2항 1항 및 제28조<%}%>
  					<%} %> 
            		</font></td>
            	</tr>
            	<tr> 
            		...
            		&nbsp;&nbsp;&nbsp;&nbsp;받도록 처리하여 주십시오.
            		<%}else{%> 
            		&nbsp;&nbsp;&nbsp;&nbsp;에 근거하여 관할법원에 이 사실을 통보하여 정당한 처분을 받도록 처리하여 주십시오.
            		<%}%>
            		</font></td>
            	</tr>    
            	<tr> 
            		<td colspan=2 align='center' height="50" style="font-size : 9pt;"><font size="2" face="바탕">== 아&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;래 ==</font></td>
            	</tr>
            	<tr bgcolor="#000000"> 
            		<td colspan="2" align='center' height="10" class=line> 
            			<table width="100%" border="0" cellspacing="1" cellpadding="0">
            				<tr bgcolor="#A6FFFF" align="center"> 
            					<td style="font-size : 10pt;" rowspan="2" width="4%" class=title><font face="바탕">연번</font></td>
            					<td style="font-size : 10pt;" rowspan="2" width="14%" class=title><font face="바탕">위반일시</font></td>
            					<td style="font-size : 8pt;" rowspan="2" width="11%" class=title><font face="바탕">차량번호</font></td>
            					<td style="font-size : 10pt;" colspan="4" height="25" class=title><font face="바탕">임차인</font></td>
            				</tr>
            				<tr bgcolor="#FFFFFF"> 
            					<td style="font-size : 8pt;" width="15%" height="25" align="center" bgcolor="#A6FFFF" class=title><font face="바탕">상호/성명</font></td>
            					<td style="font-size : 8pt;" width="18%" align="center" bgcolor="#A6FFFF" class=title><font face="바탕">생년월일<br>(운전면허번호)</font></td>
            					<td style="font-size : 8pt;" width="15%" align="center" bgcolor="#A6FFFF" class=title><font face="바탕">사업자등록번호</font></td>
            					<td style="font-size : 8pt;" width="18%" align="center" bgcolor="#A6FFFF" class=title><font face="바탕">임대기간</font></td>
            				</tr>
            			</table>
            		</td>
            	</tr>
            		...
            		<td width="100%" height="10" align='center' class=line>
            			<table width="100%" border="0" cellspacing="1" cellpadding="0">
            				...
            				<tr bgcolor="#FFFFFF" align="center">
            					...
            					<td width="18%" style="font-size : 8pt;"><font face="바탕"><input type="text" size='16' value="<%=AddUtil.ChangeDate2(FineDocListBn.getRent_start_dt())+" 00:00"%>~" class="whitetext">
