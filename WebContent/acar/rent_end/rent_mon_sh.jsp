...
<form name='form1' method='post' action='rent_mon_sc.jsp' target='c_foot'>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
    	...
    	<td colspan=5>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>예약시스템 > 영업지원 > <span class=style5>월렌트관리</span></span></td>
    ...
    <tr> 
        <td width=21% align='left'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_jhgb.gif align=absmiddle>&nbsp;
            <select name='gubun1'>
              <option value=''  <%if(gubun1.equals("")) {%>selected<%}%>>전체(취소제외)</option>
              <option value='11'  <%if(gubun1.equals("11")) {%>selected<%}%>>전체(취소포함)</option>
              <option value='1' <%if(gubun1.equals("1")){%>selected<%}%>>예약</option>
              <option value='2' <%if(gubun1.equals("2")){%>selected<%}%>>배차</option>
              <option value='3' <%if(gubun1.equals("3")){%>selected<%}%>>반차</option>
              <option value='4' <%if(gubun1.equals("4")){%>selected<%}%>>정산</option>
              <option value='5' <%if(gubun1.equals("5")){%>selected<%}%>>취소</option>
              <option value='6' <%if(gubun1.equals("6")){%>selected<%}%>>미배정(본사)</option>
              <option value='7' <%if(gubun1.equals("7")){%>selected<%}%>>미배정(부산)</option>
              <option value='8' <%if(gubun1.equals("8")){%>selected<%}%>>미배정(대전)</option>
              <option value='9' <%if(gubun1.equals("9")){%>selected<%}%>>미배정(대구)</option>
              <option value='10' <%if(gubun1.equals("10")){%>selected<%}%>>미배정(광주)</option>
            </select>
        </td>
        ...
        <td id='td_input' align='left' style="display:''" width=40%><img src=/acar/images/center/arrow_gggs.gif align=absmiddle>&nbsp;
            <select name='gubun3'>
                    <option value="1" <%if(gubun3.equals("1"))%>selected<%%>>계약일자</option>
                    <option value="2" <%if(gubun3.equals("2"))%>selected<%%>>배차일자</option>
                    <option value="3" <%if(gubun3.equals("3"))%>selected<%%>>반차일자</option>
                    <option value="4" <%if(gubun3.equals("4"))%>selected<%%>>정산일자</option>                    
            </select>			
	    &nbsp;	
	    <select name='gubun4'>
    ...
    <tr> 
        ...
        <td id='td_input' align='left' style="display:''"><img src=/acar/images/center/arrow_ys.gif align=absmiddle>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <select name="s_year">
              <option value="" <%if(AddUtil.parseInt(s_year) == 0)%>selected<%%>>전체</option>
              <%for(int i=2000; i<=AddUtil.getDate2(1); i++){%>
              <option value="<%=i%>" <%if(AddUtil.parseInt(s_year) == i)%>selected<%%>><%=i%>년</option>
              <%}%>
            </select>
        </td>
        <td>&nbsp;</td>
    ...
    <tr> 
        <td align='left' colspan="3">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_gshm.gif align=absmiddle>&nbsp;
            <select name='s_kd'>              
              ...
              <option value='4' <%if(s_kd.equals("4")){%> selected <%}%>>사유발생차량번호</option>
              <option value='5' <%if(s_kd.equals("5")){%> selected <%}%>>계약번호</option>
              <option value='6' <%if(s_kd.equals("6")){%> selected <%}%>>최초영업자</option>
              <option value='7' <%if(s_kd.equals("7")){%> selected <%}%>>관리담당자</option>              
            </select>
            <input type='text' name='t_wd' size='28' class='text' value='<%=t_wd%>' onKeyDown='javascript:enter()' style='IME-MODE: active'>
        </td>
        <td id='td_input' align='left' style="display:''"><img src=/acar/images/center/arrow_jrjg.gif align=absmiddle>&nbsp; 
            <select name='sort_gubun' onChange='javascript:search()'>
              ...
              <option value='a.rent_dt' <%if(sort_gubun.equals("a.rent_dt")){%> selected <%}%>>계약일자</option>
              <option value='nvl(a.deli_dt,a.deli_plan_dt)' <%if(sort_gubun.equals("nvl(a.deli_dt,a.deli_plan_dt)")){%> selected <%}%>>배차일자</option>
              <option value='nvl(a.ret_dt,a.ret_plan_dt)'  <%if(sort_gubun.equals("nvl(a.ret_dt,a.ret_plan_dt)")){%> selected <%}%>>반차일자</option>
              <option value='j.sett_dt' <%if(sort_gubun.equals("j.sett_dt")){%> selected <%}%>>정산일자</option>
              <option value="decode(a.cust_st, '4','(주)아마존카', d.firm_nm)"  <%if(sort_gubun.equals("decode(a.cust_st, '4','(주)아마존카', d.firm_nm)")){%> selected <%}%>>상호</option>
            </select>
