...
<%
	...
	Vector resultList = umd.getSmsResult_V5_req(gubun, dest_gubun, send_dt, st_dt, end_dt, s_bus, sort, sort_gubun, c_db.getNameById(s_bus,"USER"));
...
<!--
function open_list(cmid, senddate){
	window.open("about:blank", "result_list", "left=100, top=110, width=1040, height=650, scrollbars=no");	
	fm = document.form1;
	fm.cmid.value = cmid;
	fm.senddate.value = senddate;	
	fm.target = "result_list";
	fm.action = "./v5_req_list.jsp";
	fm.submit();
}
//-->
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <td class="line"><table width="100%"  border="0" cellspacing="1" cellpadding="0">
          <tr>
            ...
            <td width="53%" class="title">내용</td>
            <td width="19%" class="title">예약일자</td>
            </tr>
...
%>
	  <tr>
		...
		<td width="6%" align="right"><a href="javascript:open_list('<%= ht.get("CMID") %>');" onMouseOver="window.status=''; return true"><%= ht.get("CNT") %></a>&nbsp;</td>
		...
		<td width="53%" align="left">&nbsp;<%= ht.get("MSG_BODY") %></td>
		<td width="19%" align="center"><%= AddUtil.ChangeDate3((String)ht.get("REQUEST_TIME")) %></td>
		</tr>
<% 		} %>
	  <tr>
		...
		<td width="6%" class="title" style='text-align:right'><%= AddUtil.parseDecimal(sum2) %>&nbsp;</td>
		<td width="53%" class="title">&nbsp;</td>
		<td width="19%" class="title"></td>
		</tr>
<%	}else{ %>
	  <tr>
	    <td colspan="6" align="center">해당 데이터가 없습니다. </td>
	    </tr>
<% 	} %>		
    </table></td>
  </tr>
