...
<%
	...
	//발송완료리스트
	Vector resultList = umd.getSmsCreListV5(gubun, dest_gubun, send_dt, st_dt, end_dt, s_bus, sort, sort_gubun, s_bus, s_kd, t_wd);
	
	//대기중 리스트
	Vector resultList2 = umd.getSmsCreListV5Data(gubun, dest_gubun, send_dt, st_dt, end_dt, s_bus, sort, sort_gubun, s_bus, s_kd, t_wd);
...
<%	for(int i=0; i< resultList2.size(); i++){
	  ...
	  <tr>
		...
		<td width="20%" align="center"><a href="javascript:view_sms('data', '<%= ht.get("CMID") %>');" onMouseOver="window.status=''; return true"><%= ht.get("DEST_NAME2") %></a><br><%= ht.get("DEST_PHONE") %></td>
...
<%	for(int i=0; i< resultList.size(); i++){
	  ...
	  <tr>
		...
		<td width="10%" align="center"><%= ht.get("SEND_NAME2") %></td>
		<td width="20%" align="center"><a href="javascript:view_sms('log', '<%= ht.get("CMID") %>');" onMouseOver="window.status=''; return true"><%= ht.get("DEST_NAME2") %></a><br><%= ht.get("DEST_PHONE") %></td>
		<td width="50%">&nbsp;<%= ht.get("MSG_BODY") %></td>
