	...
	<tr>
		<td>
			<table border="0" cellspacing="0" cellpadding="0" width=100%>
				<tr>
					<td>
						<iframe src="sms_result_sc_in2.jsp?auth_rw=<%=auth_rw%>&&gubun=<%=gubun%>&dest_gubun=<%=dest_gubun%>&rslt_dt=<%= rslt_dt %>&st_dt=<%= st_dt %>&end_dt=<%= end_dt %>&dest_nm=<%= dest_nm %>&sort=<%= sort %>&sort_gubun=<%= sort_gubun %>" name="inner" width="100%" height="550" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
						</iframe>
					</td>
				</tr>
			</table>
