...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
  	<td><iframe src="v5_sms_req_sc_in.jsp?auth_rw=<%=auth_rw%>&gubun=<%=gubun%>&dest_gubun=<%=dest_gubun%>&send_dt=<%= send_dt %>&st_dt=<%= st_dt %>&end_dt=<%= end_dt %>&s_bus=<%= s_bus %>&sort=<%= sort %>&sort_gubun=<%= sort_gubun %>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
						</iframe></td>
  </tr>
</table>
</body>
