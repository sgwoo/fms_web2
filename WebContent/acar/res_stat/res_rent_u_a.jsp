...
<%@ page import="java.util.*, acar.util.*, acar.res_search.*, acar.fee.*"%>
...
<%
	...
	if(deli_dt.equals("") && !deli_mng_id.equals("")){
				...
				for(int i=0; i<3; i++){
					...
					if(old_pay_dt.equals("")){
						...
						if(sr_bean.getEst_dt().equals("")){
							...
							//예약인상태이고 배차예정일과 같을때는 수정					
							if(rc_bean.getUse_st().equals("1") && request.getParameter("deli_plan_dt").equals(request.getParameter("est_dt"+i))){
								sr_bean.setEst_dt	(request.getParameter("est_dt"+i)==null?"":request.getParameter("est_dt"+i));
							}
						}
						...
						if(sr_bean.getRent_s_cd().equals("")){
							...
							sr_bean.setTm		(request.getParameter("tm"+i)==null?"":request.getParameter("tm"+i));							
							sr_bean.setDly_days	("");
