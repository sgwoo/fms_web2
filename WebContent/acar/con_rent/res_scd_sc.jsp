...
<input type='hidden' name='from_page' value='res_scd_frame.jsp'>
        		...
        		<tr>
        		    <td width=100%>
            			<table border="0" cellspacing="0" cellpadding="0" width=100%>
            			    <tr>
                				<td align='center'>
                				  <iframe src="res_scd_sc_in.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&auth=<%=auth%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&s_brch=<%=s_brch%>&s_bus=<%=s_bus%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>" name="i_no" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
                				  </iframe>
                				</td>
            			    </tr>
            			</table>
    ...
    <tr>      		
        <td class='line'> 
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                <tr> 
                    <td rowspan="2" width="12%" class='title' align="center">구분</td>
	...
	if(cls_size > 0){
		for (int i = 0 ; i < cls_size ; i++){
			IncomingSBean cls = (IncomingSBean)clss.elementAt(i);%>		
                <tr> 
                    <td width="12%" align="center" class='title'><%=cls.getGubun()%></td>
                    <td width="8%"  align="right"><%if(cls.getGubun().equals("비율")){%><%=cls.getTot_su1()%>%<% }else{%><%=cls.getTot_su1()%>건<%}%>&nbsp;</td>
                    <td width="14%" align="right"><%if(cls.getGubun().equals("비율")){%><%=cls.getTot_amt1()%>%<%}else{%><%=Util.parseDecimal(cls.getTot_amt1())%>원<%}%>&nbsp;</td>
                    <td width="8%"  align="right"><%if(cls.getGubun().equals("비율")){%><%=cls.getTot_su2()%>%<% }else{%><%=cls.getTot_su2()%>건<%}%>&nbsp;</td>
                    <td width="14%" align="right"><%if(cls.getGubun().equals("비율")){%><%=cls.getTot_amt2()%>%<%}else{%><%=Util.parseDecimal(cls.getTot_amt2())%>원<%}%>&nbsp;</td>
                    <td width="8%"  align="right"><%if(cls.getGubun().equals("비율")){%><%=cls.getTot_su3()%>%<% }else{%><%=cls.getTot_su3()%>건<%}%>&nbsp;</td>
                    <td width="14%" align="right"><%if(cls.getGubun().equals("비율")){%><%=cls.getTot_amt3()%>%<%}else{%><%=Util.parseDecimal(cls.getTot_amt3())%>원<%}%>&nbsp;</td>
