...
<%
	...
	String caroff_emp_yn_origin = request.getParameter("caroff_emp_yn_origin")==null?"":request.getParameter("caroff_emp_yn_origin");
	...
	if(cmd.equals("cls_per_cng")){
		...
		bean.setCtr_cls_per		(request.getParameter("cng_cls_per")==null?0:AddUtil.parseFloat(request.getParameter("cng_cls_per")));
	...
	} else if (cmd.equals("result_select_sms_wap") || cmd.equals("result_select_sms_wap_rm")) {
		...
		for (int i = 0; i < ch_mms_id.length; i++) {
			...
			if (est_m_tel.equals("")) {
				...
				String manager_phone 	= "";					// 담당자 전화
				
				if (!caroff_emp_yn_origin.equals("3")) {
					manager_name 	= u_nm;					// 담당자 이름
					manager_phone 	= u_mt;					// 담당자 전화
				}
				
				if (cmd.equals("result_select_sms_wap")) {
					...
					//acar0065->acar0074
					List<String> fieldList = Arrays.asList(a_car_name, a_car_amount, a_gubun1, a_month, a_distance, a_rent_fee, a_esti_link, manager_name, manager_phone);
					at_db.sendMessageReserve("acar0074", fieldList, est_m_tel, u_mt, null, "", etc2);
				}
			}
		}
		...
		if(!set_code.equals("")){
			...
			for(int i = 0 ; i < size ; i++){
				...
				e_bean2.setDir_pur_commi_yn	(request.getParameter("dir_pur_commi_yn")==null?"N":request.getParameter("dir_pur_commi_yn"));
				...
				if(from_page.equals("esti_mng_atype_u.jsp")){
					...
					e_bean2.setCompare_yn	(request.getParameter("compare_yn")==null?"N":request.getParameter("compare_yn"));
					e_bean2.setPp_ment_yn	(request.getParameter("pp_ment_yn")==null?"0":request.getParameter("pp_ment_yn"));
				}
				e_bean2.setBus_st			(request.getParameter("bus_st")==null?"":request.getParameter("bus_st"));
