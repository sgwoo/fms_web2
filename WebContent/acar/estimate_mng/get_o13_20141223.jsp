...
<%
	...
	if(nm_db.getWorkAuthUser("전산팀",ck_acar_id) && rent_dt.equals("20190628"))	rent_dt = "20190701";	
...
<!--
	...
	function getShCarAmt(){	
		...
		//감면 미적용 개별소비세(인하한도 초과금액) 20210101~20210630**********************
	  	...
	  	if(<%=rent_dt%> >= 20210101){
	  	<%if(!ej_bean.getJg_w().equals("1")){ //수입차제외%>
	  	<%		if(cm_bean.getDuty_free_opt().equals("1")){//면세가표기차량 제외%>
	  	<%		}else{%>
...
// 					}	    					
		...
		fm.cd[i].value = "ch_315";				fm.nm[i].value = "차량가격(소비자차가)";							fm.value[i].value = ch_315;				i++;		
		fm.cd[i].value = "ch_326";				fm.nm[i].value = "개소세전차량가(vat포함)";						fm.value[i].value = ch_326;				i++;		
		fm.cd[i].value = "bk_176";				fm.nm[i].value = "개소세 감면 한도(교육세포함,부가세전)"; 			fm.value[i].value = bk_176;				i++;							
	  ...
	  <%if(ej_bean.getJg_g_7().equals("3")){%> //20160921이전
	  	//k_cb_5 = k_cb_5/1.1;
	  <%}%>
	  k_cb_6 = bk_129;
	  ...
	  <%}%>
		...
		fm.cd[i].value = "o_1_2";			fm.nm[i].value = "선택사양포함 초과차량가격";					fm.value[i].value = o_1_2;				i++;
		fm.cd[i].value = "o_7";				fm.nm[i].value = "선택사양포함 초과차량가격";					fm.value[i].value = o_7;				i++;
		fm.cd[i].value = "Jg_c_12";			fm.nm[i].value = "선택사양포함 초과차량가격";					fm.value[i].value = <%=em_bean.getJg_c_12()%>;				i++;
...
<%-- 				if('<%=cm_bean.getJg_code()%>' == '3871' || '<%=cm_bean.getJg_code()%>' == '4854' || '<%=cm_bean.getJg_code()%>' == '5866' || '<%=cm_bean.getJg_code()%>' == '3313111' || '<%=cm_bean.getJg_code()%>' == '3313112' || '<%=cm_bean.getJg_code()%>' == '3313113' || '<%=cm_bean.getJg_code()%>' == '3313114' || '<%=cm_bean.getJg_code()%>' == '4314111' || '<%=cm_bean.getJg_code()%>' == '6316111'){ --%>
		...
		if(<%=rent_dt%> >= 20170529){
			...
			fm.cd[i].value = "a_a_b[0]";		fm.nm[i].value = "조정최대잔가율";						fm.value[i].value = a_a_b[0];				i++;
			fm.cd[i].value = "Jg_14";			fm.nm[i].value = "조정최대잔가율";						fm.value[i].value = <%=ej_bean.getJg_14()%>;				i++;
			fm.cd[i].value = "a_o_m[0]";		fm.nm[i].value = "조정최대잔가율";						fm.value[i].value = a_o_m[0];				i++;
			fm.cd[i].value = "Jg_g_13";			fm.nm[i].value = "조정최대잔가율";						fm.value[i].value = <%=ej_bean.getJg_g_13()%>;				i++;
...
<%-- 			ro_13_amt = Math.round(<%=o_1%> * (o_13/100)); --%>
		...
		fm.cd[i].value = "b_o_13";							fm.nm[i].value = "표준최대잔가";						fm.value[i].value = b_o_13;				i++;
		fm.cd[i].value = "o_13";							fm.nm[i].value = "조정최대잔가율";						fm.value[i].value = o_13;				i++;		
		...
		if(ch_327 >0 || bk_177>0 || bk_216>0){
			parent.document.form1.o_1.value = parseDecimal(toInt(parseDigit(parent.document.form1.car_amt.value)) + toInt(parseDigit(parent.document.form1.opt_amt.value)) + toInt(parseDigit(parent.document.form1.col_amt.value)) - toInt(parseDigit(parent.document.form1.dc_amt.value)) - toInt(parseDigit(parent.document.form1.tax_dc_amt.value)));
				...
				if (toInt(<%=cm_bean.getCar_comp_id()%>) > 5) {	// 수입차
					<%-- if(<%=Integer.parseInt(ej_bean.getJg_g_7())%> > 2){ // 친환경차 구분상 전기/수소차
				...
				if(<%=Integer.parseInt(ej_bean.getJg_g_7())%> == 4){	// 수소차면 -5%
					parent.document.form1.rg_8[<%=idx%>].value = Number(parent.document.form1.rg_8[<%=idx%>].value) - 5;
					parent.document.form1.sel_rg_8_1.value = Number(parent.document.form1.sel_rg_8_1.value) - 5;	
				}
