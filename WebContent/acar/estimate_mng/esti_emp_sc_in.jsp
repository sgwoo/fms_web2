...
<%
	...
	Vector vt = e_db.getEstimateShEmpCarList(gubun1, gubun2, gubun3, gubun4, s_dt, e_dt, s_kd, t_wd, esti_m, esti_m_dt, esti_m_s_dt, esti_m_e_dt);
	int vt_size = vt.size();
...
<table border=0 cellspacing=0 cellpadding=0 width=1320>
    ...
    <tr id='tr_title' style='position:relative;z-index:1'> 
        <td width=440 class='line' id='td_title' style='position:relative;'> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr> 
                    <td width=30 class=title>연번</td>
                    <td width=80 class=title>견적번호</td>
                    <td width=110 class=title>대여상품</td>
                    <td width=70 class=title>대여기간</td>
                    <td width=80 class=title>견적일자</td>
                    <td width=70 class=title>성명</td>
                </tr>
            </table>
        </td>
    ...
    <tr> 
        <td width=440 class='line' id='td_con' style='position:relative;'> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
            <%for(int i = 0 ; i < vt_size ; i++){
				Hashtable ht = (Hashtable)vt.elementAt(i);%>
                <tr> 
                    <td width=30 align=center><%=i+1%></td>
                    <td width=80 align=center><a href="javascript:EstiDisp('<%=ht.get("EST_ID")%>')"><%=ht.get("EST_ID")%></a></td>
                    <td width=110 align=center><%=c_db.getNameByIdCode("0009", "", String.valueOf(ht.get("A_A")))%></td>
                    <td width=70 align=center><%=ht.get("A_B")%>개월</td>
                    <td width=80 align=center><%= AddUtil.ChangeDate2(String.valueOf(ht.get("RENT_DT"))) %></td>
                    <td width=70 align=center><a href="javascript:EstiDisp2('<%=ht.get("EST_ID")%>')"><%=ht.get("USER_NM")%></a></td>
                </tr>				
                  <%}%>
				<tr>						
        ...
        <td width=880 class='line'>
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                    ...
                    <td width=100 align="right"><%=Util.parseDecimal(String.valueOf(ht.get("O_1")))%></td>
                    <td width=100 align="right"><%=Util.parseDecimal(String.valueOf(ht.get("FEE_S_AMT")))%></td>
                ...
                <% if(vt_size == 0) { %>
                <tr> 
                    <td align=center colspan='9'>등록된 데이타가 없습니다.</td>
                </tr>
              <%}%>
            </table>
        </td>
