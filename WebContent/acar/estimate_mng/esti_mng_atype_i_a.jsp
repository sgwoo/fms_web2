...
<%
	...
	bean.setRent_dt			(request.getParameter("rent_dt")==null?"":request.getParameter("rent_dt"));
	...
	bean.setBus_st			(request.getParameter("bus_st")==null?"":request.getParameter("bus_st"));
	...
	//수입차출고타입
	if(AddUtil.parseInt(request.getParameter("car_comp_id")) > 5){
		bean.setImport_pur_st		(request.getParameter("import_pur_st")==null?"0":request.getParameter("import_pur_st"));
   	} else {
   		bean.setImport_pur_st("");
   	}
	//bean.setImport_pur_st		(request.getParameter("import_pur_st")==null?"0":request.getParameter("import_pur_st"));	//수입차출고타입
	bean.setCar_b_p2			(request.getParameter("car_b_p2")==null?0:AddUtil.parseDigit(request.getParameter("car_b_p2"))); //적용면세차가
	bean.setR_dc_amt			(request.getParameter("r_dc_amt")==null?0:AddUtil.parseDigit(request.getParameter("r_dc_amt"))); //렌트 세금계산서D/C
	bean.setL_dc_amt			(request.getParameter("l_dc_amt")==null?0:AddUtil.parseDigit(request.getParameter("l_dc_amt"))); //리스 세금계산서D/C
	bean.setR_card_amt		(request.getParameter("r_card_amt")==null?0:AddUtil.parseDigit(request.getParameter("r_card_amt"))); //렌트 카드결제금액
	bean.setL_card_amt			(request.getParameter("l_card_amt")==null?0:AddUtil.parseDigit(request.getParameter("l_card_amt"))); //리스 카드결제금액
	bean.setR_cash_back		(request.getParameter("r_cash_back")==null?0:AddUtil.parseDigit(request.getParameter("r_cash_back"))); //렌트 Cash Back
	bean.setL_cash_back		(request.getParameter("l_cash_back")==null?0:AddUtil.parseDigit(request.getParameter("l_cash_back"))); //리스 Cash Back
	bean.setR_bank_amt		(request.getParameter("r_bank_amt")==null?0:AddUtil.parseDigit(request.getParameter("r_bank_amt"))); //렌트 탁송썬팅비용등
	bean.setL_bank_amt		(request.getParameter("l_bank_amt")==null?0:AddUtil.parseDigit(request.getParameter("l_bank_amt"))); //리스 탁송썬팅비용등
...
// 	String gi_amt[] 			= request.getParameterValues("gi_amt");
	...
	String tint_sn_yn[] 		= request.getParameterValues("r_tint_sn_yn");			// 전면썬팅(기본형)
	...
	String tint_bn_yn[] 		= request.getParameterValues("r_tint_bn_yn");			// 블랙박스 미제공 할인
	String new_license_plate[] 	= request.getParameterValues("r_new_license_plate");	//신형번호판신청 
	String tint_cons_yn[] 		= request.getParameterValues("r_tint_cons_yn");	// 추가 탁송료
	String tint_cons_amt[] 		= request.getParameterValues("r_tint_cons_amt");	//	추가 탁송료 금액
...
// 	String eco_e_tag[]		= request.getParameterValues("eco_e_tag");
	...
	String tint_eb_yn[] 		= request.getParameterValues("r_tint_eb_yn");			// 이동형 충전기
	...
	for(int j=0; j < est_yn.length; j++){
		...
		if(est_yn[j].equals("Y")){
			...
			if(AddUtil.lengthb(r_reg_code) < 18)	r_reg_code = r_reg_code+"2"+j;
...
// 				bean.setGi_amt			(gi_amt[0]		==null?0 :AddUtil.parseDigit(gi_amt[0]));
				...
				bean.setTint_bn_yn		(tint_bn_yn[0]		==null?"N":tint_bn_yn[0]);	// 블랙박스 미제공 할인
...
// 				bean.setGi_amt			(gi_amt[j]		==null?0 :AddUtil.parseDigit(gi_amt[j]));
				...
				bean.setTint_sn_yn		(tint_sn_yn[j]	==null?"N":tint_sn_yn[j]);		// 전면썬팅미시공할인
				...
				bean.setTint_bn_yn		(tint_bn_yn[j]				==null?"N":tint_bn_yn[j]);			// 블랙박스 미제공 할인
				...
				bean.setTint_eb_yn	(tint_eb_yn[j]				==null?"N":tint_eb_yn[j]);			// 이동형 충전기
			}
			...
			if(ej_bean.getJg_b().equals("6")){
				bean.setA_h	("7"); //20191206  수소차 실등록지역 서울 -> 인천
			...
			for(int i = 0 ; i < esti_idx ; i++){
				...
				//초과운행부담금 계산위해 org, base 견적필요->20101207 현재 초과운행부담금은 계산하지 않음.->20130605 초과운행부담금 시작
				if(i==0){
					a_bean.setJob("org");
				...
				out.println("<br>ej_bean.getJg_i()="+ej_bean.getJg_i()+"<br>");
				out.println("<br>ej_bean.getJg_h()="+ej_bean.getJg_h()+"<br>");
				...
				//[20121120] 수입차 리스는 아마존카 피보험자를 할수 없다. 수입차 일반식 불가
				...
				if(est_reg_yn.equals("Y")){
					...
					//fms2에서 견적함.
					if(AddUtil.lengthb(a_bean.getEst_id()) < 15)	a_bean.setEst_id(a_bean.getEst_id()+""+"2");									
					
					count = e_db.insertEstimate(a_bean);					
				
					if(est_size==0) est_id = a_bean.getEst_id();
...
<!--
	...
	function cust_check(){
		...
		<%	if(!est_check5.equals("")){ //사전예약 고객일 경우%>		
		if(confirm_ment == ''){
		...
		if(sure){
			document.form1.action = "/acar/estimate_mng/esti_mng_abtype_proc.jsp";				
...
<%	if(count==1){%>
		...
		<%}else{%>	
		//alert('승합차(11~12인승) 혹은 수입차 리스 일때는 고객피보험자로 해야 하며, 수입차 리스와 트럭의 일반식은 견적되지 않습니다.');
