...
<%
	...
	EstimateBean [] e_r = e_db.getEstimateFmsExtList(gubun1, gubun2, gubun3, gubun4, gubun5, gubun6, s_dt, e_dt, s_kd, t_wd);
	int size = e_r.length;
%>
<html>
<head>
...
<!--
	...
	function EstiDisp(est_id, set_code){	
		...
		fm.target = 'd_content';
		fm.action = 'esti_ext_mng_u.jsp';
		fm.submit();
	}
	
	function EstiMemo(est_id, user_id){
		var SUBWIN="./esti_memo_i.jsp?est_id="+est_id+"&user_id="+user_id+"&from_page=esti_ext_sc_in.jsp";	
...
<form action="esti_ext_mng_u.jsp" name="form1" method="POST">
  ...
  <input type="hidden" name="from_page" value="esti_ext_sc_in.jsp">                    
        ...
        <td width=660 class='line' id='td_con' style='position:relative;'> 
            ...
            <%for(int i=0; i<size; i++){
                ...
                <tr> 
                    ...
                    <td width=200 align=center><span title='<%=bean.getEst_nm()%>'><%=AddUtil.substringbdot(bean.getEst_nm(), 30)%></span></td>
                </tr>
                  <%}%>
                  <% if(size == 0) { %>
                <tr> 
                    <td width=680 align=center colspan="7">등록된 데이타가 없습니다.</td>
                </tr>
          <%}%>
            </table>
        </td>
        <td width=1140 class='line'>
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <%for(int i=0; i<size; i++){
    					bean = e_r[i];%>
                <tr> 
                    <td width=80 align=center><%= AddUtil.ChangeDate3(bean.getRent_dt()) %></td>
                    ...
                    <%-- <td width=60 align=center>
                    	<a href="javascript:EstiMemo('<%=bean.getEst_id()%>','<%=user_id%>')"><%=bean.getReg_nm()%></a>
                    </td> --%>
                    <td width=60 align=center>
                    	<%=bean.getReg_nm()%>
                    </td>
