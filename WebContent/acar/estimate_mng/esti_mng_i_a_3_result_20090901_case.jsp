...
<%
	...
	if(est_id.equals("")){
		vt = e_db.getEstiMateRegCodeCaseList(reg_code, esti_table, car_mng_id, est_nm);
		vt_size = vt.size();
		
	}else{
		vt = e_db.getEstiMateEstidCase(est_id, esti_table);
		vt_size = vt.size();		
	}
...
<!--
	...
	//견적결과보기
	function estimates_view(reg_code, est_nm){
		var SUBWIN="/acar/estimate_mng/esti_mng_i_a_3_result_20090901_case.jsp?car_gu=1&rent_st=1&est_code="+reg_code+"&est_nm="+est_nm+"&esti_table=estimate";
		window.open(SUBWIN, "ResultViewCase", "left=100, top=100, width=<%=s_width%>, height=<%=s_height%>, scrollbars=yes, status=yes, resizable=yes");
	}	
//-->
</script>
</head>
