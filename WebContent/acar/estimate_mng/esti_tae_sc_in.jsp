...
<%
	...
	EstimateBean [] e_r = e_db.getEstimateTaeList(gubun1, gubun2, gubun3, gubun4, gubun5, gubun6, s_dt, e_dt, s_kd, t_wd, esti_m, esti_m_dt, esti_m_s_dt, esti_m_e_dt);
	int size = e_r.length;
%>
<html>
<head>
...
<!--
	//내용보기
	function EstiDisp(est_id, reg_id){	
...
<form action="esti_tae_mng_u.jsp" name="form1" method="POST">
  ...
  <input type="hidden" name="from_page" value="esti_tae_sc_in.jsp">                    
    ...
    <tr> 
        <td width=510 class='line' id='td_con' style='position:relative;'> 
		    ...
		    <td width=40 align=center><input type="checkbox" name="ch_l_cd" value="<%=bean.getEst_id()%>"></td>                    		    
                    ...
                    <td width=160 align=center><a href="javascript:EstiDisp('<%=bean.getEst_id()%>','<%=bean.getReg_id()%>')"><%= AddUtil.ChangeDate3(bean.getReg_dt()) %></a></td>
