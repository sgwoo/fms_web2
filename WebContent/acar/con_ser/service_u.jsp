...
<!--
	...
	function change_scd(cmd, pay_yn, idx, accid_id, serv_id){
		...
		fm.action='/acar/con_ser/mod_scd_u.jsp';
		fm.target = 'i_no';
		fm.submit();		
	}
	...
	function ServiceDisp(serv_id, serv_st, off_id){
		...
		window.open(SUBWIN, 'popwin_serv_reg','scrollbars=yes,status=no,resizable=no,width=850,height=750,top=20,left=20');
		
		/*
		if(serv_st=='순회점검'){
			var SUBWIN="/acar/car_service/service_round_c.jsp" + url;
			window.open(SUBWIN, "RoundReg", "left=100, top=50, width=730, height=380, scrollbars=no");
		}else if(serv_st=='일반수리'){
			var SUBWIN="/acar/car_service/service_general_c.jsp" + url;	
			window.open(SUBWIN, "GenReg", "left=100, top=50, width=730, height=550, scrollbars=no");
		}else if(serv_st=='보증수리'){
			var SUBWIN="/acar/car_service/service_guarantee_c.jsp" + url;	
			window.open(SUBWIN, "GuarReg", "left=100, top=50, width=730, height=380, scrollbars=no");
		}else if(serv_st=='사고수리'){
			var SUBWIN="/acar/car_service/car_accid_serv_cd.jsp" + url;	
			window.open(SUBWIN, "AccidReg", "left=100, top=50, width=730, height=550, scrollbars=no");
		}
		*/
	}	
	...
	function go_to_list(){
		...
		location = "/acar/con_ser/service_frame_s.jsp?auth_rw="+auth_rw+"&br_id="+br_id+"&user_id="+user_id+"&gubun1="+gubun1+"&gubun2="+gubun2+"&gubun3="+gubun3+"&gubun4="+gubun4+"&st_dt="+st_dt+"&end_dt="+end_dt+"&s_kd="+s_kd+"&t_wd="+t_wd+"&sort_gubun="+sort_gubun+"&asc="+asc+"&f_list="+f_list;
	}	
...
<input type='hidden' name='brch_id' value='<%=fee.get("BRCH_ID")%>'>
<table border=0 cellspacing=0 cellpadding=0 width="100%">
        ...
        <td colspan=2>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>재무회계 > 영업비용관리 > 정비비 관리 > <span class=style5>정비비 수정</span></span></td>
    ...
    <tr>
        <td align=right colspan=2><a href="javascript:go_to_list()"><img src=../images/center/button_list.gif align=absmiddle border=0></a></td>
    </tr>
    <tr>
    ...
    <tr> 
                    ...
                    <td class='line'> 
                        <table border="0" cellspacing="1" cellpadding="0" width=100%>
                            <tr> 
                              ...
                              <td width=6% class='title'>상호</td>
                              <td width=16%>&nbsp;<%=fee.get("FIRM_NM")%></td>
                              <td width=7% class='title'>고객명</td>
                              <td width=8%>&nbsp;<%=fee.get("CLIENT_NM")%></td>
                              <td width=8% class='title'>차량번호</td>
                              <td width=9%>&nbsp;<font color="#000099"><b><%=fee.get("CAR_NO")%></b></font></td>
                              <td width=6% class='title'>차명</td>
                              <td width=22%>&nbsp;<%=fee.get("CAR_NM")%></td>
                            </tr>
                            <tr> 
	        ...
	        <td class='line'>			 
                <table border="0" cellspacing="1" cellpadding="0" width=100%>
                    <tr> 
                        ...
                        <td width=11% class='title'>정비금액</td>
                        <td width=10% class='title'>DC</td>			
                        <td width=11% class='title'>지출금액</td>
                        <td width=9% class='title'>지출일자</td>
        ...
        <tr> 
            <td colspan="2"> <iframe src="/acar/con_ser/service_u_in.jsp?auth_rw=<%=auth_rw%>&m_id=<%=m_id%>&l_cd=<%=l_cd%>&c_id=<%=c_id%>&brch_id=<%=fee.get("BRCH_ID")%>" name="inner" width="100%" height="380" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 > 
            </td>
        </tr>
    </table>
</form>
