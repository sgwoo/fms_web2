...
<!--
	function search(){
		...
		if(fm.s_kd.options[fm.s_kd.selectedIndex].value == '8' || fm.s_kd.options[fm.s_kd.selectedIndex].value == '14' || fm.s_kd.options[fm.s_kd.selectedIndex].value == '15'){ //영업담당자
	...
	function list_move()
	{
		...
		else if(idx == '3') url = "/acar/con_ser/service_frame_s.jsp";
		...
		else if(idx == '14') url = "/acar/con_ser/service_frame_s.jsp";										
		fm.action = url;		
		fm.target = 'd_content';	
		fm.submit();						
	}				
...
<%
	...
	String gubun1 = request.getParameter("gubun1")==null?"14":request.getParameter("gubun1");
	...
	Vector users2 = c_db.getUserList("8888", "", ""); //협력업체
...
%>
<form name='form1' action='service_sc.jsp' target='c_body' method='post'>
        ...
        <td>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp; <span class=style1>재무회계 > 영업비용관리 ><span class=style5>정비비</span></span></td>
                ...
                <tr> 
                    <td width='190'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_jhgb.gif align=absmiddle>&nbsp;
                      <select name="gubun1" onChange="javascript:list_move()">
                        ...
                        <option value="14" <%if(gubun1.equals("14")){%>selected<%}%>>정비비</option>
                    ...
                    <td width=420> 
                      <select name="gubun4">
                        ...
                        <option value="2" <%if(gubun4.equals("2")){%>selected<%}%>>순회점검</option>
                        <option value="3" <%if(gubun4.equals("3")){%>selected<%}%>>일반수리</option>
        			    <option value="4" <%if(gubun4.equals("4")){%>selected<%}%>>보증수리</option>
                        <option value="5" <%if(gubun4.equals("5")){%>selected<%}%>>사고수리</option>
        			  </select>
                    </td>
                </tr>
                <tr> 
                    <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_gsjg.gif align=absmiddle>&nbsp;
                        <select name='s_kd' onChange="javascript:document.form1.t_wd.value='', cng_input()">
                            ...
                            <option value='10' <%if(s_kd.equals("10")){%> selected <%}%>>정비일자</option>
                            <option value='11' <%if(s_kd.equals("11")){%> selected <%}%>>정비업체</option>												
                            <option value='12' <%if(s_kd.equals("12")){%> selected <%}%>>점검내용</option>																
                            <option value='13' <%if(s_kd.equals("13")){%> selected <%}%>>주행거리</option>					
                            <option value='5' <%if(s_kd.equals("5")){%> selected <%}%>>정비비</option>
            				<%//if(br_id.equals("S1")){%>
                            ...
                            <option value='14' <%if(s_kd.equals("14")){%> selected <%}%>>점검자</option>				
                            <option value='15' <%if(s_kd.equals("15")){%> selected <%}%>>등록자</option>								
                        </select>
                    </td>
                    <td> 
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr> 
                                <td id='td_input' <%if(s_kd.equals("8") || s_kd.equals("6") || s_kd.equals("14") || s_kd.equals("15")){%> style='display:none'<%}%>> 
                                    <input type='text' name='t_wd' size='21' class='text' value='<%=t_wd%>' onKeyDown='javascript:enter()'>
                                </td>
                                <td id='td_bus' <%if(s_kd.equals("8") || s_kd.equals("14") || s_kd.equals("15")){%> style="display:''"<%}else{%>style='display:none'<%}%>> 
                                    <select name='s_bus'>
                    ...
                    <td><img src=/acar/images/center/arrow_jrjg.gif align=absmiddle>&nbsp;
                        <select name='sort_gubun' onChange='javascript:search()'>
                            ...
                            <option value='0' <%if(sort_gubun.equals("0")){%> selected <%}%>>정비일자</option>
                            <option value='2' <%if(sort_gubun.equals("2")){%> selected <%}%>>정비업체</option>
                            <option value='3' <%if(sort_gubun.equals("3")){%> selected <%}%>>정비비</option>
                        </select>
                    </td>
                    <td> 
