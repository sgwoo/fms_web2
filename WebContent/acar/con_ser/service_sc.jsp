...
<!--
	//정비비 세부내용 보기
	function view_service(m_id, l_cd, c_id, accid_id, serv_id, idx){
		...
		fm.idx.value = idx;						
		fm.target = "d_content";
		fm.action = "/acar/con_ser/service_u.jsp";
		fm.submit();
	}
	
	//정비비 세부내용 보기
	function view_serv_reg(m_id, l_cd, c_id, accid_id, serv_id, idx){
		var SUBWIN="/acar/cus_reg/serv_reg.jsp?car_mng_id=" + c_id + "&serv_id=" + serv_id; 
	...
	function  view_stat(){
		...
		document.form1.action="service_sc_stat.jsp";
		...
		<tr>
		  <td width=100%>
			<table border="0" cellspacing="0" cellpadding="0" width=100%>
			  <tr>
				<td align='center'>
				  <iframe src="/acar/con_ser/service_sc_in.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&auth=<%=auth%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>" name="i_no" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
				  </iframe>
				</td>
			  </tr>
			</table>
