...
<!--
	...
	function set_day(){
		...
		if(fm.cls_st.value == '8'){ //매입옵션 선택시 디스플레이
			...
			if(fm.opt_chk.value != '1' ){ //매입옵션 선택시 디스플레이		
			  	alert("계약당시 매입옵션 내용이 없습니다. 이같은 경우 매입옵션을 행사할수 없습니다..!!!");
			  	tr_opt.style.display 		= 'none';	//매입옵션
			  	fm.action='./lc_cls_c_nodisplay1.jsp';
				fm.cls_st.value="";				
			} else {
				...
				if ( count > 61 ) { 
					...
					if ( toInt(fm.rent_end_dt.value) >  toInt(replaceString("-","",fm.cls_dt.value))) { //만기이후 		
					    fm.dly_c_dt.value = fm.cls_dt.value;
				  		fm.action='./lc_cls_c_nodisplay1.jsp?dly_c_dt='+fm.dly_c_dt.value+'&cls_gubun=Y&cls_dt='+fm.rent_end_dt.value;
				    } else { 
				   		fm.action='./lc_cls_c_nodisplay1.jsp?cls_gubun=Y';
				    }
				}	
			}
	...
	function set_day_sui(cls_dt){
		...
		fm.action='./lc_cls_c_nodisplay1.jsp?dly_c_dt='+fm.cls_dt.value+'&cls_gubun=Y&cls_dt='+cls_dt;		
		fm.target='i_no';
		fm.submit();
	}			
				 ...
				 if ( toInt(fm.rent_end_dt.value) <= toInt(fm.dly_s_dt.value) ) { //이전 미납분이 없는 경우
			   	      ...
			   	      if (  toInt(parseDigit(fm.ex_s_amt.value)) > 0 ){
			   	    	     	fm.nfee_day.value = 0;
			   	    	     	  ...
			   	    	     	  // 대여료만기일과 정산일 비교 (실제연체일수 계산)	 20140311 수정 - 선납금액
					//	var s1_str = fm.ex_s_dt.value;
					...
					//	if ( count1 >= 0 ) { 
					//		    cal_ex_s_amt  = toInt(parseDigit(fm.nfee_s_amt.value))*count1 /30;
							    
							    //입금액  - 차감액 
					//		    if (   toInt(parseDigit(fm.r_ex_s_amt.value))  - cal_ex_s_amt) > 0   ) {
					//		       	 fm.ex_di_amt.value  =  - ( toInt(parseDigit(fm.r_ex_s_amt.value))  - cal_ex_s_amt);
					//		    }						    
					//	}
...
<input type='hidden' name='pay_e_dt' value='<%=base.get("PAY_E_DT")%>'> <!-- 입금 최종일 - 잔액아님 --> 
          ...
          <tr> 
            <td colspan="2" class='line'> 
              <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                  <td class="title" colspan="2">초과운행부담금(L)</td>
                  <td class='title' width='35%' align="center"> 
...
<!--
	...
	function set_init(){
		...
		if(fm.nfee_day.value == '31'){
	      ...
	      //개시대여료가 없는 경우 					   
	    if(fm.ifee_s_amt.value == '0' ) {
	  		//대여 스케쥴이 생성안되고, 미납이 없는 경우 scd_fee가 없어서 nfee_mon, nfee_day 구할수 없음.  --중도해약인경우	   
		   if ( toInt(fm.rent_end_dt.value) >= toInt(fm.use_s_dt.value)) { //만기이후 대여료 스케쥴이 생성이 안된 경우 
		   			 						 
			 if ( toInt(fm.rent_end_dt.value) <= toInt(fm.dly_s_dt.value) ) { //만기일 이전에 미납분이 없는 경우
			   	 alert(" 만기후 대여료스케쥴이 생성 안됨, 만기일 이전 미납분이 없는 경우");
			   	   if (toInt(fm.r_con_mon.value) > 0) {  //만기경과기준이 있다면
			   	  	  	 fm.nfee_mon.value 	= 	toInt(fm.r_con_mon.value);  //해지일 - 만기일의 개월 수 (실사용월수)
			   	   } else {  
			   	  	  	 fm.nfee_mon.value = '0';			   	  	  	 
			   	   } 
		      ...
		      if ( toInt(fm.use_e_dt.value) <= toInt(fm.dly_s_dt.value) ) { //만기스케쥴일 이전에 미납분이 없는 경우
			   //	   alert(" 만기후 대여료스케쥴이 생성 됨, 만기스케쥴일 이전 미납분이 없는 경우");     			   	 
			 	   ...
			 	   if ( toInt(fm.nfee_mon.value) < 0 ) {
		   	       ...
		   	       if (  toInt(parseDigit(fm.ex_s_amt.value)) > 0 ){
		   	    	     	fm.nfee_day.value = 0;
		   	    	     	//선납금액 30일기준으로 변경 - 해당회차 대여료를 완납한 경우		   	    	     	
	...
	if ( toInt(parseDigit(fm.nfee_s_amt.value)) < 100 ) {
				  ...
				  if ( toInt(fm.rent_end_dt.value) < toInt(replaceString("-","",fm.cls_dt.value)) ) {					  
				      if  ( fm.nfee_amt.value  != '0' ) {				      	 
				          if ( re_nfee_amt > 0 ) {	 					 
					 			...
					 			if  ( toInt(fm.df_e_dt.value) <= toInt(replaceString("-","",fm.cls_dt.value)) ) { 	 //미납스케쥴보다 해지일이 큰경우 - 20110413
			           			    fm.nfee_amt.value = parseDecimal(re_nfee_amt);	
			           		    }					 						  	    	
						  }
					  }	 
		...
		// alert( toInt(parseDigit(fm.rc_s_amt.value)) +  toInt(parseDigit(fm.rc_v_amt.value)) );
		// alert( toInt(parseDigit(fm.rr_amt.value)) );
		//  alert( toInt(parseDigit(fm.rr_s_amt.value))  );
	...
	//	                if (  toInt(fm.pay_e_dt.value)   >   toInt(fm.ex_e_dt.value) ) {
	//	                } else {
			  
	//			 	var s2_str = fm.ex_s_dt.value; 
	//			 	var e2_str = fm.ex_e_dt.value; 
	...
	//				    //31일경우 강제로 1일치 공제? - 20140626
	//			//	if (   count2  < 30 ) {  		   			 
						
	//					   if (  toInt(parseDigit(fm.ex_e_dt.value)) > 0 ){ 	  // 잔액없이 완불일 경우 :30일로 다시 					
	//		               		               				  // 대여료만기일과 정산일 비교 (실제연체일수 계산) 20140311 수정 - 선납금액
	//						var s1_str = fm.cls_dt.value;
	//						var e1_str = fm.ex_e_dt.value;
	//						var  count1 = 0;
	//						var cal_ex_s_amt =0;
												
	//						var s1_date =  new Date (s1_str.substring(0,4), s1_str.substring(5,7) -1, s1_str.substring(8,10) );	
	//						var e1_date =  new Date (e1_str.substring(0,4), e1_str.substring(4,6) -1 , e1_str.substring(6,8) );			
	...
	//							    cal_ex_s_amt  = toInt(parseDigit(fm.nfee_s_amt.value))*count1 /30;						
	//							    //입금액  - 차감액 					  
	//							     fm.ex_di_amt.value  = parseDecimal( cal_ex_s_amt  * (-1) );
	//							     fm.ex_di_v_amt.value  =   parseDecimal( toInt(parseDigit(fm.ex_di_amt.value)) * 0.1 );	
	//						}	
	//				         }	
	...
	//				//     } else  {   
				           ...
				           //	if (  toInt(parseDigit(fm.ex_s_dt.value)) > 0 ){ 	  // 잔액없이 완불일 경우 :30일로 다시 					
			               
		               				  // 대여료시작일과 정산일 비교 (실제연체일수 계산)	 20140311 수정 - 선납금액
					//		var s1_str = fm.ex_s_dt.value; 
					//		var e1_str = fm.cls_dt.value;
					//		var  count1 = 0;
					//		var cal_ex_s_amt =0;
					...
					//		count1 = 30 - (Math.floor(diff1_date/(24*60*60*1000)));
