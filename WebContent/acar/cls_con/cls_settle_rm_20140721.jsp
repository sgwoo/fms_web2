...
<!--
	...
	function view_settle(m_id, l_cd){
		window.open("/acar/cls_con/cls_settle_rm1.jsp?m_id="+m_id+"&l_cd="+l_cd, "VIEW_SETTLE1", "left=100, top=10, width=700, height=630, scrollbars=yes, status=yes");		
	}	
	  	   	  	      ...
	  	   	  	      if (  toInt(parseDigit(fm.ex_s_amt.value)) > 0 ){
				   	    	 fm.nfee_day.value = 0;
				   	  	     ...
				   	  	     if ( toInt(fm.rent_end_dt.value) == toInt(fm.use_e_dt.value) ) { // 만기일과 스케쥴 만기일 비교	 
					  	   	     fm.nfee_day.value 	= 	0;
					  	     } else if ( toInt(fm.rent_end_dt.value) > toInt(fm.use_e_dt.value) ) { // 만기일과 스케쥴 만기일 비교	 
...
<input type='hidden' name='df_s_amt' value='<%=AddUtil.parseDecimal(AddUtil.parseInt((String)base.get("DF_S_AMT")))%>'> <!-- 해지일포함된 스케쥴 연체금액 (잔액아님) --> 
 ...
 <input type='hidden' name='ex_s_dt' value='<%=base.get("EX_S_DT")%>'> 
<input type='hidden' name='ex_e_dt' value='<%=base.get("EX_E_DT")%>'> 
		   	    	     ...
		   	    	     if (  toInt(parseDigit(fm.ex_s_amt.value)) > 0 ){
		   	    	     	fm.nfee_day.value = 0;
			   	    	  	             ...
			   	    	  	             if  ( fm.nnfee_s_amt.value  == '0' ) { // 
				   	    	      		            ...
				   	    	      		            if ( fm.r_con_mon.value == '0') { //마지막 스케쥴이 연체인경우
				   	    	      		            								   	    	    	    	     //20130104
								   	    	         if  ( toInt(fm.use_e_dt.value) == toInt(replaceString("-","",fm.cls_dt.value)) ) { 	// 스케쥴 종료일과 해지일이 같다면	미납잔액이 연체금액임.
