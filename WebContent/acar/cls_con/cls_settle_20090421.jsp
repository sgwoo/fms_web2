...
<!--
	...
	function cls_display(){
		...
		var s_date =  new Date (s_str.substring(0,4), s_str.substring(4,6), s_str.substring(6,8) );
		...
		if(fm.cls_st.value == '8'){ //매입옵션 선택시 디스플레이
			...
			if(fm.opt_chk.value != '1' ){ //매입옵션 선택시 디스플레이		
				...
				if ( count > 30 ) { 
					...
					if ( toInt(fm.rent_end_dt.value) >  toInt(replaceString("-","",fm.cls_dt.value))) { //만기이후 		
				  		fm.action='./lc_cls_c_nodisplay.jsp?cls_gubun=Y&cls_dt='+fm.rent_end_dt.value;
				    } else { 
	...
	function set_day_sui(cls_dt){
		...
		fm.action='./lc_cls_c_nodisplay.jsp?cls_gubun=Y&cls_dt='+cls_dt;		
		fm.target='i_no';
		fm.submit();
	}			
			...
			if(fm.ifee_s_amt.value == '0' ) {
			   ...
			   if ( toInt(fm.rent_end_dt.value) >= toInt(fm.use_s_dt.value)) { //만기이후 대여료 스케쥴이 생성이 안된 경우 
				 if ( toInt(fm.rent_end_dt.value) <= toInt(fm.dly_s_dt.value) ) { //만기일 이전에 미납분이 있는 경우
						   ...
						   var s1_date =  new Date (s1_str.substring(0,4), s1_str.substring(4,6), s1_str.substring(6,8) );
						   var e1_date =  new Date (e1_str.substring(0,4), e1_str.substring(5,7), e1_str.substring(8,10) );
	...
	//	alert(	fm.ex_di_v_amt.value );
		...
		var no_v_amt = 0;
			...
			//각각 부가세 계산하여 더한다.	 
		if(fm.cancel_yn[0].checked == true ){
				 ...
				 no_v_amt = toInt(parseDigit(fm.ex_di_v_amt.value)) + ( toInt(parseDigit(fm.nfee_amt.value)) * 0.1) ;
...
<input type='hidden' name='ex_di_v_amt'  value='<%=AddUtil.parseDecimal(-AddUtil.parseInt((String)base.get("EX_V_AMT"))+AddUtil.parseInt((String)base.get("DI_V_AMT2")))%>'  >
            ...
            <td colspan="2" class='line'> 
              <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                  ...
                  <td class='' width='35%' align="center"> 
                    <input type='text' size='3' name='nfee_mon'  value='<%=AddUtil.parseInt((String)base.get("S_MON"))%>' readonly class='num' maxlength='4' onBlur='javascript:set_cls_amt2(this);'>
                    개월&nbsp;&nbsp;&nbsp; 
                    <input type='text' size='3' name='nfee_day'  value='<%=AddUtil.parseInt((String)base.get("S_DAY"))%>' readonly class='num' maxlength='4' onBlur='javascript:set_cls_amt2(this);'>
                    일</td>
                  <td width='35%'>&nbsp;</td>
                </tr>
                <tr> 
