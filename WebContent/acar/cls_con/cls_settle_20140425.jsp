		...
		// 환불일 경우)  rcon_mon, rcon_day:잔여대여기간  r_mon, r_day :이용기간 
		 if (  toInt(parseDigit(fm.ex_s_amt.value)) > 0 ){ 
		        ...
		        if (  toInt(parseDigit(fm.ex_e_dt.value)) > 0 ){ 	  // 잔액없이 완불일 경우 :30일로 다시 					
			      ...
			      //다시계산
			         //            if (  (  toInt(parseDigit(fm.nfee_s_amt.value)) * toInt(fm.rcon_day.value)/30 )  <  toInt(parseDigit(fm.ex_s_amt.value))   ) {
				//        //     alert(toInt(parseDigit(fm.ex_s_amt.value)));
				//                    fm.ex_di_amt.value       =   parseDecimal((toInt(parseDigit(fm.nfee_s_amt.value)) * toInt(fm.rcon_day.value)/30    * (-1)  )) ; 	
			   	//   	         //  fm.ex_di_v_amt.value  =   parseDecimal( toInt(parseDigit(fm.ex_di_amt.value)) * 0.1 );	
				//       }	 
