...
<%
	...
	//부채현황
	Vector deb1s = ad_db.getStatDebtList("stat_mng");
	int deb1_size = deb1s.size();
%>
<form name='form1' action='stat_mng_sc_in_view.jsp' target='i_view'>
<input type='hidden' name='auth_rw' value='<%=auth_rw%>'>
<input type='hidden' name='user_id' value='<%=user_id%>'>
<input type='hidden' name='br_id' value='<%=br_id%>'>
<input type='hidden' name='save_dt' value='<%=save_dt%>'>
<input type='hidden' name='brch_id' value='<%=brch_id%>'>
<table border="0" cellspacing="0" cellpadding="0" width="<%if(deb1_size==0){%>200<%}else{%><%=(deb1_size*80)+80%><%}%>">
    <tr>
        <td class=line2></td>
    </tr>
    <tr>
        <td class='line'>			
          ...
          <%	}
			        ...
			        <td align='center' width='120'>데이타가 없습니다</td>						
          <%}%>
                </tr>		  
            </table>
		</td>
