...
<%@ page import="java.util.*, acar.util.*, acar.common.*, acar.cont.*, acar.car_office.*, acar.car_mst.*, acar.estimate_mng.*, acar.user_mng.*" %>
...
<%
	...
	if(auth_rw.equals(""))	auth_rw = rs_db.getAuthRw(ck_acar_id, "11", "02", "01");
	...
	if(car_id.equals("") && !rent_mng_id.equals("")){
		//차량기본정보
		ContCarBean car 	= a_db.getContCarNew(rent_mng_id, rent_l_cd);
		car_id 	= car.getCar_id();
		car_seq = car.getCar_seq();
	}
...
<!--
	...
	function Delete(){
		...
		if(!confirm('다시 복구할 수 없습니다. 삭제하시겠습니까?')){	return;	}
		fm.cmd.value = 'd';
		fm.action = 'car_mst_u_a.jsp';
		fm.target = 'i_no';
		fm.submit();
...
<form action="./car_mst_u_a.jsp" name="form1" method="POST" >
  <input type="hidden" name="auth_rw" value="<%=auth_rw%>">
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
    <tr> 
        <td> 
            <table border="0" cellspacing="0" cellpadding="0" width=100%>
    	            ...
    	            <td>
                	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                            <tr>
                                ...
                                <td class=bar>&nbsp;&nbsp;&nbsp;<img src=../images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>Master > <span class=style5>차명관리 <%if(!from_page.equals("lc_rent")){%>(수정)<%} %></span></span></td>
                ...
                <tr> 
                    <td align="right"> 
                        <%if(!auth_rw.equals("1") && !from_page.equals("lc_rent")){%>
                        <%    if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("차종관리",user_id)){%>
                        <a href='javascript:Delete();'><img src="../images/center/button_delete.gif" align="absmiddle" border="0"></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <%    }%>                    
                        <%    if(auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6")){%>
                        <a href='javascript:Update();'><img src="../images/center/<%if(cmd.equals("u")){%>button_modify<%}else{%>button_modify_s<%}%>.gif" align="absmiddle" border="0"></a>
                        <%	      if(cmd.equals("u")){%>
                        <a href='javascript:document.form1.reset();'><img src="../images/center/button_cancel.gif" align="absmiddle" border="0"></a>
                        <%	      }%>
                        <%    }%>
                        <a href="javascript:up_grade();"><img src=../images/center/button_upgrade.gif align=absmiddle border=0></a> 
                        <%}%>
                        <%if(!from_page.equals("lc_rent")){%>
    		        <a href="javascript:go_list();"><img src="../images/center/button_list.gif" align="absmiddle" border="0"></a>
    		        <%}%>
    		    </td>    		    
                </tr>
                </table>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="3" width=100%>
                ...
                <tr> 
                    ...
                    <td > <input type="text" name="car_nm" value="<%=cm_bean.getCar_nm()%>" size="30" class=<%=white%>text> 
                    </td>
                </tr>
                <tr> 
                    <td class=title>차종</td>
                    <td > <input type="text" name="car_name" value="<%=cm_bean.getCar_name()%>" size="70" class=<%=white%>text> 
                    </td>
                </tr>
                <tr> 
                ...
                <tr> 
                    ...
                    <td > 포함차종명 : 
                      <input type="text" name="car_b_inc_name" value="<%=car_b_inc_name%>" size="40" class=text> 
                      <% if(!from_page.equals("lc_rent") && cmd.equals("u")){ %>
                      <a href="javascript:open_car_b_inc();"><img src=../images/center/button_in_cho.gif border=0 align=absmiddle></a> 
                      <% } %>
                      <br> <textarea name="car_b" cols="110" class="text" rows="8" <%=readonly%>><%=cm_bean.getCar_b()%></textarea> 
                    </td>
                </tr>
                <tr> 
                    <td class=title>기본가격</td>
                    <td > <input type="text" name="car_b_p" value="<%=AddUtil.parseDecimal(cm_bean.getCar_b_p())%>" size="10" class=<%=white%>num onBlur='javascript:this.value=parseDecimal(this.value);'>
                      원 &nbsp;&nbsp;&nbsp;
                ...
                <tr> 
                    ...
                    <td > 
                    	<input type="radio" name="use_yn" value="Y" <%if(cm_bean.getCar_yn().equals("Y"))%>checked<%%>>
                      사용 
				...
				<tr>
                	...
                	<td > <input type="radio" name="hook_yn" value="N" <%if(cm_bean.getHook_yn().equals("N"))%>checked<%%>>
                		미해당
                		<input type="radio" name="hook_yn" value="Y" <%if(cm_bean.getHook_yn().equals("Y"))%>checked<%%>>
                		해당</td>
                </tr> 
                --%>
                <tr> 
                    <td class=title>기준일자</td>
                    <td > <input type="text" name="car_b_dt" value="<%=AddUtil.ChangeDate2(cm_bean.getCar_b_dt())%>" size="12" class=<%=white%>text onBlur='javscript:this.value = ChangeDate(this.value);'> 
                    </td>
                </tr>
                <tr> 
                ...
                <tr>	<!-- 기타 추가(2018.02.08) -->
                    <td class=title>기타</td>
                    <td > <textarea name="etc2" cols="110" class="text" rows="2" placeholder="(견적서 기타란에 보여지는 부분입니다)" maxlength="200" <%=readonly%>><%=cm_bean.getEtc2()%></textarea> 
                    </td>
                </tr>               
                <tr> 
                    ...
                    <td > 
                      ...
                      단종
                      &nbsp;&nbsp;
                      (<input type="checkbox" name="end_hp_yn" value="Y" checked> 홈페이지사용여부와 연동한다.)
                    </td>
                </tr>
            </table>
        </td>
    ...
    <tr> 
        <td align="right"> 
        <%if(!from_page.equals("lc_rent")){%>
        <a href="javascript:MM_openBrWindow('car_opt.jsp?auth_rw=<%=auth_rw%>&br_id=<%=br_id%>&user_id=<%=user_id%>&car_comp_id=<%=cm_bean.getCar_comp_id()%>&car_cd=<%=cm_bean.getCode()%>&car_id=<%=car_id%>&car_seq=<%=car_seq%>&car_b_dt=<%=cm_bean.getCar_b_dt()%>&cmd=<%=cmd%>&car_nm=<%=cm_bean.getCar_nm()%>','popwin2','scrollbars=yes,status=yes,resizable=yes,width=1450,height=760,left=50, top=50')">
        <img src=../images/center/button_stsy.gif border=0></a>
        <a href="javascript:MM_openBrWindow('car_col.jsp?auth_rw=<%=auth_rw%>&br_id=<%=br_id%>&user_id=<%=user_id%>&car_comp_id=<%=cm_bean.getCar_comp_id()%>&car_cd=<%=cm_bean.getCode()%>&car_id=<%=car_id%>&car_seq=<%=car_seq%>&car_b_dt=<%=cm_bean.getCar_b_dt()%>&cmd=<%=cmd%>&car_nm=<%=cm_bean.getCar_nm()%>','popwin3','scrollbars=yes,status=yes,resizable=yes,width=1070,height=560,left=50, top=50')">
        <img src=../images/center/button_ss.gif border=0></a>
        <%}else{ %>
