...
<%@ page import="java.util.*, acar.util.*, acar.car_mst.*, acar.estimate_mng.*, acar.cont.*" %>
...
<%
	...
	cm_bean.setDuty_free_opt	(request.getParameter("duty_free_opt")==null?"":request.getParameter("duty_free_opt"));	//개소세 면세가/과세가 표기(20190429)
	...
	if(end_hp_yn.equals("Y") && cm_bean.getEnd_dt().equals("N") && cm_bean.getHp_yn().equals("Y")){
		cm_bean.setHp_yn		("N");
	}
	...
	if(cmd.equals("d")){	
		//기존계약차량이 없어야 삭제할수 있다
		int cont_cnt = a_db.getContCarIdCnt(cm_bean.getCar_id(), cm_bean.getCar_seq());
		if(cont_cnt==0){
			count = a_cmb.deleteCarNm(cm_bean);		
			//선택사양도 삭제
			count = a_cmb.deleteCarOpt(cm_bean.getCar_comp_id(), cm_bean.getCar_cd(), cm_bean.getCar_id(), cm_bean.getCar_seq());					
		}
	}else{
		count = a_cmb.updateCarNm(cm_bean);
	}
	
//	System.out.println("차종수정"+count);
%>
<html>
<head>
...
<%		}
		...
		if(count==1){%>
		alert("정상적으로 삭제되었습니다.");
		document.form1.action='car_mst_frame.jsp';
		document.form1.target='d_content';
		document.form1.submit();		
<%		}else{%>
		alert('오류발생!!');
