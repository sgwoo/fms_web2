...
<%
	...
	Vector ins_hs = aih_db.getInsurHList(br_id, gubun1, gubun2, gubun3, gubun4, st_dt, end_dt, s_kd, t_wd, sort_gubun, asc);
	    ...
	    <td class='line' width='460' id='td_con' style='position:relative;'>			
          ...
          <%		for (int i = 0 ; i < ins_h_size ; i++){
			...
			if(String.valueOf(ins_h.get("CAR_NO")).substring(4,5).equals("허")) tm_st = "2";%>
                <tr> 
                    <td <%if(ins_h.get("USE_YN").equals("N")){%>class='is'<%}%> width=60 align='center'><a name="<%=i+1%>"><%=i+1%>
                    ...
                    <td <%if(ins_h.get("USE_YN").equals("N")){%>class='is'<%}%> width=100 align='center'><a href="javascript:parent.view_ins_h('<%=ins_h.get("RENT_MNG_ID")%>','<%=ins_h.get("RENT_L_CD")%>','<%=ins_h.get("CAR_MNG_ID")%>','<%=ins_h.get("EXT_ID")%>',  '<%=i%>')" onMouseOver="window.status=''; return true"><%=ins_h.get("RENT_L_CD")%></a></td>
                    ...
                    <td <%if(ins_h.get("USE_YN").equals("N")){%>class='is'<%}%> width=90 align='center'><span title='<%=ins_h.get("CAR_NO")%>'><%=ins_h.get("CAR_NO")%></span></td>
                </tr>
                <%	}%>		  
                <tr> 
	    ...
	    <td class='line' width='1190'>			
            <table border="0" cellspacing="1" cellpadding="0" width='100%'>
              <%		for (int i = 0 ; i < ins_h_size ; i++){
    			...
    			if ( Long.parseLong(String.valueOf(ins_h.get("PAY_AMT"))) > 0) { 
    				total_amt3 = Long.parseLong(String.valueOf(ins_h.get("REQ_AMT"))) - Long.parseLong(String.valueOf(ins_h.get("PAY_AMT")));
    			 }
                ...
                <tr> 
                    ...
                    <td <%if(ins_h.get("USE_YN").equals("N")){%>class='is'<%}%> width=80 align='center'><%=ins_h.get("PAY_GU")%></td>										
                    ...
                    <td <%if(ins_h.get("USE_YN").equals("N")){%>class='is'<%}%> width=80 align='center'><%=ins_h.get("EXT_DT")%></td>					
              ...
              <%
    				total_su = total_su + 1;
    				total_amt = total_amt + Long.parseLong(String.valueOf(ins_h.get("REQ_AMT")));
    				total_amt2 = total_amt2 + Long.parseLong(String.valueOf(ins_h.get("PAY_AMT")));
