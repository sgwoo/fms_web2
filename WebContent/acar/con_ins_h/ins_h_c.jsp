...
<%
	...
	//연체료 셋팅
	boolean flag = aih_db.calDelay(c_id);
	//통계
	IncomingBean ins_h = aih_db.getInsurHScdStat(m_id, l_cd, c_id);
...
<!--
	//입금처리&입금취소&변경
	function change_scd(cmd, pay_yn, idx, accid_id, seq_no){
...
//		if('<%=user_id%>'!='000029' && cmd == 'p' && fm.req_amt.value != fm.pay_amt.value){
//			alert('청구금액과 입금금액 틀린 경우 입금처리 테스트를 해야합니다. 전산팀 정현미대리에게 얘기하세요.');
//			return;
//		}
		...
		fm.action='/acar/con_ins_h/mod_scd_u.jsp';
		fm.target = 'i_no';
//		fm.target = '_blank';		
		fm.submit();		
	}
	...
	function view_cont(accid_id, accid_st){
		...
		window.open("/acar/con_ins_m/sub_cont.jsp?gubun=h&m_id="+m_id+"&l_cd="+l_cd+"&c_id="+c_id+"&accid_id="+accid_id+"&accid_st="+accid_st+"&title="+title, "CONT", "left=100, top=100, width=700, height=300 scrollbars=yes");
	}
	...
	function go_to_list(){
		...
		location = "/acar/con_ins_h/ins_h_frame_s.jsp?auth_rw="+auth_rw+"&br_id="+br_id+"&user_id="+user_id+"&gubun1="+gubun1+"&gubun2="+gubun2+"&gubun3="+gubun3+"&gubun4="+gubun4+"&st_dt="+st_dt+"&end_dt="+end_dt+"&s_kd="+s_kd+"&t_wd="+t_wd+"&sort_gubun="+sort_gubun+"&asc="+asc+"&idx="+idx;
	}	
...
<!--        	  <select name="gubun1" onChange="javascript:page_move()">
          	  ...
          	  <option value="5" <%if(gubun1.equals("5")){%>selected<%}%>><%=title%></option>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
    ...
    <tr>
    	<td colspan='2'>
    	  <iframe src="/acar/con_ins_h/ins_h_c_in.jsp?auth=<%=auth%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&m_id=<%=m_id%>&l_cd=<%=l_cd%>&c_id=<%=c_id%>&accid_id=<%=accid_id%>&serv_id=<%=serv_id%>&brch_id=<%=brch_id%>" name="i_in" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
    	  </iframe>
    	</td>
    </tr>
    <tr>		
