...
<script language='javascript'>
	...
	function bank_doc_reg(){	
		...
		var sh_fm = parent.document.form1;		
		
		fm.doc_id.value 	= sh_fm.doc_id.value;
//		fm.doc_dt.value 	= sh_fm.doc_dt.value;
		fm.doc_dt.value		= fm.exp_dt.value;
		fm.fund_yn.value		= fm.fund_yn.value;
		...
		fm.action = "bank_doc_reg_cg_a.jsp";
		fm.target = "i_no";
		fm.submit()
	}	
</script>
...
<input type='hidden' name='doc_dt' value='<%=exp_dt%>'>
...
<% 	
	...
	float  tax_amt = 0;  //취득세 -- 모닝,마티즈 자가용 취득세 제외
	...
	for(int i=0; i<vid.length;i++){
		...
		car_st = String.valueOf(loan.get("CAR_ST"));
				         
	         	// 추후 데이타 화 
	         	//롯데캐피탈 , 외환캐피탈, 효성캐피탈, 우리파이낸셜, KT 캐피탈, IBK캐피탈, HK저축은행, 무림캐피탈, 수협 , SBI 3저축은행(0068) , 한국씨티그룹(0069), 세람저축(0072), SBI 4 저축(0073), 하나저축(0074) , aj인베스트(0076);
	         	//현대저축(0081), 페퍼저축(0083), 흥국저축(0084) ,kb저축(0085) , 남양저축(0088), 인성저축은행(0089), 신한저축은행(0090), gkstlswjcnrdmsgod(0094) , 오투저축(0100) , 제이티(0101) , bnk캐피탈(0102) , kb국민(0103) , 스타저축(0104)
	         	//한화저축(0111), ibk(0113) , 롯데카드(0114),금화저축(0115)
		 if (		ch_bank_id.equals("0038") || ch_bank_id.equals("0040") || ch_bank_id.equals("0039") || ch_bank_id.equals("0051") || ch_bank_id.equals("0057") || ch_bank_id.equals("0059")  
		        ...
		        loan_amt = AddUtil.parseFloat(String.valueOf(loan.get("CAR_AMT"))) *  90/100;
				 ...
				 || ch_bank_id.equals("0079") || ch_bank_id.equals("0056") || ch_bank_id.equals("0091") || ch_bank_id.equals("0116") || ch_bank_id.equals("0093") ) { //신한캐피탈 or 신한은행 or 하나은행 or 우리은행 , 한국캐피탈 , 오릭스(0086)  , 메리츠(0079) , KDB(0010), 애큐온(0056- 20200212변경), db캐피탈(0091-202009변경)
				loan_amt = AddUtil.parseFloat(String.valueOf(loan.get("CAR_AMT"))) *  90/100;
				 ...
				 || ch_bank_id.equals("0087")  ) { //산은캐피탈 , BS캐피탈, 메리츠, 한화생명보험, 농심캐피탈, 메리츠캐피탈(0079),  대신저축(0082)  , orix (0086),  롯데오토리스(0093), 
				//	loan_amt = Math.round(AddUtil.parseFloat(String.valueOf(loan.get("CAR_AMT"))) /  AddUtil.parseFloat("1.1"));   //오케이아프로(0087)
				loan_amt = AddUtil.parseFloat(String.valueOf(loan.get("CAR_AMT"))) /  AddUtil.parseFloat("1.1");   //오케이아프로(0087)
		} else if ( ch_bank_id.equals("0043") || ch_bank_id.equals("0044") || ch_bank_id.equals("0041") ||  ch_bank_id.equals("0002") || ch_bank_id.equals("0063") ||  ch_bank_id.equals("0118") ) { //두산캐피탈, nh캐피탈 , 하나캐피탈 , 외환은행, 우리금융
	    ...
	    } else if ( ch_bank_id.equals("0037") || ch_bank_id.equals("0005") ) { //산업은행 - 20180508
	    ...
	    } else if ( ch_bank_id.equals("0046")  ) { //우리카드  - 20210401	
	    ...
	    } else if ( ch_bank_id.equals("0033") ) { //광주은행,전북은행
				loan_a_amt = AddUtil.parseFloat(String.valueOf(loan.get("CAR_S_AMT"))) * 100/100;
				loan_a_amt1 = (int) loan_a_amt;
		        loan_a_amt1 = AddUtil.ten_th_rnd(loan_a_amt1); 	 	            	 	         
		} else if ( ch_bank_id.equals("0011") || ch_bank_id.equals("0108") ) { //현대캐피탈 구입가격 대출, 현대커머셜(0108)
		...
		if (ch_bank_id.equals("0043") || ch_bank_id.equals("0044") || ch_bank_id.equals("0041") || ch_bank_id.equals("0002") || ch_bank_id.equals("0011") ||  ch_bank_id.equals("0063")
		 || ch_bank_id.equals("0029") || ch_bank_id.equals("0033") || ch_bank_id.equals("0046") || ch_bank_id.equals("0118")    ) {
			loan_amt1 = loan_a_amt1;
		} else if (ch_bank_id.equals("0026")   ) { 
			loan_amt1 = loan_amt1;
	//	} else if (ch_bank_id.equals("0018")  || ch_bank_id.equals("0037")   ) { //삼성제외 -20191213
        ...
        int ep2 = 0;
            ...
            //산은, 두산, 하나, 삼성카드, 무림는  은 취득세 - 산은제외(20130911) , 메리츠(0079) 20160217
	...
	//System.out.println("car_use: "+String.valueOf(loan.get("CAR_NO")).indexOf("허"));			

	
		   	 if ((String.valueOf(loan.get("CAR_NM")).equals("모닝") ||  String.valueOf(loan.get("CAR_NM")).equals("스파크")) && (ep == 2 ||   ep3 == 2 ||ep4 == 2  ) ) {
			//  if ((String.valueOf(loan.get("CAR_NM")).equals("모닝") ||  String.valueOf(loan.get("CAR_NM")).equals("스파크")) && ep == 2) {
...
%>		  
 		        <tr align="center"> 
        			...
        			<input type='hidden' name='car_f_amt' value='<%=loan.get("CAR_F_AMT")%>'> <!-- 우리카드(0046)만 사용 -->
        			<input type='hidden' name='car_dc_amt' value='<%=loan.get("CAR_DC_AMT")%>'> <!-- 우리카드(0046)만 사용 -->
