...
function make_chunggu(){
	...
	if(fm.gubun1.value  != '2'){ alert("정산일로 조회하세요!!."); return; }		
	if(fm.s_kd.value != '3'){ alert("정산회차를 선택하세요!!."); return; }		
	if(fm.t_wd.value == ''){ alert("정산회차를 입력하세요!!."); return; }		
	
	if(!confirm('해당정비요청건을  결재청구하시겠습니까?')){	return;	}
	
	fm.target = "i_no";
	fm.action = "cus_samt_req_a.jsp";
	fm.submit();	
}	
...
<table width="100%" border="0" cellspacing="1" cellpadding="0">
  ...
  <tr> 
    <td><iframe src="./cus_samt_s_sc_in.jsp?acct=<%=acct%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&gubun1=<%=gubun1%>&s_year=<%=s_year%>&s_mon=<%=s_mon%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort=<%=sort%>&asc=<%=asc%>" name="inner" width="100%" height="<%=height - 30 %>" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe></td>
  </tr>
  <tr> 
