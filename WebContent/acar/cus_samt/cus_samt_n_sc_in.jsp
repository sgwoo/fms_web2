				...
				<td style="width: 620px;">
					<div style="width: 620px;">
						<table class="inner_top_table left_fix" style="height: 60px;">
							...
							<tr> 
								<td rowspan=2 class='title title_border' style="width: 65px;">연번</td>
								...
								<td colspan=2  class='title title_border border_bottom_none' style="width: 70px;">과실비율</td>
								<td rowspan=2 class='title title_border' style="width: 35px;">사진</td>
								<td rowspan=2 class='title title_border' style="width: 90px;">차량번호</td>								
	             			</tr>
	              			<tr>
								<td class='title title_border' style="width: 35px;">당사</td>
								<td class='title title_border border_right_none' style="width: 35px;">상대</td>  
             	  			</tr>
						</table>
					</div>
				</td>
			       		...
			       		<%if (ser_size > 0) {%>	
							...
							%>
							  <tr style="height: 25px;"> 
								<td class="content_border center" style="width: 65px;">
			        		    ...
			        		    <td class="content_border center" style="width: 35px;"><%=exp.get("OUR_FAULT_PER")%></td>	
			        		    <td class="content_border center" style="width: 35px;"><%=Math.abs(AddUtil.parseInt(String.valueOf(exp.get("OUR_FAULT_PER")))-100)%></td>	
			        		    <td class="content_border center" style="width: 35px;"><%=exp.get("PIC_CNT")%></td>	
			        		    <td class="content_border center" style="width: 90px;"><%=exp.get("CAR_NO")%></td>
							</tr>
							<%}%>
							<tr> 
								<td class="title content_border" align='center'></td>
				        		<td class="title content_border" colspan=8 align='center'>합계</td>
				                <td class="title content_border">&nbsp;</td>
							</tr>
						<%} else {%>
							<tr>
								<td colspan=10 class="content_border" align='center'>등록된 데이타가 없습니다</td>
							</tr>	              
		 				<%}	%>
						</table>
					</div>
						...
						<%	if(ser_size > 0){%>
							...
							%>
							  <tr style="height: 25px;"> 
								...
								<td class="content_border center" width='71'><%=exp.get("IPGODT")%></td>
						        <td class="content_border center" width='71'><%=exp.get("CHULGODT")%></td>
						        <td class="content_border center" width='60'><%=c_db.getNameById(String.valueOf(exp.get("CHECKER")), "USER")%></td>			  
							  	<td class="content_border left" width='170'>
							  		&nbsp;<%=Util.subData(String.valueOf(exp.get("CLIENT_NM")), 12)%>
							  	</td>
				  			    <td class="content_border left" width='200'>				  			    	
					  			    <%if (String.valueOf(exp.get("CNT")).equals("1")) {%>
				  			 	...
				  			 	<td class="content_border right" width='80'><%=AddUtil.parseDecimal(exp.get("SUP_AMT"))%>&nbsp;</td>      
				                <td class="content_border right" width='80'><%=AddUtil.parseDecimal(v_labor - vc_rate)%>&nbsp;</td>
				                <td class="content_border right" width='80'><%=AddUtil.parseDecimal(v_amt)%>&nbsp;</td>
				                <td class="content_border right" width='60'><%=AddUtil.parseDecimal(v_dc_sup_amt)%>&nbsp;</td>
				                <td class="content_border right" width='70'><%=AddUtil.parseDecimal(v_ext_amt)%>&nbsp;</td>
				                <td class="content_border right" width='80'><%=AddUtil.parseDecimal(v_labor - vc_rate + v_amt  - v_dc_sup_amt -  v_ext_amt  )%>&nbsp;</td>
				                <td class="content_border right" width='80'><%=AddUtil.parseDecimal(exp.get("CUST_AMT"))%>&nbsp;</td>
				                <td class="content_border right" width='70'><%=AddUtil.parseDecimal(exp.get("CLS_AMT"))%>&nbsp;</td>				                
		                	  	<td class="content_border center">
				                	<%if ( !String.valueOf(exp.get("SSS_ST")).equals("0") ) {%>-
					                <%} else {%>
				                ...
				                <td class="content_border right"><%=exp.get("ACCT_DT")%>&nbsp;</td>
	                	  	</tr>
	                	  	<%
			             		amt[0]   = amt[0] + v_labor;
			             		amt[1]   = amt[1] + v_amt;
			               	...
			               	<tr> 
				                <td class="title content_border" colspan=7></td>
				                ...
				                <td width='60' class="title content_border right"><%=Util.parseDecimal(amt[10])%>&nbsp;</td>
				                ...
				                <td width='70' class="title content_border right"><%=Util.parseDecimal(amt[12])%>&nbsp;</td>
				                <td class="title content_border center" width='40'></td>
		                	  	<td class="title content_border center" width='70'></td>
			              	</tr>
			            <%} else  {%>  
					       	<tr>
								<td class="content_border" colspan="17" align='center'>&nbsp;</td>
							</tr>
			   			<%}	%>
						</table>
					</div>
