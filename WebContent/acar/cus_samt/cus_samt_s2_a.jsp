...
<%
	...
	String[] c_id = request.getParameterValues("car_mng_id");	
	String[] s_id = request.getParameterValues("serv_id");	
	
	String[] j_st = request.getParameterValues("jung_st");	//정산회차
	String[] s_dt = request.getParameterValues("set_dt");	 // 정산일 - 정비일의 월의 마지막일자 
	
	String[] r_labor = request.getParameterValues("r_labor");  //공임
  	String[] v_amt = request.getParameterValues("v_amt");      //부픔
  	String[] v_g_dc_amt = request.getParameterValues("v_g_dc_amt");  //공임 dc
 	String[] v_ext_amt = request.getParameterValues("v_ext_amt");  //공임 dc
 	String[] v_dc_amt = request.getParameterValues("v_dc_amt");  //dc -할인율 
		
	for(int i=0; i < vt_size; i++){
								
		//정비테이블에 정산일 및 회차 등록 							
		if(!c_id[i].equals("")){
		//	if ( j_st[i].equals("4")) j_st[i] = "3";
		//	System.out.println("c_id[i]= " + c_id[i] + ", s_id[i]= " + s_id[i] + ", s_dt[i]= " + s_dt[i] + ", j_st[i]= "+ j_st[i]);
			 flag1 = cs_db.updateServiceSetDt( c_id[i], s_id[i], s_dt[i], j_st[i], user_id);	//정산확정
		}
	...
	//		System.out.println("s_jung_st="+ s_jung_st);
		...
		s_r_labor = r_labor[i]	==null?0 :AddUtil.parseDigit(r_labor[i]);  //공임 
		s_v_amt = v_amt[i]	==null?0 :AddUtil.parseDigit(v_amt[i]);  //부픔 
		s_v_g_dc_amt = v_g_dc_amt[i]	==null?0 :AddUtil.parseDigit(v_g_dc_amt[i]);  //공임 dc
		s_v_ext_amt = v_ext_amt[i]	==null?0 :AddUtil.parseDigit(v_ext_amt[i]);  //선납
		s_v_dc_amt = v_dc_amt[i]	==null?0 :AddUtil.parseDigit(v_dc_amt[i]);  //dc -할인율  (0) 
