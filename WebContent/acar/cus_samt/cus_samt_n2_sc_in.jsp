...
<%
	...
	String sort = request.getParameter("sort")==null?"6":request.getParameter("sort");
				...
				<td style="width: 940px;">
					<div style="width: 940px;">
						<table class="inner_top_table left_fix" style="height: 60px;">
				             ...
				             <tr> 
			                    <td width='7%'   rowspan=2  class='title title_border'>연번</td>
			            	    <td width='4%'  rowspan=2 class='title title_border'><input type="checkbox" name="ch_all" value="Y" onclick="javascript:AllSelect();"></td>		
			                    <td width='7%'  rowspan=2 class='title title_border'>청구일</td>
			                    <td width='7%'  rowspan=2 class='title title_border'>견적서</td>
			                    <td width='14%'  rowspan=2 class='title title_border'>업체</td>
			                    <td width='7%'  rowspan=2 class='title title_border'>정산</td>
			                    <td width='6%'  rowspan=2 class='title title_border'>사고<br>유형</td>  
			                    <td width='10%'  rowspan=2 class='title title_border'>구분</td>
			                    <td colspan=2  class='title title_border' >과실비율</td>
			                    <td width='8%'  rowspan=2 class='title title_border'>차량번호</td> 
			                    <td width='14%'  rowspan=2 class='title title_border'>차명</td>                  
			                    <td width='7%'  rowspan=2 class='title title_border'>정비일</td>                               
			                </tr>
			                <tr>
			                    <td width='3%' class='title title_border'>당사</td>
			                    <td width='3%' class='title title_border'>상대</td>  
			                </tr>
						...
						<table class="inner_top_table table_layout" style="height: 60px;">
			                ...
			                <tr>			                         
			                   ...
			                   <td width='60'  rowspan=2 class='title title_border'>등록일</td>
			                    ...
			                    <td width='80'  class="title title_border" rowspan=2 >사전정산일</td>                 
			                </tr>
			                <tr>             
			...
			<%	if(ser_size > 0){%>
			         <%for(int i = 0 ; i < ser_size ; i++){
			                ...
			                <tr style="height: 25px;"> 
			                    <td width='7%' class='center content_border'>			                 
			                      <% if ( exp.get("JUNG_ST").equals("미정산") && String.valueOf(exp.get("PRE_SET_DT")).equals("")  ) { %>
			                    ...
			                    <%if(exp.get("USE_YN").equals("N")){%>   	(해약) 
			                  	<%}%>
			                  	<% if ( exp.get("JUNG_ST").equals("미정산") && String.valueOf(exp.get("PRE_SET_DT")).equals("") ) { %> </a> <% } %>
			                    </td>
			                      <td width='4%' class='center content_border'><input type="checkbox" name="ch_cd" value="<%=exp.get("CAR_MNG_ID")%>^<%=exp.get("SERV_ID")%>^"  <% if (!String.valueOf(exp.get("PRE_SET_DT")).equals("")  ) {%>disabled <% } %> ></td>
			                      <td width='7%' class='center content_border'><%=exp.get("REQ_DT")%></td>
			                      <td width='7%' class='center content_border'>
			                      ...
			                      <%}%>
			                     <td width='14%' class='center content_border'><%=AddUtil.subData(String.valueOf(exp.get("OFF_NM")), 8)%></td>                     
			                    <td width='7%' class='center content_border'><%=exp.get("JUNG_ST")%></td>
			                    ...
			                    <td width='3%' class='center content_border'><%=exp.get("OUR_FAULT_PER")%></td>
			                    <td width='3%' class='center content_border'><%=Math.abs(AddUtil.parseInt(String.valueOf(exp.get("OUR_FAULT_PER")))-100)%></td>
			                    <td width='8%' class='center content_border'><%=exp.get("CAR_NO")%></td>
			                    <td width='14%' class='left content_border'>&nbsp;<%=Util.subData(String.valueOf(exp.get("CAR_NM")), 10)%></td>
				                <td width='7%' class='center content_border'><%=exp.get("SERV_DT")%></td>
			                            
			                </tr>
			       <%		}%>
			                <tr> 
			                    <td colspan="13" class='title content_border center' >합계&nbsp;</td>
			                </tr>
			           <%} else  {%>  
						 <tr>
				...
				%>		
				           <tr style="height: 25px;"> 				             
			                   ...
			                   <td width='70' class='right content_border'><%=AddUtil.parseDecimal(v_dc_sup_amt)%>&nbsp;</td>  
			                    ...
			                    <td width='80' class='center content_border'><%=String.valueOf(exp.get("SET_DT"))%></td> 
			                     <td width='80' class='center content_border'><%=String.valueOf(exp.get("PRE_SET_DT"))%></td>
			      ...
			      <%} else  {%>  
					       <tr>
						        <td width="1440" colspan="16" class='center content_border'>&nbsp;</td>
						   </tr>	              
			   <%}	%>
				          </table>
				        </div>
