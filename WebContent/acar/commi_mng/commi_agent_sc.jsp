...
<%@ page import="java.util.*, acar.util.*, acar.con_ins.*, acar.user_mng.*"%>
...
<!--
	//지급수수료 세부내용 보기
	function view_commi(car_off_id)
	{
		var fm = document.form1;
		fm.car_off_id.value=car_off_id;
		fm.target='d_content';
		fm.action='/acar/commi_mng/commi_agent_c.jsp';
		fm.submit();
	}
	...
	function execl(){
		var fm = document.form1;
		fm.target ='_blank';
		fm.action = 'commi_agent_sc_in_excel.jsp';
		fm.submit();
	}		
//-->
</script>
	...
	<tr>
		<td>
			<table border="0" cellspacing="0" cellpadding="0" width=100%>
				<tr>
					<td>
						<iframe src="/acar/commi_mng/commi_agent_sc_in.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>&f_list=<%=f_list%>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
						</iframe>
					</td>
				</tr>
			</table>
