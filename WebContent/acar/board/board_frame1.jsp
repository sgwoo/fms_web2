...
<frameset rows="130, *, 0" border=1>
	<frame name="c_body"    src="./board_sh.jsp?br_id=<%=br_id%>&user_id=<%=user_id%>&member_id=<%=member_id%>&client_id=<%=client_id%>&r_site=<%=r_site%>&auth_rw=<%=auth_rw%>&gubun=<%=gubun%>&s_yy=<%=s_yy%>&s_mm=<%=s_mm%>&s_dd=<%=s_dd%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&idx=<%=idx%>" marginwidth=10 marginheight=10 scrolling='no' noresize>
	<frame name="c_foot"    src="./board_sc.jsp?br_id=<%=br_id%>&user_id=<%=user_id%>&member_id=<%=member_id%>&client_id=<%=client_id%>&r_site=<%=r_site%>&auth_rw=<%=auth_rw%>&gubun=<%=gubun%>&s_yy=<%=s_yy%>&s_mm=<%=s_mm%>&s_dd=<%=s_dd%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&idx=<%=idx%>" marginwidth=10 marginheight=10 scrolling="auto">
