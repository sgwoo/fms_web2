...
<%
	...
	Vector buss1 = ad_db.getFeeScdSum("0001", "", "3", "3", "3");
	int bus_size1 = buss1.size();
	
	Vector buss2 = ad_db.getFeeScdSum("0002", "", "3", "3", "3");
	int bus_size2 = buss2.size();
		
	Vector buss3 = ad_db.getFeeScdSum("9999", "", "3", "3", "3");
	int bus_size3 = buss3.size();
	
	Vector buss4 = ad_db.getFeeScdSum("0007", "", "3", "3", "3");
	int bus_size4 = buss4.size();
					
	Vector buss5 = ad_db.getFeeScdSum("0008", "", "3", "3", "3");
...
<!--
	...
	function stat_search(mode, bus_id2){	
		...
		parent.location.href = "/acar/arrear/arrear_frame_s.jsp?gubun1=1&gubun2=3&gubun3=3&gubun4=3&s_kd=8&t_wd="+bus_id2;		
	}		
//-->
</script>
    ...
    <tr>
	    <td class='line' width='30%' id='td_con' style='position:relative;'> 
          ...
          <%for (int i = 0 ; i < bus_size1 ; i++){
            ...
            <%	if(i==0){%>
                    ...
                    <%	}else{}%>
                    <td width=30% align="center"><a href="javascript:stat_search('d','<%=ht.get("BUS_ID2")%>');" onMouseOver="window.status=''; return true"><font color="#0000FF"><%=ht.get("BUS_NM2")%></font></a></td>
                    <td width=30% align="center"><%=AddUtil.ChangeDate2(String.valueOf(ht.get("ENTER_DT")))%> </td>
		 ...
		 <%  if  (bus_size1 > 0 ) { %>  	
          ...
          <%for (int i = 0 ; i < bus_size2 ; i++){
                ...
                <tr> 
            <%	if(i==0){%>
                    <td align="center" width=40% rowspan="<%=bus_size2%>">고객지원팀</td>
                    <%	}else{}%>
		  ...
		  <%  if  (bus_size2 > 0 ) { %>  		
           ...
           <%		
            ...
            <%for (int i = 0 ; i < bus_size3 ; i++){
                ...
                <tr> 
            <%	if(i==0){%>
                    <td align="center" width=40% rowspan="<%=bus_size3%>">기타 </td>
                    <%	}else{}%>
          ...
          <%for (int i = 0 ; i < bus_size4 ; i++){
                ...
                <tr> 
            <%	if(i==0){%>
                    <td align="center" width=40% rowspan="<%=bus_size4%>">부산지점</td>
                    <%	}else{}%>
                    <td align="center" width=30%><a href="javascript:stat_search('d','<%=ht.get("BUS_ID2")%>');" onMouseOver="window.status=''; return true"><font color="#0000FF"><%=ht.get("BUS_NM2")%></font></a></td>
                    <td align="center" width=30%><%=AddUtil.ChangeDate2(String.valueOf(ht.get("ENTER_DT")))%> </td>
          ...
          <%for (int i = 0 ; i < bus_size5 ; i++){
                ...
                <tr> 
            <%	if(i==0){%>
                    <td align="center"  rowspan="<%=bus_size5%>">대전지점</td>
                    <%	}else{}%>
                    ...
                    <td align="center" width="76"><%=AddUtil.ChangeDate2(String.valueOf(ht.get("ENTER_DT")))%> </td>
	    ...
	    <td class='line' width='70%'>
          ...
          <%for (int i = 0 ; i < bus_size1 ; i++){
				...
				%>
                <tr> 
                    <td align="right" width="16%"><%=Util.parseDecimal(AddUtil.parseInt(String.valueOf(ht.get("TOT_D_CNT"))) + AddUtil.parseInt(String.valueOf(ht.get("TOT_H_CNT")))) %>건&nbsp;</td>
                    <td align="right" width="17%"><%=Util.parseDecimal(AddUtil.parseInt(String.valueOf(ht.get("TOT_D_AMT"))) + AddUtil.parseInt(String.valueOf(ht.get("TOT_H_AMT")))) %>원&nbsp;</td>
                    <td align="right" width="16%"><%=Util.parseDecimal(ht.get("TOT_D_CNT"))%>건&nbsp;</td>
                    <td align="right" width="17%"><%=Util.parseDecimal(ht.get("TOT_D_AMT"))%>원&nbsp;</td>
                    <td align="right" width="16%"><%=Util.parseDecimal(ht.get("TOT_H_CNT"))%>건&nbsp;</td>
                    <td align="right" width="18%"><%=Util.parseDecimal(ht.get("TOT_H_AMT"))%>원&nbsp;</td>																																				
                </tr>
          <%		
			}%>
		  <%  if  (bus_size1 > 0 ) { %>  		
                <tr> 
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(t_cnt[0])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(t_amt[0])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(d_cnt[0])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(d_amt[0])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(h_cnt[0])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=18%><%=Util.parseDecimal(h_amt[0])%>원&nbsp;</td>																																				
                </tr>
           <%		
			}%>
          <%for (int i = 0 ; i < bus_size2 ; i++){
		 ...
		 <%  if  (bus_size2 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(t_cnt[1])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(t_amt[1])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(d_cnt[1])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(d_amt[1])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(h_cnt[1])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=18%><%=Util.parseDecimal(h_amt[1])%>원&nbsp;</td>																																				
                </tr>
             <%		
			}%>
           <%for (int i = 0 ; i < bus_size3 ; i++){
		 ...
		 <%  if  (bus_size3 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(t_cnt[2])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(t_amt[2])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(d_cnt[2])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(d_amt[2])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(h_cnt[2])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=18%><%=Util.parseDecimal(h_amt[2])%>원&nbsp;</td>																																				
                </tr>
             <%		
			}%>
		 ...
		 <%  if  (bus_size4 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(t_cnt[3])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(t_amt[3])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(d_cnt[3])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(d_amt[3])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(h_cnt[3])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=18%><%=Util.parseDecimal(h_amt[3])%>원&nbsp;</td>																																				
                </tr>
           <%		
			}%>
          <%for (int i = 0 ; i < bus_size5 ; i++){
		 ...
		 <%  if  (bus_size5 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(t_cnt[4])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(t_amt[4])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(d_cnt[4])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=17%><%=Util.parseDecimal(d_amt[4])%>원&nbsp;</td>
                    <td class=title style='text-align:right' width=16%><%=Util.parseDecimal(h_cnt[4])%>건&nbsp;</td>
                    <td class=title style='text-align:right' width=18%><%=Util.parseDecimal(h_amt[4])%>원&nbsp;</td>																																				
                </tr>
            <%		
			}%> 
                <tr> 
                    <td class=title_p style='text-align:right' width=16%><%=Util.parseDecimal(t_cnt[0]+t_cnt[1]+t_cnt[2]+t_cnt[3]+t_cnt[4])%>건&nbsp;</td>
                    <td class=title_p style='text-align:right' width=17%><%=Util.parseDecimal(t_amt[0]+t_amt[1]+t_amt[2]+t_amt[3]+t_amt[4])%>원&nbsp;</td>
                    <td class=title_p style='text-align:right' width=16%><%=Util.parseDecimal(d_cnt[0]+d_cnt[1]+d_cnt[2]+d_cnt[3]+d_cnt[4])%>건&nbsp;</td>
                    <td class=title_p style='text-align:right' width=17%><f%=Util.parseDecimal(d_amt[0]+d_amt[1]+d_amt[2]+d_amt[3]+d_amt[4])%>원&nbsp;</td>
                    <td class=title_p style='text-align:right' width=16%><%=Util.parseDecimal(h_cnt[0]+h_cnt[1]+h_cnt[2]+h_cnt[3]+h_cnt[4])%>건&nbsp;</td>
                    <td class=title_p style='text-align:right' width=18%><%=Util.parseDecimal(h_amt[0]+h_amt[1]+h_amt[2]+h_amt[3]+h_amt[4])%>원&nbsp;</td>																																				
                </tr>
            </table>
	    </td>
	</tr>
