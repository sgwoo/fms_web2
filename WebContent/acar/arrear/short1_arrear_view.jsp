		 ...
		 <%  if  (bus_size1 > 0 ) { %>  	
		  ...
		  <%  if  (bus_size2 > 0 ) { %>  		
           ...
           <%		
            ...
            <%for (int i = 0 ; i < bus_size3 ; i++){
                ...
                <tr> 
            <%	if(i==0){%>
                    <td align="center" rowspan="<%=bus_size3%>">기타 </td>
            <%	}else{}%>
		  ...
		  <%  if  (bus_size5 > 0 ) { %>  		
           ...
           <%		
                ...
                <tr> 
                    <td class="title_p" colspan="3">&nbsp;총합계</td>
	    ...
	    <td class='line' width='70%'>
          ...
          <%for (int i = 0 ; i < bus_size1 ; i++){
                ...
                <tr> 
                    ...
                    <td align="right" width="18%"><%=Util.parseDecimal(AddUtil.parseInt(String.valueOf(ht.get("TOT_D_AMT"))) + AddUtil.parseInt(String.valueOf(ht.get("TOT_H_AMT")))) %>원&nbsp;</td>
                    ...
                    <td align="right" width="17%"><%=Util.parseDecimal(ht.get("TOT_H_AMT"))%>원&nbsp;</td>																																				
                </tr>
          <%		
			}%>
		  <%  if  (bus_size1 > 0 ) { %>  		
                <tr> 
                    <td class="title" style='text-align:right'><%=Util.parseDecimal(t_cnt[0])%>건&nbsp;</td>
                    <td class="title" style='text-align:right'><%=Util.parseDecimal(t_amt[0])%>원&nbsp;</td>
                    <td class="title" style='text-align:right'><%=Util.parseDecimal(d_cnt[0])%>건&nbsp;</td>
                    <td class="title" style='text-align:right'><%=Util.parseDecimal(d_amt[0])%>원&nbsp;</td>
                    <td class="title" style='text-align:right'><%=Util.parseDecimal(h_cnt[0])%>건&nbsp;</td>
                    <td class="title" style='text-align:right'><%=Util.parseDecimal(h_amt[0])%>원&nbsp;</td>																																				
                </tr>
           <%		
			}%>
          <%for (int i = 0 ; i < bus_size2 ; i++){
				...
				%>
                <tr> 
                    <td align="right"><%=Util.parseDecimal(AddUtil.parseInt(String.valueOf(ht.get("TOT_D_CNT"))) + AddUtil.parseInt(String.valueOf(ht.get("TOT_H_CNT")))) %>건&nbsp;</td>
                    <td align="right"><%=Util.parseDecimal(AddUtil.parseInt(String.valueOf(ht.get("TOT_D_AMT"))) + AddUtil.parseInt(String.valueOf(ht.get("TOT_H_AMT")))) %>원&nbsp;</td>
                    <td align="right"><%=Util.parseDecimal(ht.get("TOT_D_CNT"))%>건&nbsp;</td>
                    <td align="right"><%=Util.parseDecimal(ht.get("TOT_D_AMT"))%>원&nbsp;</td>
                    <td align="right"><%=Util.parseDecimal(ht.get("TOT_H_CNT"))%>건&nbsp;</td>
                    <td align="right"><%=Util.parseDecimal(ht.get("TOT_H_AMT"))%>원&nbsp;</td>																																				
                </tr>
          <%		
			}%>
		 <%  if  (bus_size2 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_cnt[1])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_amt[1])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_cnt[1])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_amt[1])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_cnt[1])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_amt[1])%>원&nbsp;</td>																																				
                </tr>
             <%		
			}%>
           <%for (int i = 0 ; i < bus_size3 ; i++){
		 ...
		 <%  if  (bus_size3 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_cnt[2])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_amt[2])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_cnt[2])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_amt[2])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_cnt[2])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_amt[2])%>원&nbsp;</td>																																				
                </tr>
             <%		
			}%>
		 ...
		 <%  if  (bus_size4 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_cnt[3])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_amt[3])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_cnt[3])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_amt[3])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_cnt[3])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_amt[3])%>원&nbsp;</td>																																				
                </tr>
           <%		
			}%>
          <%for (int i = 0 ; i < bus_size5 ; i++){
		 ...
		 <%  if  (bus_size5 > 0 ) { %>  			
                <tr> 
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_cnt[4])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(t_amt[4])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_cnt[4])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(d_amt[4])%>원&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_cnt[4])%>건&nbsp;</td>
                    <td class=title style='text-align:right'><%=Util.parseDecimal(h_amt[4])%>원&nbsp;</td>																																				
                </tr>
            <%		
			}%> 
                <tr> 
                    <td class=title_p style='text-align:right'><%=Util.parseDecimal(t_cnt[0]+t_cnt[1]+t_cnt[2]+t_cnt[3]+t_cnt[4])%>건&nbsp;</td>
                    <td class=title_p style='text-align:right'><%=Util.parseDecimal(t_amt[0]+t_amt[1]+t_amt[2]+t_amt[3]+t_amt[4])%>원&nbsp;</td>
                    <td class=title_p style='text-align:right'><%=Util.parseDecimal(d_cnt[0]+d_cnt[1]+d_cnt[2]+d_cnt[3]+d_cnt[4])%>건&nbsp;</td>
                    <td class=title_p style='text-align:right'><%=Util.parseDecimal(d_amt[0]+d_amt[1]+d_amt[2]+d_amt[3]+d_amt[4])%>원&nbsp;</td>
                    <td class=title_p style='text-align:right'><%=Util.parseDecimal(h_cnt[0]+h_cnt[1]+h_cnt[2]+h_cnt[3]+h_cnt[4])%>건&nbsp;</td>
                    <td class=title_p style='text-align:right'><%=Util.parseDecimal(h_amt[0]+h_amt[1]+h_amt[2]+h_amt[3]+h_amt[4])%>원&nbsp;</td>																																				
                </tr>
            </table>
	    </td>
	</tr>
