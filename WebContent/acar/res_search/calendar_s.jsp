...
<!--
	function set_deli_dt(year, month, day){
		...
		fm.rent_start_dt.value = year+"-"+month+"-"+day;
	}
...
/**********          년,월,일,날짜수를 출력 한다.          **********/
	...
	/*
	//차량스케줄 연동 일자 조회
	Vector conts = rs_db.getResCalendar(c_id, Integer.toString(year), AddUtil.addZero2(month), days);
	int cont_size = conts.size();
		for(int i = 0 ; i < cont_size ; i++){
		Hashtable ht = (Hashtable)conts.elementAt(i);
		out.println(ht.get("DT"));
	}
	if(1==1)return;
	*/
%>
 ...
 <input type='hidden' name='date' value=''>    
  <table border=0 cellspacing=0 cellpadding=0 width=280>
    <tr> 
      ...
      <td align="right" height="22" width="120">&nbsp;
        <font color="#999999">대여개시일</font>
        </td>
    </tr>
    <tr> 
...
//			for (int j =1; j< days+1;j++){
			for (int j =1; j< dt.length+1;j++){
				tr++;
		%>
            <td height="35" valign="top" align="center" width="35" <% if((nowyear == year) && (nowmonth == month) && (nowdate == j)) { out.println("bgcolor='#FFFFCC'"); } else { out.println("bgcolor='#FFFFFF'"); } %>> 
			  <!--사용여부-->
