...
<!--
	...
	function CarRentSearch(car_no) {
		...
		if (keyValue =='13'){
			...
			window.open(SUBWIN, "CarRentList", "left=100, top=100, width=650, height=400, scrollbars=yes");
		}
		return;
	}	
	...
	function resDelete(s_cd){
		var fm = document.form1;
		fm.s_cd.value = s_cd;
		fm.target = "i_no";		
		fm.action = "car_res_d.jsp";
		fm.submit();		
	}
	...
	function rent_end_set_display(){
		var fm = document.form1;
		if(fm.rent_start_dt_y.value != '' && fm.rent_start_dt_m.value != '' && fm.rent_start_dt_d.value != '' && fm.rent_mon.value != ''){
			fm.target = "i_no";
	...
	function view_car()
	{		
		window.open("/acar/car_register/car_view.jsp?rent_mng_id=<%=res.get("RENT_MNG_ID")%>&rent_l_cd=<%=res.get("RENT_L_CD")%>&car_mng_id=<%=c_id%>&cmd=ud", "VIEW_CAR", "left=100, top=100, width=850, height=700, scrollbars=yes");
	}		
	
	//차종내역 보기
	function view_car_nm(){
		window.open("/acar/car_mst/car_mst_u.jsp?car_id=<%=res.get("CAR_ID")%>&car_seq=<%=res.get("CAR_SEQ")%>", "VIEW_CAR_NM", "left=20, top=20, width=875, height=650, scrollbars=yes");		
	}		
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>예약시스템 > 영업지원 > 예약관리 > <span class=style5>차량별 예약/대여 리스트</span></span></td>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr> 
                    ...
                    <td> 
                      &nbsp;<input type="text" name="car_no" size="13" value="<%=res.get("CAR_NO")%>" class='text' onKeydown="CarRentSearch(this.value)">
                    </td>
                    <td class=title>차명</td>
                    <td align="left" colspan="3">&nbsp;<%=res.get("CAR_NM")%>&nbsp;<a href='javascript:view_car_nm()' onMouseOver="window.status=''; return true" title="클릭하세요"><%=res.get("CAR_NAME")%></a></td>
                ...
                <tr> 
                    ...
                    <td width=23% colspan="3">&nbsp; 
                      ...
                      <input type="text" name="maint_end_dt" value="<%=AddUtil.ChangeDate2(String.valueOf(res.get("MAINT_END_DT")))%>" size="10" class=whitetext>
                      	<%	
        		    		int car_maint_d_day = c_db.getCar_D_day("car_maint_dt", c_id);
						%>
						<%if(car_maint_d_day <= 30){ %><font color=red>(D-day <%=car_maint_d_day%>일)</font><%} %>
                    </td>
                    ...
                    <td>&nbsp; 
                      <input type="text" name="car_end_dt" value="<%=AddUtil.ChangeDate2(String.valueOf(res.get("CAR_END_DT")))%>" size="10" class=whitetext>
                      	<%if(String.valueOf(res.get("CAR_USE")).equals("1")){
							int car_end_d_day = c_db.getCar_D_day("car_end_dt", c_id);							
						%>
						<%if(car_end_d_day <= 30){ %><br>&nbsp; <font color=red>(D-day <%=car_end_d_day%>일)</font><%} %>
						<%} %>                      
                    </td>
			...
			<%if(ins.getAge_scp().equals("8")){%>48세이상<%}%>
                ...
                <tr> 
                    ...
                    <td colspan="9">&nbsp;월대여료 : <%=AddUtil.parseDecimal(String.valueOf(hp.get("RM1")))%>원 (공급가)                    
    ...
    <tr> 
        <td align="right">
	    <a href="javascript:javascript:MM_openBrWindow('car_img_add_all.jsp?c_id=<%=c_id%>&car_no=<%=res.get("CAR_NO")%>','ImgAdd','scrollbars=no,status=yes,resizable=yes,width=820,height=600,left=50, top=50');" onMouseOver="window.status=''; return true"><img src="/acar/images/center/button_reg_p.gif"  align="absmiddle" border="0"></a>
	    &nbsp;	  
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="1" width=100%>
                <tr> 
                    ...
                    <td class=title rowspan='2' width="3%">상태</td>
                    <td class=title colspan='2'>자동차</td>										
                    <td class=title rowspan='2' width="23%">대여기간</td>
                    <td class=title rowspan='2' width="23%">상호/성명</td>					
                    <td class=title rowspan='2' width="6%">담당자</td>
                    <td class=title rowspan='2' width="6%">등록자</td>
                    <td class=title rowspan='2' width="9%">등록일자</td>
                    <td class=title rowspan='2' width="5%">스케줄</td>			
                </tr>
				<tr>
              ...
              <%	if(cont_size > 0){
                ...
                <tr> 
                    ...
                    <td <%if(!String.valueOf(reservs.get("USE_ST")).equals("예약")&&!String.valueOf(reservs.get("USE_ST")).equals("배차"))%>class="title_p"<%%> align="center"><%=AddUtil.ChangeDate3(String.valueOf(reservs.get("DELI_DT")))%>~<%=AddUtil.ChangeDate3(String.valueOf(reservs.get("RET_DT")))%></td>
        	    ...
        	    <td <%if(!String.valueOf(reservs.get("USE_ST")).equals("예약")&&!String.valueOf(reservs.get("USE_ST")).equals("배차"))%>class="title_p"<%%> align="center">
        	        <%if(!String.valueOf(reservs.get("USE_ST")).equals("취소")){%><a class=index1 href="javascript:MM_openBrWindow('/acar/rent_diary/car_scd_add.jsp?c_id=<%=c_id%>&s_cd=<%=reservs.get("RENT_S_CD")%>&car_no=<%=res.get("CAR_NO")%>','ImgAdd','scrollbars=yes,status=yes,resizable=yes,width=650,height=650,left=50, top=50')"><img src="/acar/images/center/button_in_see.gif" align="absmiddle" border="0"></a>
                        <%}else{%>                        
                        -                            
                        <%}%>                        
                    </td>
