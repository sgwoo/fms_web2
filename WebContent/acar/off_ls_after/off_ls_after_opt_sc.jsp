...
function view_detail(auth_rw,car_mng_id,seq)
{
	...
	var from_page = document.inner.form1.from_page.value;
	var url = "?from_page="+from_page+"&auth_rw="+auth_rw+"&car_mng_id="+car_mng_id+"&seq="+seq+"&gubun="+gubun+"&gubun_nm="+gubun_nm+"&migr_dt=<%= migr_dt %>&migr_gu=<%= migr_gu %>&st_dt=<%= st_dt %>&end_dt=<%= end_dt %>&car_st=<%= car_st %>&com_id=<%= com_id %>&car_cd=<%= car_cd %>";
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
	...
	<tr>
        ...
        <table border="0" cellspacing="0" cellpadding="0" width=100%>
          <tr> 
            <td> <iframe src="./off_ls_after_opt_sc_in.jsp?auth_rw=<%=auth_rw%>&brch_id=<%=brch_id%>&gubun=<%=gubun%>&gubun_nm=<%=gubun_nm%>&dt=<%= dt %>&migr_dt=<%= migr_dt %>&migr_gu=<%= migr_gu %>&st_dt=<%= st_dt %>&end_dt=<%= end_dt %>&car_st=<%= car_st %>&com_id=<%= com_id %>&car_cd=<%= car_cd %>&sh_height=<%=height%>&height=<%=height%>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 ></iframe></td>
          </tr>
        </table>
		</td>
	</tr>
