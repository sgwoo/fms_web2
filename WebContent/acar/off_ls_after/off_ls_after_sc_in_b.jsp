...
<%
	...
	//차량 기본정보	
	detail = olfD.getAfter_detail(car_mng_id);
	...
	//계약기본정보
	Hashtable cls_8_ht = a_db.getContViewOffLs8CarCase(car_mng_id);
...
function apprslUpd(ioru)
{
	...
	if(apprsl_dt != ""){
		...
		fm.gubun.value = ioru;
		fm.action="/acar/off_ls_after/off_ls_after_apprsl_upd.jsp";
		fm.target = "i_no";	
		fm.submit();
	}else{
		alert("평가일자를 입력하세요!");
...
function auctionUpd(ioru){
	...
	fm.gubun.value = ioru;
	fm.action="/acar/off_ls_after/off_ls_after_auction_upd.jsp";
	fm.target = "i_no";	
	fm.submit();
}
function imgAppend(){
...
//자동차 양도증명서		2018.01.17
function view_car_transfer_certificate(){
	window.open("./car_transfer_certificate.jsp?", "CAR_TRANSFER_CERTIFICATE", "left=100, top=100, width=840, height=700, scrollbars=yes");
}
//-->
</script>
</head>
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
    ...
    <tr> 
        <td class=line colspan="2"> 
            <table border="0" cellspacing="1" cellpadding='0' width="100%" >
               ...
               <%if(!String.valueOf(cls_8_ht.get("RENT_L_CD")).equals("") && !String.valueOf(cls_8_ht.get("RENT_L_CD")).equals("null")){//매입옵션일때 담당자 표시%>
               <tr> 
                ...
                <tr> 
                    <td align="center"><%=c_db.getNameById(String.valueOf(cls_8_ht.get("BUS_ID")),"USER")%></td>
                    <td align="center"><%=c_db.getNameById(String.valueOf(cls_8_ht.get("BUS_ID2")),"USER")%></td>
                    <td align="center"><%=c_db.getNameById(String.valueOf(cls_8_ht.get("MNG_ID")),"USER")%></td>
                    <td align="center"></td>
                    <td align="center"></td>
                </tr>    
                <%}%>
    ...
    <%-- <tr>
        <td colspan="2">
        	<%if(user_id.equals("000302")){ %>
        	<input type="button" name="test_temp" value="자동차양도증명서_개발중" onclick="javascript:view_car_transfer_certificate()"
        	onMouseOver="window.status=''; return true" title='동차양도증명서'><!-- 추후 사용 예정 2018.02.08 -->
        	<%} %>
        </td>
    </tr> --%>
    <tr> 
