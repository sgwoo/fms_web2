...
<%
	...
	if(est_st.equals("2")){
		...
		if(!mode.equals("cmp") && !from_page.equals("/fms2/lc_rent/search_car_esti_s.jsp")) 	fee_opt_amt 	= 0;		//연장계약시 매입옵션 있는경우 매입옵션금액
	}
...
<!--
	...
	function getShCarAmt(){	
		...
		fm.cd[i].value = "ex_n_h";			fm.nm[i].value = "현시점 수출효과+주행거리 상쇄효과 합계";	fm.value[i].value = ex_n_h;				i++;		
		...
		o_s = Math.round(o_s);
		fm.cd[i].value = "o_s";					fm.nm[i].value = "현시점 경매장 예상낙찰가(주행거리반영)-재리스시작시 일반승용 LPG차령 60개월이상 잔가율 조정";	fm.value[i].value = o_s;					i++;	
...
//			return;
		...
		fm.cd[i].value = "";			fm.nm[i].value = "---------------------------"; fm.value[i].value = '주행거리 --------------------------------------';			i++;						
		fm.cd[i].value = "tot_dist";		fm.nm[i].value = "확인주행거리";				fm.value[i].value = <%=ht.get("TOT_DIST")%>;				i++;		
		fm.cd[i].value = "today_dist";		fm.nm[i].value = "적용주행거리";				fm.value[i].value = fw_941;						i++;		
