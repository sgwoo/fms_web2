...
<%@ page import="java.util.*, acar.util.*, acar.common.*, acar.secondhand.*, acar.estimate_mng.*, acar.cont.*, acar.car_mst.*, acar.user_mng.*" %>
...
<%
	...
	int car_amt 		= request.getParameter("car_c_amt")	==null? 0:Util.parseDigit(request.getParameter("car_c_amt"));
	int opt_amt 		= request.getParameter("opt_c_amt")	==null? 0:Util.parseDigit(request.getParameter("opt_c_amt"));
	int col_amt 		= request.getParameter("col_c_amt")	==null? 0:Util.parseDigit(request.getParameter("col_c_amt"));
	int dc_amt 			= 0;
	int tax_dc_amt 	= request.getParameter("tax_dc_amt")	==null? 0:Util.parseDigit(request.getParameter("tax_dc_amt"));
	int o_1 				= request.getParameter("tot_c_amt")		==null? 0:Util.parseDigit(request.getParameter("tot_c_amt"));

	String rent_dt 		= request.getParameter("sh_day_bas_dt")==null?"":request.getParameter("sh_day_bas_dt");
	String init_reg_dt = request.getParameter("sh_init_reg_dt")==null?"":request.getParameter("sh_init_reg_dt");
	String rent_st 		= request.getParameter("rent_st")==null?"1":request.getParameter("rent_st");
	String est_st 		= request.getParameter("est_st")==null?"":request.getParameter("est_st");
	String mode 	  	= request.getParameter("mode")==null?"":request.getParameter("mode");
	String today_dist	= request.getParameter("sh_km")==null?"":request.getParameter("sh_km");
	...
	int cust_sh_car_amt = 0;								//중고차리스시 구입예정 중고차가격
	int accid_serv_amt1 = request.getParameter("accid_serv_amt")	==null? 0:Util.parseDigit(request.getParameter("accid_serv_amt"));
	int accid_serv_amt2 = 0;
	
	rent_dt = AddUtil.replace(rent_dt,"-","");
	init_reg_dt = AddUtil.replace(init_reg_dt,"-","");
	...
	out.println("init_reg_dt="+init_reg_dt+"<br>");
	out.println("jg_b_dt="+jg_b_dt+"<br>");
	out.println("em_a_j="+em_a_j+"<br>");
	out.println("jg_code="+cm_bean.getJg_code()+"<br>");
	
	//공통변수
	EstiCommVarBean em_bean = e_db.getEstiCommVarCase("1", em_var_seq);
	
	//차량등록 경과기간(차령)
	Hashtable carOld 	= c_db.getOld(init_reg_dt, rent_dt);
	
	out.println("today_dist="+today_dist);
	
	String duty_free_opt = cm_bean.getDuty_free_opt();
...
<!--
	...
	function getShCarAmt(){	
		...
		var init_reg_dt = <%=AddUtil.replace(init_reg_dt,"-","")%>;
		fm.cd[i].value = "init_reg_dt";		fm.nm[i].value = "신차등록일";									fm.value[i].value = init_reg_dt;			i++;		

		var rent_dt = <%=AddUtil.replace(rent_dt,"-","")%>;
		...
		if(<%=rent_dt%> >= 20151013){
			...
			%>
				...
				fm.cd[i].value = "jg_opt_az[0]";	fm.nm[i].value = "색상 및 사양 잔가반영 - 현재차령 적용값";		fm.value[i].value = jg_opt_az[0];		i++;			
				fm.cd[i].value = "jg_opt_az[1]";	fm.nm[i].value = "색상 및 사양 잔가반영 - 종료시점 적용값";		fm.value[i].value = jg_opt_az[1];		i++;			
			...
			fm.cd[i].value = "t_jg_opt_az[0]";			fm.nm[i].value = "색상 및 사양 잔가반영 - 현재차령 적용합계";	fm.value[i].value = t_jg_opt_az[0];					i++;	
			fm.cd[i].value = "t_jg_opt_az[1]";			fm.nm[i].value = "색상 및 사양 잔가반영 - 종료시점 적용합계";	fm.value[i].value = t_jg_opt_az[1];					i++;		
		...
		parent.document.form1.sh_year.value				= <%=carOld.get("YEAR")%>;
		parent.document.form1.sh_month.value			= <%=carOld.get("MONTH")%>;
		parent.document.form1.sh_day.value				= <%=carOld.get("DAY")%>;
		
		if(parent.document.form1.sh_base_dt[0].value == ''){
			parent.document.form1.sh_base_dt[0].value	= parent.document.form1.sh_day_bas_dt.value;
		}
		if(parent.document.form1.sh_base_dt[1].value == ''){
			parent.document.form1.sh_base_dt[1].value	= parent.document.form1.sh_day_bas_dt.value;
		}
		
		alert('중고차 재리스잔존가 계산이 완료되었습니다.');
