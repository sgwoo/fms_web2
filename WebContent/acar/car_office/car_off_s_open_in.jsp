        ...
        <%for(int i=0; i<co_r.length; i++){
			co_bean = co_r[i];	%>
                <tr> 
                    ...
                    <td width='19%' align='center'><%= c_db.getNameById(co_bean.getCar_comp_id(),"CAR_COM") %></td>
                    <td width='14%' align='center'><%if(co_bean.getCar_off_st().equals("1")){ out.print("지점");
        		  								}else if(co_bean.getCar_off_st().equals("2")){ out.print("대리점");
        										} %></td>
                    <td width='37%' align='center'><a href="javascript:parent.SetCarOff('<%= co_bean.getCar_comp_id() %>','<%= co_bean.getCar_off_id() %>','<%= co_bean.getCar_off_nm() %>','<%= co_bean.getCar_off_st() %>','<%= co_bean.getCar_off_tel() %>','<%= co_bean.getCar_off_fax() %>','<%= co_bean.getCar_off_post() %>','<%= co_bean.getCar_off_addr() %>')"><%= co_bean.getCar_off_nm() %></a></td>
