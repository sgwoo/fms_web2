...
%>
        <tr> 
          ...
          <td align="center" width=120><%= c_db.getNameById(coev_bean.getVis_nm(), "USER") %></td>
          <td width=230>&nbsp;<span title="<%= coev_bean.getVis_cont() %>"><a href="javascript:UpdateVis('<%=coev_bean.getSeq_no()%>','<%=coev_bean.getVis_dt()%>','<%=coev_bean.getVis_nm()%>','<%=coev_bean.getSub()%>','<%=Util.htmlR(coev_bean.getVis_cont())%>')"><%= coev_bean.getSub() %></a></span></td>
          <td align="center" width=120><%= coev_bean.getVis_dt() %></td>
        </tr>
        <%}%>
        <% if(coev_r.length == 0) { %>
        <tr> 
