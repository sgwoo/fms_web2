...
*/		
	...
	function TodaySearch(today){
		...
		i_fm.save_dt.value = '';
		i_fm.target='i_view';
		i_fm.action='stat_bus_sc_in_view.jsp';		
		i_fm.submit();				
	}
	//조회
	function Search(){
	...
	function OpenHelp(){
		var fm = document.form1;		
		var SUBWIN = "stat_bus_help.jsp?auth_rw="+fm.auth_rw.value+"&user_id="+fm.user_id.value+"&br_id="+fm.br_id.value+"&save_dt="+fm.save_dt.value;
...
<%
	...
	auth_rw = rs_db.getAuthRw(user_id, "09", "04", "04");
	...
	if(save_dt.equals(""))	save_dt = ad_db.getMaxSaveDt("stat_bus");
...
<table border=0 cellspacing=0 cellpadding=0 width="100%">
    	...
    	<td colspan=3>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>경영정보 > 영업현황 > <span class=style5>사원별 영업누계현황</span></span></td>
    ...
    <tr> 
	  ...
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=../images/center/arrow_yus.gif align=absmiddle>
        ...
        <input type='text' name='view_dt' size='11' value='<%if(save_dt.equals("")){%><%=AddUtil.getDate()%><%}else{%><%=AddUtil.ChangeDate2(save_dt)%><%}%>' class="white" readonly>
        &nbsp;&nbsp;</td>
    </tr>
    <tr>
        <td class=h></td>
    ...
    <tr> 
        <td colspan="3"><iframe src="./stat_bus_sc_in_view.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&save_dt=<%=save_dt%>&brch_id=<%=brch_id%>" name="i_view" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe> 
        </td>
    </tr>
    <tr>
        <td class=h></td>
    ...
    <tr> 
        <td colspan="3"><font color="#FF00FF">♣ 도움말 : </font> <a href="javascript:OpenHelp()">가중치 부여방법</a> <a href="javascript:OpenHelp()"><img src=../images/center/button_see.gif border=0 align=absmiddle></a> (최초영업자)
        </td>
    </tr>
    <tr>
        <td></td>
    ...
    <tr> 
        <td colspan="3"><iframe src="./stat_bus_sc_in_list.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&save_dt=<%=save_dt%>" name="i_list" width="100%" height="45" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe> 
        </td>
    </tr>
  </table>
</form>
