...
<!--
	...
	function SearchopenBrWindow() { //v2.0
		...
		theURL = "/acar/pop_search/s_caroff.jsp?type=search&gubun="+fm.s_kd.value+"&gu_nm="+fm.t_wd.value+"&m_st=<%= m_st %>&m_st2=<%= m_st2 %>&m_cd=<%= m_cd %>&go_url=<%= go_url %>";
		window.open(theURL,'search_open','scrollbars=yes,status=no,resizable=yes,width=570,height=380,left=50,top=50');
	}		
//-->
</script>
  <tr> 
    <td bgcolor="#878751" colspan="<%= colspan %>"> <table width="0%" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          <td width="12">&nbsp;</td>
          <td><font color="#ffffff">검색</font></td>
          <td width="5">&nbsp;</td>
          <td>         <select name="s_kd">
          <option value='1' <%if(s_kd.equals("1") || s_kd.equals("")){%> selected <%}%>> 영업소 </option>
        </select>
