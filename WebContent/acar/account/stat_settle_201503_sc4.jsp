...
<%@ page import="java.util.*, acar.common.*, acar.util.*, acar.settle_acc.*, acar.user_mng.*, acar.estimate_mng.*, acar.account.*"%>
...
<table width="<%if(s_width.equals("768")){%><%=AddUtil.parseInt(s_width)-100%><%}else{%>100%<%}%>" border="0" cellspacing="0" cellpadding="0">
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=../images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>경영정보 > 캠페인관리 > <span class=style5>채권관리캠페인(에이전트)</span></span></td>
        ...
        <td class=line> 
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                ...
                <tr> 
                    ...
                    <!--<td class='title' width="10%">총받을어음</td>-->
    ...
    <tr> 
        <td><img src=../images/center/icon_arrow.gif align=absmiddle> <span class=style2>미수금 현황 그래프</span></td>
    </tr>
    <tr>
        <td class=line2></td>
    </tr>
        ...
        <td class="line"> 
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                <tr> 
                    ...
                    <td width=% rowspan="2" class="title">구분</td>									
                    <td width=13% rowspan="2" class="title">소속</td>									
                    <td width=6% rowspan="2" class="title">성명</td>
                ...
                <tr>
          		...
          		<%	if(vt_size > 0){
						...
						for (int i = 0 ; i < vt_size ; i++){
							...
							String emp_nm = c_db.getNameById(feedp.getGubun(), "CAR_OFF_EMP");
			%>		
                <tr>
                  <td align="center"><%=i+1%></td>
                  <td align="center"><%=feedp.getBr_id()%></td>				  
                  <td align="center"><%if(feedp.getDept_id().equals("김안")){%>(주)다이렉트리스
									  <%}else if(feedp.getDept_id().equals("이광희")){%>(주)하나캐피탈
									  <%}else if(feedp.getDept_id().equals("김인형")){%>현대 오금역대리점
									  <%}else if(feedp.getDept_id().equals("박석호")){%>기아 삼전대리점
									  <%}else if(feedp.getDept_id().equals("최준원")){%>신한카드 여의도대리점
									  <%}else if(feedp.getDept_id().equals("고상진")){%>(주)에스지앤카
									  <%}else{%>
									  <%=feedp.getDept_id()%>
									  <%	if(feedp.getGubun_sub().equals("E") ){%>
									  	<b>[견적]</b>
									  <%	}%>
									  <%}%>
				  </td>				  
                  <td align="center"><%=emp_nm%></td>                  
                  <td align="right"><%=Util.parseDecimalLong(feedp.getTot_amt1())%></td>
                  <td align="right"><%=Util.parseDecimal(feedp.getTot_amt2())%>&nbsp;<a href="javascript:MM_openBrWindow('stat_settle_sc_in_view_sub_list3.jsp?bus_id2=<%=feedp.getGubun()%>&r_dly_amt=<%=feedp.getTot_amt2()%>','list_id1','scrollbars=yes,status=no,resizable=yes,width=960,height=860,top=50,left=50')"><img src="/images/esti_detail.gif"  width="14" height="15" align="absmiddle" border="0" alt="리스트보기"></a></td>			
    ...
    <tr> 
        <td height="19"><iframe src="./stat_settle_sc_in_list4.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&save_dt=<%=save_dt%>&from_page=/acar/account/stat_settle_201503_sc4.jsp" name="i_list" width="100%" height="45" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe> 
        </td>
    </tr>
