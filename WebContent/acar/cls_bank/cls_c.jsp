...
<%
	...
	String cls_rtn_dt = request.getParameter("cls_rtn_dt")==null?"":request.getParameter("cls_rtn_dt");
	...
	ClsBankBean cls = as_db.getClsBankCase(lend_id, rtn_seq, cls_rtn_dt);
	
	Vector scds = abl_db.getBankClsBScds(lend_id, rtn_seq, cls_rtn_dt);
...
<!--
	...
	function save(){
		...
		if(!confirm('수정하시겠습니까? 스케줄은 별도 수정해야 합니다.'))
			return;
		fm.target='i_no';
      ...
      <td class='line' colspan="2"> 
        <table border="0" cellspacing="1" cellpadding="0" width=800>
          ...
          <tr>             
    				...
    				<td colspan='5'>&nbsp;<%=lend_bank.get("CLS_RTN_ETC")%></td>
			    </tr>					
      ...
      <td class='line'> 
        <table border="0" cellspacing="1" cellpadding="0" width="800">
                <tr> 
                    ...
                    <td>&nbsp; 
                      <input type='text' name='cls_rtn_dt' size='12' class='whitetext' onBlur='javascript: this.value = ChangeDate(this.value)' value='<%=cls.getCls_rtn_dt()%>' readonly></td>
                </tr>		
                <tr> 
            ...
            <td>&nbsp; 
              ...
              원&nbsp;</td>
                ...
                <tr> 
                    ...
                    <td>&nbsp; 
                      <%=AddUtil.ChangeDate2(cls.getReg_dt())%> <%=c_db.getNameById(cls.getReg_id(),"USER")%></td>
                </tr>	          
        </table>
      </td>
    </tr>
...
<%		for(int i = 0 ; i < scd_size ; i++){
			BankScdBean scd = (BankScdBean)scds.elementAt(i);%>
				<tr>
					...
					<td width=14% align='right'><%=AddUtil.parseDecimal(scd.getAlt_prn_amt())%></td>
					<td width=14% align='right'><%=AddUtil.parseDecimal(scd.getAlt_int_amt())%></td>
					<td width=14% align='right'><%=AddUtil.parseDecimal(scd.getAlt_prn_amt()+scd.getAlt_int_amt())%></td>
					<td width=18% align='right'><%=AddUtil.parseDecimal(scd.getAlt_rest())%></td>
					<td width=10% align='center'><%if(scd.getPay_yn().equals("0")){%>N<%}else{%>Y<%}%></td>
