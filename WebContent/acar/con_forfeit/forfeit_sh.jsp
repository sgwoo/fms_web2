...
<!--
	function search(){
		...
		if(fm.s_kd.options[fm.s_kd.selectedIndex].value == '8' || fm.s_kd.options[fm.s_kd.selectedIndex].value == '14' || fm.s_kd.options[fm.s_kd.selectedIndex].value == '16'){ //영업담당자
...
%>
<form name='form1' action='forfeit_sc.jsp' target='c_body' method='post'>
        ...
        <td>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>재무회계 > 수금관리 > <span class=style5>과태료 수금관리</span></span></td>
	...
	<tr>
		<td>			
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr> 
                    <td width='195'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_jhgb.gif align=absmiddle>&nbsp;
        ...
        <!--                <option value="4" <%if(gubun3.equals("4")){%>selected<%}%>>선수금</option>-->
                    ...
                    <td width=435>
                      <select name="gubun5">
                        <option value="1" <%if(gubun5.equals("1")){%>selected<%}%>>전체</option>
                        <option value="3" <%if(gubun5.equals("3")){%>selected<%}%>>고객과실</option>
                        <option value="5" <%if(gubun5.equals("5")){%>selected<%}%>>업무상과실</option>
						<option value="12" <%if(gubun5.equals("12")){%>selected<%}%>>업무대여과실</option>
						<option value="2" <%if(gubun5.equals("2")){%>selected<%}%>>외부업체과실</option>
						<option value="6" <%if(gubun5.equals("6")){%>selected<%}%>>해지-미청구분</option>
						<%if(nm_db.getWorkAuthUser("전산팀",user_id)){%>   
						<option value="7" <%if(gubun5.equals("7")){%>selected<%}%>>해지-퇴사자분</option>
						<option value="8" <%if(gubun5.equals("8")){%>selected<%}%>>해지-미거래분</option>						
						<%}%>
						<%if(nm_db.getWorkAuthUser("전산팀",user_id)||nm_db.getWorkAuthUser("과태료관리자",user_id)){%>   
						<option value="9" <%if(gubun5.equals("9")){%>selected<%}%>>보유차-미연결분</option>
						<%}%>
						<option value="10" <%if(gubun5.equals("10")){%>selected<%}%>>통행료</option>
						<option value="11" <%if(gubun5.equals("11")){%>selected<%}%>>통행료외</option>						
        			  </select>			
                    </td>
                </tr>
                <tr> 
                    <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_gsjg.gif align=absmiddle>&nbsp;
                      <select name='s_kd' onChange="javascript:document.form1.t_wd.value='', cng_input()">
                        ...
                        <option value='13' <%if(s_kd.equals("13")){%> selected <%}%>>청구기관</option>																
                        ...
                        <option value='14' <%if(s_kd.equals("14")){%> selected <%}%>>영업담당자</option>
                        <option value='8' <%if(s_kd.equals("8")){%> selected <%}%>>관리담당자</option>
                        <option value='16' <%if(s_kd.equals("16")){%> selected <%}%>>과태료담당</option>						
						<option value='17' <%if(s_kd.equals("17")){%> selected <%}%>>업무과실자</option>						
                      </select>
                      </td>
                    <td> 
                      <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr> 
                          <td id='td_input' <%if(s_kd.equals("8") || s_kd.equals("6") || s_kd.equals("14") || s_kd.equals("16")){%> style='display:none'<%}%>> 
                            <input type='text' name='t_wd' size='21' class='text' value='<%=t_wd%>' onKeyDown='javascript:enter()' style='IME-MODE: active'>
                          </td>
                          <td id='td_bus' <%if(s_kd.equals("8") || s_kd.equals("14") || s_kd.equals("16")){%> style="display:''"<%}else{%>style='display:none'<%}%>> 
                            <select name='s_bus'>
                    ...
                    <td><img src=/acar/images/center/arrow_jrjg.gif align=absmiddle>&nbsp;
                      <select name='sort_gubun' onChange='javascript:search()'>
                        <option value='0' <%if(sort_gubun.equals("0")){%> selected <%}%>>납부기한</option>
        ...
        <!--                <option value='4' <%if(sort_gubun.equals("4")){%> selected <%}%>>연체일수</option>-->
                        ...
                        <option value='6' <%if(sort_gubun.equals("6")){%> selected <%}%>>영업담당자</option>
                      </select>
                    </td>
                    <td> 
