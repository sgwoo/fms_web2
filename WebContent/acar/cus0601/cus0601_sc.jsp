...
function view_detail(off_id){
	var url = "?auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&sort=<%=sort%>&gubun1=<%=gubun1%>&br_id=<%=br_id%>";
...
<form name="form1" action="" method="post">
    <tr> 
      <td><iframe src="cus0601_sc_in.jsp?auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&sort=<%=sort%>&gubun1=<%=gubun1%>&br_id=<%=br_id%>" name="serv_offList" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
    </tr>
