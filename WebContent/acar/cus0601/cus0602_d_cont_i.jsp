...
<%
	...
	if(auth_rw.equals(""))	auth_rw = rs_db.getAuthRw(ck_acar_id, "17", "04", "02");
...
function ServOffReg(){
	...
	if(fm.own_nm.value==""){ alert("대표자를 입력해 주세요!"); fm.own_nm.focus(); return; }
	if(fm.ent_no.value==""){ alert("사업자번호를 입력해 주세요!"); fm.ent_no.focus(); return; }
	if(!isTel(fm.ent_no.value)){ alert("사업자번호를 다시 확인해 주세요!"); fm.ent_no.focus(); return; }

	if(!confirm('해당탁송업체를 등록하시겠습니까?')){ return; }
	fm.action = "cus0602_d_cont_in.jsp";
	fm.target = "i_no";
	fm.submit();
}
function search_zip()
...
<input type="hidden" name="reg_id" value="<%=user_id%>">
    ...
    <tr> 	    
        <td align="right">
            <%if(!auth_rw.equals("1")){%>
            <a href='javascript:ServOffReg()' onMouseOver="window.status=''; return true"><img src=../images/center/button_reg.gif border=0 align=absmiddle></a> 
            <%}%>
