...
<!--
	function search(){
		...
		if(fm.s_kd.options[fm.s_kd.selectedIndex].value == '12'){ //금융사
	...
	function search2(){
		...
		if(fm.end_dt.value != ''){ 						fm.end_dt.value = ChangeDate3(fm.end_dt.value);	}
		if(fm.st_dt.value !='' && fm.end_dt.value==''){ fm.end_dt.value = fm.st_dt.value; }				
		window.open('lend_list.jsp?st_dt='+fm.st_dt.value+'&end_dt='+fm.end_dt.value+'&cpt_cd='+fm.gubun4.value, "LIST", "left=100, top=30, width=800, height=750, scrollbars=yes, status=yes");
	}	
	//대출리스트
	function search3(){
		var fm = document.form1;
		...
		if(fm.end_dt.value != ''){ 						fm.end_dt.value = ChangeDate3(fm.end_dt.value);	}
		if(fm.st_dt.value !='' && fm.end_dt.value==''){ fm.end_dt.value = fm.st_dt.value; }				
		window.open('lend_list2.jsp?st_dt='+fm.st_dt.value+'&end_dt='+fm.end_dt.value+'&cpt_cd='+fm.gubun4.value, "LIST", "left=100, top=30, width=800, height=750, scrollbars=yes, status=yes");
	}		
	...
	function cng_input(){
		...
		if(fm.s_kd.options[fm.s_kd.selectedIndex].value == '8'){ //영업담당자
			...
			td_cpt.style.display	= 'none';			
		}else if(fm.s_kd.options[fm.s_kd.selectedIndex].value == '12'){ //금융사
			td_input.style.display	= 'none';
			td_bus.style.display	= 'none';			
			td_brch.style.display	= 'none';		
...
//	CodeBean[] banks = c_db.getCodeAll("0003"); /* 코드 구분:은행명 */	
	CodeBean[] banks = c_db.getAllotCptCdAll(); /* 코드 구분:은행명 allot */	
	int bank_size = banks.length;
	
	Vector users = c_db.getUserList("", "", "EMP"); //영업담당자 리스트
	int user_size = users.size();	
%>
<form name='form1' action='debt_scd_sc.jsp' target='c_body' method='post'>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
        ...
        <td>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp; <span class=style1>재무회계 > 구매자금관리 ><span class=style5>할부금상환스케줄조회</span></span></td>
	...
	<tr>
                  ...
                  <tr> 
                    <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=../images/center/arrow_gsjg.gif align=absmiddle>&nbsp;
                      <select name='s_kd' onChange="javascript:document.form1.t_wd.value='', cng_input()">
                        ...
                        <option value='12' <%if(s_kd.equals("12")){%> selected <%}%>>금융사코드</option>
        <!--            <option value='7' <%if(s_kd.equals("7")){%> selected <%}%>>사용본거지</option>
                        ...
                        <option value='13' <%if(s_kd.equals("13")){%> selected <%}%>>대출금액</option>				
                      </select>
                    </td>
                    <td> 
                      <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr> 
                          ...
                          <td id='td_brch' <%if(s_kd.equals("6")){%> style="display:''"<%}else{%>style='display:none'<%}%>> 
                           ...
                           <td id='td_cpt' <%if(s_kd.equals("12")){%> style="display:''"<%}else{%>style='display:none'<%}%>> 
        			        <select name='s_cpt' onChange='javascript:search();'>
        			          <option value=''>전체</option>
        			          <%Vector cpts = c_db.getBankList(); //금융기관 조회
        						int cpt_size = cpts.size();					  
        					  	if(cpt_size > 0){
        								for (int i = 0 ; i < cpt_size ; i++){
        									Hashtable cpt = (Hashtable)cpts.elementAt(i);%>
        			          <option value='<%= cpt.get("CODE") %>'  <%if(t_wd.equals(String.valueOf(cpt.get("CODE")))){%>selected<%}%>> 
        			          <%= cpt.get("NM")%> </option>
        			          <%		}
        							}		%>
        			        </select>
                    ...
                    <td><img src=../images/center/arrow_jrjg.gif align=absmiddle>&nbsp;
                      <select name='sort_gubun' onChange='javascript:search()'>
                        ...
                        <option value='6' <%if(sort_gubun.equals("6")){%> selected <%}%>>대출번호</option>						
                      </select>
                    </td>
                    <td> 
        			...
        			<%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("회계업무",user_id)){%>
						&nbsp;<a href='javascript:search2()' title='대출리스트-근저당설정용'><img src=/acar/images/center/button_list1.gif align=absmiddle border=0></a>
						&nbsp;<a href='javascript:search3()' title='대출리스트-금융사제출용'><img src=/acar/images/center/button_list2.gif align=absmiddle border=0></a>						
					<%}%>
        			<%if(auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6")){%>
						&nbsp;<a href ="/acar/excel_reg/excel.jsp?action=scd_alt.jsp" target="_blank" ><img src=../images/center/button_reg_excel.gif align=absmiddle border=0></a>
					<%}%>                   
                    &nbsp;&nbsp;&nbsp;&nbsp;</td>
                </tr>
            </table>
