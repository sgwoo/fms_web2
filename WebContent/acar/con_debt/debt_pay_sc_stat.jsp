        ...
        <td>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>재무회계 > 영업비용관리 > <span class=style5>할부금현황</span></span></td>
...
<%	//할부금 현황
	Vector debts = ac_db.getDebtStat(br_id, "", "", "");
		...
		for (int i = 0 ; i < debt_size ; i++){
			IncomingSBean debt = (IncomingSBean)debts.elementAt(i);%>		
                <tr> 
                    ...
                    <td align="right"><%if(debt.getSt()==1){%><%=debt.getTot_su1()%>%<% }else{%><%=debt.getTot_su1()%>건<%}%>&nbsp;</td>
                    <td align="right"><%if(debt.getSt()==1){%><%=debt.getTot_amt1()%>%<%}else{%><%=Util.parseDecimal(debt.getTot_amt1())%>원<%}%>&nbsp;</td>
                    <td align="right"><%if(debt.getSt()==1){%><%=debt.getTot_su2()%>%<% }else{%><%=debt.getTot_su2()%>건<%}%>&nbsp;</td>
                    <td align="right"><%if(debt.getSt()==1){%><%=debt.getTot_amt2()%>%<%}else{%><%=Util.parseDecimal(debt.getTot_amt2())%>원<%}%>&nbsp;</td>
                    <td align="right"><%if(debt.getSt()==1){%><%=debt.getTot_su3()%>%<% }else{%><%=debt.getTot_su3()%>건<%}%>&nbsp;</td>
                    <td align="right"><%if(debt.getSt()==1){%><%=debt.getTot_amt3()%>%<%}else{%><%=Util.parseDecimal(debt.getTot_amt3())%>원<%}%>&nbsp;</td>
                    <td align="right"><%if(debt.getSt()==1){%>-<%}else{%><%=Integer.parseInt(debt.getTot_su2())+Integer.parseInt(debt.getTot_su3())%>건<%}%>&nbsp;</td>  
        			<td align="right">
