...
<%
	...
	Vector accids = as_db.getAccidS7List(br_id, gubun1, gubun2, gubun3, gubun4, gubun5, gubun6, brch_id, st_dt, end_dt, s_kd, t_wd, sort, asc, s_st);
	    ...
	    <td class='line' width='63%'>
    	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
                <tr> 
                    ...
                    <td width='13%' class='title'>보험사</td>
                    <td width='11%' class='title'>정비비</td>
                    <td width='11%' class='title'>면책금</td>
                    <td width='10%' class='title'>휴/대차료</td>
                    <td width='10%' class='title'>대인</td>
                    <td width='10%' class='title'>대물</td>
                    <td width='9%' class='title'>자손</td>
                    <td width='11%' class='title'>자차</td>
                </tr>
            </table>
	    </td>
    </tr>
	    ...
	    <td class='line' width='37%' id='td_con' style='position:relative;'>			
            <table border="0" cellspacing="1" cellpadding="0" width='100%'>
              <% 		for (int i = 0 ; i < accid_size ; i++){
                ...
                <tr> 
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='13%' align='center'><a name="<%=i+1%>"><%=i+1%> 
    	    ...
    	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
              <%		for (int i = 0 ; i < accid_size ; i++){
                ...
                <tr> 
                    ...
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='13%' align='center'><%=accid.get("OUR_INS")%> 
                      <%if(String.valueOf(accid.get("OUR_INS")).equals("")){%>
                      <%=as_db.getInserNm(String.valueOf(accid.get("CAR_MNG_ID")))%> 
                      <%}%>
                    </td>
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='11%' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("TOT_AMT")))%></td>
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='11%' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("CUST_AMT")))%></td>
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='10%' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("REQ_AMT")))%></td>
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='10%' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("HUM_AMT")))%></td>
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='10%' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("MAT_AMT")))%></td>
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='9%' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("ONE_AMT")))%></td>
                    <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='11%' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("TOT_AMT")))%></td>
                </tr>
              <%		total_amt1 = total_amt1 + Long.parseLong(String.valueOf(accid.get("TOT_AMT")));
    					total_amt2 = total_amt2 + Long.parseLong(String.valueOf(accid.get("CUST_AMT")));
    					total_amt3 = total_amt3 + Long.parseLong(String.valueOf(accid.get("REQ_AMT")));
    					total_amt4 = total_amt4 + Long.parseLong(String.valueOf(accid.get("HUM_AMT")));
    					total_amt5 = total_amt5 + Long.parseLong(String.valueOf(accid.get("MAT_AMT")));
    					total_amt6 = total_amt6 + Long.parseLong(String.valueOf(accid.get("ONE_AMT")));
    		  		}%>
                <tr> 
                    <td  class='title'>&nbsp;</td>
                    <td  class='title'>&nbsp;</td>
