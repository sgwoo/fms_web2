	    ...
	    <td class='line' width='980'>
    	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
                <tr> 
                    ...
                    <td width='60' class='title'>출고경과</td>			
                    <td width='90' class='title'>차량가액</td>
                    <td width='90' class='title'>수리비용</td>			
                    <td width='80' class='title'>청구가능금액</td>
                    ...
                    <td width='70' class='title'>사고접수자</td>
...
<%	if(accid_size > 0){%>
        ...
        <table border="0" cellspacing="1" cellpadding="0" width='100%'>
          <% 		for (int i = 0 ; i < accid_size ; i++){
			Hashtable accid = (Hashtable)accids.elementAt(i);
				if(AddUtil.parseInt(String.valueOf(accid.get("DLV_MON")).equals("")) > 2 ){
					continue;
				} %>
          <tr> 
            ...
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='100' align='center'><a href="javascript:parent.AccidentDisp('<%=accid.get("RENT_MNG_ID")%>', '<%=accid.get("RENT_L_CD")%>', '<%=accid.get("CAR_MNG_ID")%>', '<%=accid.get("ACCID_ID")%>', '<%=accid.get("ACCID_ST")%>','<%=i%>', '<%=accid.get("AMOR_EST_ID")%>')" onMouseOver="window.status=''; return true"><%=accid.get("RENT_L_CD")%></a></td>
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='150' align='center'><span title='<%=accid.get("FIRM_NM")%>'><a href="javascript:parent.view_client('<%=accid.get("RENT_MNG_ID")%>', '<%=accid.get("RENT_L_CD")%>', '1')" onMouseOver="window.status=''; return true"><%=Util.subData(String.valueOf(accid.get("FIRM_NM")), 8)%></a></span></td>
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='100' align='center'><%=accid.get("CAR_NO")%></td>
          </tr>
          <%		}%>
          <tr> 
	...
	<td class='line' width='980'>
	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
          ...
          <tr> 
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='100' align='center'><span title='<%=accid.get("CAR_NM")%>'><%=Util.subData(String.valueOf(accid.get("CAR_NM")), 6)%></span></td>		  
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='100' align='center'><%=accid.get("ACCID_DT")%></td>
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='60' align='center'><%=accid.get("DLV_MON")%>년이내</td>
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='90' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("CAR_AMT")))%></td>
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='90' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("TOT_AMT")))%></td>
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='80' align='right'><%=Util.parseDecimal(String.valueOf(accid.get("REQ_EST_AMT")))%></td>
            ...
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='70' align='center'><%=accid.get("REG_NM")%></td>
            <td <%if(accid.get("USE_YN").equals("N")){%>class='is'<%}%> width='70' align='center'><%=accid.get("MNG_NM")%></td>			
          </tr>
