...
<%
	...
	int r_dc_per = request.getParameter("r_dc_per")==null?0:AddUtil.parseDigit(request.getParameter("r_dc_per"));
	...
	siBn.setR_amt(r_amt);
	siBn.setR_dc(r_dc);
	siBn.setR_dc_per(r_dc_per);
	siBn.setR_j_amt(r_j_amt);
	siBn.setBus_id2(bus_id2);
	siBn.setSh_amt(sh_amt);
	if (  serv_st.equals("1") ||  serv_st.equals("2") ||  serv_st.equals("3")   )   {	
		System.out.println("정비 담당자=" + bus_id2 + ":" + car_mng_id );
	}
	...
	if(car_mng_id.equals("") || serv_id.equals("")){
		result = 3;
	 	...
	 	result = cr_db.updateService_g(siBn);
	}
...
<%if(result==1){%>
	...
	//parent.window.close();
		parent.location.href = "serv_reg.jsp?car_mng_id=<%= car_mng_id %>&serv_id=<%= serv_id %>&accid_id=<%= accid_id %>&rent_mng_id=<%= rent_mng_id %>&rent_l_cd=<%= rent_l_cd %>";
		parent.opener.location.reload();
<%}else if(result == 3){%>
