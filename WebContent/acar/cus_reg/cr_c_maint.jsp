...
<%if(car_size >= 2){ %>
	var SUBWIN="./cr_l_maint.jsp?auth_rw=<%= auth_rw %>&s_gubun1=<%= s_gubun1 %>&s_kd=<%= s_kd %>&t_wd=<%= t_wd %>";
...
<%}else if(car_size == 1){
	...
	parent.parent.c_body.location.href = "cus_reg_maint.jsp?mdata=<%=mdata%>&client_id=<%= car.get("CLIENT_ID") %>&car_mng_id=<%= car.get("CAR_MNG_ID") %>";	
<%}else{ %>
