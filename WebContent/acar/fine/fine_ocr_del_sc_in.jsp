...
<%
	...
	Vector vt = fdb.getFineOcrDelList(s_kd, t_wd, gubun1, st_dt, end_dt, gubun2, del_yn);
	int vt_size = vt.size();
%>
...
<%-- 								<td  width='10%' class='center content_border'><span><a href="javascript:parent.fine_reg('<%=String.valueOf(ht.get("PAID_NO"))%>', '<%=String.valueOf(ht.get("CAR_NO"))%>','<%=gubun1%>','<%=st_dt%>','<%=end_dt%>', '<%=ht.get("SEQ")%>','<%=ht.get("FILE_NAME")%>','<%=ht.get("SAVE_FOLDER")%>')" onMouseOver="window.status=''; return true" onFocus="this.blur()"><img src="/acar/images/center/button_reg.gif" align="absmiddle" border="0"></a></span></td> --%>
								<%if(del_yn.equals("N")){ %>
								<td  width='10%' class='center content_border'><input class="button btn-submit" type="button" value="삭제" onclick="notReg('<%=gubun1%>','<%=st_dt%>', '<%=end_dt%>', '<%=s_kd%>', '<%=t_wd%>','<%=ht.get("SEQ")%>','<%=ht.get("PAID_NO")%>','<%=ht.get("VIO_DT")%>','<%=ht.get("CAR_NO")%>','<%=del_yn%>')" style="width: 39; height: 20; padding:0px; margin:0px;"></td>
								<%} %>
