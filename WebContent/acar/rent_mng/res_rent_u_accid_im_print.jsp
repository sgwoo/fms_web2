        ...
        <td class=line colspan="2"> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                    ...
                    <td width=35%> 
                      &nbsp;<%=AddUtil.ChangeDate2(ma_bean.getIns_use_st())%>&nbsp;<%=i_start_h%>시<%=i_start_s%>분
                    </td>
                    <td class=title width=15%>반차일자</td>
                    <td width=35%>
                      &nbsp;<%=AddUtil.ChangeDate2(ma_bean.getIns_use_et())%>&nbsp;<%=i_end_h%>시<%=i_end_s%>분
        			</td>
                </tr>
                <tr> 
...
<!-- 사고대차 화면 출력 양식 요청해서 임시로 작업함. 2009-05-08 여기서 부터-->    
    ...
    <tr>
    	<td>&nbsp;&nbsp;&nbsp;&nbsp;위 고객은 <input name="car_no2" type="text" class="whitetext" value="<%=cont.get("CAR_NO")%>" size="12"> <b><%=cont.get("CAR_NM")%></b> 차량을 
    	  장기계약하여 이용하던중 대차 서비스를 
    	  <p>&nbsp;&nbsp;&nbsp;
    	  <b><%=AddUtil.ChangeDate2(ma_bean.getIns_use_st())%>&nbsp;<%=i_start_h%>시<%=i_start_s%>분  ~     	
    	    <%=AddUtil.ChangeDate2(ma_bean.getIns_use_et())%>&nbsp;<%=i_end_h%>시<%=i_end_s%>분 까지
    	  </b>     	
