...
<!--
	...
	function RentMemo(s_cd, c_id, user_id){
		var SUBWIN="/acar/con_rent/res_memo_i.jsp?msg_st=ret_msg&s_cd="+s_cd+"&c_id="+c_id+"&user_id="+user_id;	
...
<%
	...
	String mng_mode = ""; 
	if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("본사월렌트담당",user_id) || nm_db.getWorkAuthUser("면책금관리자",user_id) || nm_db.getWorkAuthUser("본사관리팀장",user_id)){
		mng_mode = "A";
	}	
	String mng_mode2 = ""; 
        ...
        <td class='line' width='470' id='td_title' style='position:relative;'> 
    	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
                <tr> 
                    ...
                    <td width='70' class='title'>반차알림</td>
	    ...
	    <td class='line' width='470' id='td_con' style='position:relative;'> 
    		...
    		<%	for(int i = 0 ; i < cont_size ; i++){
                ...
                <tr> 
                  ...
                  <td width='50' align='center'>
        		    <%if(auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6")){%>
        		    <%	if(!String.valueOf(reserv.get("RENT_ST")).equals("월렌트") || mng_mode.equals("A")){%>
        		    <a href="javascript:parent.reserve_action('A', '<%=reserv.get("RENT_S_CD")%>', '<%=reserv.get("CAR_MNG_ID")%>','','','');" onMouseOver="window.status=''; return true" onFocus="this.blur()"><img src="/acar/images/center/button_in_yj.gif"  align="absmiddle" border="0"></a>                    
        		    <%	}else{%>
        		    -
        		    <%	}%>
        			<%}else{%>
                  ...
                  <td width='70' align='center'>        		        
        			<a href="javascript:RentMemo('<%=reserv.get("RENT_S_CD")%>', '<%=reserv.get("CAR_MNG_ID")%>','<%=user_id%>');" onMouseOver="window.status=''; return true" onFocus="this.blur()">[반차알림]</a>                    				
                  </td>
	    ...
	    <td class='line' width='1500'>
    		...
    		<%	for(int i = 0 ; i < cont_size ; i++){
        	    ...
        	    <%if(mng_mode.equals("A")){%>
                      <a href ="javascript:parent.reserve_action('N', '<%=reserv.get("RENT_S_CD")%>', '<%=reserv.get("CAR_MNG_ID")%>','<%=reserv.get("CAR_NO")%>','<%=reserv.get("FIRM_NM")%>','<%=reserv.get("CUST_NM")%>');" title="취소"><img src="/acar/images/center/button_in_cancel.gif"  align="absmiddle" border="0"></a>                      
        	    <%}else{%>
