...
<%
	...
	double hppr = detail.getHppr();
	double stpr = detail.getStpr();
	if(carpr==0){
		hppr_per = 0;
		stpr_per = 0;
	}else{
...
function auctionUpd(ioru){
	...
	fm.gubun.value = ioru;
	fm.action="/acar/off_ls_jh/off_ls_jh_auction_upd.jsp";
	fm.target = "i_no";	
	fm.submit();
}
...
function carPurUpd(ioru){
	...
	if(ioru=="i"){
		...
		fm.action="/acar/off_ls_jh/off_ls_jh_car_pur_upd.jsp";
	}
        ...
        <td class='line' colspan="2" > 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                    ...
                    <td class="title" align="center">구조변경</td>
                    <td><%if(detail.getCar_cha_yn().equals("1")){%>
                      &nbsp;있음 
                      <%}else{%>
                      &nbsp;없음 
                      <%}%></td>
...
<%}%> 
                ...
                <tr> 
                    <td align="center"> 
                      <select name="determ_id">
                        <option value='' <%if(detail.getDeterm_id().equals("")){%>selected<%}%>>선택</option>
                        <%	if(user_size > 0){
        						for (int i = 0 ; i < user_size ; i++){
        							Hashtable user = (Hashtable)users.elementAt(i);	
        %>
                        <option value='<%=user.get("USER_ID")%>' <%if(detail.getDeterm_id().equals(user.get("USER_ID"))){%>selected<%}%>><%=user.get("USER_NM")%></option>
                        <%
        						}
        					}		%>
                      </select>
                    ...
                    <td align="center"> 
                      ...
                      원 </td>
                    <td align="center"> 
                      <input  class="num" type="text" name="hppr" size="12" value="<%=AddUtil.parseDecimal(detail.getHppr())%>" onChange="javascript:getHpprPer()">
                      (<input  class="white" type="text" name="hppr_per" size="5" value="<%=AddUtil.parseDecimal(hppr_per)%>">%)</td>
                    <td align="center"> 
                      <input  class="num" type="text" name="stpr" size="12" value="<%=AddUtil.parseDecimal(detail.getStpr())%>" onChange="javascript:getStprPer()">
    ...
    <tr> 
        <td colspan="2"><iframe src="./off_ls_jh_sc_in_b_car_pur.jsp?auth_rw=<%=auth_rw%>&car_mng_id=<%=car_mng_id%>" name="in_apprsl" width="100%" height="100" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe></td>
    </tr>
