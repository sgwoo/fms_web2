...
<%
	...
	int result = 0;
	if(gubun.equals("u")){
		result = olaD.updAuction_ban(auction_ban);
	}else if(gubun.equals("i")){
		result = olaD.insAuction_ban(auction_ban);
	}
%>
<html>
...
<%if(result >= 1){
	...
	<%}else if(gubun.equals("u")){%>
		alert("수정되었습니다.");
	<%}%>
	parent.location.href = "off_ls_jh_actn_mng_ban.jsp?auth_rw=<%=auth_rw%>&car_mng_id=<%=car_mng_id%>&seq=<%=seq%>&actn_cnt=<%=actn_cnt%>";
<%}else{%>
