...
<%
	...
	String apprsl_dt = request.getParameter("apprsl_dt_s")==null?"":request.getParameter("apprsl_dt_s");
	String determ_id = request.getParameter("determ_id")==null?"":request.getParameter("determ_id");
	String gubun = request.getParameter("gubun")==null?"":request.getParameter("gubun");
System.out.println("ar="+auth_rw+" ci="+car_mng_id);
	//로그인사용자정보 가져오기
...
<%if(result >= 1){
	...
	<%}else if(gubun.equals("u")){%>
		alert("수정되었습니다.");
	<%}%>
	parent.parent.location.href = "off_ls_jh_sc_in_detail_frame.jsp?auth_rw=<%=auth_rw%>&car_mng_id=<%=car_mng_id%>";
<%}else{%>
