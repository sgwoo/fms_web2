...
<%	if(result >= 1 || flag1 == 0 ){%>
	alert("비대상으로 등록되었습니다.\n");
	parent.parent.location.href = "/acar/call/rent_cond_frame.jsp?mode=2&auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&use_yn=<%=use_yn%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>";
<%}else{%>
	alert("데이터베이스에 문제가 발생하였습니다.\n 문의하세요 !");
	parent.c_foot.inner.location.href = "/acar/call/rent_cond_frame.jsp?mode=2&auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&use_yn=<%=use_yn%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>";
<%}%>
//-->
