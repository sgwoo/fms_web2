...
//	AddUserMngDatabase a_umd = AddUserMngDatabase.getInstance();
    ...
    if(cmd.equals("ai")){
	    ...
	    umd.insertAuthMa(user_id, m_st, m_st2, v);
    }

	MenuBean bme_r [] = umd.getMaMenuAll(m_st, m_st2, "b"); //대메뉴리스트
	
//	MenuBean bme_r [] = a_umd.getMenuAll(m_st, "b");//대메뉴 리스트
	MenuBean mme_r [] = umd.getMaMenuAll(m_st, m_st2, "m"); //중메뉴
	MenuBean sme_r [] = umd.getMaMenuAll(m_st, m_st2, "s"); //소메뉴
	
	AuthBean au_r [] = umd.getAuthMaAll(user_id, m_st, m_st2);
...
//		var t_wd = fm.user_nm[0].value;
		...
		fm.action = "./user_search.jsp?nm="+nm+"&idx="+idx;
...
<form action="./user_auth_i.jsp" name="SMenuSearchForm" method="post">
<input type="hidden" name="user_id" value="<%= user_id %>">
<input type="hidden" name="m_st" value="<%=m_st%>">
<input type="hidden" name="m_st2" value="<%=m_st2%>">
</form>

<form action="./user_auth_copy.jsp" name="SMenuCopyForm" method="post">
<table border=0 cellspacing=0 cellpadding=0 width="100%">
    <tr>
    ...
    <tr>
        ...
        <td colspan="3">&nbsp; 
        	<input name="user_nm" type="text" class="text"  size="12"> 
            <input type="text" name="s_user_id" class=text>  
            <a href="javascript:User_search('s_user_id', '0');"><img src=../images/pop/button_search.gif border=0 align=absmiddle></a>
