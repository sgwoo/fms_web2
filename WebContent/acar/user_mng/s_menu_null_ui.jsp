...
<%
	...
	if(cmd.equals("i")||cmd.equals("u")){
		...
		if(cmd.equals("i")){
			count = umd.insertMaMenu(sme_bean,"s");
		}else if(cmd.equals("u")){
			count = umd.updateMaMenu(sme_bean);
		}
	    ...
	    count = umd.deleteSMaMenu(m_st, m_st2, v);
		...
		if(user_size > 0){
			for(int i = 0 ; i < user_size ; i++){
				...
				search_auth = umd.getMaMenuAuth(String.valueOf(user.get("USER_ID")), m_st, m_st2, m_cd);
				...
				if(!String.valueOf(user.get("DEPT_ID")).equals("8888")){
					if(search_auth.equals("")){
						if(umd.setMenuBaseAuth(m_st, m_st2, m_cd, String.valueOf(user.get("USER_ID")), auth)) count=1;
					}else if(search_auth.equals("0")){
						if(umd.updateMenuBaseAuth(m_st, m_st2, m_cd, String.valueOf(user.get("USER_ID")), auth)) count=1;
					}
				}
			}
		}
