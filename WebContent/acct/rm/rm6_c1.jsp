...
<%	
	...
	if(st_dt.equals("") && end_dt.equals("") && s_cnt.equals("")){	
		...
		vt = at_db.getRm6C1List(st_dt, end_dt, s_cnt);
		vt_size = vt.size();
	}
...
<form name="form1" method="post" action="">
  ...
  <input type='hidden' name='acct_st' 	value='rm6_c1'>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <td>
        <table width=100% border=0 cellpadding=0 cellspacing=0>
          <tr>
            ...
            <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acct/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>내부통제 > 대여차량관리Cycle > <span class=style5>차량정비전표승인</span></span></td>
	...
	<table border="0" cellspacing="1" cellpadding="0" width="100%">
	  <tr>
	    ...
	    <td class="title" width="25%">지출처</td>
	    <td class="title" width="20%">전표번호</td>	
	    <td class="title" width="15%">승인자</td>    	    
	  ...
	  <%	for(int i = 0 ; i < vt_size ; i++){
			...
			if(String.valueOf(ht.get("USER_NM2")).equals("")){
			//	exception_cnt++;
			}
			
			%>
          <input type='hidden' name="seq" 	value="<%=i+1%>">
	  <input type='hidden' name="value1" 	value="<%=AddUtil.ChangeDate2(String.valueOf(ht.get("P_PAY_DT")))%>">
	  <input type='hidden' name="value2" 	value="<%=ht.get("OFF_NM")%>">
	  <input type='hidden' name="value3" 	value="<%=ht.get("AUTODOCU_WRITE_DATE")%>(<%=ht.get("AUTODOCU_DATA_NO")%>)">
	  <input type='hidden' name="value4" 	value="<%=ht.get("USER_NM2")%>">
	  <input type='hidden' name="value5" 	value="<%=ht.get("USER_DT2")%>">
	  ...
	  <tr>
	    ...
	    <td align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("P_PAY_DT")))%></td>            
            ...
            <td align='center'><%=ht.get("AUTODOCU_WRITE_DATE")%>(<%=ht.get("AUTODOCU_DATA_NO")%>)</td>                        
            <td align='center'><%=ht.get("USER_NM2")%></td>            
            <td align='center'><%=ht.get("USER_DT2")%></td>
            <!--
	    <td align='center'>
	      <select name="result">
	...
	<table border="0" cellspacing="1" cellpadding="0" width="100%">
	  <tr>
	    <td class="title" width="20%">서브프로세스명</td>
	    <td width="80%">&nbsp;
	      대여차량유지보수(사고처리포함)</td>	    
	  </tr>     
	  <tr>
	    <td class="title" width="20%">통제활동번호/명</td>
	    <td width="80%">&nbsp;
	      C1/전표에 대한 승인 </td> 
	  </tr>     
	  <tr>
	    <td class="title" width="20%">테스트 절차</td>
	    <td width="80%">&nbsp;
	      차량정비전표를 샘플링하여 정당한 총무팀장의 승인여부를 확인한다.
	    </td>	    
	  </tr>     
	</table>
      </td>
