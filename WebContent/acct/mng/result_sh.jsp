...
<%
	...
	//평가기간
	Vector dts = at_db.getAcctDtList();
	int dt_size = dts.size();
...
<!--
	...
	function Search()
	{
		...
		if(fm.gubun2.value == ''){ alert('평가기간을 선택하십시오.'); return; }		
		fm.target = "c_foot";		
      ...
      <td>
        <table width=100% border=0 cellpadding=0 cellspacing=0>
          <tr>
            ...
            <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acct/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>내부통제결과 > <span class=style5>평가결과관리</span></span></td>
            ...
            <table border=0 cellspacing=1>
	    		...
	    		<select name='gubun2'>
                    	  ...
                    	  <%	for (int i = 0 ; i < dt_size ; i++){
                        		Hashtable dt = (Hashtable)dts.elementAt(i); %>
		    	  <option value="<%=dt.get("S_DT")%><%=dt.get("E_DT")%>" <%if(gubun2.equals(String.valueOf(dt.get("S_DT"))+""+String.valueOf(dt.get("E_DT"))))%>selected<%%>><%=dt.get("S_DT")%>~<%=dt.get("E_DT")%></option>
		    	  <%	}%>
            		</select>	
            		&nbsp;&nbsp;
