...
//	System.out.println("email_fine_bill m_id=" +m_id + "|l_cd=" + l_cd + "|dem_dt=" + dem_dt+"|e_dem_dt="+e_dem_dt);
	...
	//Vector fines = afm_db.getFinePreDemList2( m_id, l_cd, dem_dt, e_dem_dt, sort, asc);
                            ...
                            <tr>
								...
								<%if (!coll_dt.equals("")){%>
								  <span class=style2><span class=style1><b>
								    <%if (coll_dt.length()>7){%>
								    <%=coll_dt.substring(0,4)%>년&nbsp;<%=coll_dt.substring(5,7)%>월&nbsp;<%=coll_dt.substring(8,10)%>일
									<%}else{%>
									<%=coll_dt%>
									<%}%>
								  </b></span> 결제하신 금액은 <span class=style3><b><%=AddUtil.parseDecimal(tot_fine)%></b></span> 원입니다.</span></td>
								  <%}%>
                            </tr>
                            <tr>
                                <td height=10></td>
