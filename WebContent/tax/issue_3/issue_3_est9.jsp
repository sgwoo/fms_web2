...
<%
	...
	//대차료 스케줄
	Vector grts = ScdMngDb.getMyAccidLScdTaxList(s_br, "", "N");
...
<!--
	function client_select(car_mng_id, accid_id, client_id, site_id, firm_nm, rent_mng_id, rent_l_cd, seq_no, rent_s_cd, cust_st){
		...
		fm.cust_st.value = cust_st;								
		fm.action = "issue_3_sc9.jsp";
		fm.target = "d_content";
		fm.submit();
	}
//-->
...
<%@ include file="/include/search_hidden.jsp" %>
  ...
  <table width=100% border=0 cellpadding=0 cellspacing=0> 
    <tr> 
      <td colspan="2"><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>대차료 미발행리스트 (거래명세서 기발행분)</span></td>
    </tr> 
    ...
    <tr>
      <td colspan="2" class="line">
        ...
        <tr>
          ...
          <td width="5%" rowspan="2" class='title'>사고구분</td>
          <td width='7%' rowspan="2" class='title'>보험회사</td>
		...
		<%	int count = 0;
			if(grt_size > 0){
				for (int i = 0 ; i < grt_size ; i++){
					...
					if(String.valueOf(grt.get("CAR_MNG_ID")).equals("037044") && String.valueOf(grt.get("ACCID_ID")).equals("079111")) continue;
					count++;
		%>		
        <tr>
          <td align="center"><%=count%></td>
          <td align="center"><%=grt.get("ST_NM")%></td>		  
          <td align="center"><a href="javascript:client_select('<%=grt.get("CAR_MNG_ID")%>','<%=grt.get("ACCID_ID")%>','<%=grt.get("CLIENT_ID")%>','','<%=grt.get("FIRM_NM")%>','<%=grt.get("RENT_MNG_ID")%>','<%=grt.get("RENT_L_CD")%>','<%=grt.get("SEQ_NO")%>','<%=grt.get("RENT_S_CD")%>','<%=grt.get("CUST_ST")%>')"><%=grt.get("RENT_L_CD")%></a></td>
          ...
          <td align="center"><span title='<%=grt.get("INS_COM")%>'><%=AddUtil.subData(String.valueOf(grt.get("INS_COM")), 4)%></span></td>
          <td align="right"><%=Util.parseDecimal(String.valueOf(grt.get("MC_AMT")))%>원&nbsp;</td>
          <td align="center"><%=AddUtil.ChangeDate2(String.valueOf(grt.get("REQ_DT")))%></td>
          <td align="right"><%=Util.parseDecimal(String.valueOf(grt.get("EXT_PAY_AMT")))%>원&nbsp;</td>
