...
<%@ page import="java.util.*, acar.util.*, acar.cont.*, acar.fee.*, acar.common.*, tax.*, acar.bill_mng.*, acar.client.*, acar.car_register.*"%>
...
<%
	...
	if(auth_rw.equals(""))	auth_rw = rs_db.getAuthRw(user_id, "05", "06", "17");
	...
	//타시스템 계산서 발행인 경우
	String print_nm = "";
	if (client.getPrint_st().equals("9")) {
		print_nm = "타시스템발행";
	}
	...
	//분할청구정보
	Vector rtn = af_db.getFeeRtnClientList(client_id, rent_mng_id, rent_l_cd);
	...
	int grt_size = 0;
	if(!client_id.equals("")){
		grts = ScdMngDb.getAccidServScdList("", client_id, "N");
		grt_size = grts.size();	
	}
	...
	//세금계산서일자가 오늘보다 크면 오늘날짜
	if(tax_est_dt.equals("") || AddUtil.parseInt(AddUtil.replace(tax_est_dt,"-","")) > AddUtil.parseInt(today)){
		tax_est_dt = today;
	}
	tax_est_dt = AddUtil.replace(tax_est_dt,"-","");
%>
...
<!--
	...
	function tax_reg(){
		...
		if(fm.scd_size.value == '0'){	alert('스케줄이 없습니다.'); return; }
		if(fm.count.value == '0'){		alert('선택된 스케줄이 없습니다.'); return; }		
	...
	function scd_select(idx){
		...
		if(size == 1){//1건
			if(fm.ch_l_cd.checked == true){
				...
				fm.tax_bigo.value 		= "차량수리("+ch_split[4]+")";
...
<%@ include file="/include/search_hidden.jsp" %>
  ...
  <table width=100% border=0 cellpadding=0 cellspacing=0>
          ...
          <td class="line">
            ...
            <tr>
			  ...
			  <input type="hidden" name="i_site_id" value="<%=site_id%>">
				...
				<%}%>  
				<!--본사-->
		          	<option value='00||<%=client.getFirm_nm()%>||<%=client.getClient_nm()%>||<%=client.getO_addr()%>||<%=client.getBus_cdt()%>||<%=client.getBus_itm()%>||<%=client.getTaxregno()%>||0' <%if(site_id.equals("")){%>selected<%}%>><%if(!client.getEnp_no1().equals("")){%><%=client.getEnp_no1()%>-<%=client.getEnp_no2()%>-<%=client.getEnp_no3()%> 본사<%}else{%><%=client.getSsn1()%>-<%=client.getSsn2()%><%}%></option>				  
		          	<!--지점-->
				<%	for(int i = 0 ; i < c_site_size ; i++){
          			...
          			<option value='00||<%=client2.getFirm_nm()%>||<%=client2.getClient_nm()%>||<%=client2.getO_addr()%>||<%=client2.getBus_cdt()%>||<%=client2.getBus_itm()%>||<%=client2.getTaxregno()%>||<%=client2.getClient_id()%>' ><%if(!client2.getEnp_no1().equals("")){%><%=client2.getEnp_no1()%>-<%=client2.getEnp_no2()%>-<%=client2.getEnp_no3()%><%}else{%><%=client2.getSsn1()%>-<%=client2.getSsn2()%><%}%> 승계전업체 <%=client2.getFirm_nm()%></option>				  
          			<%}%>
          			<!--계약승계 후계약 업체-->
				...
				<%	for(int i = 0 ; i < rtn_size ; i++){
						Hashtable rtn_ht = (Hashtable)rtn.elementAt(i);%>	  		
			      	<option value='<%=rtn_ht.get("SEQ")%>||<%=rtn_ht.get("FIRM_NM")%>||<%=rtn_ht.get("CLIENT_NM")%>||<%=rtn_ht.get("O_ADDR")%>||<%=rtn_ht.get("BUS_CDT")%>||<%=rtn_ht.get("BUS_ITM")%>||<%=rtn_ht.get("TAXREGNO")%>||<%=rtn_ht.get("CLIENT_ID")%>'><%=rtn_ht.get("ENP_NO")%> <%=rtn_ht.get("FIRM_NM")%></option>				  
          			<%	}%>
            ...
            <tr>
              ...
              <td colspan="3">&nbsp;<input type="text" name="i_firm_nm" size="50" value="<%=i_firm_nm%>" class="whitetext" readonly><font color=red><%=print_nm%></font></td>
              </tr>
            <tr>
    ...
    <tr>
      <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>기타 스케줄</span></td>
      <td align="right">
	  </td>
    </tr>
    ...
    <tr>
      <td colspan="2" class="line">
        ...
        <tr>
          ...
          <td width='9%' class='title'>면책금</td>
          <td width='9%' class='title'>사고일자</td>
        ...
        <tr>	  		
          <td align="center"><input type="checkbox" name="ch_l_cd" onclick="javascript:scd_select(<%=i%>)" value="<%=grt.get("RENT_L_CD")%>,<%=grt.get("CAR_MNG_ID")%>,<%=grt.get("SERV_ID")%>,차량수리비,<%=grt.get("CAR_NO")%>,<%=grt.get("CAR_NM")%>,<%=grt.get("CUST_S_AMT")%>,<%=grt.get("CUST_V_AMT")%>,<%=grt.get("CUST_AMT")%>,,<%=AddUtil.ChangeDate2(String.valueOf(grt.get("IPGODT")))%>,<%=AddUtil.ChangeDate2(String.valueOf(grt.get("CHULGODT")))%>" <%if(grt_size==1)%>checked<%%>></td>
          <td width='9%' align="center"><%=grt.get("ST_NM")%></td>		  
          <td width='10%' align="center"><a href="javascript:client_select('<%=grt.get("CLIENT_ID")%>','','<%=grt.get("FIRM_NM")%>')"><%=grt.get("RENT_L_CD")%></a></td>
          ...
          <td align="center"><%=Util.parseDecimal(String.valueOf(grt.get("TOT_AMT")))%></td>
          <td align="center"><%=Util.parseDecimal(String.valueOf(grt.get("CUST_AMT")))%></td>
    ...
    <tr>
      <td colspan="2" class="line">
        ...
        <tr>
          ...
          <td height="22" align="center" style="border-left: #000000 1px solid; border-bottom: #000000 1px solid"><span class="ledger_contC">
            <input type="text" name="item_car_nm" size="25" value="" class="whitetext" >
          </span></td>
          <td height="22" align="center" style="border-left: #000000 1px solid; border-bottom: #000000 1px solid"><span class="ledger_contC">
    ...
    <tr>
      <td colspan="2"><table style="BORDER-COLLAPSE: collapse" cellspacing=0 cellpadding=0 width=100%>
        <tbody>
		  ...
		  <%	String req_y = AddUtil.getDate(1);
				String req_m = AddUtil.getDate(2);
			  	String req_d = AddUtil.getDate(3);
				if(tax_est_dt.length() == 8){
					req_y = tax_est_dt.substring(0,4);
					req_m = tax_est_dt.substring(4,6);
					req_d = tax_est_dt.substring(6,8);
				}
		  %>
          <tr>
            <td class=ledger_contC style="BORDER-RIGHT: #0166a9 1px solid; BORDER-TOP: #0166a9 1px solid; BORDER-LEFT: #0166a9 2px solid; BORDER-BOTTOM: #0166a9 2px solid" height=25 align="center"><input type="text" name="req_y" size="4" value="<%=req_y%>" class="taxtext"></td>
            <td class=ledger_contC style="BORDER-RIGHT: #0166a9 1px solid; BORDER-TOP: #0166a9 1px solid; BORDER-LEFT: #0166a9 1px solid; BORDER-BOTTOM: #0166a9 2px solid" height=25 align="center"><input type="text" name="req_m" size="2" value="<%=req_m%>" class="taxtext" onBlur="javascript:document.form1.tax_m.value=this.value;"></td>
            <td class=ledger_contC style="BORDER-RIGHT: #0166a9 1px solid; BORDER-TOP: #0166a9 1px solid; BORDER-LEFT: #0166a9 1px solid; BORDER-BOTTOM: #0166a9 2px solid" height=25 align="center"><input type="text" name="req_d" size="2" value="<%=req_d%>" class="taxtext" onBlur="javascript:document.form1.tax_d.value=this.value;"></td>
    ...
    <%if(!client2.getFirm_nm().equals("")){%>	
    <tr>
      <td>* 승계업체가 공급받는자일 경우 이메일 수신자 정보를 알맞게 넣어 주십시오.</td>
    </tr>
    <%}%>
    <%if(rtn_size>0){%>	
    <tr>
      <td>* 분할업체가 공급받는자일 경우 이메일 수신자 정보를 알맞게 넣어 주십시오.</td>
    </tr>
    <%}%>
    <tr>
      <td class=h></td>
      ...
      <td colspan="2" class="line"><table border="0" cellspacing="1" cellpadding="0" width='100%'>
        ...
        <tr>
          ...
          <td width='90%'>&nbsp;
              ...
              리스사업매출&nbsp;
              <%if(client.getClient_id().equals("009821")){//주식회사 두바이카%>
              <input type="radio" name="car_use" value="4">
              카드사캐쉬백수익&nbsp;
              <%}%>
			  </td>
          </tr>
...
//		fm.item_value.value 		= parseDecimal(toInt(parseDigit(ch_split[8])) - toInt(parseDigit(fm.item_supply.value)));		
		...
		fm.tax_g.value			= fm.item_g.value;
		fm.tax_bigo.value 		= "차량수리("+ch_split[4]+")" + " " + fm.br_bigo.value;					
		fm.tax_supply.value 		= fm.item_supply.value;
		fm.tax_value.value 		= fm.item_value.value;	
		fm.tax_amt.value 		= fm.item_amt.value;		
		fm.count.value 			= 1;
