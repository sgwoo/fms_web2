...
<%@ page import="java.util.*, acar.util.*, tax.*, acar.fee.*"%>
...
<%
	out.println("세금계산서 개별-통합 발행하기 1단계"+"<br><br>");
	...
	//사용할 item_id 가져오기
	item_id = IssueDb.getItemIdNext(tax_out_dt);
	...
	for(int i=0;i < vid_size;i++){
		...
		//이미 발행한 계산서 중복체크하기
		if(IssueDb.getTaxMakeCheck2(ch_l_cd, ch_fee_tm, ch_rent_st, ch_rent_seq) > 0) continue;
		
		//이미 발행한 청구서 중복체크하기
		if(IssueDb.getTaxItemMakeCheck(ch_l_cd, ch_fee_tm, ch_rent_st, ch_rent_seq) > 0) continue;
		...
		out.println(base.getCar_mng_id()+ch_l_cd+tax_out_dt+"<br>");
