...
<!--
	...
	function select(car_off_id, ven_code){		
		var fm = document.form1;
		<%if(!go_url.equals("")){%>
		if(fm.go_target.value == 'd_content'){
			opener.parent.location.href = "<%=go_url%><%=hidden_value%>&car_off_id="+car_off_id+"&ven_code="+ven_code+"&app_yn=<%=app_yn%>";		
		}else{
			opener.parent.c_foot.location.href = "<%=go_url%><%=hidden_value%>&car_off_id="+car_off_id+"&ven_code="+ven_code;
		}
		<%}else{%>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
    ...
    <tr> 
      <td align='left'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	    <select name='s_kd'>
          <option value="1" <%if(s_kd.equals("1")){%> selected <%}%>>영업소명</option>          
        </select>
    ...
    <tr> 
      <td colspan="2"><iframe src="./s_car_off_in.jsp?s_br=<%=s_br%>&type=<%=type%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>" name="inner" width="100%" height="250" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 ></iframe></td>
    </tr>
    <tr> 
      <td colspan="2">&nbsp;</td>
    </tr>	
