...
<%
	...
	int height = AddUtil.parseInt(s_height)-emp_top_height-sh_height-(h_cnt*25)-310;//현황 라인수만큼 제한 아이프레임 사이즈
%>
...
<!--
	...
	function CardUseList(acct_code_nm, acct_code){
		...
		fm.action = "card_use_card_us_sc_in.jsp";
		fm.target = "inner";
		fm.submit();
	}
//-->
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>	
        ...
        <td class="line">
            <table border="0" cellspacing="1" cellpadding="0" width='100%'>      	
                ...
                <%	for(int i = 0 ; i < vt_size ; i++){
        					...
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("51100")) acct_code_yn[0] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("54000")) acct_code_yn[1] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("51200")) acct_code_yn[2] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("53900")) acct_code_yn[3] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("82900")) acct_code_yn[4] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("51300")) acct_code_yn[5] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("81400")) acct_code_yn[6] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("21200")) acct_code_yn[7] = "Y";
        					if(String.valueOf(ht.get("ACCT_CODE")).equals("13100")) acct_code_yn[8] = "Y";
        					%>
                <tr>
    ...
    <tr> 
        <td height="<%=height%>" colspan="2"><iframe src="card_use_card_us_sc_in.jsp<%=hidden_value%>&acct_code=<%=acct_code%>&acct_code_nm=<%=acct_code_nm%>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 > 
