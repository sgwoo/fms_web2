...
<!--
	...
	function CardUseList(chk2, t_date, cardno){
		...
		if(chk2 != ''){
			fm.chk2.value = chk2;
			fm.chk1.value = '3';
			fm.st_dt.value = t_date;
			fm.action = "card_use_card_d_frame.jsp";
		}else{
    ...
    <tr>
      <td class="line"><table border="0" cellspacing="1" cellpadding="0" width='100%'>
        ...
        <tr>
          ...
          <td colspan="3" class='title'>소유자</td>
          <td colspan="2" class='title'>당일</td>
          <td colspan="2" class='title'>전일</td>
          <td colspan="2" class='title'>전전일</td>
          </tr>
        <tr>
        ...
        <tr>
          ...
          <td align="right"><a href="javascript:CardUseList('1','<%=ht.get("T1")%>','<%=ht.get("CARDNO")%>')"><%=cnt2[1]%></a></td>
          <td align="right"><a href="javascript:CardUseList('1','<%=ht.get("T1")%>','<%=ht.get("CARDNO")%>')"><%=Util.parseDecimal(amt2[1])%></a></td>
          <td align="right"><a href="javascript:CardUseList('2','<%=ht.get("T2")%>','<%=ht.get("CARDNO")%>')"><%=cnt2[2]%></a></td>
          <td align="right"><a href="javascript:CardUseList('2','<%=ht.get("T2")%>','<%=ht.get("CARDNO")%>')"><%=Util.parseDecimal(amt2[2])%></a></td>
          <td align="right"><a href="javascript:CardUseList('3','<%=ht.get("T3")%>','<%=ht.get("CARDNO")%>')"><%=cnt2[3]%></a></td>
          <td align="right"><a href="javascript:CardUseList('3','<%=ht.get("T3")%>','<%=ht.get("CARDNO")%>')"><%=Util.parseDecimal(amt2[3])%></a></td>
          </tr>
        <%	}%>
        <tr>
