...
<%
	...
	auth_rw = rs_db.getXmlAuthRw(ck_acar_id, "04", "07", "12");
	
	
	//거래처정보
	TradeBean[] vens = neoe_db.getBaseTradeList(s_kd, t_wd); //-> neoe_db 변환
...
<form name='form1' method='post' action='sc_vendor_list2.jsp'>
...
<input type='hidden' name='from_page' value='<%=from_page%>'>
  <table border="0" cellspacing="0" cellpadding="0" width=100%>
      ...
      <td class='line'> 
        <table border="0" cellspacing="1" cellpadding="0" width=100%>
          ...
          <tr> 
            ...
            <td align="center">
			  <%if(from_page.equals("/fms2/pay_mng/pay_excel_reg.jsp")){//출금원장엑셀등록에서 검색 호출시%>
			  <%= ven.getS_idno()%>
			  <%}else{%>
			  <a href="javascript:setVendor('<%= ven.getCust_code()%>','<%= AddUtil.replace(ven.getCust_name(),"'","")%>','<%= ven.getS_idno()%>', '<%=ven_st%>' );" title='거래처 선택'><%= ven.getS_idno()%></a>			  
			  <%}%>			  
			</td>
            <td>&nbsp;
			  <%if(from_page.equals("/fms2/pay_mng/pay_excel_reg.jsp")){//출금원장엑셀등록에서 검색 호출시%>
			  [<span title='거래처코드'><font color=red><%= ven.getCust_code()%></font></span>]&nbsp;<%= ven.getCust_name()%>&nbsp;<%if(!ven.getDc_rmk().equals("")){%>(<%= ven.getDc_rmk()%>)<%}%>
			  <%}else{%>
			  <a href="javascript:setVendor('<%= ven.getCust_code()%>','<%= AddUtil.replace(ven.getCust_name(),"'","")%>','<%= ven.getS_idno()%>', '<%=ven_st%>');" title='거래처 선택'><%= ven.getCust_name()%>&nbsp;<%if(!ven.getDc_rmk().equals("")){%>(<%= ven.getDc_rmk()%>)<%}%></a>
			  <%}%>
			</td>            
			...
			<a href="javascript:CardDocHistory('<%= ven.getCust_code()%>');" title='카드전표에 입력된 거래처 이력보기'><img src=/acar/images/center/button_in_ir.gif align=absmiddle border=0></a>
			</td>            						
          </tr>
                <%	}
				}%>		  
	...
	<%if(from_page.equals("/fms2/pay_mng/pay_excel_reg.jsp")){//출금원장엑셀등록에서 검색 호출시%>
     <tr> 
      <td align='left'>&nbsp;&nbsp;&nbsp;※ 거래처명에 있는 [거래처코드]를 참고하시면 됩니다.
     </td>
    </tr>
	<%}else{%>
     <tr> 
