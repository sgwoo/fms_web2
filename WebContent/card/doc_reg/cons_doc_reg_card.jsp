...
<%
	...
	String card_name 	= request.getParameter("card_name")==null?"":request.getParameter("card_name");
	String card_sdate 	= request.getParameter("card_sdate")==null?"":request.getParameter("card_sdate");
	String card_edate 	= request.getParameter("card_edate")==null?"":request.getParameter("card_edate");
...
//sc_in에서 넘어온 것
	...
	function setCardCode(cardno, card_name, card_sdate, card_edate, buy_dt, ven_name, ven_code, buy_s_amt, buy_v_amt, buy_amt, buy_id){
		var fm = opener.opener.document.form1;	
		
		fm.cardno.value 		= cardno;		
		fm.card_name.value 	= card_name;
		fm.card_sdate.value 	= ChangeDate(card_sdate);
		if(card_edate.length == 6){
			card_edate = card_edate+getMonthDateCnt(card_edate.substring(0,4), card_edate.substring(4,6));
		}
		fm.card_edate.value 	= ChangeDate(card_edate);
		
		fm.buy_dt.value 		= ChangeDate(buy_dt);		
		fm.ven_name.value 	= ven_name;
		fm.ven_code.value 		= ven_code;		
		fm.buy_s_amt.value 	= buy_s_amt;		
		fm.buy_v_amt.value 	= buy_v_amt;		
		fm.buy_amt.value 	= buy_amt;	
		fm.buy_id.value 	= buy_id;
...
<script type="text/javascript">
    ...
    gridQString = "../doc_mng/sc_doc_mng_xml.jsp<%=hidden_value%>&from_page=<%=from_page%>";
