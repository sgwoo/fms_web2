...
<!--
	...
	function Disp2(idx1, title, nm, tel, m_tel){
		var fm = document.form1;
		opener.form1.call_t_nm[<%=idx1%>].value 	= nm;
		opener.form1.call_t_tel[<%=idx1%>].value 	= m_tel;
		if(m_tel == '' && tel !=''){	opener.form1.call_t_tel[<%=idx1%>].value 	= tel;	}		
		window.close();
	}		
...
<!-- 					<a href="javascript:search('<%=idx1%>','<%=t_wd%>');"><img src=/acar/images/center/button_search.gif border=0 align=absmiddle></a> -->
        ...
        <td class=line>
    	    <table border="0" cellspacing="1" width=100%>
                ...
                <tr align="center">
                    ...
                    <td><%=ht.get("GUBUN")%></td>
                    <td><%=ht.get("TITLE")%></td>		  
                    <td><a href="javascript:Disp2('<%=idx1%>', '<%=ht.get("TITLE")%>', '<%=ht.get("NM")%>', '<%=ht.get("TEL")%>', '<%=ht.get("M_TEL")%>')" onMouseOver="window.status=''; return true"><%=ht.get("NM")%></a></td>
                    <td><%=ht.get("TEL")%></td>
                    <td><%=ht.get("M_TEL")%></td>
                </tr>
                <%		}%>
