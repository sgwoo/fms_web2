...
<!--
	function Search(){
		...
		fm.action="rent_search.jsp?idx1="+<%=idx1%>;
		fm.submit();
	}
	function enter() {
	...
	function setCardCode(idx,rent_mng_id, rent_l_cd, client_id, car_no, first_car_no, car_nm, firm_nm, car_mng_id, mgr_nm, mgr_tel, bus_id2, bus_nm2, bus_id, bus_nm, tot_dist, serv_dt){
		var fm = document.form1;
		<%if(go_url.equals("/fms2/pay_mng/pay_dir_reg.jsp") || go_url.equals("/fms2/pay_mng/pay_dir_user_reg.jsp")){%>
		...
		<%}else if(go_url.equals("/doc_mng/sc_doc_reg_u.jsp")){%>
			opener.form1.rent_l_cd[idx].value = rent_l_cd;	
			opener.form1.item_name[idx].value	= car_no;
			opener.form1.item_code[idx].value	= car_mng_id;
	    	opener.form1.acct_cont[idx].value = firm_nm + " - " + car_no;	
		<%}else{%>
		 	opener.form1.rent_l_cd[idx].value = rent_l_cd;	
			opener.form1.item_name[idx].value	= car_no;
			opener.form1.item_code[idx].value	= car_mng_id;
	    	opener.form1.acct_cont[idx].value = firm_nm + " - " + car_no;	
	    	//opener.form1.rent_l_cd[0].value = rent_l_cd;		//<--상단에서 null일 때 0 처리 하므로 문제없을듯.(문제시 위의 소스대신 대체할것!(기존)) 	
			//opener.form1.item_name[0].value	= car_no;		//2017.12.01
			//opener.form1.item_code[0].value	= car_mng_id;
	    	//opener.form1.acct_cont[0].value = firm_nm + " - " + car_no;	
			opener.form1.last_dist[idx].value=tot_dist;
			opener.form1.last_serv_dt[idx].value=serv_dt;
			opener.form1.tot_dist[idx].value="";
			opener.form1.oil_liter[idx].value="";
	    ...
	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
          ...
          <%	if(ck_size > 0){
					for (int i = 0 ; i < ck_size ; i++){
						...
						if ( acct_code.equals("00004") ) { // 전표 유류대인 경우 
							ht9 = csd.getCarInfoOil(String.valueOf(card_kind.get("CAR_MNG_ID")));
						} else {
							ht9 = csd.getCarInfo(String.valueOf(card_kind.get("CAR_MNG_ID")));
						}
