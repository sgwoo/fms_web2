...
<%
	...
	Vector vts = CardDb.getCardDocSearchList2(s_br, chk1, chk2, chk3, chk4, gubun1, gubun2, gubun3, gubun4, gubun5, gubun6, gubun7, gubun8, "",  st_dt, end_dt, s_kd, t_wd1, t_wd2, sort, asc, cgs_ok);
	    ...
	    <td class='line' width='40%' id='td_title' style='position:relative;'> 
	        <table border="0" cellspacing="1" cellpadding="0" width='100%'>
                <tr> 
                    ...
                    <td width='26%' class='title'>거래처</td>
                </tr>
            </table>
        </td>
	    ...
	    <td class='line' width='40%' id='td_con' style='position:relative;'> 
          ...
          <%for(int i = 0 ; i < vt_size ; i++){
                ...
                <tr> 
                    ...
                    <td width='8%' align="center"><%=Util.subData(String.valueOf(ht.get("OWNER_NM")), 4)%></td>
                    <td width='8%' align="center"><%=Util.subData(String.valueOf(ht.get("USER_NM")), 4)%></td>
                    <td width='11%' align="center"><%=AddUtil.ChangeDate2(String.valueOf(ht.get("BUY_DT")))%></td>
                    <td width='26%' align="center"><a href="javascript:CardDocUpd('<%=ht.get("CARDNO")%>','<%=ht.get("BUY_ID")%>','<%=ht.get("DOC_MNG_ID")%>','<%=ht.get("BUY_USER_ID")%>')"><span title='<%=ht.get("VEN_NAME")%>'><%=Util.subData(String.valueOf(ht.get("VEN_NAME")), 6)%></span></a></td>
                </tr>
          <%}%>
                <tr align="center">
                    <td class="title"></td>
	    ...
	    <td class='line' width='60%'>
    	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
              <%for(int i = 0 ; i < vt_size ; i++){
    				Hashtable ht = (Hashtable)vts.elementAt(i);%>
                <tr> 
                    <td width='7%' align="center"><a href="javascript:CardDocHistory('<%=ht.get("VEN_CODE")%>','<%=ht.get("CARDNO")%>','<%=ht.get("BUY_ID")%>')"><%=ht.get("BUY_ST")%></a></td>			
