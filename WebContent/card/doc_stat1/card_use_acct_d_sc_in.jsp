...
<!--
	...
	function CardUseList(acct_code_g, acct_code_g_nm){
		...
		fm.action = "card_use_acct_d_sc_list.jsp";
		fm.target = "inner_in";
		fm.submit();
	}
//-->
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>	
	...
	<%	if(vt_size2 > 0){
		int height = 340-(vt_size2*25);
	%>
    <tr> 
        <td></td>
    </tr>		
    <tr> 
        <td height="<%=height%>" colspan="2"><iframe src="card_use_acct_d_sc_list.jsp<%=hidden_value%>&acct_code=<%=acct_code%>&acct_code_nm=<%=acct_code_nm%>&acct_code_g=<%=acct_code_g%>" name="inner_in" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 > 
