...
function updateVar(){
	...
	fm.action = "/fms2/mis/man_cost_var_iu_2015_2.jsp";
...
//계산식보기
function view_sik(){
	window.open("man_cost1_jung.jsp?popup=Y","sik","left=50,top=50,width=1300,height=750,scrollbars=yes");
}


//변수수정 -캠페인포상건만 저장 ( 20200316 ) 
function updateVarS(){
	var fm = document.form1;
	if(!confirm("캠페인 포상값을 저장하시겠습니까?"))	return;	
	fm.action = "campaign2019_magam_i.jsp?gubun=1";
	fm.target = 'i_no';	
//	fm.target = '_blank';	
	fm.submit();
}
...
//프린트하기
function cmp_print(){
	window.open("man_cost1_settle_print.jsp?save_dt=<%=save_dt%>&auth_rw=<%=auth_rw%>","print","left=30,top=50,width=950,height=600,scrollbars=yes");	
}
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>경영정보 > 캠페인관리 > <span class=style5>관리비용절감캠페인(1군 - 정비)</span></span></td>
 	  ...
 	  <td class="line">
	    <table border="0" cellspacing="1" cellpadding="0" width='900'>
         <tr> 
           ...
           <td width='100' class='title' rowspan=2>1대당 월평균<br>정비비용 </td>
...
<% } %>      	 
	   ...
	   <% 	} %>
	      ...
	      <tr> 
	        ...
	        <td class=title style='text-align:right;'><%= Util.parseDecimal(t_ave_amt) %><br><%= Util.parseDecimal(t_ave_amt/vt_size2) %></td>
	 		<td class=title style='text-align:right;'><%= Util.parseDecimal(sum_cam_amt)%><br><%= Util.parseDecimal(sum_cam_amt/vt_size2)%></td>
...
<!-- 평가기준 -->
 ...
 -->  	
      ...
      <td colspan=2><font color="#FF00FF">♣ 관리비용</font>     : ((기본식평균/전체기본식평균)*(기본식비용/비용(본인))+(일반식평균/전체일반식평균)*(일반식비용/비용(본인)))*1대당평균관리비용(전체비용/전체대수)  
        &nbsp;&nbsp;</td> -->
           <td colspan=2><font color="#FF00FF">♣ 관리비용</font>     : ((본인기본식1대당평균비용/전체기본식1대당평균비용)*(본인기본식총비용/본인총비용)+(본인일반식1대당평균비용/전체일반식1대당평균비용)*(본인일반식총비용/본인총비용))*1대당평균관리비용(전체비용/전체대수)
        &nbsp;&nbsp;</td> 
   	</tr>
   	   <tr> 
      <td colspan=2><font color="#FF00FF">♣ 예비차 정비</font> :   기본식 ->지점별 평균비용(지점별 예비차 전체비용/지점인원수 )/캠페인전체대수 * 기본식전체대수 ,  일반식 -> 지점별 평균비용(지점별 예비차 전체비용/지점인원수 )/캠페인전체대수 *일반식전체대수</td>
   	</tr>
    <tr> 
    ...
    <tr> 
     ...
     <tr> 
      ...
      <td>: 
		...
		피해사고수리비 적용금액: 100만원 미만 - 10만원 적용, 100만원 초과 - 초과분의 5% 적용
		<br>* 사고등록일로 60일이내 - 100% , 60~120:40% , 120초과:10%  반영 <!--  피해사고수리비입력건 차감 (폐차제외) -->
		</td>	
    </tr>
...
<% if ( auth_rw.equals("6") || auth_rw.equals("4") ) { %>   
  <table width="900" border="0" cellspacing="0" cellpadding="0">
    <tr> 
      <td align=right><a href='javascript:updateVar()' onMouseOver="window.status=''; return true"><img src="/acar/images/center/button_modify_bs.gif"  align="absmiddle" border="0"></a>
