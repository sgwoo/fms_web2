...
//비용캠페인변수 : cost_campaign 테이블
	Hashtable ht3 = ac_db.getCostCampaignVar("J");
	...
	if(save_dt.equals(""))	save_dt = ad_db.getMaxSaveDt("stat_bus_cost_j_cmp");
			
	Vector vts2 = ac_db.getCostManCampaignNewJ(save_dt, "J", base_cnt, "", "");
	...
	<td class='line' width='72%'>
	    <table border="0" cellspacing="1" cellpadding="0" width='100%' height=71>
         <tr>
         	...
         	<td width='5%' class="title" rowspan=3>차량정비비</td>
         	<td width='5%' class='title' rowspan=3 >대차비용</td>
         	<td width=16% class="title" colspan=5>기타</td>
           <td class="title" rowspan=3 width="5%">차량관리비용계<br>(정비비+대차<br>+기타)</td>  
           <td width="5%"  class="title" rowspan=3>예비차</td>  
            <td width='6%' class='title' rowspan=3>비용정산계<br>(차량관리비용<br>+활동비)</td>                	
          </tr>
          <tr>
  ...
  <tr height=100>
	  <td class='line' width='28%' id='td_con' style='position:relative;'> 
        ...
        <%	
         ...
         long t_amt3[] = new long[1]; //탁송료(일반식)
        	long t_amt4[] = new long[1];   //탁송료(기본식)
        	long t_amt5[] = new long[1];  //탁송유류대(일반식)
        	long t_amt6[] = new long[1]; //탁송유류대(기본식)     
        	long t_amt7[] = new long[1]; //자동차검사(일반식)    
        	long t_amt8[] = new long[1]; //자동차검사(기본식)    
        	long t_amt9[] = new long[1]; //긴급출동(일반식)
        	long t_amt10[] = new long[1];  //긴급출동(기본식)   
        	long t_amt11[] = new long[1];  //대차비용(일반식)
         	long t_amt12[] = new long[1];  //대차비용(기본식) 
         	
        	long t_amt13[] = new long[1];  //정비비(기본식) 
	    ...
	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
          <%for(int i = 0 ; i < vt_size2 ; i++){
					...
					for(int j=0; j<1; j++){	
						...
						t_amt7[j] += AddUtil.parseLong(String.valueOf(ht.get("AMT7")));  //자동차검사비(일반식)
						t_amt8[j] += AddUtil.parseLong(String.valueOf(ht.get("AMT8"))); //자동차검사비(기본식)
						...
						t_amt13[j] += AddUtil.parseLong(String.valueOf(ht.get("AMT13"))); //정비비(기본식)													
...
%>   
	...
	<td width="5%" align="right"><%=Util.parseDecimal(String.valueOf(ht.get("WAY1_AMT")))%> </td>  <!-- 일반식 관리비용-->  
            ...
            <td width="5%" align="right"><a href="javascript:MM_openBrWindow('man_j_cost_jung_detail_list3.jsp?mng_id=<%= ht.get("USER_ID") %>&mng_nm=<%=ht.get("USER_NM")%>&save_dt=<%=save_dt%>&c_e_dt=<%=ce_dt%>&bb_cnt=<%=bb_cnt%>&ii_cnt=<%=ii_cnt%>&bus_cost_per=<%=bus_cost_per%>&mng_cost_per=<%=mng_cost_per%>&dt=<%=dt%>&ref_dt1=<%=cs_dt%>&ref_dt2=<%=ce_dt%>&amt1=<%=amt1%>&amt2=<%=amt2%>&amt1_per=<%=amt1_per%>&amt2_per=<%=amt2_per%>&amt3_per=<%=amt3_per%>&gg=01','popwin_list3','scrollbars=yes,status=no,resizable=yes,width=950,height=760,top=30,left=30')"><%=Util.parseDecimal(AddUtil.parseLong(String.valueOf(ht.get("AMT1"))) + AddUtil.parseLong(String.valueOf(ht.get("AMT2"))) + AddUtil.parseLong(String.valueOf(ht.get("AMT13"))) )%></a></td>  <!--정비비 -->
       	   <td width="5%" align="right"><a href="javascript:MM_openBrWindow('man_j_cost_jung_detail_list3.jsp?mng_id=<%= ht.get("USER_ID") %>&mng_nm=<%=ht.get("USER_NM")%>&save_dt=<%=save_dt%>&c_e_dt=<%=ce_dt%>&bb_cnt=<%=bb_cnt%>&ii_cnt=<%=ii_cnt%>&bus_cost_per=<%=bus_cost_per%>&mng_cost_per=<%=mng_cost_per%>&dt=<%=dt%>&ref_dt1=<%=cs_dt%>&ref_dt2=<%=ce_dt%>&amt1=<%=amt1%>&amt2=<%=amt2%>&amt1_per=<%=amt1_per%>&amt2_per=<%=amt2_per%>&amt3_per=<%=amt3_per%>&gg=05','popwin_list3','scrollbars=yes,status=no,resizable=yes,width=950,height=760,top=30,left=30')"><%=Util.parseDecimal(AddUtil.parseLong(String.valueOf(ht.get("AMT11"))) + AddUtil.parseLong(String.valueOf(ht.get("AMT12")))  )%></a></td> <!-- 대차비용 -->
            
            <td width="4%" align="right"><a href="javascript:MM_openBrWindow('man_j_cost_jung_detail_list3.jsp?mng_id=<%= ht.get("USER_ID") %>&mng_nm=<%=ht.get("USER_NM")%>&save_dt=<%=save_dt%>&c_e_dt=<%=ce_dt%>&bb_cnt=<%=bb_cnt%>&ii_cnt=<%=ii_cnt%>&bus_cost_per=<%=bus_cost_per%>&mng_cost_per=<%=mng_cost_per%>&dt=<%=dt%>&ref_dt1=<%=cs_dt%>&ref_dt2=<%=ce_dt%>&amt1=<%=amt1%>&amt2=<%=amt2%>&amt1_per=<%=amt1_per%>&amt2_per=<%=amt2_per%>&amt3_per=<%=amt3_per%>&gg=07','popwin_list3','scrollbars=yes,status=no,resizable=yes,width=950,height=760,top=30,left=30')"><%=Util.parseDecimal(AddUtil.parseLong(String.valueOf(ht.get("AMT3"))) + AddUtil.parseLong(String.valueOf(ht.get("AMT4")))  )%></a></td> <!--탁송료 --> 
            <td width="4%" align="right"><a href="javascript:MM_openBrWindow('man_j_cost_jung_detail_list3.jsp?mng_id=<%= ht.get("USER_ID") %>&mng_nm=<%=ht.get("USER_NM")%>&save_dt=<%=save_dt%>&c_e_dt=<%=ce_dt%>&bb_cnt=<%=bb_cnt%>&ii_cnt=<%=ii_cnt%>&bus_cost_per=<%=bus_cost_per%>&mng_cost_per=<%=mng_cost_per%>&dt=<%=dt%>&ref_dt1=<%=cs_dt%>&ref_dt2=<%=ce_dt%>&amt1=<%=amt1%>&amt2=<%=amt2%>&amt1_per=<%=amt1_per%>&amt2_per=<%=amt2_per%>&amt3_per=<%=amt3_per%>&gg=10','popwin_list3','scrollbars=yes,status=no,resizable=yes,width=950,height=760,top=30,left=30')"><%=Util.parseDecimal(AddUtil.parseLong(String.valueOf(ht.get("AMT9"))) + AddUtil.parseLong(String.valueOf(ht.get("AMT10")))  )%></a></td> <!-- 긴급출동-->
            <td width="4%" align="right"><a href="javascript:MM_openBrWindow('man_j_cost_jung_detail_list3.jsp?mng_id=<%= ht.get("USER_ID") %>&mng_nm=<%=ht.get("USER_NM")%>&save_dt=<%=save_dt%>&c_e_dt=<%=ce_dt%>&bb_cnt=<%=bb_cnt%>&ii_cnt=<%=ii_cnt%>&bus_cost_per=<%=bus_cost_per%>&mng_cost_per=<%=mng_cost_per%>&dt=<%=dt%>&ref_dt1=<%=cs_dt%>&ref_dt2=<%=ce_dt%>&amt1=<%=amt1%>&amt2=<%=amt2%>&amt1_per=<%=amt1_per%>&amt2_per=<%=amt2_per%>&amt3_per=<%=amt3_per%>&gg=03','popwin_list3','scrollbars=yes,status=no,resizable=yes,width=950,height=760,top=30,left=30')"><%=Util.parseDecimal(AddUtil.parseLong(String.valueOf(ht.get("AMT7"))) + AddUtil.parseLong(String.valueOf(ht.get("AMT8")))  )%></a> </td> <!-- 검사비 -->    
            <td width="4%" align="right"><a href="javascript:MM_openBrWindow('man_j_cost_jung_detail_list3.jsp?mng_id=<%= ht.get("USER_ID") %>&mng_nm=<%=ht.get("USER_NM")%>&save_dt=<%=save_dt%>&c_e_dt=<%=ce_dt%>&bb_cnt=<%=bb_cnt%>&ii_cnt=<%=ii_cnt%>&bus_cost_per=<%=bus_cost_per%>&mng_cost_per=<%=mng_cost_per%>&dt=<%=dt%>&ref_dt1=<%=cs_dt%>&ref_dt2=<%=ce_dt%>&amt1=<%=amt1%>&amt2=<%=amt2%>&amt1_per=<%=amt1_per%>&amt2_per=<%=amt2_per%>&amt3_per=<%=amt3_per%>&gg=08','popwin_list3','scrollbars=yes,status=no,resizable=yes,width=950,height=760,top=30,left=30')"><%=Util.parseDecimal(AddUtil.parseLong(String.valueOf(ht.get("AMT5"))) + AddUtil.parseLong(String.valueOf(ht.get("AMT6")))  )%></a></td> <!--유류대 -->    
            ...
            <td width="6%" align="right"><%=Util.parseDecimal(g_tot)%></td>
          </tr>
          <%}%>
          ...
          <tr> 
            ...
            <td class=title style='text-align:right;'><%= Util.parseDecimal(t_amt6[0]+t_amt5[0]+t_amt3[0]+t_amt4[0]+t_amt9[0]+t_amt10[0]+t_amt7[0]+t_amt8[0])%></td>
    
            <td class=title style='text-align:right;'><%= Util.parseDecimal(t_amt1[0]+t_amt2[0]+t_amt13[0]+t_amt11[0]+t_amt12[0] +t_amt3[0]+t_amt4[0]+t_amt9[0]+t_amt10[0]+t_amt7[0]+t_amt8[0]  +t_amt5[0]+t_amt6[0]   )%></td>     

            <td class=title style='text-align:right;'><%= Util.parseDecimal(t_amt35[0])%></td><!--예비차관련 적용-->    
            <td class=title style='text-align:right;'><%= Util.parseDecimal(t_amt1[0]+t_amt2[0]+t_amt13[0]+t_amt11[0]+t_amt12[0] +t_amt3[0]+t_amt4[0]+t_amt9[0]+t_amt10[0]+t_amt7[0]+t_amt8[0]  +t_amt5[0]+t_amt6[0] +t_amt35[0])%></td>
