...
//비용캠페인변수 : cost_campaign 테이블
	Hashtable ht3 = ac_db.getCostCampaignVar("6");
	...
	int amt1_per 		= AddUtil.parseInt((String)ht3.get("AMT1_PER")); //제안포상금 적용비율
	...
	Vector vts2 = ac_db.getPropCampaign(save_dt, "6", "", "", "");
...
function updateVar(){
	...
	fm.action = "/fms2/mis/prop_var_iu.jsp";
	fm.target = 'i_no';
	fm.submit();
}
...
//프린트하기
function cmp_print(){
	window.open("prop_settle6_print.jsp?save_dt=<%=save_dt%>&auth_rw=<%=auth_rw%>","print","left=30,top=50,width=950,height=600,scrollbars=yes");	
}
...
<input type="hidden" name="gubun" value="6"> 
<input type="hidden" name="from_page" value="/fms2/mis/prop_settle6.jsp"> 
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>경영정보 > 캠페인관리 > <span class=style5>제안캠페인(외근)</span></span></td>
...
<table width="900" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
 	  <td class="line">
	    <table border="0" cellspacing="1" cellpadding="0" width='920'>
         <tr> 
           <td width='40' class='title' rowspan=2>순위</td>
           <td width='110' class='title' rowspan=2>부서</td>
	    ...
	    int ji_size = vt_size2 / 2;   	
     	...
     	for(int i = 0 ; i < vt_size2 ; i++){
				...
				for(int j=0; j<1; j++){
					...
					t_amt2[j]  += AddUtil.parseLong(String.valueOf(ht.get("R_AMT"))) ; //포상금액
 ...
 %>   		
	      <tr> 
	         ...
	         <%if (ht.get("LOAN_ST").equals("2")){ %>
                  		 <%=ht.get("DEPT_NM")%>&nbsp;<% if ( String.valueOf(ht.get("DEPT_NM")).equals("부산지점") ||  String.valueOf(ht.get("DEPT_NM")).equals("대전지점")   ||  String.valueOf(ht.get("DEPT_NM")).equals("대구지점")   ||  String.valueOf(ht.get("DEPT_NM")).equals("광주지점")  ||  String.valueOf(ht.get("DEPT_NM")).equals("수원지점") ) { %><%}%> 
             <%}else if( ht.get("LOAN_ST").equals("1")){%>
           		<%=ht.get("DEPT_NM")%>&nbsp;<% if ( String.valueOf(ht.get("DEPT_NM")).equals("부산지점") ||  String.valueOf(ht.get("DEPT_NM")).equals("대전지점")  ||  String.valueOf(ht.get("DEPT_NM")).equals("대구지점")   ||  String.valueOf(ht.get("DEPT_NM")).equals("광주지점")   ||  String.valueOf(ht.get("DEPT_NM")).equals("수원지점")  ) { %> <%}%> 
	         <% } %>
	         </td>
