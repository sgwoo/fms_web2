...
<%
	...
	if(doc_no.equals("")){
		vt = t_db.getCarTintNotPayOffList(off_id, req_dt, br_id2);
	}else{
		vt = t_db.getCarTintReqDocList(req_code);
		if(vt.size()>0){
			for(int i = 0 ; i < 1 ; i++){
          ...
          <tr>
            <td width="800" class=line>
			  ...
			  <tr><td class=line2></td></tr>
                <tr>
                  <td width="10%" align='center'>용품업체</td>		  
	  ...
	  <td class=line>
		...
		<tr><td class=line2></td></tr>
		  <tr valign="middle" align="center"> 
		    ...
		    <td style="font-size:8pt" width='100' rowspan="2" align='center'>상호</td>
		    <td style="font-size:8pt" width='70' rowspan="2" align='center' >차량번호</td>
		    <td style="font-size:8pt" width='100' rowspan="2" align='center'>차명</td>
		    <td style="font-size:8pt" width='50' rowspan="2" align='center'>영업구분</td>
		    <td style="font-size:8pt" width='100' rowspan="2" align='center'>납품일자</td>
		    <td style="font-size:8pt" colspan="7" align='center'>청구금액</td>
		    <td style="font-size:8pt" width="50" rowspan="2" align='center'>의뢰자</td>					    
	      </tr>
		  <tr valign="middle" align="center">
		    <td style="font-size:8pt" width='60' align='center'>측후면썬팅</td>
		    <td style="font-size:8pt" width='60' align='center'>전면썬팅</td>
		    <td style="font-size:8pt" width='60' align='center'>블랙박스</td>
		    <td style="font-size:8pt" width='60' align='center'>내비게이션</td>
		    <td style="font-size:8pt" width='60' align='center'>기타용품</td>
		    <td style="font-size:8pt" width='60' align='center'>이동형<br>충전기</td>
		    <td style="font-size:8pt" width='60' align='center'>합계</td>
		  </tr>
<%		for(int i = 0 ; i < vt_size ; i++){
		  ...
		  <tr> 
			...
			<td colspan="4" style="font-size:8pt" align='center'>[대량용품]<span title='<%=ht.get("COM_MODEL_NM")%>'><%=Util.subData(String.valueOf(ht.get("COM_MODEL_NM")), 25)%></span></td>
			<%}else{%>
		    ...
		    <td style="font-size:8pt" align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("S1_AMT")))%></td>
		    <td style="font-size:8pt" align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("S2_AMT")))%></td>			
		    <td style="font-size:8pt" align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("B_AMT")))%></td>
		    <td style="font-size:8pt" align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("N_AMT")))%></td>
		    <td style="font-size:8pt" align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("E_AMT")))%></td>
		    <td style="font-size:8pt" align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("EV_AMT")))%></td>
  ...
  <%		total_amt1 	= total_amt1 + AddUtil.parseLong(String.valueOf(ht.get("S1_AMT")));
	...
	<tr>
		<td align="right">
		  <a href="javascript:print()"><img src=/acar/images/center/button_print.gif border=0 align=absmiddle></a><font color=#CCCCCC>&nbsp;(※인쇄TIP : A4, 세로방향)</font>&nbsp;&nbsp;
	  ...
	  <td>
		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
		  ...
		  <%if(br_id2.equals("S1")||br_id2.equals("S2")||br_id2.equals("I1")){%>
                  <td width='90' align='center'>용품관리자</td>
		  ...
		  <%}%>
                ...
                <tr>
                  ...
                  <td align='center'><%=c_db.getNameById(doc.getUser_id2(),"USER_PO")%><br><%=doc.getUser_dt2()%><%if(!doc.getUser_dt1().equals("") && doc.getUser_dt2().equals("")){%><%if(doc.getUser_id2().equals(user_id) || nm_db.getWorkAuthUser("전산팀",user_id)){%><a href="javascript:doc_sanction('2')">결재</a><br>&nbsp;<%}%><%}%></td>				
				  <%}else{%>
                  <td align='center'><%=c_db.getNameById(doc.getUser_id1(),"USER_PO")%><br><%=doc.getUser_dt1()%><%if(doc.getUser_dt1().equals("")){%><a href="javascript:doc_sanction('1')">결재</a><br>&nbsp;<%}%></td>
				  <%}%>				  
                </tr>
              </table>
			</td>
	...
	<!--미지급금전표발행-->
	<%		if(!doc.getUser_id1().equals("") && ( nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("본사고객지원팀출납",user_id) || nm_db.getWorkAuthUser("탁송관리자",user_id) || nm_db.getWorkAuthUser("매입옵션관리자",user_id) ) ){%>
	<tr>
		<td>&nbsp;</td>	
	</tr>
	<tr>
            ...
            <td width="480" class=line>
			  ...
			  <tr><td class=line2></td></tr>
                <tr>
                  ...
                  <td width="70%">&nbsp;<input type='text' name='autodocu_dt' value='<%=AddUtil.ChangeDate2(req_dt)%>' maxlength='12' size='12' class='text' onBlur='javscript:this.value = ChangeDate(this.value);'></td>
                </tr>
                <tr>
                  <td width="30%" align='center'>금액</td>
                  <td width="70%">
			&nbsp;공급가 : <input type='text' name='autodocu_s_amt' value='<%=total_amt7%>' maxlength='15' size='10' class='num' >
			&nbsp;부가세 : <input type='text' name='autodocu_v_amt' value='<%=AddUtil.parseFloatTruncZero(String.valueOf(total_amt7*0.1))%>' maxlength='15' size='10' class='num' >				  
				  </td>
                </tr>
              </table>
			</td>
			<td width="520">&nbsp;<a href="javascript:autodocu_reg()" onMouseOver="window.status=''; return true" onfocus="this.blur()"><img src=/acar/images/center/button_jpbh_mgg.gif border=0 align=absmiddle></a></td>
          </tr>
        </table>		  
	  </td>	
	</tr>		
...
<script language='javascript'>
<!--
	document.form1.req_amt.value = '<%=Util.parseDecimal(total_amt7)%>';
//-->
</script>
