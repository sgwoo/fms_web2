...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
	...
	<tr>
		<td>
			<iframe src="/fms2/con_fee/fee_scd_sc_in.jsp?auth=<%=auth%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&auth=<%=auth%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>" name="i_inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
			</iframe>
		</td>
	</tr>
	<tr>
		<td>* 지연대차 기생성분은 미등록 검색시 최종사용일이 2일이상 되면 나옵니다.
		</td>
	</tr>	
</table>
</form>
