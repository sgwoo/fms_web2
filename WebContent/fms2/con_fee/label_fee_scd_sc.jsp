...
<!--
	...
	function select_label(){
		...
		fm.target = "_blank";
		<%if(user_id.equals("000131")){%>
		fm.action = "label_print_select2.jsp?";
		<%}else{%>
...
<input type='hidden' name='t_wd' value='<%=user_id%>'>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
	...
	<tr>
		<td>
			<iframe src="/fms2/con_fee/label_fee_scd_sc_in.jsp?auth=<%=auth%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&auth=<%=auth%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=user_id%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>" name="i_inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
			</iframe>
		</td>
	</tr>
</table>
