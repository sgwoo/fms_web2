...
<%
	...
	Vector fee_scd = af_db.getFeeScdPrint2(l_cd, "", true);	//2018.04.16
	    ...
	    <td class='line' width='18%' id='td_title' style='position:relative;'>			
            <table border="0" cellspacing="1" cellpadding="0" width='100%'>
                <tr> 
                    ...
                    <td width='45%' class='title'>세금계산서일자</td>
                </tr>
	        </table>
	    </td>
...
<%		for(int i = 0 ; i < fee_scd_size ; i++){
			FeeScdBean a_fee = (FeeScdBean)fee_scd.elementAt(i);%>		
				...
				<%if(a_fee.getRc_yn().equals("0")){ //미입금%>
					...
					<td <%if(a_fee.getTm_st2().equals("3")){%>class="im"<%}%> align='center' width='45%'><input type='text' name='t_fee_est_dt' size='9' class='whitetext' value='<%=a_fee.getTax_out_dt()%>' readonly></td>
				<%}else{//입금된것%>
					...
					<td <%if(a_fee.getTm_st2().equals("3")){%>class="im"<%}%> class='is' align='center' width='45%'><input type='text' name='t_fee_est_dt' size='9' class='istext' value='<%=a_fee.getTax_out_dt()%>' readonly></td>
				<%}%>
				</tr>
<%		}%>				
	        </table>
