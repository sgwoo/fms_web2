...
<%
	...
	String value4[]  = request.getParameterValues("tm_st1");
	String value5[]  = request.getParameterValues("tm_st2");
	...
	String node_code= l_cd.substring(0,2)+"01";
...
//	out.println(insert_id);
	...
	for(int i=0 ; i < scd_size ; i++){
		...
		if(pay_amt > 0){
			...
			if(value0[i].equals("scd_fee")){
				...
				FeeScdBean pay_fee = af_db.getScdNew(m_id, l_cd, value1[i], value2[i], value3[i], value4[i]);
			...
			//연체료
				...
				dly_bean.setRent_mng_id	(m_id);
				dly_bean.setRent_l_cd	(l_cd);
	...
	if(autodoc.equals("Y") && vt.size() > 0){
		...
		String row_id = neoe_db.insertSetAutoDocu(rc_dt,  vt);
	...
	if(stop_size >0){
		...
		for(int j=0;j < vid_size;j++){
			...
			if(!stop_seq.equals("") && !cancel_dt.equals("")){
				...
				//세금계산서 발행 일시중지 등록 ----------------------------------------------------------------------
				FeeScdStopBean fee_scd = af_db.getFeeScdStop(m_id, l_cd, stop_seq);
