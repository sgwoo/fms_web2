...
<%	if(fine_size > 0){
		for(int i = 0 ; i < fine_size ; i++){
				...
				String file_type1 = ht.get("FILE_TYPE")+"";%>
		        <tr>
        		    ...
        		    <%if(!(ht.get("SAVE_FILE")+"").equals("")){%>
									<%if(file_type1.equals("image/jpeg")||file_type1.equals("image/pjpeg")||file_type1.equals("application/pdf")){%>
										<a href="javascript:openPopP('<%=file_type1%>','<%=ht.get("FILE_SEQ")%>');" title='보기' ><img src="/acar/images/center/button_in_see.gif" align="absmiddle" border="0"></a>
									<%}else{%>
									...
									<%}%> 
					<%}else{%>
										<a href="https://fms3.amazoncar.co.kr/data/stop_doc/<%=ht.get("FILENAME")%>" target="_blank"><img src=/acar/images/center/button_in_see.gif border=0 align=absmiddle></a>
					<%}%> 
