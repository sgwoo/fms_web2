...
<%@ page import="acar.pay_mng.*, acar.master_car.*, acar.doc_settle.*, acar.car_sche.*, acar.user_mng.*, acar.bill_mng.*"%>
...
<%
	...
	String user_dt1 	= request.getParameter("user_dt1")==null?"":request.getParameter("user_dt1");
	...
	Vector vt = ps_db.getPayEstAmt14_2List("9", off_id+""+s_yy+ ""+ s_mm, "", "", "");
	...
	for(int i = 0 ; i < vt_size ; i++){
			...
			bean.setAmt				((long)autodocu_s_amt+autodocu_v_amt);  //미지급전표금액으로 (query 금액이 아님!)
			...
			item.setP_gubun			("11");  //차량정비비 (45700) 
			...
			item.setP_cd4			(String.valueOf(ht.get("P_CD4"))==null?"":String.valueOf(ht.get("P_CD4")));
			item.setP_cd5			(String.valueOf(ht.get("P_CD5"))==null?"":String.valueOf(ht.get("P_CD5")));
			item.setI_amt			((long)autodocu_s_amt+autodocu_v_amt);
			item.setI_s_amt			((long)autodocu_s_amt);
			item.setI_v_amt			((long)autodocu_v_amt);
			item.setSub_amt1		((long)autodocu_s_amt);
			item.setSub_amt2		((long)autodocu_v_amt);
			item.setSub_amt3		(String.valueOf(ht.get("SUB_AMT3"))==null?0:AddUtil.parseDigit4(String.valueOf(ht.get("SUB_AMT3"))));
			item.setSub_amt4		(String.valueOf(ht.get("SUB_AMT4"))==null?0:AddUtil.parseDigit4(String.valueOf(ht.get("SUB_AMT4"))));
			item.setSub_amt5		(String.valueOf(ht.get("SUB_AMT5"))==null?0:AddUtil.parseDigit4(String.valueOf(ht.get("SUB_AMT5"))));
