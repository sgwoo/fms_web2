...
<%@ page import="acar.cus_reg.*, acar.doc_settle.*, acar.coolmsg.*, acar.car_sche.*, acar.user_mng.*"%>
...
<%
	...
	String etc 	= request.getParameter("etc")==null?"":request.getParameter("etc"); //협력업체관련 
...
<%
	...
	if(!doc_no.equals("") ) {
		...
		//	doc = d_db.getDocSettle(doc_no);
			...
			url 		= "/fms2/service/serv_doc_frame.jsp";
			sub 	= off_nm + " 정비비  지출결의서 결재 요청 ";
		
			cont 	= "[자동차 정비업체: "+off_nm+", 정산일자: "+jung_dt+"] 정비비 지출결의서 결재를 요청합니다.";
					
			if(doc_bit.equals("3"))	{
				url 		= "/fms2/service/serv_doc_frame.jsp";
				sub 		= "정비비  지출결의서 결재 완료";
				cont 	= "[자동차 정비업체: "+off_nm+", 정산일자: "+jung_dt+"] 정비비 지출결의서 결재를  완결하니 출금 집행하세요";				
			}
		...
		//	if(!cs_bean.getWork_id().equals("")) 	target_id = cs_bean.getWork_id();
			...
			if(doc_bit.equals("3"))	{
				System.out.println("쿨메신저(자동차정비 비용결재)"+doc.getDoc_id()+"-----------------------"+target_bean.getUser_nm()+" "+cont);		
			}
