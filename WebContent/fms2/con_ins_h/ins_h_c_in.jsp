...
<%@ page import="java.util.*, acar.user_mng.*, acar.util.*, acar.cont.*, acar.con_ins_h.*"%>
...
<%
	...
	Vector ins_h_scd = ae_db.getInsurHScd(m_id, l_cd, c_id, accid_id, seq_no, ext_id);
	   		...
	   		if(ins_hs.getGubun().equals("미수금")){ //미입금%>
                <tr> 
                    ...
                    <td align='right'> 
                      ...
                      원</td>
                    <td align='center'><input type='text' name='req_dt' size='11' value='<%=ins_hs.getReq_dt()%>' class='text' onBlur='javascript:this.value=ChangeDate(this.value)'></td>
                    <td align='center'>
					  <select name="pay_gu">
                    ...
                    <td align='center'> 
                      <input type='text' name='pay_dt' size='11' value='<%=ins_hs.getPay_dt()%>' class='text' onBlur='javascript:this.value=ChangeDate(this.value)'>
                    </td>
                    <td align='right'> 
                    ...
                    <td align='center'> 
                      ...
                      <!--<a href="javascript:parent.change_scd('p', 'N', '<%=i%>', '<%=ins_hs.getAccid_id()%>','<%=ins_hs.getSeq_no()%>', '<%=ins_hs.getExt_tm()%>')"><img src=/acar/images/center/button_in_ig.gif align=absmiddle border=0></a>-->
                      <%	}else{%>
                      - 
                      <%	}%>
                    ...
                    <td align='center'> 
                      ...
                      <%	if((auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6"))){%>
                      <a href="javascript:parent.change_scd('u', 'N', '<%=i%>', '<%=ins_hs.getAccid_id()%>','<%=ins_hs.getSeq_no()%>', '<%=ins_hs.getExt_tm()%>')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a> 
                         <a href="javascript:parent.change_scd('d', 'N', '<%=i%>', '<%=ins_hs.getAccid_id()%>','<%=ins_hs.getSeq_no()%>', '<%=ins_hs.getExt_tm()%>')">d</a> 
                      <%	}else{%>
                      - 
                      <%	}%>
          ...
          <%			}else{//입금%>
                <tr> 
                    ...
                    <td class='is' align='right'> 
                      ...
                      원</td>
                    <td class='is' align='center'><input type='text' name='req_dt' size='11' value='<%=ins_hs.getReq_dt()%>' class='text' onBlur='javascript:this.value=ChangeDate(this.value)'></td>
                    <td align='center'>
					  <select name="pay_gu">
                    ...
                    <td class='is' align='center'> 
                      ...
                      <%	if((auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6"))){%>
                      <a href="javascript:parent.change_scd('c', 'Y', '<%=i%>', '<%=ins_hs.getAccid_id()%>','<%=ins_hs.getSeq_no()%>', '<%=ins_hs.getExt_tm()%>')"><img src=/acar/images/center/button_in_cancel.gif align=absmiddle border=0></a> 
                      <%	}else{%>
                      - 
                      <%	}%>
                      <%//}%>
                    ...
                    <td class='is' align='center' width='7%'> 
                      ...
                      <%	if((auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6"))){%>
                      <a href="javascript:parent.change_scd('u', 'Y', '<%=i%>', '<%=ins_hs.getAccid_id()%>','<%=ins_hs.getSeq_no()%>', '<%=ins_hs.getExt_tm()%>')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a> 
                       
                       <%	if( nm_db.getWorkAuthUser("전산팀",user_id) ){%>       
                       <!--해당건 스케쥴추가 - 정리된 휴대차료 청구가 추후 입금될때 사용, 전산팀만  잔액 생성 -->
                      <a href="javascript:parent.change_scd('p', 'Y', '<%=i%>', '<%=ins_hs.getAccid_id()%>','<%=ins_hs.getSeq_no()%>', '<%=ins_hs.getExt_tm()%>')">ad</a> 
                      &nbsp;
                      <a href="javascript:parent.change_scd('m', 'Y', '<%=i%>', '<%=ins_hs.getAccid_id()%>','<%=ins_hs.getSeq_no()%>', '<%=ins_hs.getExt_tm()%>')">ch</a> 
                      <% } %>
                      <%	}else{%>
                      - 
                      <%	}%>
