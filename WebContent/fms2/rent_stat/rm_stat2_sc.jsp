...
<%
	...
	int amt3[]	 	= new int[kd_size+1];	
	int amt4[]	 	= new int[kd_size+1];		
	...
	int h_amt3[]	 	= new int[br_size];	
	int h_amt4[]	 	= new int[br_size];		
	...
	int t_amt3[]	 	= new int[kd_size+1];	
	int t_amt4[]	 	= new int[kd_size+1];		
...
<!--
	...
	function view_stat2(s_br, s_br_nm, car_kd, car_kd_nm, st, st_nm){
		window.open('rm_stat2_list.jsp?gubun4=<%=gubun4%>&start_dt=<%=start_dt%>&end_dt=<%=end_dt%>&s_br='+s_br+'&s_br_nm='+s_br_nm+'&car_kd='+car_kd+'&car_kd_nm='+car_kd_nm+'&st='+st+'&st_nm='+st_nm, "STAT2_LIST", "left=0, top=0, width=900, height=568, scrollbars=yes, status=yes, resize");
	}
//-->
</script>
</head>
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
    <tr> 
		...
		<%		Vector vt = rs_db.getRmRentStat2List(br_id_cd[k], gubun4, start_dt, end_dt);
				int vt_size = vt.size();			
		%>
		<%		//배열에 값 넘김	
