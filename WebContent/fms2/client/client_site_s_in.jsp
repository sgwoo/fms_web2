...
<%			Vector c_sites = al_db.getClientSiteList(client_id, h_wd);
			...
			if(c_site_size > 0){
				...
				<tr>
					...
					<td width='5%' align='center'><%=site.getSeq()%></td>		
					<td width='20%' align='center'><a href="javascript:parent.select('<%= site.getSeq()%>','<%= from_page%>', '<%= site.getR_site()%>')" onMouseOver="window.status=''; return true"><span title='<%=site.getR_site()%>'><%=AddUtil.subData(site.getR_site(), 15)%></span></a></td>
					<td width='15%' align='center'><a href="javascript:parent.select('<%= site.getSeq()%>','<%= from_page%>', '<%= site.getR_site()%>')" onMouseOver="window.status=''; return true"><%=site.getSite_jang()%></a></td>
					<td width='25%' align='center'><span title='<%=site.getAddr()%>'><%=AddUtil.subData(site.getAddr(), 16)%></span></td>					
