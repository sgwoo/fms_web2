...
<!--
	...
	function view_cont(m_id, l_cd, use_yn){
		...
		fm.target ='d_content';
		if(use_yn == '대기')		fm.action = '/fms2/lc_rent/lc_b_s.jsp';
		else				fm.action = '/fms2/lc_rent/lc_c_frame.jsp';
...
%>
				<tr>
					...
					<td <%if(String.valueOf(cont.get("IS_RUN")).equals("해약")){%>class=is<%}%> align='center' width='13%'><%=cont.get("CAR_NM")%></td>
					<td <%if(String.valueOf(cont.get("IS_RUN")).equals("해약")){%>class=is<%}%> align='center' width='13%'><%=cont.get("RENT_START_DT")%>&nbsp;~&nbsp;<%=cont.get("RENT_END_DT")%></td>
					...
					<td <%if(String.valueOf(cont.get("IS_RUN")).equals("해약")){%>class=is<%}%> align='center' width='11%'><%=cont.get("R_SITE")%>&nbsp;<%=cont.get("R_SITE_SEQ")%></td>
