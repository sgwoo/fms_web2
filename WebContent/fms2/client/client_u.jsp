...
<!--
	function modify()
	{
		...
		// 공동 대표자 이메일 주소 - 개인사업자
		if(fm.repre_email2_1.value != '' && fm.repre_email2_2.value != ''){
			fm.repre_email2.value = fm.repre_email2_1.value+'@'+fm.repre_email2_2.value;
		}else{
			fm.repre_email2.value = '';
		}
		
		//예외처리
		<%if(nm_db.getWorkAuthUser("스케줄생성자",user_id) || nm_db.getWorkAuthUser("스케줄변경담당자",user_id)){%>
		<%}else{%>
...
<body>
<form name='form1' method='post' action='/fms2/client/client_u_a.jsp'>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
			        ...
			        <td class=line>
			            <table border="0" cellspacing="1" cellpadding="0" width=100%>
							...
							<tr>
							  ...
							  <td colspan=3>&nbsp;
								<input type="text" name='t_zip' id="t_zip" size="7"  value='<%=client.getRepre_zip()%>' maxlength='7'>
								<input type="button" onclick="openDaumPostcode()" value="우편번호 찾기"><br>
								&nbsp;&nbsp;<input type="text" name='t_addr' id="t_addr" value='<%=client.getRepre_addr()%>' size="100">
							  </td>
							</tr>
				<%	String email_1 = "";
					String email_2 = "";
    			    ...
    			    <td class=line>
    			        <table border="0" cellspacing="1" cellpadding="0" width=100%>
            				...
            				<tr>
            		            <td class=title>주소</td>
							  <td colspan=3>&nbsp;
								<input type="text" name='t_zip' id="t_zip1" size="7"  value='<%=client.getRepre_zip()%>' maxlength='7'>
								<input type="button" onclick="openDaumPostcode1()" value="우편번호 찾기"><br>
								&nbsp;&nbsp;<input type="text" name='t_addr' id="t_addr1" value='<%=client.getRepre_addr()%>' size="100">
							  </td>
            				</tr>
            		  		<%	email_1 = "";
        					email_2 = "";
					  ...
					  <select id="repre_email_domain" align="absmiddle" onChange="javascript:document.form1.repre_email_2[1].value=this.value;">
						...
						<input type='hidden' name='repre_email' value='<%=client.getRepre_email()%>'>
            		        ...
            		        <tr>
            					...
            					<td>&nbsp;
                		            이름 : <input type='text' size='10' name='repre_nm2' value='<%=client.getRepre_nm2()%>' maxlength='50' class='text' value=''>
            			        </td>
            		            <td class='title'>생년월일</td>
            		            <td>&nbsp;<input type='text' name='repre_ssn2_1' value='<%=client.getRepre_ssn2_1()%>' maxlength='6' size='6' class='text'>
            						-*******
            						<input type='hidden' name='repre_ssn2_2' value='<%=client.getRepre_ssn2_2()%>' maxlength='7' size='7' class='text'></td>
            			        </tr>
            			        <script>
								function openDaumPostcode3() {
									new daum.Postcode({
										oncomplete: function(data) {
											document.getElementById('repre_zip2').value = data.zonecode;
											document.getElementById('repre_addr2').value = data.address;
										}
									}).open();
								}
							</script>	
            			    <tr>
            		        	<td class=title>주소</td>
								<td colspan=3>&nbsp;
									<input type="text" name='repre_zip2' id="repre_zip2" size="7"  value='<%=client.getRepre_zip2()%>' maxlength='7'>
									<input type="button" onclick="openDaumPostcode3()" value="우편번호 찾기"><br>
									&nbsp;&nbsp;<input type="text" name='repre_addr2' id="repre_addr2" value='<%=client.getRepre_addr2()%>' size="100">
								</td>
            				</tr>
            		        <%	String repre_email2_1 = "";
									String repre_email2_2 = "";
									if(!client.getRepre_email2().equals("")){
										int repre_mail_len = client.getRepre_email2().indexOf("@");
										if(repre_mail_len > 0){
											repre_email2_1 = client.getRepre_email2().substring(0,repre_mail_len);
											repre_email2_2 = client.getRepre_email2().substring(repre_mail_len+1);
										}
									}
							%>							
            		        <tr>
            		            <td class='title'>이메일주소</td>
            		            <td colspan=3>&nbsp;
					<input type='text' size='35' name='repre_email2_1' value='<%=repre_email2_1%>' maxlength='100' class='text' style='IME-MODE: inactive'>
					@
					<input type='text' size='20' name='repre_email2_2' value='<%=repre_email2_2%>' maxlength='100' class='text' style='IME-MODE: inactive'>
					  <select id="repre_email2_domain_1" align="absmiddle" onChange="javascript:document.form1.repre_email2_2.value=this.value;">
						<option value="" selected>선택하세요</option>
						<option value="hanmail.net">hanmail.net</option>
						<option value="naver.com">naver.com</option>
						<option value="nate.com">nate.com</option>
						...
						<input type='hidden' name='repre_email2' value='<%=client.getRepre_email2()%>'>
            		            </td> 
            		            </tr>   
            		        <tr>
            		            <td colspan="2" class='title'>사무실번호</td>
    			    ...
    			    <td class=line>
    			        <table border="0" cellspacing="1" cellpadding="0" width=100%>
            		        ...
            		        <tr>
            		            ...
            		            <td>&nbsp;<input type='text' size='30' name='o_tel' value='<%=client.getO_tel()%>' maxlength='20' class='text'></td>
            		        ...
            		        <tr>
            		            ...
            		            <td colspan="3">&nbsp;<input type='text' name='t_zip' id="t_zip2" value='<%=client.getComm_zip()%>' size='7' class='text' readonly >
								<input type="button" onclick="openDaumPostcode2()" value="우편번호 찾기"><br>
            		                <input type='text' name='t_addr' id="t_addr2"  value='<%=client.getComm_addr()%>' maxlength='60' size='50' class='text' style='IME-MODE: active'></td>
            		            </td>
            		        </tr>
            		        <tr>
	    ...
	    <td colspan="2" class=line>
	        <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr>
                    ...
                    <td >&nbsp;
                      <input type='text' size='30' name='lic_no' maxlength='30' class='whitetext' value='<%=client.getLic_no()%>' readonly>
                      &nbsp;(개인,개인사업자,일부 법인사업자)
                    </td>
                    <td colspan="2" style="border-left: none;">
                    	&nbsp;※ 계약자의 운전면허번호를 기재하고, 계약자가 운전면허가 없는 경우 차량이용자의 운전면허를 입력<br>
			            &nbsp;※ 법인번호가 없는 법인사업자도 운전면허번호를 입력(비영리법인 등)<br>
			            &nbsp;※ 일괄변경에서 수정할 수 있습니다.
			        </td>
                </tr>	 	        	
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="3">&nbsp;<input name="etax_not_cau" type="text" value="<%=client.getEtax_not_cau()%>" size="90" class='<%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%>text<%}else{%>whitetext<%}%>'></td>
                </tr>	
                <tr>
                    <td class='title'>거래명세서메일수신여부</td>
                    <td width="35%">&nbsp;
                    <select name='item_mail_yn' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                    ...
                    <td width="35%">&nbsp;
					<select name='tax_mail_yn' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                ...
                <tr>
                    ...
                    <td width="35%">&nbsp;
                    <select name='print_st' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                    ...
                    <td width="35%">&nbsp;
                    <select name='print_car_st' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                ...
                <tr>
                    ...
                    <td>&nbsp;
                    <select name='im_print_st' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                    ...
                    <td >&nbsp;
					<select name='tm_print_yn' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                ...
                <tr>					
                    ...
                    <td>&nbsp;
			<select name='bigo_yn' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                            ...
                            <option value='C' <%if(client.getBigo_yn().equals("C")) out.println("selected");%>>기본+지점추가삽입분만 표시</option>						
                            <option value='D' <%if(client.getBigo_yn().equals("D")) out.println("selected");%>>차량번호만 표시</option>						
			...
			(기본 : 차량번호+차명)					
                    ...
                    <td >&nbsp;
					<select name='etax_item_st' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
                ...
                <tr>
					...
					고정값 : <input type='text' size='50' name='bigo_value1' value='<%=client.getBigo_value1()%>' class='<%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%>text<%}else{%>whitetext<%}%>'> 
					&nbsp;
					<img src=/acar/images/center/arrow.gif align=absmiddle>
					변동값 : NO.<input type='text' size='10' name='bigo_value2' value='<%=client.getBigo_value2()%>' class='<%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%>text<%}else{%>whitetext<%}%>'>                     
					<br>&nbsp;(변동값은 숫자만 가능합니다. 계산서 발행시 이값에 1을 증가하여 비고에 표시합니다.)
					</td>	                    							
                </tr>	
                <tr>
                    <td class='title'>계산서 청구발행 구분</td>
                    <td>&nbsp;
					<select name='pubform' <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id) || nm_db.getWorkAuthUser("월렌트관리",user_id)){%><%}else{%>disabled<%}%>>
    ...
    <%}else{%>
	  <tr>
	    <td>※ 세금계산서 관련항목 수정은 총무팀 계산서담당자에게 요청하세요.</td>
    </tr>
    <%}%>
</table>
