...
%>
<form name='form1' action='/fms2/client/client_sc.jsp' target='c_foot' method='post'>
				...
				<tr>
					<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_search.gif align=absmiddle>&nbsp;
						<select name='s_kd'>
							...
							<option value='7' <%if(s_kd.equals("7")){%> selected <%}%>>사업자번호/생년월일</option>
							<option value='8' <%if(s_kd.equals("8")){%> selected <%}%>>경매장</option>
							<option value='12' <%if(s_kd.equals("12")){%> selected <%}%>>고객관리번호</option>
							<option value='14' <%if(s_kd.equals("14")){%> selected <%}%>>등록일자</option>
							<option value='13' <%if(s_kd.equals("13")){%> selected <%}%>>거래명세서메일수신거부</option>
						</select>
						<input type='text' size='35' class='text' name='t_wd' value='<%=t_wd%>' onKeyDown='javascript:enter()' style='IME-MODE: active'>
