...
<%
	...
	if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("스케줄생성자",user_id) || nm_db.getWorkAuthUser("스케줄변경담당자",user_id)){
		upd_auth  = "Y";
	}
%>
...
<!--
	...
	function modify()
	{
		...
		//사업자등록증 수정시(등록x) 개인사업자의 대표자 이름 변경시 체크
		var cl_st = $("#cl_st_after option:selected").val();
		$("#cl_nm_change").val("");
		if(cl_st=='3' || cl_st=='4' || cl_st=='5'){	//개인사업자인 경우
			var vt_size1 = <%=vt_size1%>;
			if(vt_size1 > 0){	//사업자등록증 등록내역이 이미 있는경우
    ...
    if(document.form1.client_st.value == '2'){
    	...
    	if (chk != document.form1.ssn2.value.substring(6,7)) {
    		if ('<%=client.getClient_st()%>' == '2' && '<%=client.getNationality()%>' == '2') {
            	alert ('맞지 않는 주민등록번호입니다만 외국인으로 점검예외합니다..');
            	return true;    			
    		}else{
            	document.form1.ssn1.value = "";
            	document.form1.ssn2.value = "";
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>영업지원 > 고객관리 > <span class=style5>사업자등록증 변경</span></span> : 사업자등록증 변경</td>
            ...
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr>
                    ...
                    <td>&nbsp;
					  ...
					  &nbsp;&nbsp;
					  (종사업자번호 : <input type='text' size='4' name='taxregno' maxlength='4' <%if(!upd_auth.equals("Y")){%>readonly class='whitetext'<%}else{%>class='text'<%}%> value='<%=client.getTaxregno()%>'>)					  
					  </td>
                </tr>
                <tr>
                    <td class='title'><%if(client.getClient_st().equals("1")){%>법인번호<%}else if(client.getClient_st().equals("2")){%>주민번호<%}else{%>생년월일<%}%></td>
