...
<%
	...
	Vector vt = ai_db.getCardCmsContRmList(s_kd, t_wd, st_dt, end_dt, gubun1, gubun2, gubun3, gubun4, gubun5);
	int vt_size = vt.size();
		...
		<td class='line' width='1090'>
			<table border="0" cellspacing="1" cellpadding="0" width=100%>
				<tr>
					...
					<td width='80'  class='title'>NICE</td>
					<td width='120' class='title'>카드사</td>
					<td width='130' class='title'>카드번호</td>
					<td width='60' class='title'>유효기간<br>(yy/mm)</td>
					<td width='150' class='title'>소유주</td>			
					<td width='60' class='title'>신청자</td>
					<td width='80' class='title'>신청일자</td>		
					<td width='80' class='title'>최초인출일</td>									
...
%>
				<tr>
					...
					<td  width='100' align='center'><a href="javascript:parent.cms_mng('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("CLIENT_ID")%>')" onMouseOver="window.status=''; return true"><%=ht.get("RENT_L_CD")%></a></td>
					<td  width='80' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("RENT_DT")))%></td>
					<td  width='140'>&nbsp;<span title='<%=ht.get("FIRM_NM")%>'><a href="javascript:parent.view_client('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("RENT_ST")%>')" onMouseOver="window.status=''; return true"><%=Util.subData(String.valueOf(ht.get("FIRM_NM")), 8)%></a></span></td>
...
<%
		...
		//	if (String.valueOf(ht.get("R_CMS_ACC_NO")).trim().length()> 12) 		{
				cardno =  String.valueOf(ht.get("CMS_ACC_NO")); 				
		//		cardno =  String.valueOf(ht.get("CMS_ACC_NO")).substring(0,12)+ "****";			
		//	}else{
		//		cardno =  "" ; // String.valueOf(ht.get("CMS_ACC_NO"));
		//	}
...
%>			
				<tr>
					...
					<td  width='80' align='center'><%=ht.get("CBIT")%></td>					
					<td  width='120' align='center'><%=Util.subData(String.valueOf(ht.get("CMS_BANK")), 7)%></td>					
					<td  width='130' align='center' ><%=String.valueOf(ht.get("CMS_ACC_NO"))%></td>
				   <td  width='60' align='center'><%=String.valueOf(ht.get("C_YYYY"))%>/<%=String.valueOf(ht.get("C_MM"))%></td>
					...
					<td  width='80' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("CMS_START_DT")))%></td>									
			...
			<!--		<td  width='60' align='center'>
					<%if(String.valueOf(ht.get("CAR_ST")).equals("4")){%>
					월렌트
					<%}else{%>
