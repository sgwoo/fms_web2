...
<%
	...
	ContCmsBean bean = a_db.getCardCmsMng(m_id, l_cd);
	...
	bean.setCms_bank	(request.getParameter("cms_bank")==null?"":request.getParameter("cms_bank")); //카드사 
	
	bean.setCms_acc_no	(request.getParameter("cms_acc_no")==null?"":request.getParameter("cms_acc_no")); //카드번호 
...
//	bean.setCms_dep_ssn	(request.getParameter("cms_dep_ssn")==null?"":request.getParameter("cms_dep_ssn")); 
	bean.setC_yyyy	(request.getParameter("c_yyyy")==null?"":request.getParameter("c_yyyy")); //유효기간 
	bean.setC_mm	(request.getParameter("c_mm")==null?"":request.getParameter("c_mm"));
	...
	bean.setCms_m_tel	(request.getParameter("cms_m_tel")==null?"":request.getParameter("cms_m_tel"));  //연락처 (필수) 
	...
	if(bean.getSeq().equals("")){
		...
		if(!a_db.insertContCardCmsMng(bean))	flag += 1;
		...
		if(!a_db.updateContCardCmsMng(bean))	flag += 1;
...
<%	}else{%>		
		alert("처리되었습니다");		
		parent.location='card_cms_mng.jsp<%=valus%>';		
<%	}%>
</script>