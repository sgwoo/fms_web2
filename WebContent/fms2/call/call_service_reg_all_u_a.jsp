...
<%	}else{	%>
		alert("수정되었습니다");
		parent.location='call_service_reg_frame.jsp?mode=2&m_id=<%=m_id%>&l_cd=<%=l_cd%>&c_id=<%=c_id%>&s_id=<%=s_id%>&p_gubun=<%=p_gubun%>&auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&use_yn=<%=use_yn%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>';
<%	}	%>
</script>
</body>
</html>
