...
<!--
	...
	function scan_reg(file_st){
		window.open("reg_scan.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&rent_mng_id=<%=rent_mng_id%>&rent_l_cd=<%=rent_l_cd%>&emp_id=<%=emp4.getEmp_id()%>&agnt_st=4&from_page=/fms2/commi/commi_doc_proxy_i.jsp&file_st="+file_st, "SCAN", "left=10, top=10, width=620, height=250, scrollbars=yes, status=yes, resizable=yes");
	}
	...
	function search_bank_acc(){
		var fm = document.form1;
		window.open("s_emp_bank_acc.jsp?from_page=/fms2/commi/commi_doc_u.jsp&emp_id=<%=emp4.getEmp_id()%>", "SEARCH_EMP_ACC", "left=50, top=50, width=950, height=600, scrollbars=yes");		
	}	
	...
	function save(){
		...
		if(confirm('수정 하시겠습니까?')){	
			fm.action='commi_doc_proxy_u_a.jsp';		
			fm.target='d_content';			
			fm.submit();
		}							
	}	
	...
	function delete_proxy(){
		if(confirm('삭제 하시겠습니까?')){	
			if(confirm('정말 삭제 하시겠습니까?')){	
				if(confirm('마지막으로 진짜 삭제 하시겠습니까?')){					
					fm.action='commi_doc_proxy_d_a.jsp';		
					fm.target='d_content';			
					fm.submit();
				}
			}
...
<form action="" name="form1" method="POST">
  ...
  <input type='hidden' name="emp_id" 		value="<%=emp4.getEmp_id()%>">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr> 
                    ...
                    <td width=15%>&nbsp;<input type='text' name="emp_acc_nm" value='<%=emp4.getEmp_acc_nm()%>' size="12" class='text'>					  
					  <a href="javascript:search_bank_acc()"><span title="<%=emp4.getEmp_nm()%> 영업사원의 실수령인을 조회합니다."><img src=/acar/images/center/button_in_search.gif align=absmiddle border=0></span></a>					  
					</td>
                    <td class=title width=10%>관계</td>
                    <td width=15%>&nbsp;<input type='text' name="rel" value='<%=emp4.getRel()%>' size="16" class='text'></td>
                    <td class=title width=10%>타소득</td>
                    <td width=15%>&nbsp;
        			  <select name="rec_incom_yn">
                        <option value="">==선택==</option>
        				<option value="1" <%if(emp4.getRec_incom_yn().equals("1")){%> selected <%}%>>유</option>
        				<option value="2" <%if(emp4.getRec_incom_yn().equals("2")){%> selected <%}%>>무</option>							
        			  </select>
        			</td>
                    ...
                    <td width=15%>&nbsp;
        			  <select name="rec_incom_st" onChange="javascript:set_amt()">
                        <option value="">==선택==</option>
        				<option value="2" <%if(emp4.getRec_incom_st().equals("2")){%> selected <%}%>>사업소득</option>
        				<option value="3" <%if(emp4.getRec_incom_st().equals("3")){%> selected <%}%>>기타사업소득</option>							
        			  </select>
                    </td>
		        </tr>	
                <tr> 
                    <td class=title>거래은행</td>
                    <td width=15%>&nbsp;
                    	<input type='hidden' name="emp_bank" 			value="<%=emp4.getEmp_bank()%>">
                        ...
                        <%	if(bank_size > 0){
        								...
        								%>
                        <option value='<%= bank.getCode()%>' <%if(emp4.getEmp_bank().equals(bank.getNm())||emp4.getBank_cd().equals(bank.getCode())){%> selected <%}%>><%=bank.getNm()%></option>
                        <%		}
        					}%>
                      </select></td>
                    <td class=title width=10%>계좌번호</td>
                    <td colspan="3">&nbsp;<input type='text' name="emp_acc_no" value='<%=emp4.getEmp_acc_no()%>' size="31" class='text'></td>
                    <td class=title width=10%>주민번호</td>
                    <td width=15%>&nbsp;<input type='text' name="rec_ssn" value='<%=emp4.getRec_ssn()%>' size="16" class='text'></td>
		        </tr>	
                ...
                <tr> 
                    <td class=title>주소</td>
					<td colspan="3">&nbsp;<input type="text" name="t_zip"  id="t_zip" size="7" maxlength='7' value="<%=emp4.getRec_zip()%>">
						<input type="button" onclick="openDaumPostcode()" value="우편번호 찾기"><br>
						&nbsp;<input type="text" name="t_addr" id="t_addr" size="40" value="<%=emp4.getRec_addr()%>">
					</td>
	...
	<tr>
	    <td style='height:18'><span class=style4>&nbsp;* 실수령인에는 영업사원에 등록된 데이타를 디폴트로 보여줍니다. 기존에 기안한 영업수당문서에 등록된 실수령인을 조회할 수도 있습니다.</span> </td>
	</tr>			  	
	<tr>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr> 
                    ...
                    <td align="center"><input type='text' name='commi' maxlength='10' value='<%=Util.parseDecimal(emp4.getCommi())%>' class='num' size='10' onBlur='javascript:this.value=parseDecimal(this.value); set_amt();'></td>
                    <td align="center">&nbsp;</td>
                    <td align="center">&nbsp;
        			  <select name="add_st1" onchange="javascript:set_amt()">
                        <option value="">==선택==</option>
        				<option value="1" <%if(emp4.getAdd_st1().equals("1"))%>selected<%%>>세전</option>
        				<%if(!a_co_bean.getDoc_st().equals("2")){%><option value="2" <%if(emp4.getAdd_st1().equals("2"))%>selected<%%>>세후</option><%}%>
        			  </select>
        			</td>			
                    <td align="center"><input type='text' name='add_amt1' maxlength='8' value='<%=Util.parseDecimal(emp4.getAdd_amt1())%>' class='num' size='8' onBlur='javascript:this.value=parseDecimal(this.value); set_amt();'></td>
                    <td>&nbsp;
                ...
                <input type="hidden" name="inc_per" value="<%=emp4.getInc_per()%>">
                <input type="hidden" name="res_amt" value="<%=Util.parseDecimal(emp4.getRes_amt())%>">
                <input type="hidden" name="res_per" value="<%=emp4.getRes_per()%>">
                <input type="hidden" name="c_amt" value="<%=Util.parseDecimal(emp4.getTot_amt())%>">
                <input type="hidden" name="tot_per" value="<%=emp4.getTot_per()%>">
                ...
                <tr>
                    ...
                    <td align="center"><input type='text' name='inc_per' maxlength='3' value='<%=emp4.getInc_per()%>' class='num' size='3' onBlur='javascript:this.value=parseDecimal(this.value); set_amt();'>%</td>
                    <td align="center">-</td>
                    <td align="center">-</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    ...
                    <td align="center"><input type='text' name='res_per' maxlength='3' value='<%=emp4.getRes_per()%>' class='num' size='3' onBlur='javascript:this.value=parseDecimal(this.value); set_amt();'>%</td>
                    <td align="center">-</td>
                    <td align="center">-</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    ...
                    <td align="center"><input type='text' name='tot_per' maxlength='3' value='<%=emp4.getTot_per()%>' class='num' size='3' onBlur='javascript:this.value=parseDecimal(this.value);'>%</td>
                    <td width=3% class=title>소계</td>
                    <td>&nbsp;</td>
                    <td align="center"><input type='text' name='add_tot_amt' maxlength='8' value='<%=Util.parseDecimal(emp4.getAdd_amt1()+emp4.getAdd_amt2()+emp4.getAdd_amt3())%>' class='num' size='8' onBlur='javascript:this.value=parseDecimal(this.value); set_amt();'></td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
    ...
    <tr>
	    ...
	    <a href="javascript:delete_proxy();"><img src=/acar/images/center/button_delete.gif align=absmiddle border=0></a>
	    <%}%>	    
	    </td>
	</tr>	
