...
<!--
	...
	function save(mode){
		...
		if(mode == 'guar'){
			...
			// 출고보전수당 지급여부가 있음 일 때 출고보전수당이 0이면 수정을 막는다		2017. 12. 06
			var dlv_con_commi_yn = <%=cont_etc.getDlv_con_commi_yn().equals("Y")%>;	// 출고보전수당 지급여부
			var dlv_con_commi = $("#dlv_con_commi").val();													// 출고보전수당 금액
						
			if(dlv_con_commi_yn && dlv_con_commi==0){
				alert('출고보전수당이 0원일 수 없습니다. 확인하십시오.'); return;
			}
			...
			if(fm.dlv_con_commi.value == 0){
				if(!confirm('(주)오프라이스 출고보전수당이 없습니다. 수정 하시겠습니까?')){	
					return;
				}
			}
			<%}%>
	...
	function doc_email(){
		...
		if(fm.emp_email.value == ''){ alert('이메일주소를 입력하십시오.'); return; }
        ...
        <td>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;영업지원 > 계출관리 > <span class=style1><span class=style5>영업수당지급요청</span></span></td>
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr> 
                    ...
                    <td width=15%>&nbsp;
        			  <%String pp_st = "";
        				...
        				<%if(pay_amt>0 && pp_st.equals("완결")){%>(<%=Util.parseDecimal(pay_amt)%>원)<%}%>
        			</td>
                ...
                <tr> 
                    ...
                    <td width=15%>&nbsp;
        			  ...
        			  <%=gi_st%><%if(!gi_st.equals("면제")){%>(<%=Util.parseDecimal(gins.getGi_amt())%>원)<%}%>
        			</td>
                    <td class=title width=10%>완결예정일</td>
                ...
                <tr> 
                    <td rowspan="2" class=title width=10%>연대보증</td>
                    <td width=15%>&nbsp;
        			...
        			<%if(doc.getUser_id8().equals(user_id) || doc.getUser_id7().equals(user_id) || doc.getUser_id2().equals(user_id) || nm_db.getWorkAuthUser("전산팀",user_id)){%>
...
<%}%>
        ...
        <td class=line> 
            ...
            <td colspan='5'>&nbsp;
              <%if(String.valueOf(alink_lc_rent.get("RENT_L_CD")).equals(rent_l_cd)){%>[전자계약서]<%}%>
                ...
                <tr> 
                    <td class=title width=10%>출고구분</td>
                    <td>&nbsp;<%if(pur.getOne_self().equals("Y")){%>자체출고<%}else if(pur.getOne_self().equals("N")){%>영업사원출고<%}%>
                    </td>
                    <td class=title width=10%>출고영업소명</td>
                    <td colspan="2">&nbsp;<%=emp2.getCar_off_nm()%></td>
    		</tr>	                        
                <tr> 
                    <td class=title width=10%>영업구분</td>
                    <td>&nbsp;<%String pur_bus_st = pur.getPur_bus_st();%><%if(pur_bus_st.equals("1")){%>자체영업<%}else if(pur_bus_st.equals("2")){%>영업사원영업<%}else if(pur_bus_st.equals("3")){%>실적이관<%}else if(pur_bus_st.equals("4")){%>에이전트<%}%>
                        <%if(!pur_bus_st.equals("4") && base.getBus_st().equals("7")){%>(에이전트)<%}%>
                    </td>
    ...
    <tr> 
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr> 
                    <td width=3% rowspan="3" class=title>영<br>
                      업<br>
                      담<br>
                    당</td>
                ...
                <tr> 
                    <td width=7% class=title>거래은행</td>
                    <td width=15%>&nbsp;
...
따라서 영업담당자의 직장동료 등 타인은 실수령인으로 등록할 수가 없습니다.</td>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr>
                    ...
                    <td align="center"><%if(pur.getPur_bus_st().equals("3") && (pur.getDlv_brch().equals("B2B사업운영팀")||pur.getDlv_brch().equals("특판팀")||pur.getDlv_brch().equals("법인판촉팀")||pur.getDlv_brch().equals("법인판매팀"))){%><a href="javascript:dlv_tns_commi();"><img src=/acar/images/center/button_sd_sjig.gif align=absmiddle border=0 alt='실적이관권장수당'></a><%}%></td>
                    <td align="center">&nbsp;
	...
	<%}%>
	<%}%>	
	<%if(pur.getDlv_brch().equals("B2B사업운영팀")||pur.getDlv_brch().equals("특판팀")||pur.getDlv_brch().equals("법인판촉팀")||pur.getDlv_brch().equals("법인판매팀")){%>
	<tr>
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr> 
					...
					%>						  
        			  ...
        			  휴 대 폰 : <input type='text' name='emp_m_tel' size='13' value='<%=coe_bean.getEmp_m_tel()%>' class='text'>&nbsp;
					  <%if(doc.getDoc_step().equals("3")){%>
					  <%	if(nm_db.getWorkAuthUser("본사출납",user_id) || nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("영업팀내근직",user_id)){  %>
				...
				<%if(doc.getDoc_step().equals("3")){%>
					  <%	if(nm_db.getWorkAuthUser("본사출납",user_id) || nm_db.getWorkAuthUser("전산팀",user_id)){  %>
                <tr> 					
	...
	<%if(doc.getUser_dt8().equals("") || nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("영업수당회계관리자",user_id) || nm_db.getWorkAuthUser("채권관리팀",user_id) || nm_db.getWorkAuthUser("에이전트관리",user_id)){%>
	<tr>
	    <td align="right">&nbsp;</td>
	<tr> 
	...
	<%if(!emp1.getSup_dt().equals("") && nm_db.getWorkAuthUser("전산팀",user_id)){
		//수정기한
	...
	<tr>
	    <td align='center'>
	    	<input type="button" class="button" value="지급완료후 미지급분 발생 -> 문서 재결재처리" onclick="javascript:re_doc_sanction(6);">&nbsp;	    
	    </td>
	</tr>			  	
	<%}}%>
	<%if(emp1.getSup_dt().equals("") && doc.getDoc_step().equals("3") && nm_db.getWorkAuthUser("전산팀",user_id)){
	%>
    <tr>
	    <td align="right">&nbsp;</td>
	</tr>  
	<tr>
	    <td align='center'>
	    	<input type="button" class="button" value="지급전 문서결재취소" onclick="javascript:re_doc_sanction(0);">&nbsp;	    
	    </td>
	</tr>			  	
	<%}%>
	<tr>
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr> 
                    ...
                    <td align="center"><!--채권관리자--><%=c_db.getNameById(doc.getUser_id7(),"USER_PO")%><br><%=doc.getUser_dt7()%>
        			  ...
        			  %>
        			  <%	if(doc.getUser_id7().equals(user_id) || user_id7.equals(user_id) || nm_db.getWorkAuthUser("전산팀",user_id)  || nm_db.getWorkAuthUser("계약서류점검담당자",user_id) || nm_db.getWorkAuthUser("영업수당관리자",user_id) || nm_db.getWorkAuthUser("채권관리팀",user_id) || nm_db.getWorkAuthUser("출고관리자",user_id) || nm_db.getWorkAuthUser("대출관리자",user_id)){%>
