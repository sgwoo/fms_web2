...
<%
	...
	Vector vt = d_db.getCommiDocList(s_kd, t_wd, gubun1);
	int vt_size = vt.size();
%>
...
<!--
	...
	{
		window.open("view_scan_commi.jsp?m_id="+m_id+"&l_cd="+l_cd, "VIEW_SCAN", "left=100, top=100, width=720, height=500, scrollbars=yes, status=yes");								
	}	
//-->
</script>
</head>
...
<body onLoad="javascript:init()">
    <table border="0" cellspacing="0" cellpadding="0" width=<%if(!gubun1.equals("2")){%>'1730'<%}else{%>'1475'<%}%>>
        <tr>
	    ...
	    <td class='line' width=<%if(!gubun1.equals("2")){%>'1160'<%}else{%>'905'<%}%>>
		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
		    <tr>
			...
			<%} %>
			<td rowspan='2' width='70' class='title'>가감내용</td>
			...
			<td width='65' style="font-size:7pt" class='title'>보증보험</td>
			<td width='65' style="font-size:7pt" class='title'>초기선납금</td>			
			...
			<td width='75' class='title'>출고보전</td>
			<td width='65' class='title'>이관권장</td>
...
%>
			...
			<td  width='110'>&nbsp;<span title='<%=ht.get("FIRM_NM")%>'><a href="javascript:parent.view_client('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("RENT_ST")%>')" onMouseOver="window.status=''; return true"><%=AddUtil.substringbdot(String.valueOf(ht.get("FIRM_NM")), 11)%></a></span></td>
			<td  width='75' align='center'><%=ht.get("CAR_NO")%></td>			
			<td  width='50' align='center'><%=c_db.getNameById(String.valueOf(ht.get("BUS_ID")),"USER")%></td>					
			<td  width='50' align='center'><%if(!String.valueOf(ht.get("BUS_AGNT_ID")).equals("")){%><%=c_db.getNameById(String.valueOf(ht.get("BUS_AGNT_ID")),"USER")%><%}%></td>
		    </tr>
<%
		}
%>
...
%>			
			...
			<td  width='65' align='center'><%if(gi_st.equals("미가입")){%><font color=red><%}%><%=gi_st%><%if(gi_st.equals("미가입")){%></font><%}%></td>
			<td  width='65' align='center'><%if(pp_st.equals("미입금")||pp_st.equals("잔액")){%><font color=red><%}%><span title='<%=Util.parseDecimal(String.valueOf(ht.get("JAN_AMT")))%>원'><%=pp_st%></span><%if(pp_st.equals("미입금")||pp_st.equals("잔액")){%></font><%}%></td>							
			...
			<td  width='65' align='center'>
				<%if(String.valueOf(ht.get("USER_DT1")).equals("")){%>
					<a href="javascript:parent.reg_addamt('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>')" onMouseOver="window.status=''; return true">
			...
			<td  width='75' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("DLV_CON_COMMI")))%></td>
			<td  width='65' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("DLV_TNS_COMMI")))%></td>
