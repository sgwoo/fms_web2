...
<%
	...
	//그린카부분 check
	int gCnt = 0;
	gCnt = as_db.getGreenCarCount(asset_code) ; 
	System.out.println(" 상각 복원 asset_code=" + asset_code + " | gisu = " + gisu );
	s_flag =  as_db.call_sp_insert_assetmove_ret(gisu, asset_code, user_id );
	
	//그린카인 경우 처리 
   if ( gCnt > 0  ) {
     	s_flag =  as_db.call_sp_insert_assetmove_green_ret(gisu, asset_code, user_id );
	}
	
	System.out.println("자산매각 취소 =" + s_flag);
