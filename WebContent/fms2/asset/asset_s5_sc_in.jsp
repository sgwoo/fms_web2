...
<%
	...
	vt = as_db.getAssetSaleList(chk1, st_dt,end_dt,s_kd, s_au, t_wd, sort, asc, gubun, gubun_nm);
...
<!--
	...
	//팝업윈도우 열기
	function open_accident(car_mng_id)
	
	{
		window.open("/acar/off_lease/off_lease_accident.jsp?car_mng_id="+car_mng_id, "accident_car", "left=100, top=20, width=1000, height=900, scrollbars=yes");
	}
    ...
    <td class='line' width='720' id='td_title' style='position:relative;' > 
	  <table border="0" cellspacing="1" cellpadding="0" width='100%' height=43>
        <tr> 
          ...
          <td width="100" class='title'>취득일자</td>
          <td width="100" class='title'>매각일자</td>
          <td width="140" class='title'>경매장</td>
        </tr>
      </table>
	</td>
    ...
    <td class='line' width='620' id='td_con' style='position:relative;'> 
	  <table border="0" cellspacing="1" cellpadding="0" width='100%'>
        ...
        <tr> 
          ...
          <td <%=td_color%> width='60' align='center'><a href="javascript:parent.view_asset('<%=ht.get("ASSET_CODE")%>')" onMouseOver="window.status=''; return true"><%=ht.get("ASSET_CODE")%></a></td>
          ...
          <td <%=td_color%> width='100' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("GET_DATE")))%></td>
          <td <%=td_color%> width='100' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("ASSCH_DATE")))%></td>
          <td <%=td_color%> width='140' align='center'>
          ...
          <% } else { %><span title='<%=olaD.getActn_nm(String.valueOf(ht.get("ACTN_ID")))%>'><%=AddUtil.subData(olaD.getActn_nm(String.valueOf(ht.get("ACTN_ID"))),7)%></a></td>
          <% } %>    
        </tr>
        <%		}	%>
        <tr> 
			...
			//	f_sup_amt = AddUtil.parseFloat(String.valueOf(ht.get("SALE_AMT")))  /  AddUtil.parseFloat("1.1") ; //과세표준
	  		...
	  		//	sup_amt= (int) f_sup_amt;
	   		//	vat_amt = su_bean.getSale_amt() -  sup_amt;	//부가세	
				...
				use_mon += 	AddUtil.parseInt(String.valueOf(ht.get("USE_MON")));	
				
				car_age_avg += 	Float.parseFloat(String.valueOf(ht.get("USE_MON")));	
		%>
		<tr>
		  <td <%=td_color%> width='100' align='right'><%=Util.parseDecimal(sup_amt)%></td>  <!-- 매각액 -->
		  <td <%=td_color%> width='100' align='right'><%=Util.parseDecimal(sup_amt - (t1 + t2 - t6 - t8 - t12) )%></td> <!-- 손익 -->
		  <td <%=td_color%> width='100' align='right'><%=Util.parseDecimal(t1+t2)%></td> <!-- 기초가액 -->
		  <td <%=td_color%> width='100' align='right'><%=Util.parseDecimal(t6+ t8)%></td>	 <!-- 상각누계액-->	 
		   <td <%=td_color%> width='100' align='right'><%=Util.parseDecimal(t12)%> </td>	<!-- 구매보조금 (잔액) -->
		  ...
		  <td <%=td_color%> width='80' align='right'>
		   ...
		   <td <%=td_color%> width='70' align='center'>
                                ...
                                <%}%>
                                &nbsp;                                
                       <a href="javascript:open_accident('<%=ht.get("CAR_MNG_ID")%>')" onMouseOver="window.status=''; return true" hover><img src="http://fms1.amazoncar.co.kr/acar/images/center/icon_memo.gif"  align="absmiddle" border="0"></a>         
          ...
          <td <%=td_color%> width='50' align='center'>
            <%=ht.get("USE_MON")%>
		  </td>						
		</tr>
<%		}	%>
		<tr>
		  ...
		  <td class=title colspan='3'  style="text-align:right">평균&nbsp;</td>
		  <td class=title><%=AddUtil.parseFloatCipher2(car_age_avg/cont_size,1)%></td>
		</tr>
	  </table>
	</td>
<%	}else{	%>                     
