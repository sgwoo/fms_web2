...
<%
	...
	if(nm_db.getWorkAuthUser("아마존카이외",user_id) || acar_de.equals("1000")){
		white = "white";
		disabled = "disabled";
	}
%>
...
<body <%if(nm_db.getWorkAuthUser("외부_탁송업체",ck_acar_id) || acar_de.equals("1000")){%><%}else{%>onload="javascript:document.form1.t_wd.focus();"<%}%>>
<form name='form1' action='/fms2/cons_pur/consp_mng_sc.jsp' target='c_foot' method='post'>
    	...
    	<td colspan=10>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>협력업체 > 배달탁송관리 > <span class=style5>탁송의뢰현황</span></span></td>
        ...
        <td colspan="2" class=line>
	        <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr>		
                    ...
                    <td width=40%>&nbsp;
                          <select name='gubun6'>
                          <option value='1' <%if(gubun6.equals("1")){%>selected<%}%>>등록일자</option>
                          <option value='2' <%if(gubun6.equals("2")){%>selected<%}%>>출고예정일</option>
                        </select>
                        &nbsp;
            		    <select name='gubun1'>
                          <option value='3' <%if(gubun1.equals("3")){%>selected<%}%>>당일</option>
                          <option value='5' <%if(gubun1.equals("5")){%>selected<%}%>>익일</option>
                          <option value='6' <%if(gubun1.equals("6")){%>selected<%}%>>익익일</option>
                          <option value='7' <%if(gubun1.equals("7")){%>selected<%}%>>익익익일</option>
                          <option value='8' <%if(gubun1.equals("8")){%>selected<%}%>>익익익익일</option>
                ...
                <tr>
                    ...
                    <td width=40%>&nbsp;
            		    <select name='gubun5'>
                          <option value=''  <%if(gubun5.equals("")){%>selected<%}%>>선택</option>
                          <option value='1' <%if(gubun5.equals("1")){%>selected<%}%>>본사</option>
                          <option value='2' <%if(gubun5.equals("2")){%>selected<%}%>>부산지점</option>
                          <option value='3' <%if(gubun5.equals("3")){%>selected<%}%>>대전지점</option>
                          <option value='5' <%if(gubun5.equals("5")){%>selected<%}%>>대구지점</option>
                          <option value='6' <%if(gubun5.equals("6")){%>selected<%}%>>광주지점</option>
                          <option value='4' <%if(gubun5.equals("4")){%>selected<%}%>>고객 </option>
                        </select>
        		    </td>
                </tr>                
        		<tr>
                    <td class=title width=10%>검색조건</td>
                    <td <%if(nm_db.getWorkAuthUser("외부_탁송업체",ck_acar_id) || acar_de.equals("1000")){%>colspan="3"<%}%>>&nbsp;
                    	<%if(acar_de.equals("1000")){%>
            		    		<select name='s_kd'>
                          <option value='5' selected>의뢰자</option>
                        </select>
            						&nbsp;&nbsp;&nbsp;
            						<%=t_wd%>
              ...
              <%if(nm_db.getWorkAuthUser("외부_탁송업체",ck_acar_id) || acar_de.equals("1000")){%>
