...
//  request.send(sendXML);
 ...
 function WM_CONTRACT_ARMAZON_DELETE_ADMIN(doc_type,key_index)
 {
    
  webMethodCall("http://www.papyless.co.kr/AmazonCar.asmx", "WM_CONTRACT_ARMAZON_DELETE_ADMIN", "WK_DOC_TYPE=" + doc_type + "&WK_KEY_INDEX=" + key_index   , RtnService);
 }
...
<%if(flag1){%>
		...
		var v_doc_type = '<%=ht.get("DOC_TYPE")%>';
		var v_key_index = '<%=ht.get("DOC_TYPE")%><%=ht.get("RENT_L_CD")%><%=ht.get("RENT_ST")%><%=ht.get("IM_SEQ")%>';
		
		<%
				String doc_type = String.valueOf(ht.get("DOC_TYPE"));
				String reg_dt   = String.valueOf(ht.get("S_REG_DT"));
				String doc_type2 = doc_type;
				
				if(AddUtil.parseInt(reg_dt) >= 20181206){
		...
		<%if(!doc_type.equals(doc_type2)){%>
			v_doc_type  = '<%=doc_type2%>';
			v_key_index = '<%=doc_type2%><%=ht.get("RENT_L_CD")%><%=ht.get("RENT_ST")%><%=ht.get("IM_SEQ")%>';
		<%}%>
		
		<%if(link_type.equals("4") && link_table.equals("rm_rent_link_m")){%>
				//모바일 -> 프로시저에서 모두 처리
		<%}else{%>
				//파피리스	
				WM_CONTRACT_ARMAZON_DELETE_ADMIN(v_doc_type,v_key_index);
		<%}%>
