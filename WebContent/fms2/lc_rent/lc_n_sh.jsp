...
<!--
	...
	function search(){
		...
		if(fm.t_wd.value == '' && fm.st_dt.value == '' && fm.end_dt.value == ''){
			alert('검색어나 기간을 입력하십시오.'); return;
		}							
		fm.t_wd.value = replaceString("&","AND",fm.t_wd.value);
		fm.action = 'lc_n_sc.jsp';
		fm.target='c_foot';
		fm.submit();
	}
	function enter(){
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>영업지원 > 계출관리 > <span class=style5>미점검현황</span></span></td>
        ...
        <td class=line>
    	    <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr>
                    ...
                    <td width="40%">&nbsp;
			<select name='gubun4'>
                            <option value="1" <%if(gubun4.equals("1"))%>selected<%%>>계약등록일</option>                           
                        </select>			
			&nbsp;	
			<select name='gubun5'>			   
			...
			<input type="text" name="end_dt" size="10" value="<%=end_dt%>" class="text">
                    ...
                    <td width=40%>&nbsp;
            		    <select name='gubun1'>
                          ...
                          <option value='1' <%if(gubun1.equals("1")){%>selected<%}%>>신규계약</option>
                          <option value='2' <%if(gubun1.equals("2")){%>selected<%}%>>연장계약</option>
 		              	  <option value='3' <%if(gubun1.equals("3")){%>selected<%}%>>승계계약</option>
                        </select>&nbsp;
            			<select name='gubun3'>
                          <option value='1' <%if(gubun3.equals("1")){%>selected<%}%>>미점검</option>
                        </select>
        			</td>		
                </tr>        					        	    
                <tr>
                    ...
                    <td colspan='3'>&nbsp;
            		    <select name='s_kd'>
                          ...
                          <option value='4' <%if(s_kd.equals("4")){%>selected<%}%>>영업지점코드 </option>
