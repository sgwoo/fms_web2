...
<%
	...
	String mail_yn = request.getParameter("mail_yn")	==null? "Y":request.getParameter("mail_yn");
...
<body leftmargin="15" topmargin="1" <%if(!mail_yn.equals("Y")){%>onLoad="javascript:onprint()"<%} %> >
...
<form action="" name="form1" method="POST" >
  <table width='600' height="230" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr> 
      <td height="50" style="font-size : 11pt;"><font face="굴림">■&nbsp;&nbsp;계약사항 변경내용</font></td>
    </tr>
    <tr> 
      ...
      <td align='center' height="30"> 
	    <table width="100%" height="100%" border="0" cellspacing="1" cellpadding="0">
          <tr bgcolor="#FFFFFF" align="center"> 
            ...
            <td style="font-size : 10pt; font-weight:bold" width="20%"><font face="굴림">변경전</font></td>
            <td style="font-size : 10pt; font-weight:bold" width="20%"><font face="굴림">변경후</font></td>			
          </tr>
        </table>
	  </td>
    </tr>
      ...
      <td width="100%" align='center'>
	    <table width="100%" border="0" cellspacing="1" cellpadding="0">
          ...
          <tr bgcolor="#FFFFFF" align="center"  height="50">
            <td style="font-size : 10pt;" width="20%" <%if(ins_cha_size+fee_cng+opt_cng+cls_cng>1){%>rowspan='<%=ins_cha_size+fee_cng+opt_cng+cls_cng%>'<%}%>><font face="굴림"><%=cont.get("CAR_NO")%><%if(ins_cha_size>1){%><br>(<%=ins_cha_size%>건)<%}%></font></td>
            <td style="font-size : 10pt;" width="20%" <%if(ins_cha_size+fee_cng+opt_cng+cls_cng>1){%>rowspan='<%=ins_cha_size+fee_cng+opt_cng+cls_cng%>'<%}%>><font face="굴림"><%=AddUtil.ChangeDate2(d_bean.getCh_dt())%> 24시</font></td>
			<%		for(int j = 0 ; j < 1 ; j++){
						InsurChangeBean cha = (InsurChangeBean)ins_cha.elementAt(j);%>
            <td style="font-size : 10pt;" width="20%" ><font face="굴림">
            ...
            <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=cha.getCh_before()%></font>
            <%if(cha.getCh_item().equals("5")){%>
            km/1년            
            <%	if(d_bean.getO_rtn_run_amt()-d_bean.getN_rtn_run_amt()>0 || d_bean.getO_rtn_run_amt()-d_bean.getN_rtn_run_amt()<0){ %>
            <br>미운행 1km당 <%=AddUtil.parseDecimal(d_bean.getO_rtn_run_amt())%>원(부가세별도) 환급<br>초과운행 1km당 <%=AddUtil.parseDecimal(d_bean.getO_over_run_amt())%>원(부가세별도) 납입
            <%	}%>
            <%}%>
            </td>
            <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=cha.getCh_after()%></font>
            <%if(cha.getCh_item().equals("5")){%>
            km/1년
            <%	if(d_bean.getO_rtn_run_amt()-d_bean.getN_rtn_run_amt()>0 || d_bean.getO_rtn_run_amt()-d_bean.getN_rtn_run_amt()<0){ %>
            <br>미운행 1km당 <%=AddUtil.parseDecimal(d_bean.getN_rtn_run_amt())%>원(부가세별도) 환급<br>초과운행 1km당 <%=AddUtil.parseDecimal(d_bean.getN_over_run_amt())%>원(부가세별도) 납입
            <%	}%>
            <%}%>
            </td>			
			<%		}%>
		...
		<%			}%>                
          <% 	}%>
           <%		if(fee_cng>0){%>
		    ...
		    <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=AddUtil.parseDecimal(d_bean.getO_fee_amt())%>원
		    	<%if(d_bean.getO_fee_amt()>0){%><br>(vat 포함)<%}%>
		    </font></td>
		    <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=AddUtil.parseDecimal(d_bean.getN_fee_amt())%>원
		    	<%if(d_bean.getN_fee_amt()>0){%><br>(vat 포함)<%}%>
		    </font></td>
                </tr>                
                <%		}%>
           ...
           <%		if(opt_cng>0){%>
		    ...
		    <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=AddUtil.parseDecimal(d_bean.getO_opt_amt())%>원
		    	<%if(d_bean.getO_opt_amt()>0){%><br>(vat 포함)<%}%>
		    </font></td>
		    <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=AddUtil.parseDecimal(d_bean.getN_opt_amt())%>원
		    	<%if(d_bean.getN_opt_amt()>0){%><br>(vat 포함)<%}%>
		    </font></td>
                </tr>                
                <%		}%>
           <%		if(cls_cng>0){%>
		    ...
		    <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=d_bean.getO_cls_per()%>%</font></td>
		    <td style="font-size : 10pt;" width="20%"><font face="굴림"><%=d_bean.getN_cls_per()%>%</font></td>
                </tr>                
                <%		}%>                                
                <% 	}%>
      </table></td>
  ...
  <table width='600' height="<%//=height%>" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr> 
      ...
      <td colspan=3 style="font-size : 13pt;"><font face="굴림">상기와 같이 계약사항의 변경을 요청합니다.</font></td>
    </tr>
    <tr> 
