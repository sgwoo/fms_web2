...
<!--
 	//계약서 내용 보기
	function view_cont(rent_mng_id, rent_l_cd, rent_st, now_stat, san_st, reg_step, client_id, car_st, bus_st){
		...
		if(bus_st=='에이젼트'){
			if(now_stat != '계약승계' && now_stat != '차종변경' && now_stat != '연장' && reg_step != '4'){
			...
			if(client_id!= '000228' && reg_step != '' && san_st == '미결' && now_stat != '계약승계' && now_stat != '차종변경' && now_stat != '연장' && reg_step != '4'){
				if(reg_step == '1')		fm.action = 'lc_reg_step2.jsp';
				else if(reg_step == '2')	fm.action = 'lc_reg_step3.jsp';
				else if(reg_step == '3')	fm.action = 'lc_reg_step4.jsp';
				if(reg_step == '2' && car_st == '업무대여')		fm.action = 'lc_reg_step4.jsp';
			}else{
				if(car_st == '중고차')	fm.action = 'lc_b_u_ac.jsp';				
			}							
		}
		if(san_st == '요청')	fm.action = 'lc_b_c.jsp';
		
		fm.submit();
	}
	
 	//계약서 내용 보기
	function view_cont_c(rent_mng_id, rent_l_cd, rent_st, now_stat, san_st, reg_step, client_id, car_st){
		var fm = document.form1;
		fm.rent_mng_id.value 		= rent_mng_id;
		fm.rent_l_cd.value 		= rent_l_cd;
		fm.rent_st.value 		= rent_st;
...
<form name='form1' method='post' target='d_content'>
  ...
  <input type='hidden' name='from_page' value='/fms2/lc_rent/lc_b_frame.jsp'>  
