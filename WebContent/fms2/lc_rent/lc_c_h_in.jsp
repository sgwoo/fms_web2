            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>		  
              <tr> 
                ...
                <td>&nbsp;<%=cont_etc.getEst_area()%>&nbsp;<%=cont_etc.getCounty()%>
					        <%if(!base.getCar_gu().equals("2")){%><a href="javascript:parent.update('est_area')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a><%}%>
				        </td>
              </tr>
              ...
              <tr>
                ...
                <td>&nbsp;<%String bus_st = base.getBus_st();%><%if(bus_st.equals("1")){%>인터넷<%}else if(bus_st.equals("2")){%>영업사원<%}else if(bus_st.equals("3")){%>업체소개<%}else if(bus_st.equals("4")){%>catalog<%}else if(bus_st.equals("5")){%>전화상담<%}else if(bus_st.equals("6")){%>기존업체<%}else if(bus_st.equals("7")){%>에이젼트<%}else if(bus_st.equals("8")){%>모바일<%}%>
					      <a href="javascript:parent.update('bus_st')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a>
    			          <%if(bus_st.equals("2")){%><font color='#999999'><%=emp1.getEmp_nm()%>(<%=emp1.getCar_off_nm()%>)</font><%}%>
    			      </td>
              </tr>
    		  <%}%>
              <tr> 
                <td width="15%" class=title>용도구분</td>
                <td width="34%">&nbsp;<%String car_st = base.getCar_st();%><%if(car_st.equals("1")){%>렌트<%}else if(car_st.equals("2")){%>예비<%}else if(car_st.equals("3")){%>리스<%}else if(car_st.equals("4")){%>월렌트<%}else if(car_st.equals("5")){%>업무대여<%}%>
    			      <%if(!base.getCar_gu().equals("2") && !car_st.equals("4")){%>&nbsp;<a href="javascript:parent.update('car_st')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a><%}%></td>
                <td width="15%" class=title>관리구분</td>
                <td width="36%">&nbsp;<%String rent_way = max_fee.getRent_way();%><%if(rent_way.equals("1")){%>일반식<%}else if(rent_way.equals("3")){%>기본식<%}else if(rent_way.equals("2")){%>맞춤식<%}%>
    			      <%if(!base.getCar_gu().equals("2") && !car_st.equals("4")){%>&nbsp;<a href="javascript:parent.update('rent_way')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a><%}%></td>
              </tr>
    		  <tr>
                <td class=title>영업지점</td>
                <td>&nbsp;<%=c_db.getNameById(base.getBrch_id(),"BRCH")%></td>		    
                <td class=title>관리지점</td>
                <td>&nbsp;<%=c_db.getNameById(cont_etc.getMng_br_id(),"BRCH")%>
    			      &nbsp;<a href="javascript:parent.update('mng_br_id')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a></td>
    		  </tr>
    		  <tr>
    		  <!-- 연장이 아닌 경우 계약승계, 차종변경등은 bus_id 임, ext_agnt가 있어도... ) //c_db.getNameById(fee.getExt_agnt(),"USER") -->
                ...
                <td>
                    <%if(base.getCar_st().equals("4")){%>
                        ...
                        <%if(!cont_etc.getSpe_est_id().equals("") && base.getUse_yn().equals("")){%>
                        ...
                        &nbsp;<a href="javascript:parent.update('bus_id')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a>
                        <%			}%>
                        <%}%>
                    <%}else{%>
    			              <%if(fee_size ==1){%>
    		  ...
    		  <tr>
                ...
                <td>&nbsp;<%=c_db.getNameById(base.getBus_id2(),"USER")%>
    			    &nbsp;<a href="javascript:parent.update('bus_id2')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a></td>
                <td class=title>관리담당자</td>
                <td>&nbsp;<%=c_db.getNameById(base.getMng_id(),"USER")%>
    			    &nbsp;<a href="javascript:parent.update('mng_id')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a>
    		  ...
    		  <tr>
    			...
    			<%}%>
    			<%if(fee_size==1){%>
    			&nbsp;<a href="javascript:parent.update('bus_agnt_id')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a>
    			<%}%>
    			  </td>
                <td class=title><%if(base.getCar_st().equals("4")){%>예비차관리자<%}else{%>예비배정자<%}%></td>
                <td>&nbsp;<%=c_db.getNameById(base.getMng_id2(),"USER")%>
                &nbsp;<a href="javascript:parent.update('mng_id2')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a>
    			  </td>
    		  </tr>
    		  <tr>
                <td class=title>점검자</td>
                <td colspan="3">&nbsp;<%=c_db.getNameById(fee_etc.getChk_id(),"USER")%>&nbsp;<%=AddUtil.ChangeDate2(fee_etc.getChk_dt())%></td>
    		  </tr>
    		  <%if(!fee_etc.getCng_chk_dt().equals("")){%>
    		  <tr>
                <td class=title>계약변경확인</td>
                <td colspan="3">&nbsp;<%=c_db.getNameById(fee_etc.getCng_chk_id(),"USER")%>&nbsp;<%=AddUtil.ChangeDate2(fee_etc.getCng_chk_dt())%>&nbsp;<%=fee_etc.getCng_chk_st()%></td>
    		  </tr>
    		  <%}%>
    		  <%}%>
            </table>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>		  
              <tr>
                ...
                <%	if(cls.getCls_st().equals("계약승계")){
                		String modify_deadline = rs_db.addDay(cls.getReg_dt(), 30); //해지등록부터 30일이내 수정가능                		
                		if(AddUtil.parseInt(cls.getReg_dt()) <= AddUtil.getDate2(4) && AddUtil.parseInt(modify_deadline) >= AddUtil.getDate2(4)){
               	%>
    			&nbsp;<a href="javascript:parent.update('suc_cls_dt')"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a>
    			<%		}%>
    			<%	}%>    			
    			<%}%>
                </td>
