...
<%
	...
	if(auth_rw.equals(""))	auth_rw = rs_db.getAuthRw(user_id, "01", "01", "25");
	...
	if(car.getCar_origin().equals("")) car.setCar_origin("1");
	
	//자동차회사 리스트
	CarCompBean cc_r [] = cod.getCarCompAllNew(car.getCar_origin());
...
<!--
	...
	function sub_list(idx){
		...
		var SUBWIN="search_esti_sub_list.jsp?from_page=/fms2/lc_rent/lc_cng_car_frame.jsp&idx="+idx+"&car_comp_id="+car_comp_id+"&car_cd="+fm.code.value+"&car_id="+fm.car_id.value+"&car_seq="+fm.car_seq.value+"&car_nm="+fm.car_nm.value+"&garnish_yn_opt_st="+garnish_yn_opt_st+"&hook_yn_opt_st="+hook_yn_opt_st;	
		window.open(SUBWIN, "SubList", "left=100, top=100, width=1050, height=600, scrollbars=yes, status=yes");
	}
...
<form action='' name="form1" method='post'>
  ...
  <input type="hidden" name="lkas_yn" value="<%=cm_bean.getLkas_yn()%>">				<!-- 차선이탈 제어형 (차종포함) 2017.12.26 추가-->
  <input type="hidden" name="lkas_yn_opt_st" value="">	<!-- 차선이탈 제어형 (옵션) -->
  <input type="hidden" name="ldws_yn" value="<%=cm_bean.getLdws_yn()%>">				<!-- 차선이탈 경고형 (차종포함) -->
  <input type="hidden" name="ldws_yn_opt_st" value="">	<!-- 차선이탈 경고형 (옵션) -->
  <input type="hidden" name="aeb_yn" value="<%=cm_bean.getAeb_yn()%>">				<!-- 긴급제동 제어형 (차종포함) -->
  <input type="hidden" name="aeb_yn_opt_st" value="">	<!-- 긴급제동 제어형 (옵션) -->
  <input type="hidden" name="fcw_yn" value="<%=cm_bean.getFcw_yn()%>">				<!-- 긴급제동 경고형 (차종포함) -->
                        ...
                        <table width="100%" border="0" cellpadding="0">
                            <tr>
                            	...
                            	<%	for(int i=0; i<cc_r.length; i++){
                            	  ...
                            	  <option value="<%= cc_bean.getNm_cd() %><%= cc_bean.getCode() %>"  <%if(cm_bean.getCar_comp_id().equals(cc_bean.getCode()))%>selected<%%>><%= cc_bean.getNm() %></option>
                            	<%	}	%>
                          		</select>
                          	-->
                            ...
                            <tr>
                                ...
                                <!--               
									...
									<%	for(int i = 0 ; i < car_size ; i++){
											Hashtable h_car = (Hashtable)cars.elementAt(i);%>
										<option value="<%=h_car.get("CODE")%>"  <%if(cm_bean.getCode().equals(String.valueOf(h_car.get("CODE"))))%>selected<%%>>[<%=h_car.get("CAR_CD")%>]<%=h_car.get("CAR_NM")%></option>	
									<%	}%>		
                              		</select>
                              		-->
...
//-->
    ...
    <tr>
        <td>※ 개시전차종변경은 같은 제조사, 차명에서 <font color=red>차종 및 옵션의 변경</font>이 있을때만 처리됩니다. 나머지는 출고전해지하시고 계약 등록하세요</td>
    </tr>
