...
<!--
	...
	function save(){
		...
		//fm.rent_end_dt.value = addMonth(fm.rent_start_dt.value, fm.add_tm.value); 계산값이 다름
		...
		*/
		...
		if(toInt(replaceString('-','','<%=car_end_dt_max%>')) < toInt(replaceString('-','',fm.rent_end_dt.value)) ){
			alert('대여기간 만료일이 차령만료일보다 큽니다. 확인하십시오.'); 						
			fm.rent_end_dt.focus(); 	
			tr_imreq.style.display = '';
			return;
		}
		<%}%>	
		
		<%if(ext_fee.getPp_s_amt() > 0){%>
		if(!confirm('선납금이 있는 계약입니다. \n\n선납금으로 인해 대여료 인하가 된 만큼 기간을 경과하여 임의연장하는 경우 선납금이 미반영된 금액으로 연장되어야 합니다.\n\n대여료조정이 필요하므로 임의연장 등록후 관리팀장과 상의하여 대여료 조정하십시오.')){
			return;		
		}			
		<%}%>
		
		if(toInt(parseDigit(fm.fee_amt.value)) == 0){ alert('월대여료가 0원입니다. 입력하십시오.'); return; }		
...
//			fm.target='i_no';
	...
	function send_im_req_cont(){
		var fm = document.form1;
		if(fm.im_req_cont.value == ''){ alert('요청사유를 입력하십시오.'); return; }
		fm.action='lc_im_renew_c_req_a.jsp';
		fm.target='i_no';
		fm.submit();				
	}
//-->
...
//    				if(!fees.getCon_mon().equals("")){%>	
	        ...
	        <table border="0" cellspacing="1" cellpadding="0" width='100%'>	 
        		...
        		<%	for(int j = start_size ; j < fee_scd_size ; j++){
							...
							if(j == fee_scd_size-1){
								...
								f_fst_dt 		=  c_db.addMonth(String.valueOf(ht.get("FEE_EST_DT")), 1);
								f_req_dt 		=  c_db.addMonth(String.valueOf(ht.get("REQ_DT")), 1);
								f_tax_dt 		=  c_db.addMonth(String.valueOf(ht.get("TAX_OUT_DT")), 1);
								last_fee_tm = String.valueOf(ht.get("FEE_TM"));
								
								if(AddUtil.parseInt(AddUtil.replace(f_use_s_dt,"-","")) > AddUtil.parseInt(AddUtil.replace(f_fst_dt,"-",""))){
									f_fst_dt 	=  f_use_e_dt;
									f_tax_dt  =  f_use_e_dt;
									f_req_dt 	=  c_db.addDay  (f_use_s_dt, 15);
								}
								...
								//개시대여료 있어서 소진된 경우
								if(ext_fee.getFee_s_amt() >0 && ext_fee.getIfee_s_amt()/ext_fee.getFee_s_amt() >0 && AddUtil.parseInt(String.valueOf(ht.get("USE_E_DT"))) < AddUtil.parseInt(ext_fee.getRent_end_dt()) ){
									ifee_cnt = ext_fee.getIfee_s_amt()/ext_fee.getFee_s_amt();
									
									Hashtable dt_ht0 = af_db.getUseMonDay(ext_fee.getRent_end_dt(), String.valueOf(ht.get("USE_E_DT")));
									
									if( AddUtil.parseInt(String.valueOf(dt_ht0.get("U_MON"))) > ext_fee.getIfee_s_amt()/ext_fee.getFee_s_amt() ){
										ifee_cnt = AddUtil.parseInt(String.valueOf(dt_ht0.get("U_MON")));
									}
	...
	<%	
			...
			int pp_fee_v_amt = 0;
			if(ext_fee.getPp_s_amt()>0){
				pp_fee_s_amt = ext_fee.getPp_s_amt()/AddUtil.parseInt(ext_fee.getCon_mon());
				pp_fee_v_amt = ext_fee.getPp_v_amt()/AddUtil.parseInt(ext_fee.getCon_mon());
			}
			ext_fee.setFee_s_amt(ext_fee.getFee_s_amt()+pp_fee_s_amt);
			ext_fee.setFee_v_amt(ext_fee.getFee_v_amt()+pp_fee_v_amt);
			if(f_use_s_dt.equals("")){
				f_use_s_dt 	=  c_db.addDay  (fees.getRent_end_dt(), 1);
				f_use_e_dt 	=  c_db.addMonth(fees.getRent_end_dt(), 1);
				f_fst_dt 	=  f_use_e_dt;
				f_req_dt 	=  c_db.addDay  (f_use_e_dt, 15);
				f_tax_dt 	=  f_use_e_dt;
			}
	%>
	<tr>
		      ...
		      <%}%>			  
                ...
                <tr>
                    ...
                    <td>&nbsp;<input type='text' name='rent_start_dt' value='<%=AddUtil.ChangeDate2(f_use_s_dt)%>' size='10' class='<%if(fee_scd_size==0){%>whitetext<%}else{%>whitetext<%}%>' onBlur='javscript:this.value = ChangeDate4(this, this.value); <%if(f_use_s_dt.equals("")){%>ScdfeeSet();<%}%>'>
        			  ~
        			  <input type='text' name='rent_end_dt' value='' size='10' class='<%if(fee_scd_size==0 || base.getCar_st().equals("4")){%>text<%}else{%>whitetext<%}%>' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>
        			  &nbsp;&nbsp;<b>(임의연장이 반영후 임의연장 첫회 대여료 납입일(자동이체일)은 <font color=red><input type='text' name='f_est_dt' value='<%=AddUtil.ChangeDate2(f_fst_dt)%>' size='10' class='default' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>입니다.<!-- <%//=AddUtil.replace(f_fst_dt,"-","").substring(6,8)%>일</font>입니다.) --></b>
        			  
        			</td>
                </tr>							
                <tr>
                    <td colspan="2" class='title'>월대여료</td>
                    <td>&nbsp;<input type='text' name='fee_amt' <%if(ext_fee.getFee_s_amt()>0){%>readonly<%}%> value='<%=AddUtil.parseDecimal(ext_fee.getFee_s_amt()+ext_fee.getFee_v_amt())%>' size='10' class='<%if(ext_fee.getFee_s_amt()==0 || base.getCar_st().equals("4")){%>num<%}else{%>whitenum<%}%>' onBlur='javascript:this.value=parseDecimal(this.value); cal_sv_amt(this);'>원&nbsp;
						(임의연장은 계약서를 쓰지 않고, 대여요금은 기존과 동일하게 합니다.)		
						&nbsp;&nbsp;
						공급가:<input type='text' name='fee_s_amt' <%if(ext_fee.getFee_s_amt()>0){%>readonly<%}%> value='<%=AddUtil.parseDecimal(ext_fee.getFee_s_amt())%>' size='10' class='whitenum' onBlur='javascript:this.value=parseDecimal(this.value); cal_sv_amt(this);'>원, 
						부가세:<input type='text' name='fee_v_amt' <%if(ext_fee.getFee_s_amt()>0){%>readonly<%}%> value='<%=AddUtil.parseDecimal(ext_fee.getFee_v_amt())%>' size='10' class='whitenum' onBlur='javascript:this.value=parseDecimal(this.value); cal_sv_amt(this);'>원
        			</td>
                </tr>								
                ...
                <tr id=tr_im4 style='display:<%if(fee_scd_size==0){%>none<%}else{%>none<%}%>'>
                    ...
                    <td>
                        &nbsp;<input type='text' name='o_f_est_dt' value='<%=AddUtil.ChangeDate2(f_fst_dt)%>' maxlength='10' size='10' class='default' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>
                    </td>
                </tr>
    ...
    <%if(pp_fee_s_amt>0){%>
    <tr>
    	<td><font color=green>* 선납금이 있습니다. 선납금/대여기간 금액이 월대여료에 합산됩니다.</font></td>
    </tr>
    <%}%>
    ...
    <tr tr id=tr_imreq style="display:none">    
        <td>● 영업용차량일 경우 차령만료일 체크해서 연장만료일자 경과를 할 경우 고객지원팀장에게 등록 요청
        	<br><br>요청사유 : <input type='text' name="im_req_cont" value='' size="100" class='text'>&nbsp;<a href="javascript:send_im_req_cont();">[메시지발송]</a><br><br>
        </td>
    </tr>	
