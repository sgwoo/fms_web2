...
<%
	...
	Vector vt = a_db.getCmsContList(s_kd, t_wd, st_dt, end_dt, gubun1, gubun2, gubun3, gubun4, gubun5);
	int vt_size = vt.size();
%>
		...
		<td class='line' width='1120'>
			<table border="0" cellspacing="1" cellpadding="0" width=100%>
				<tr>
					...
					<td width='100'  class='title'>결제원</td>
					<td width='80' class='title'>CMS코드</td>
					<td width='120' class='title'>은행명</td>
					<td width='130' class='title'>계좌번호</td>
...
%>
				<tr>
					...
					<td  width='120'>&nbsp;<span title='<%=ht.get("FIRM_NM")%>'><a href="javascript:parent.view_client('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("RENT_ST")%>')" onMouseOver="window.status=''; return true"><%=Util.subData(String.valueOf(ht.get("FIRM_NM")), 8)%></a></span></td>
...
%>			
				<tr>
					...
					<td  width='100' align='center'><%=ht.get("CBIT")%></td>		
					<td  width='80' align='center'><%=ht.get("CMS_CODE")%></td>						
					<td  width='120' align='center'><%=Util.subData(String.valueOf(ht.get("CMS_BANK")),6)%></td>					
					<td  width='130' align='center'><%=ht.get("CMS_ACC_NO")%></td>
