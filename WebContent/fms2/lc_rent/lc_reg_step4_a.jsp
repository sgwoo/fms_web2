...
<%@ page import="acar.cont.*, acar.car_mst.*, acar.car_office.*, acar.res_search.*,  acar.ext.*, acar.client.*, acar.estimate_mng.*"%>
...
//			flag12 = cm_db.insertCoolMsg(msg2);	
	...
	//전기차/수소차 아니면 인천 20190701 && !ej_bean.getJg_g_7().equals("4") 수소차는 인천등록
	...
	if(fee.getFee_pay_st().equals("1") && !cms.getCms_acc_no().equals("") && !car_st.equals("2")){
	...
	if(!car_st.equals("2")){
		...
		if(!tae_car_mng_id.equals("") && fee.getPrv_dlv_yn().equals("Y")){
			...
			//예약시스템 예약등록
			/*
			if(a_db.getRentContChk(taecha.getCar_mng_id(), base.getClient_id(), taecha.getCar_rent_st()) == 0){//기등록체크
				RentContBean rc_bean = new RentContBean();
				rc_bean.setRent_s_cd	("");
				rc_bean.setCar_mng_id	(taecha.getCar_mng_id());
				rc_bean.setRent_st		("10");
				rc_bean.setCust_st		("1");
				rc_bean.setCust_id		(base.getClient_id());
				rc_bean.setRent_dt		(base.getRent_dt());
				rc_bean.setBrch_id		(base.getBrch_id());
				rc_bean.setBus_id		(base.getBus_id());
				if(!taecha.getCar_rent_st().equals(""))		rc_bean.setRent_start_dt(taecha.getCar_rent_st()+"0000");
				if(!taecha.getCar_rent_et().equals(""))		rc_bean.setRent_end_dt	(taecha.getCar_rent_et()+"0000");
				if(!taecha.getCar_rent_st().equals(""))		rc_bean.setDeli_plan_dt	(taecha.getCar_rent_st()+"0000");
				if(!taecha.getCar_rent_et().equals(""))		rc_bean.setRet_plan_dt	(taecha.getCar_rent_et()+"0000");
				rc_bean.setUse_st		("1");
				...
				String rent_s_cd = a_db.getRentContChkGetCD(taecha.getCar_mng_id(), base.getClient_id(), taecha.getCar_rent_st());
				int rs_count = 1;
				//단기대여관리 수정
				RentContBean rc_bean = rs_db.getRentContCase(rent_s_cd, taecha.getCar_mng_id());
	...
	if(!car_gu.equals("0")){
		...
		if(dir_pur_yn.equals("Y") && cm_bean.getCar_comp_id().equals("0001") && emp_id[1].equals("")){
			//emp_id[1] = "030849";	
		}
		if(dir_pur_yn.equals("Y") && cm_bean.getCar_comp_id().equals("0003") && emp_id[1].equals("")){
			//emp_id[1] = "030879";	
		}
	...
	if(!ins_chk1.equals("") || !ins_chk2.equals("") || !ins_chk3.equals("") || !ins_chk4.equals("")  ){
			...
			String cont 	= "[ "+rent_l_cd+" "+firm_nm+" ]  &lt;br&gt; &lt;br&gt; "+ins_chk1+"  &lt;br&gt; &lt;br&gt; "+ins_chk2+"  &lt;br&gt; &lt;br&gt; "+ins_chk3+"  &lt;br&gt; &lt;br&gt; "+ins_chk4+" &lt;br&gt; &lt;br&gt; ";
	...
	//재리스보유차주행거리 입력시 입력점검 프로시저 호출
	if(car_gu.equals("0")){
		String  d_flag3 =  ad_db.call_sp_dist_case_ck(base.getCar_mng_id(), ck_acar_id);
	}	
