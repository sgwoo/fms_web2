	    ...
	    <td class='line' width='720'>
    	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<tr>
        	        ...
        	        <td width='80' class='title'>정상위약율</td>
        	        <td width='80' class='title'>계약위약율</td>
    ...
    <tr>		
        <td class='line' width='300' id='td_con' style='position:relative;'> 
            ...
            <%	for(int i = 0 ; i < cont_size ; i++){
                ...
                <tr> 
                    ...
                    <td <%=td_color%> width='100' align='center'><a href="javascript:parent.view_cont('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("USE_YN")%>', '<%=ht.get("CAR_ST")%>')" onMouseOver="window.status=''; return true"><%=ht.get("RENT_L_CD")%></a>&nbsp;<%if(!String.valueOf(ht.get("USE_YN")).equals("") && nm_db.getWorkAuthUser("관리자모드",user_id)){%><a href="javascript:parent.view_cont('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '')" onMouseOver="window.status=''; return true">.</a><%}%></td>
        ...
        <%	for(int i = 0 ; i < cont_size ; i++){
        		...
        		<tr>
                    ...
                    <td <%=td_color%> width='40' align='center'><a href="javascript:parent.view_scan('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>')" onMouseOver="window.status=''; return true" title='스캔관리'>보기</a></td>										
        		</tr>
<%		}	%>
          </table>
	    </td>
