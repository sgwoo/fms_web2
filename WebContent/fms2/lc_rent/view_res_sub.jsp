        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                    ...
                    <!--<td align="center"><a class=index1 href="javascript:MM_openBrWindow('/acar/rent_diary/rent_cont.jsp?c_id=<%=c_id%>&s_cd=<%=reservs.get("RENT_S_CD")%>&car_no=<%=res.get("CAR_NO")%>','RentCont','scrollbars=yes,status=yes,resizable=yes,width=850,height=400,left=10, top=10')"><%=reservs.get("CUST_NM")%></a></td>-->
                    <td align="center"><%=reservs.get("FIRM_NM")%></td>
        			<!--<td align="center"><%if(!String.valueOf(reservs.get("USE_ST")).equals("취소")){%><a class=index1 href="javascript:MM_openBrWindow('/acar/rent_diary/car_scd_add.jsp?c_id=<%=c_id%>&s_cd=<%=reservs.get("RENT_S_CD")%>&car_no=<%=res.get("CAR_NO")%>','ImgAdd','scrollbars=yes,status=yes,resizable=yes,width=620,height=600,left=50, top=50')"> 
                    보기</a><%}else{%>-<%}%></td>-->
                </tr>
                <%		}
      			}else{%>
