...
<!--	
	...
	{
		window.open("/fms2/client/client_s_frame.jsp?car_st=<%=base.getCar_st()%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/fms2/lc_rent/lc_reg_step2.jsp", "CLIENT", "left=10, top=10, width=1100, height=600, scrollbars=yes, status=yes, resizable=yes");
	}		
...
<table border='0' cellspacing='0' cellpadding='0' width='100%'>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>         
                ...
                <tr id=tr_lic_no2 style="display:''">
		            ...
		            <td width='12%'>&nbsp;이름 : <input type='text' name='mgr_lic_emp' value=''  size='10' class='text' onBlur='javascript:CheckLen(this.value,20)'</td>
		            <td width='12%'>&nbsp;관계 : <input type='text' name='mgr_lic_rel' value=''  size='10' class='text' onBlur='javascript:CheckLen(this.value,20)'></td>
		            <td width='48%'>&nbsp;(개인사업자)&nbsp;※ 계약자가 운전면허가 없는 경우 차량이용자의 운전면허를 입력</td>
                </tr>  
        ...
        <td class=line> 
    		...
    		<%	for(int i=0; i<=4; i++){%>
                <tr> 
					...
					<!--<input type='text' name='mgr_email' size='18' class='text' onBlur='javascript:CheckLen(this.value,30)' style='IME-MODE: inactive'>-->
					</td>
    		...
    		<%	} %>    		    
                ...
                <tr> 
					...
					<td colspan=8>&nbsp;
						...
						&nbsp;&nbsp;<input type="text" name="t_addr" id="t_addr1" size="100" value="">
					</td>
    ...
    <tr>
        <td align='right'>
    	 <a href="lc_reg_step3.jsp?rent_mng_id=<%=rent_mng_id%>&rent_l_cd=<%=rent_l_cd%>&auth_rw=<%=auth_rw%>" target='d_content'><img src=/acar/images/center/button_3step.gif align=absmiddle border=0></a>&nbsp;
