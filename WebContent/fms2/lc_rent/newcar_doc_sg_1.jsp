<%@ page language="java" contentType="text/html;charset=euc-kr"%>	
	...
	<tr>
		...
		<td width=55% class="center"><span class="style8">(계약승계용)</span>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<td width=20% class="cnum right">
			<table>
				<tr>
					<th width=45% class="center">차량번호</th>
					<td width=55% class="center"><%=cr_bean.getCar_no()%></td>
				</tr>
			</table>
		</td>
	</tr>
		...
		<td colspan="3" class="doc">
			<table class="center">
				<tr>
					...
					<th width=45%>선택사양</th>
					<th width="9%">색상</th>
					<th width="16%">차량가격<br><span class="fs">-제조사 소비자가격-</span></th>
					<th width=5%">대수</th>
				</tr>
				<tr>
					<td>&nbsp;<%=cm_bean.getCar_nm()%>&nbsp;<%=cm_bean.getCar_name()%></td>
					<td>&nbsp;<%=car.getOpt()%></td>
					<td  style="height:55px;">&nbsp;<%=car.getColo()%> 
			...
			<table class="center">
				...
				<tr>
					<%-- <td rowspan="4" class="left"><input type="checkbox" name="checkbox" value="checkbox" <%if(cont_etc.getCom_emp_yn().equals("Y")){%> checked <%}%>>계약자의 임직원<br>&nbsp;&nbsp;&nbsp;&nbsp;<span class="fs">(법인 임직원 한정운전<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 특약 가입)</span><br> --%>
					<td rowspan="4" class="left"><input type="checkbox" name="checkbox" value="checkbox" <%if(cont_etc.getCom_emp_yn().equals("Y")){%> checked <%}%>>계약자의 임직원<br><span class="fs">(임직원 한정운전 특약 가입)</span><br>
				...
				<tr>
					<td colspan="5" class="left" style="height:30px;">* 여기서 가족이란 부모, 배우자, 배우자의 부모, 자녀, 며느리, 사위를 말합니다. (형제·자매는 포함되지 않습니다)<br>
					<%-- <%if(base.getCar_st().equals("3")){	//리스 %>
	...
	<tr>
		<td colspan="3" class="right fss">( 계약번호 : <%=rent_l_cd%>, Page 1/2, <%=AddUtil.ChangeDate2(fee.getRent_dt())%> )</td>
	</tr>