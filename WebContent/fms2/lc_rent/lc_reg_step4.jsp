...
//<!--	
	...
	function estimate(rent_st, st){
		...
		if(fm.car_gu.value == '1' && fm.one_self[0].checked == false && fm.one_self[1].checked == false){	alert('출고구분을 선택하여 주십시오.'); fm.one_self[0].focus(); return; }
		...
		if(fm.insurant.value == '1' && fm.insur_per.value == '2'){
			if(fm.ins_s_amt.value == '0'){
				alert('피보험자가 고객일 땐, 원보험료를 입력해주세요');
				return;
			}
		}
...
// 						fm.car_ext.value = '7'; // 전기화물차 인천 등록. 20210729
		...
		if(fm.car_st.value != '2'){
		  ...
		  if(fm.car_st.value != '2' && fm.car_st.value != '5'){
			...
			if(toInt(parseDigit(fm.tot_pp_amt.value))>0 && fm.pp_est_dt.value == '')
										{ alert('선수금 입금예정일을 입력하십시오.'); 			fm.pp_est_dt.focus(); 		return;}
			...
			if(ja_r_amt == 0)						{ alert('대여요금-적용잔가금액을 입력하십시오.'); 			fm.ja_r_amt.focus(); 		return; }
			if(fm.opt_chk[0].checked == false && fm.opt_chk[1].checked == false)				
										{ alert('대여요금-매입옵션 여부를 입력하십시오.'); 			fm.opt_chk.focus(); 		return; }
		...
		<%}%>
			...
			<%if(!base.getCar_st().equals("5")){%>
				...
				if(fm.car_gu.value == '1' && '<%=base.getReject_car()%>' != 'Y'){//신차 (인수거부차량제외)
						...
						if(fm.tae_req_st.value == '1'){
							...
							if(fm.tae_rent_fee_st[0].checked == false && fm.tae_rent_fee_st[1].checked == false){
								...
								if(fm.tae_rent_fee_st[0].checked == true){
									...
									if(toInt(parseDigit(fm.tae_rent_fee.value)) >= toInt(parseDigit(fm.tae_rent_fee_cls.value)))	{ alert('신차 해지시 요금정산(월렌트정상요금)값이 출고지연대차 월대여료보다 크지 않습니다. 확인하십시오.'); 			fm.tae_rent_fee.focus(); 	return; }
								}else{
				...
				if('<%=base.getReject_car()%>' != 'Y'){//신차 (인수거부차량제외)
			
					if(fm.emp_id[0].value != ''){
						if(fm.bus_st.value == '2'){
							if(fm.comm_rt.value == '' || fm.comm_rt.value == '0')	{ alert('영업담당영업사원-영업수당 최대수수료율를 입력하십시오.'); 	fm.comm_rt.focus(); 		return; }
						}else{
						...
						if(fm.bus_st.value != '2' && fm.bus_st.value != '7')	{ alert('영업담당영업사원이 있는데 영업구분이 영업사원이 아닌 다른것으로 되어 있습니다. 변경하십시오.'); fm.bus_st.focus(); return; }
					...
					if(fm.car_gu.value == '1' && fm.bus_st.value == '2'){
						if(fm.emp_nm[0].value == '')			{ alert('영업구분이 영업사원인 계약입니다. 영업사원을 선택하십시오.'); 	fm.emp_nm[0].focus(); 		return; }
					}
			...
			(가니쉬: <input type='text' name="garnish_col" size='20' class='text' value='<%=car.getGarnish_col()%>'>)
                ...
                <tr>
        		    ...
        		    <%if(pur.getUdt_st().equals("")){%>
                            <option value='1' <%if(base.getBrch_id().equals("S1")||base.getBrch_id().equals("S2")||base.getBrch_id().equals("I1")||base.getBrch_id().equals("K1")||base.getBrch_id().equals("K2")||base.getBrch_id().equals("K3")||base.getBrch_id().equals("S3")||base.getBrch_id().equals("S4")||base.getBrch_id().equals("S5")||base.getBrch_id().equals("S6"))%>selected<%%>>서울본사</option>
                    ...
                    <td colspan="3" >&nbsp;
                      <select name="car_ext" id="car_ext" class='default'>
                        ...
                        <%if(cr_bean.getCar_ext().equals("")){%>
                        	...
                        	<%}else{%>
			                  	<option value='7' <%if(base.getBrch_id().equals("S1"))%>selected<%%>>인천</option>
                ...
                <tr>
        	      ...
        	      &nbsp;&nbsp;내용 <input type="text" name="tint_ps_nm" class="text" size="10" value='<%=car.getTint_ps_nm()%>' style="width: 150px;">
        	      &nbsp; 용품점 지급금액 <input type="text" name="tint_ps_amt" class='num' size="10" value='<%=car.getTint_ps_amt()%>'> 원 (부가세별도)
      		     <br>
                       &nbsp;
                ...
                <tr>
                    ...
                    <td colspan="5">&nbsp;
        				...
        				&nbsp;<input type='text' name='extra_amt' size='10' maxlength='20' class='num' value='<%=AddUtil.parseDecimal(car.getExtra_amt())%>' onBlur='javascript:this.value=parseDecimal(this.value);'>
        					  ...
        					  &nbsp;<input type="checkbox" name="serv_b_yn" value="Y" <%if(car.getServ_b_yn().equals("Y")){%>checked<%}%>> 블랙박스
                ...
                <tr>
                  ...
                  <td colspan="5">&nbsp;
				    <input type='text' name='sh_km' size='6' value='<%if(base.getCar_gu().equals("0")){%><%= AddUtil.parseDecimal((String)sh_ht2.get("TOT_DIST")) %><%}%>' class='defaultnum' >
                        ...
                        <%if(base.getCar_gu().equals("2")){%>
                        <a href="javascript:getSecondhandCarAmt_h2();">확인</a>
                        <%}%>
                ...
                <tr> 
                    ...
                    <td colspan='3'>&nbsp;<%if(cont_etc.getInsur_per().equals("")) cont_etc.setInsur_per("1");%>
                        <select name='insur_per' onChange='javascript:display_ip(); set_insur_serv();'>
                          <option value="">선택</option>
                          <option value="1" <%if(cont_etc.getInsur_per().equals("1")||cont_etc.getInsur_per().equals("")||base.getCar_st().equals("5")){%> selected <%}%>>아마존카</option>
                          <%if(car_st.equals("3")){%>
              ...
              <tr> 
                ...
                <td>&nbsp;
                    <select name='driving_age'>
                      <option value="">선택</option>
                      <option value="0" <%if(base.getDriving_age().equals("0")||base.getCar_st().equals("5")){%> selected <%}%>>26세이상</option>
                      <%if(car_st.equals("3")){%>
              ...
              <tr>
                ...
                <td class=''>&nbsp;
                    <select name='gcp_kd'>
                      ...
                      <option value="2" <% if(base.getGcp_kd().equals("2")||base.getCar_st().equals("5")) out.print("selected"); %>>1억원</option>
                ...
                <td class=''>&nbsp;
                    <select name='bacdt_kd'>
                      <option value="">선택</option>
                      <option value="2" <% if(base.getBacdt_kd().equals("2")||base.getCar_st().equals("5")) out.print("selected"); %>>1억원</option>
              ...
              <tr>
                ...
                <td>&nbsp;<%if(cont_etc.getCanoisr_yn().equals("")) cont_etc.setCanoisr_yn("Y");%>
                  <select name='canoisr_yn'>
                    <option value="">선택</option>
                    <option value="Y" <%if(cont_etc.getCanoisr_yn().equals("Y")||base.getCar_st().equals("5")){%> selected <%}%>>가입</option>
                ...
                <td class=''>&nbsp;<%if(cont_etc.getCacdt_yn().equals("")) cont_etc.setCacdt_yn("N");%>
                  <select name='cacdt_yn' class='default'>
                    ...
                    <option value="N" <%if(cont_etc.getCacdt_yn().equals("N")||base.getCar_st().equals("5")){%> selected <%}%>>미가입</option>
                  </select>              </td>
                ...
                <td class=''>&nbsp;<%if(cont_etc.getEme_yn().equals("")) cont_etc.setEme_yn("N");%>
                  <select name='eme_yn'>
                    ...
                    <option value="N" <%if(cont_etc.getEme_yn().equals("N")||base.getCar_st().equals("5")){%> selected <%}%>>미가입</option>
                  </select></td>
              </tr>
              <tr>
...
// 			if(cm_bean.getJg_code().equals("4854") || cm_bean.getJg_code().equals("5866") || cm_bean.getJg_code().equals("3871") || cm_bean.getJg_code().equals("4314111") || cm_bean.getJg_code().equals("6316111") || cm_bean.getJg_code().equals("3313111") || cm_bean.getJg_code().equals("3313112") || cm_bean.getJg_code().equals("3313113") || cm_bean.getJg_code().equals("3313114")){
			...
			결재일자 : <input type='text' size='11' name='credit_sac_dt' maxlength='10' class="text" value='<%=AddUtil.ChangeDate2(fee_etc.getCredit_sac_dt())%>' onBlur='javscript:this.value = ChangeDate(this.value);'>
                ...
                <tr>
                    ...
                    <td colspan="2" align="center">
                    	...
                    	<select name="br_to_st" class="default">
                    		...
                    		<option value="9" <%if (fee_etc.getBr_to_st().equals("9")) {%>selected<%}%>>해당없음</option>
              ...
              <tr>
				...
				월대여료납입방식 :
					  <select name='fee_chk'>
                              <option value="">선택</option>
                              <option value="0" <%if(fee.getFee_chk().equals("0")||base.getCar_st().equals("5"))%>selected<%%>>매월납입</option>
              ...
              <tr id=tr_emp_bus style="display:<%if(!base.getCar_gu().equals("2")){%>''<%}else{%>none<%}%>">
                <td rowspan="3" class='title'>기<br>타</td>               
                ...
                <td align="center">
    		      ...
    		      %</td>
                <td align='center'>
    				[최대 <input type='text' name="comm_rt" value='<%//=emp1.getComm_rt()%>' size="3" class='fixnum' readonly>
    			  %]</td>
              </tr>                                    
              <tr>
                    	...
                    	<div style="float: left; margin-left: 10px; height: 100%; display: table;">
					  	...
					  	<div style="float: left; margin-left: 10px; height: 100%; display: table;">
					  		...
					  		<%if (base.getCar_gu().equals("1") && AddUtil.parseInt(AddUtil.getDate(4)) >= 20200831 && AddUtil.parseInt(AddUtil.getDate(4)) < 20210101) {//개소세 한시적 인하 기간 %>                 
								<!-- <input type="button" onclick="setMentConEtc('0')" value="개별소비세 인상금액 안내문구"><br> -->
            ...
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
              <tr>
                ...
                <td width="20%">&nbsp;매월
                  <select name='fee_est_day'>
                    ...
                    <% } %>
                    <option value='99' <%if(fee.getFee_est_day().equals("99")||base.getCar_st().equals("5")){%> selected <%}%>> 말일 </option>
              ...
              <tr>
                ...
                <td width="20%">&nbsp;
                  <select name='fee_sh'>
                    <option value="">선택</option>
                    <option value="0" <%if(fee.getFee_sh().equals("0")||base.getCar_st().equals("5")){%> selected <%}%>>후불</option>
                ...
                <td>&nbsp;
    			  ...
    			  <td>&nbsp;
    			    사유 : <input type='text' name='cms_not_cau' size='25' value='<%//=fee_etc.getCms_not_cau()%><%if(base.getCar_st().equals("5")){%>급여공제<%}%>' class='text'>
    			  </td>
                </tr>		  		  		  
              <tr>
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2><%if(!base.getCar_gu().equals("2")){%>영업사원-출고담당<%if(!cop_bean.getRent_l_cd().equals("")){%>&nbsp;<font color=red>(사전계약 <%=cop_bean.getCom_con_no()%>)</font><%}%><%}else if(base.getCar_gu().equals("2")){%>중고차구입처<%}%></span></td>
	</tr>	
	<tr>
        <td class=line2></td>
    </tr>
    <tr id=tr_emp_dlv style="display:<%if(!base.getCar_gu().equals("0")){%>''<%}else{%>none<%}%>"> 
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>	
              <tr>
                <td width="3%" rowspan="<%if(!base.getCar_gu().equals("2")){%>4<%}else if(base.getCar_gu().equals("2")){%>5<%}%>" class='title'>출<br>
                  고</td>
    ...
    <tr>
	    <td>* 출고영업소에 선수금 및 임시운행보험료를 지급할 카드/계좌정보를 입력하십시오. 개인명의 계좌는 사용할 수 없습니다. 카드는 지급수단과 지급요청일만 입력하십시오.</td>
    </tr>
