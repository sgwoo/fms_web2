...
<%@ page import="acar.car_office.*, acar.user_mng.*, acar.con_tax.*, acar.insur.*, acar.tint.*, acar.offls_pre.*,acar.client.*, acar.doc_settle.*"%>
<jsp:useBean id="a_db" scope="page" class="acar.cont.AddContDatabase"/>
<jsp:useBean id="af_db" scope="page" class="acar.fee.AddFeeDatabase"/>
<jsp:useBean id="t_db" scope="page" class="acar.con_tax.TaxDatabase"/>
<jsp:useBean id="tint_db" scope="page" class="acar.tint.TintDatabase"/>
...
<%
	...
	//납품관리
	TintBean tint 	= tint_db.getTint(rent_mng_id, rent_l_cd);
	
	TintBean tint3 	= tint_db.getCarTint(rent_mng_id, rent_l_cd, "3");
	
	//차량정보
	Off_ls_pre_apprsl olp_bean = rs_db.getCarBinImg2(base.getCar_mng_id());
	...
	// 임시운행 임시보험료 스캔파일이 있는지 확인한다. 2018.04.19
	Vector attach_vt = new Vector();
	attach_vt = c_db.getAcarAttachFileListLcScanEqual("LC_SCAN", rent_mng_id, rent_l_cd, "26", 0);
...
<!--
	...
	function update_car_f_amt(){
		window.open("/acar/car_register/register_pur_id.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&rent_mng_id=<%=rent_mng_id%>&rent_l_cd=<%=rent_l_cd%>", "CAR_F_AMT", "left=10, top=10, width=820, height=300, scrollbars=yes, status=yes, resizable=yes");
	}
	...
	function opt(car_id, car_seq){		
		var fm = document.form1;
		var SUBWIN="lc_c_c_car_opt.jsp?car_id="+car_id+"&car_seq="+car_seq+"&from_page=<%=from_page%>";
	...
	//저공해스티커 재발급요청
	function eco_sticker_req(){
		var fm = document.form1;
		window.open("reg_eco_sticker_req.jsp?rent_mng_id="+fm.rent_mng_id.value+"&rent_l_cd="+fm.rent_l_cd.value, "ECO_STICKER", "left=100, top=100, height=200, width=620, scrollbars=yes, status=yes");
	}
...
<form action='lc_b_u_a.jsp' name="form1" method='post'>
  ...
  <input type='hidden' name="car_mng_id" id="car_mng_id" 		value="<%=base.getCar_mng_id()%>">
  ...
  <input type='hidden' name='from_page2'	 	value='/fms2/lc_rent/lc_c_c_car.jsp'>
<table border='0' cellspacing='0' cellpadding='0' width='100%'>
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>대여차량 (<%=base.getCar_mng_id()%>)<%if(!mode.equals("view")){%><%if(user_id.equals("000000") || base.getBus_id().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id) || nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("계출담당",user_id) || nm_db.getWorkAuthUser("회계업무",user_id) || nm_db.getWorkAuthUser("보유차관리",user_id) || nm_db.getWorkAuthUser("차량등록자",user_id)){%>&nbsp;<a href="javascript:update('car')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><%}%><font color="#CCCCCC"> ( 계출담당 : 색상,등록지역,썬팅,LPG장착,추가장착,비고 수정 )</font><%}%></span></td>
	</tr>
	<tr>
	    <td class=line2></td>
	</tr>
        ...
        <td class='line'> 			
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr>
                    ...
                    <td>&nbsp;<%if(cr_bean.getTaking_p() == 0){%><%=ej_bean.getJg_g_17()%><%}else{%><%=cr_bean.getTaking_p()%><%}%></td>
                ...
                <tr>
                    ...
                    <td class='title'>-</td>
                    <td>&nbsp;<%if(ej_bean.getJg_g_16().equals("1")){ %><input type="button" class="button" id="pre_cls" value='저공해스티커 재발급요청' onclick="javascript:eco_sticker_req();"><%}%></td>
                </tr>				
                <tr>
          ...
          <tr <%if ((ej_bean.getJg_b().equals("5") || ej_bean.getJg_b().equals("6")) && !car.getEco_e_tag().equals("1")) {%>style="display: none;"<%}%>>
            ...
            <td colspan="5">&nbsp;<%String eco_e_tag = car.getEco_e_tag();%><%if(eco_e_tag.equals("0")){%>미발급<%}else if(eco_e_tag.equals("1")){%>발급<%}%>
          ...
          <%}%>
                ...
                <tr>
                    ...
                    <td colspan="5">&nbsp;
                      ...
                      &nbsp;
                      <%if(car.getTint_s_yn().equals("Y")){%><input type="checkbox" name="tint_s_yn" value="Y" <%if(car.getTint_s_yn().equals("Y")){%>checked<%}%>> 전면 썬팅(기본형), 가시광선투과율:<%=car.getTint_s_per()%>%<%}%>
                      &nbsp;
...
<%-- 	                   <%}%> --%>
                ...
                <tr>
                    ...
                    <td colspan="5">&nbsp;<%=car.getExtra_set()%>&nbsp;<%=AddUtil.parseDecimal(car.getExtra_amt())%>원<font color="#666666">(부가세포함금액, 견적미반영분)</font>
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>차량가격 
	        <%if(!base.getCar_st().equals("4") && !mode.equals("view")){%>
	            <%if(nm_db.getWorkAuthUser("계출담당",user_id) || nm_db.getWorkAuthUser("회계업무",user_id)){%>
	            &nbsp;<a href="javascript:update('car_amt')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a>	            
	            <%}else if(pur.getPur_est_dt().equals("") && (base.getBus_id().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id))){%>
	            &nbsp;<a href="javascript:update('car_amt')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a>
	            <%}%>
	            <font color="#CCCCCC"> ( 계출담당 : 과세구분,출처,차가,특소세 수정 )</font>
	        <%}%>
	        </span>
	    </td>
	</tr>
    ...
    <tr id=tr_car1 style="display:<%if(!base.getCar_gu().equals("0") && !base.getCar_gu().equals("3")){%>''<%}else{%>none<%}%>"> 
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="3" class='title'>구입가격<%if(user_id.equals("000000") || base.getBus_id().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id) || nm_db.getWorkAuthUser("계출담당",user_id) || nm_db.getWorkAuthUser("회계업무",user_id)){%>&nbsp;<a href="javascript:update_car_f_amt()"><img src=/acar/images/center/button_in_modify.gif align=absmiddle border=0></a><%}%></td>
                </tr>
                <tr>
...
<input type='text' name='sh_km_bas_dt' size='11' value='<%= AddUtil.ChangeDate2(sh_km_bas_dt) %>' class='white' >
    ...
    <%}%>               
    <!--연장시 중고차가-->     
    <%for(int i=2; i<=fee_size; i++){
    		ContCarBean fee_etcs = a_db.getContFeeEtc(rent_mng_id, rent_l_cd, Integer.toString(i));
    		
			sh_car_amt 	= fee_etcs.getSh_car_amt();
			sh_year 	= fee_etcs.getSh_year();
			sh_month 	= fee_etcs.getSh_month();
			sh_day	 	= fee_etcs.getSh_day();
			sh_amt		= fee_etcs.getSh_amt();
			sh_ja		= fee_etcs.getSh_ja();
			sh_km		= fee_etcs.getSh_km();
			sh_tot_km	= fee_etcs.getSh_tot_km();
			sh_km_bas_dt	= fee_etcs.getSh_km_bas_dt();
			sh_init_reg_dt	= fee_etcs.getSh_init_reg_dt();
			sh_day_bas_dt	= fee_etcs.getSh_day_bas_dt();
			
    %>
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2><%=i-1%>차 연장 중고차가</span></td>
    </tr>    		
    <tr>    		
        <td class=line> 			  
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
...
<input type='text' name='sh_km_bas_dt' size='11' value='<%= AddUtil.ChangeDate2(sh_km_bas_dt) %>' class='white' >
            ...
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr>
                    ...
                    <td align="center">
                    	...
                    	<%} %>
                    	<%if(base.getCar_mng_id().equals("") && trf_pay_dt.equals("") && trf_pay_dt.equals("") && trf_req_dt.equals("") && trf_nm.equals("임시운행보험료")){ %>
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2><%if(!base.getCar_gu().equals("2")){%>출고<%}else if(base.getCar_gu().equals("2")){%>중고차구입처<%}%> <%if(!mode.equals("view")){%><%if(user_id.equals("000000") || base.getBus_id().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id) || nm_db.getWorkAuthUser("계출담당",user_id) || nm_db.getWorkAuthUser("차량등록자",user_id) || nm_db.getWorkAuthUser("납품준비상황등록업무",user_id) || nm_db.getWorkAuthUser("출고일자등록",user_id) || nm_db.getWorkAuthUser("전산팀",user_id)){%>&nbsp;<a href="javascript:update('emp2','')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><%}%><font color="#CCCCCC"> ( 계출담당 : 영업사원-출고담당 수정 )</font><%}%></span></td>
	</tr>
	<tr>
	    <td class=line2></td>
	</tr>	
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>보험사항 <%if(!mode.equals("view")){%><%if(base.getBus_id().equals(user_id) || base.getBus_id2().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id) || nm_db.getWorkAuthUser("보험담당",user_id) || nm_db.getWorkAuthUser("본사관리팀장",user_id)){%>&nbsp;<a href="javascript:update('insur')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><%}%><font color="#CCCCCC"> ( 보험담당 : 보험조건, 가입조건 수정 )</font><%}%></span></td>
	</tr>
	<tr>
	    <td class=line2></td>
	</tr>	
        ...
        <td class=line>	
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                    ...
                    <td>&nbsp;
                    <b><%String driving_age = base.getDriving_age();%>
                    	...
                    	<a href="javascript:age_search();">
                    		<img src=/acar/images/center/button_in_search.gif border=0 align=absmiddle>
                    	</a>
                    </td>
                ...
                <tr>
                    ...
                    <td width="20%" class=''>&nbsp;<%String gcp_kd = base.getGcp_kd();%><%if(gcp_kd.equals("1")){%>5천만원<%}else if(gcp_kd.equals("2")){%>1억원<%}else if(gcp_kd.equals("3")){%>5억원<%}else if(gcp_kd.equals("4")){%>2억원<%}else if(gcp_kd.equals("8")){%>3억원<%}else if(gcp_kd.equals("9")){%>10억원<%}%></td>
                ...
                <tr>
                    ...
                    <td colspan="5">&nbsp;
					 ...
					 <input type="checkbox" name="ev_yn" 	value="Y" <%if(cont_etc.getEv_yn().equals("Y")){%>checked<%}%> disabled>
					  전기차   
					 <input type="checkbox" name="hook_yn" 	value="Y" <%if(cont_etc.getHook_yn().equals("Y")){%>checked<%}%>>
					  견인고리(트레일러용) 
			...
			ContFeeRmBean fee_rm = a_db.getContFeeRm(rent_mng_id, rent_l_cd, "1");%>
                <tr>
                    ...
                    <td colspan="5">&nbsp;<%if(fee_rm.getMy_accid_yn().equals("Y")){%>고객부담<%}else if(fee_rm.getMy_accid_yn().equals("N")){%>면제<%}%></td>
                </tr>                
                <%}%>
            </table>
        </td>
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>영업사원-출고담당 <%if(!mode.equals("view")){%><%if(base.getBus_id().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id) || nm_db.getWorkAuthUser("계출담당",user_id)){%><a href="javascript:update('emp2','')"><img src="/images/u.gif" width="16" height="16" aligh="absmiddle" border="0"></a><%}%><font color="#CCCCCC"> ( 계출담당 : 영업사원-출고담당 수정 )</font><%}%></span></td>
	</tr>
	<tr>
	    <td class=line2></td>
	</tr>	
