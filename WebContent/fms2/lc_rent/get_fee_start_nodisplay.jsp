...
<%
	...
	if(from_page.equals("/fms2/lc_rent/lc_c_u_start.jsp")){
		...
		if(reg_type.equals("2") && !fee_est_day3.equals("")){
			...
			//매월30일	
				...
				System.out.println("매월결제일자="+fee_est_day3);
				System.out.println("시작일자="+f_use_s_dt2);
				System.out.println("시작월말일자="+fee_est_dt2);
				...
				if(use_days ==0 || use_days < 0){
					...
					System.out.println("일할계산");
					System.out.println("1회차사용일수="+use_days);
					System.out.println("시작익월말일자="+fee_est_dt2);
				...
				for(int i = 0 ; i < fee_pay_tm ; i++){
					...
					if(reg_type.equals("2") && fee_est_day3.equals("30")){
						if(i > 0 && i < (fee_pay_tm-1)){
							cont_fee_amt = fee_s_amt+fee_v_amt;	
						}
					}
					...
					out.println(i+"회차 사용기간="+use_s_dt[i]+"~"+use_e_dt[i]+", 대여료="+AddUtil.parseDecimal(scd_fee_amt[i])+"원"+u_mon+"월"+u_day+"일<br>");
					//System.out.println(i+"회차 사용기간="+use_s_dt[i]+"~"+use_e_dt[i]+", 대여료="+AddUtil.parseDecimal(scd_fee_amt[i])+"원"+u_mon+"월"+u_day+"일<br>");
...
<script language='javascript'>
	...
	<%	if(reg_type.equals("1")){%>
		...
		parent.document.form1.fee_est_day1.value 			= parseDecimal(<%=fee_est_day %>);		
		...
		parent.document.form1.fee_fst_dt1.value 			= ChangeDate3('<%=f_use_e_dt%>');
		parent.document.form1.fee_fst_amt1.value 			= parseDecimal(<%=f_fee_amt%>);
		parent.document.form1.fee_fst_amt1_etc.value 	= '<%=f_etc%>';
		parent.document.form1.fee_lst_dt1.value 			= ChangeDate3('<%=l_use_e_dt%>');
		parent.document.form1.fee_lst_amt1.value 			= parseDecimal(<%=l_fee_amt%>);
	...
	<%	}else if(reg_type.equals("2")){%>
		...
		parent.document.form1.fee_est_day2.value 			= parseDecimal(<%=fee_est_day %>);		
		...
		parent.document.form1.fee_fst_dt2.value 			= ChangeDate3('<%=f_use_e_dt%>');
		parent.document.form1.fee_fst_amt2.value 			= parseDecimal(<%=f_fee_amt%>);
		parent.document.form1.fee_lst_dt2.value 			= ChangeDate3('<%=l_use_e_dt%>');
		parent.document.form1.fee_lst_amt2.value 			= parseDecimal(<%=l_fee_amt%>);
		...
		<%//if(!f_use_e_dt.equals(f_use_e_dt2)){%>
		parent.document.form1.fee_fst_dt3.value 			= ChangeDate3('<%=use_e_dt[0]%>');
		parent.document.form1.fee_fst_amt3.value 			= parseDecimal(<%=scd_fee_amt[0]%>);
		parent.document.form1.fee_fst_amt3_etc.value 	= '<%=f_etc%>';
		<%	if(scd_fee_amt[fee_pay_tm] > 0){%>		
		parent.document.form1.fee_lst_dt3.value 			= ChangeDate3('<%=use_e_dt[fee_pay_tm]%>');
		parent.document.form1.fee_lst_amt3.value 			= parseDecimal(<%=scd_fee_amt[fee_pay_tm]%>);		
		<%	}else{%>
		parent.document.form1.fee_lst_dt3.value 			= ChangeDate3('<%=use_e_dt[fee_pay_tm-1]%>');
		parent.document.form1.fee_lst_amt3.value 			= parseDecimal(<%=scd_fee_amt[fee_pay_tm-1]%>);		
		<%	}%>
		parent.document.form1.fee_lst_amt3_etc.value 	= '<%=l_etc%>';
		<%//}else{%>
		//parent.document.form1.fee_fst_dt3.value 		= ChangeDate3('<%=f_use_e_dt2%>');
		//parent.document.form1.fee_fst_amt3.value 		= parseDecimal(<%=f_fee_amt2%>);
		//parent.document.form1.fee_lst_dt3.value 		= ChangeDate3('<%=l_use_e_dt2%>');
		//parent.document.form1.fee_lst_amt3.value 		= parseDecimal(<%=l_fee_amt2%>);
		<%//}%>
		parent.document.form1.end_chk.value						= 'Y';
