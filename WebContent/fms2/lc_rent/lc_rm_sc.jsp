...
<!--
 	//계약서 내용 보기
	function view_cont(m_id, l_cd, use_yn, car_st, now_stat, reg_step){
		var fm = document.form1;
		fm.rent_mng_id.value = m_id;
		fm.rent_l_cd.value = l_cd;
		fm.now_stat.value = now_stat;
		//if(car_st == '예비') fm.c_st.value = 'car';
		//if(use_yn == '')	fm.action = 'lc_b_u_rm.jsp';
		//else							fm.action = 'lc_c_frame.jsp';
		fm.action = 'lc_c_frame.jsp';
		
		if(reg_step == '1')		fm.action = 'lc_reg_step2_rm.jsp';
		else if(reg_step == '2')	fm.action = 'lc_reg_step4_rm.jsp';
		else if(reg_step == '3')	fm.action = 'lc_reg_step4_rm.jsp';
...
<form name='form1' method='post'>
  ...
  <input type='hidden' name='from_page' value='/fms2/lc_rent/lc_rm_frame.jsp'>
	...
	<tr>
		<td>
			<table border="0" cellspacing="0" cellpadding="0" width=100%>
				<tr>
					<td>
						<iframe src="lc_rm_sc_in.jsp<%=valus%>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
						</iframe>
					</td>
				</tr>
			</table>
