				...
				<tr>
					...
					<td class="pd">임차인은 계약기간 동안 임대인을 피보험자로 하는 이행(지급)보증보험증권( 
						...
						<%=AddUtil.parseDecimal(ext_gin.getGi_amt())%>원, 
							...
							<%if(!ext_gin.getGi_month().equals("")){ %><%=ext_gin.getGi_month()%>개월<%}else{ %><%=fee.getCon_mon()%>개월<%} %>
						<%}else{%>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					  원, &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;개월
					  <%}%>
...
<%-- 					<%if(rtn_run_amt == 0){ // 환급대여료가 0이면 기존 양식 %> --%>
				...
				<tr>
					...
					<td colspan="4">(1) 본 계약은 <span class="style7">&nbsp;&nbsp;<%=begin.get("FIRM_NM")%>&nbsp;&nbsp;</span>와 
					(주)아마존카 간에 <%=AddUtil.getDate3(String.valueOf(begin.get("RENT_DT")))%> 계약하여 이용하고 있는 
					<%if(base.getCar_st().equals("3")){	//리스 %>자동차 시설대여(리스)<%}else{	//렌트 %>자동차 대여이용<%} %>
					계약을 승계하는 것임. &nbsp;계약승계수수료 <span class=style7>&nbsp;&nbsp;<%=AddUtil.parseDecimal(cont_etc.getRent_suc_commi())%> 원(부가세포함)&nbsp;&nbsp;</span><br>
					(2) 기존임차인은 기존 계약에 관한 일체의 권리와 의무를 변경 임차인 <span class="style7">&nbsp;&nbsp;<%=client.getFirm_nm()%>&nbsp;&nbsp;</span>에게 승계하는 것을 인정함.<br>
					(3) 승계시점 주행거리       <span class=style7>&nbsp;&nbsp;<%=AddUtil.parseDecimal(cont_etc.getRent_suc_dist())%><%if(cont_etc.getRent_suc_dist()==0){%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%}%> km&nbsp;&nbsp;</span>  &nbsp;&nbsp;&nbsp; 20 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;년 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;월 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;일 &nbsp;(기존임차인) 
					<span class="style7">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(인)</span><br>
<%-- 					<%if(rtn_run_amt == 0){ // 환급대여료가 0이면 기존 양식 %> --%>
					<%if(first_rent_dt < 20220415){ // 최초 계약일이 2022.04.15 이전이면 기존 양식 %>
						&nbsp;※ 승계시점 주행거리는 참고 기재사항이며, 초과운행대여료 정산은 대여 종료시(중도해지 포함)에 승계전 원계약을 기준으로 정산합니다.
					<%} else{ // 환급대여료 값 있는 경우 신규 양식_20220415 개정 %>
						&nbsp;※ 승계시점 주행거리는 참고 기재사항이며, 환급대여료 및 초과운행대여료 정산은 대여 종료시(중도해지 포함)에 승계전 원계약을 기준으로 정산합니다.
					<%} %>
					</td>
				</tr>
...
<%-- 						<%if(rtn_run_amt == 0){ // 환급대여료가 0이면 기존 양식 %> --%>
	...
	<tr>
		...
		<%-- <%if(client.getClient_st().equals("1") && cont_etc.getClient_share_st().equals("1")){%> --%>
     	 	...
     	 	<table class="center">
				<tr>
					<td style="height:15px;padding:0px;" colspan=4 class="center"><span class=style2>계 약 일 &nbsp;&nbsp;:&nbsp;&nbsp; <%=AddUtil.getDate(1)%> &nbsp;&nbsp;년&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;월 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;일</span></td>
				</tr>
		        <tr>
		...
		<!-- 연대보증인일 경우 -->		
			<table class="center">
				<tr>
					<td height=15 colspan=4 class="center"><span class=style2>계 약 일 &nbsp;&nbsp;:&nbsp;&nbsp; <%=AddUtil.getDate(1)%> &nbsp;&nbsp;년&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;월 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;일</span></td>
				</tr>
		        <tr>
		          	<td width="39%" class="name left">&nbsp;&nbsp;<span class=style2>대여제공자(임대인)</span><br>&nbsp;&nbsp;서울시 영등포구 의사당대로 8,<br>&nbsp;&nbsp;802호 (여의도동, 태흥빌딩)<br><br>
		      			&nbsp;&nbsp;<span class=style6>(주)아마존카 대표이사 &nbsp;&nbsp;&nbsp;조 &nbsp;&nbsp;&nbsp;성 &nbsp;&nbsp;&nbsp;희 &nbsp;&nbsp;(인)</span></td>
     	 	...
     	 	<div id="Layer3" style="position:absolute; margin-top: -105px; margin-left: 617px; z-index:1;"><img src="http://fms1.amazoncar.co.kr/acar/images/rect.png"></div>
     	 	<div id="Layer5" style="position:absolute; margin-top: -50px; margin-left: 605px; z-index:1;"><img src="http://fms1.amazoncar.co.kr/acar/images/tri.png"></div>	
		</td>
	</tr>
	<tr>	        
...
<%-- 			<%if(rtn_run_amt == 0){ // 환급대여료가 0이면 기존 양식 %> --%>
	...
	<tr>	        
		<td colspan="3" class="right fss">( 계약번호 : <%=rent_l_cd%>, Page 2/2, <%=AddUtil.ChangeDate2(fee.getRent_dt())%> )</td>
	</tr>