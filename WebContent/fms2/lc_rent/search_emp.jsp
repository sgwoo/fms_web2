...
<!--
	...
	function set_emp(emp_id, emp_nm, car_off_nm, car_off_id, car_off_st, cust_st, emp_bank, emp_acc_no, emp_acc_nm, bank_cd){
		...
		if(document.form1.gubun.value == 'BUS'){
			...
			<%}%>
			<%if(gubun.equals("DLV") && one_self.equals("Y") && jg_w.equals("") && !pur_bus_st.equals("1")){//자첼출고영업소%>
				opener.dlv_con_commi_yn_tr.style.display	= '';
				opener.document.form1.dlv_con_commi_yn[1].checked	=true;
			<%}%>
			<%if(gubun.equals("DLV") && one_self.equals("N") && jg_w.equals("") && !pur_bus_st.equals("1")){//자첼출고영업소%>
...
<input type='hidden' name='pur_bus_st' value='<%=pur_bus_st%>'>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
	...
	<td>&nbsp;&nbsp;<img src=/acar/images/center/arrow.gif> 영업소/영업사원/연락처 :	
        ...
        <td align="right">        
            <%//if(!auth_rw.equals("1")){%>
            <a href='reg_emp_i.jsp?gubun=<%=gubun%>' onMouseOver="window.status=''; return true"><img src=/acar/images/center/button_reg.gif align=absmiddle border=0></a>
            <%//}%>
        </td>
    </tr>
    <tr>
	...
	<td class="line" colspan=2>
		...
		%>
                <tr align="center">
                    ...
                    <td><a href="javascript:set_emp('<%= coe_bean.getEmp_id() %>','<%= coe_bean.getEmp_nm() %>','<%= coe_bean.getCar_off_nm() %>','<%= coe_bean.getCar_off_id() %>','<%= coe_bean.getCar_off_st() %>','<%= coe_bean.getCust_st() %>','<%= ht.get("BANK") %>','<%= ht.get("ACC_NO") %>','<%= ht.get("ACC_NM") %>','')" onMouseOver="window.status=''; return true"><%= coe_bean.getEmp_nm() %></a></td>
                ...
                <%		if(car_comp_id.equals("0001")){//현대법인판매팀장%>
                <tr align="center">
                ...
                <!--영업담당-에이전트영업-->
		<%}else if(gubun.equals("BUS") && pur_bus_st.equals("4")){//에이전트영업
						count = 0;
						CarOfficeDatabase umd = CarOfficeDatabase.getInstance();
        		CarOffEmpBean coe_r [] = umd.getCarOffEmpAgentAll("mix", t_wd, "car_off_nm", "asc", bus_id);
        		if(coe_r.length > 0){
        			for(int i = 0 ; i < coe_r.length ; i++){
        				coe_bean = coe_r[i];
        				if(coe_bean.getEmp_nm().equals("박규숙")) continue;
		...
		%>    
                <tr align="center">
                    ...
                    <td><%= coe_bean.getCar_comp_nm() %></td>
                    <td><%= coe_bean.getCar_off_nm() %></td>
                    <td><a href="javascript:set_emp('<%= coe_bean.getEmp_id() %>','<%= coe_bean.getEmp_nm() %>','<%= coe_bean.getCar_off_nm() %>','<%= coe_bean.getCar_off_id() %>','<%= coe_bean.getCar_off_st() %>','<%= coe_bean.getCust_st() %>','<%= coe_bean.getEmp_bank() %>','<%= coe_bean.getEmp_acc_no() %>','<%= coe_bean.getEmp_acc_nm() %>','<%= coe_bean.getBank_cd() %>')" onMouseOver="window.status=''; return true"><%= coe_bean.getEmp_nm() %></a></td>
                    <td><%= coe_bean.getEmp_pos() %></td>		  
                    <td><%= coe_bean.getEmp_m_tel() %><%if(coe_bean.getEmp_m_tel().equals("")){%><%= coe_bean.getCar_off_tel() %><%}%></td>
                </tr>
                <%		}%>		
		...
		<%}else{%>
          <%	if(!t_wd.equals("")){
                ...
                <tr align="center">
                    ...
                    <td><%= coe_bean.getCar_off_nm() %></td>
                    <td>                    
                        <%if(!coe_bean.getFraud_care().equals("")){//사기거래관련주의요망자%><font color=red><%}%>
                        <%if(gubun.equals("DLV")){//출고담당%>
			...
			<%}%>
			<%if(!coe_bean.getFraud_care().equals("")){//사기거래관련주의요망자%><br>사기거래관련주의요망:<%=coe_bean.getFraud_care()%></font><%}%>
		    </td>
                    <td><%= coe_bean.getEmp_pos() %></td>		  
                    <td><%= coe_bean.getEmp_m_tel() %><%if(coe_bean.getEmp_m_tel().equals("")){%><%= coe_bean.getCar_off_tel() %><%}%></td>
                </tr>
