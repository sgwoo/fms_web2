...
<%
	...
	//if(!t_wd.equals("")){
		vt = a_db.getContBcAddList(s_kd, t_wd, andor, gubun1, gubun2, gubun3);
	//}
...
<table border="0" cellspacing="0" cellpadding="0" width='1630'>
    ...
    <tr id='tr_title' style='position:relative;z-index:1'>		
	    ...
	    <td class='line' width='72%'>
	        <table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<tr>
        		    <td width="12%" class='title' style='height:34'>차종</td>
        		    <td width="8%" class='title'>차량번호</td>
        	            <td width='6%' class='title'>차량구분</td>		  
        	            <td width='4%' class='title'>계약<br>구분</td>
        	            <td width='4%' class='title'>용도<br>구분</td>
        	            <td width='4%' class='title'>관리<br>구분</td>		  
        	            <td width='4%' class='title'>약정<br>기간</td>
        	            <td width='7%' class='title'>견적일자</td>		  
        	            <td width='7%' class='title'>대여개시일</td>
        	            <td width='8%' class='title'>적용<br>신용등급</td>
        		    <td width='7%' class='title'>견적대여료</td>
        		    <td width='7%' class='title'>기준대여료(c)</td>
        		    <td width='7%' class='title'>정상대여료(g)</td>
        		    <td width='7%' class='title'>계약대여료(h)</td>
        		    <td width='9%' class='title'>등록자</td>
        		</tr>
	        </table>
	    </td>
    </tr>
  <%if(cont_size > 0){%>
    <tr>		
        <td class='line' width='28%' id='td_con' style='position:relative;'> 
    	    <table border="0" cellspacing="1" cellpadding="0" width='100%'>
                ...
                <tr> 
                    ...
                    <td <%=td_color%> width='25%' align='center'><a href="javascript:parent.view_cont('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("RENT_ST")%>', 'a')" onMouseOver="window.status=''; return true" title='계약약식내역'><%=ht.get("RENT_L_CD")%></a></td>		  
		...
		%>
                <tr>
        		    <td <%=td_color%> width='12%' align='center'><span title='<%=ht.get("CAR_NM")%>'><%=AddUtil.subData(String.valueOf(ht.get("CAR_NM")), 8)%></span></td>
        		    <td <%=td_color%> width='8%' align='center'><%=ht.get("CAR_NO")%></td>					
        		    <td <%=td_color%> width='6%' align='center'><%=ht.get("CAR_GU")%></td>
        		    <td <%=td_color%> width='4%' align='center'><%=ht.get("CONT_ST")%></td>		  		  
        		    <td <%=td_color%> width='4%' align='center'><%=ht.get("CAR_ST")%></td>
        		    <td <%=td_color%> width='4%' align='center'><%=ht.get("RENT_WAY")%></td>	
        		    ...
        		    <td <%=td_color%> width='8%' align='center'><%=ht.get("SPR_KD")%></td>
        		    <td <%=td_color%> width='7%' align='right'><%=Util.parseDecimal(String.valueOf(ht.get("INV_S_AMT")))%>원</td>		  
        		    <td <%=td_color%> width='7%' align='right'><%=Util.parseDecimal(String.valueOf(ht.get("BC_S_C")))%>원</td>
        		    <td <%=td_color%> width='7%' align='right'><%=Util.parseDecimal(String.valueOf(ht.get("BC_S_G")))%>원</td>
        		    <td <%=td_color%> width='7%' align='right'><%=Util.parseDecimal(String.valueOf(ht.get("FEE_S_AMT")))%>원</td>
        		    <td <%=td_color%> width='9%' align='center'><%=ht.get("REG_ID")%></td>
        		</tr>
<%		}	%>
	        </table>
	    </td>
