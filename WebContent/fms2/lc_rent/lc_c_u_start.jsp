...
<%
	...
	String car_deli_est_dt = cont_etc.getCar_deli_est_dt();
...
<!--
	...
	function go_to_list(){
		...
		<%if(from_page.equals("/fms2/lc_rent/lc_start2_frame.jsp")){%>
		fm.action = '/fms2/lc_rent/lc_start2_frame.jsp';			
		<%}%>	
		fm.target = 'd_content';	
		fm.submit();
	}	
	...
	function update(){
		...
		if(<%=rs_cont_size%> > 0){
			if(!confirm('예약시스템-지연대차에 반차 미처리분이 있습니다. 하단 지연대차 미반차 리스트에서 출고전대차 수정할 수 있습니다. 대여개시 진행하시겠습니까?')){	return;	}
		}		
		...
		if(fm.car_deli_dt.value == '')			{ alert('대여개시-차량인도일을 입력하십시오.'); 			fm.car_deli_dt.focus(); 	return; }
		...
		//출고지연 장기화로 막음
		<%}else{%>
		//[입력값 체크]1.차량인도일 현재보다 한달이상 차이가 나면 안된다.
		var est_day = getRentTime('m', fm.car_deli_dt.value, '<%=AddUtil.getDate()%>');				
		if( est_day > 2 || est_day < -2){ 
			alert('입력하신 차량인도일이 현재날짜와 한달이상 차이납니다.\n\n확인하십시오.'); 				fm.car_deli_dt.focus(); 	return; 
		}
		//[입력값 체크]1.대여개시일 현재보다 한달이상 차이가 나면 안된다.
		est_day = getRentTime('m', fm.rent_start_dt.value, '<%=AddUtil.getDate()%>');				
		if( est_day > 2 || est_day < -2){ 
		...
		<%if(!fee.getFee_chk().equals("1") && !fee.getPp_chk().equals("0") ){%>
			...
			if(fm.reg_type[0].checked == false && fm.reg_type[1].checked == false && fm.reg_type[2].checked == false){	alert('대여료스케줄 구분을 선택하십시오.');	return;}
			
			
			if(fm.reg_type[0].checked == true){

				ChangeDate4(fm.fee_fst_dt1, fm.fee_fst_dt1.value);
				...
				if(toInt(parseDigit(fm.fee_fst_amt1.value)) == 0){ alert('1회차대여료 납입금액을 확인하십시오.'); 		return; }
				...
				est_day = getRentTime('d', '<%=AddUtil.getDate(4)%>', fm.fee_fst_dt1.value);
			...
			}else if(fm.reg_type[2].checked == true){
    ...
    <tr> 
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr> 
                    <td class=title width=15%>계약번호<%=base.getCar_mng_id()%></td>
...
<%		}%>  	
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr>
                    ...
                    <td width="35%">&nbsp;
                        ...
                        &nbsp;(탁송요청일 디폴트)
                        <%if(!car_deli_est_dt.equals("")){%>
                        (차량인도예정일 : <%=AddUtil.ChangeDate2(cont_etc.getCar_deli_est_dt())%>)
                        <%}%>
		    </td>
                </tr>
                <tr>                        
                    ...
                    <td>&nbsp;
                        <input type="text" name="rent_end_dt" value="" size="11" maxlength='10' class='text' onBlur='javascript:this.value=ChangeDate(this.value);'>
                    </td>
                </tr>
            </table>
	</td>
    ...
    <tr id=tr_fee style="display:''"> 
        <td class='line'> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr>
                    <td width="33%" align="center" class=title><input type='radio' name="reg_type" value='1' onClick="javascript:cng_input(1)"> 대여개시일과 납입일자가 정상 (한달단위청구)</td>
                    <td width="33%" align="center" class=title><input type='radio' name="reg_type" value='2' onClick="javascript:cng_input(2)"> 납입일자변경 (일할계산)</td>
                    <td width="34%" align="center" class=title><input type='radio' name="reg_type" value='3' onClick="javascript:cng_input(3)"> 대여료 선납/분산청구 및 분산납부 (다중스케줄)</td>
                </tr>
            </table>
	</td>
    </tr>  
        ...
        <td class='line'> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>                   
                ...
                <tr>
                    ...
                    <td>&nbsp;
                        <input type='text' name='fee_fst_dt1' value='' maxlength='10' size='10' class='whitetext' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>
                    </td>
                </tr>		  		  		  		  
                <tr>                
                ...
                <tr>
                    ...
                    <td class='title'>납입일자</td>
                    <td>&nbsp;
                        <input type='text' name='fee_lst_dt1' value='' maxlength='10' size='10' class='whitetext' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>
                    </td>
                </tr>		  		  		  		  
                <tr>                
                    <td class='title'>납입금액</td>
        ...
        <td class='line'> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%> 
                ...
                <tr>
                    ...
                    <td class='title'>납입일자</td>
                    <td>&nbsp;
                        <input type='text' name='fee_fst_dt2' value='' maxlength='10' size='10' class='whitetext' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>
                    </td>
                    <td rowspan='2' class='title'>1회차대여료</td>
                    <td class='title'>납입일자</td>
                    <td>&nbsp;
                        <input type='text' name='fee_fst_dt3' value='' maxlength='10' size='10' class='text' onBlur='javscript:set_start_date(2); this.value = ChangeDate4(this, this.value); '>
                    </td>
                </tr>		  		  		  		  
                <tr>                
                    <td class='title'>납입금액</td>
                ...
                <tr>
                    ...
                    <td class='title'>납입일자</td>
                    <td>&nbsp;
                        <input type='text' name='fee_lst_dt2' value='' maxlength='10' size='10' class='whitetext' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>
                    </td>
                    <td rowspan='2' class='title'>마지막회차대여료</td>
                    <td class='title'>납입일자</td>
                    <td>&nbsp;
                        <input type='text' name='fee_lst_dt3' value='' maxlength='10' size='10' class='whitetext' onBlur='javscript:this.value = ChangeDate4(this, this.value);'>
                    </td>
                </tr>		  		  		  		  
                <tr>                
                    <td class='title'>납입금액</td>
