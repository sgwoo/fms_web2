...
<%
	...
	//추가연장대여정보
	Hashtable fee_add = a_db.getContFeeAdd(rent_mng_id, rent_l_cd, "s");
	
	
	if(String.valueOf(fee_add.get("RENT_L_CD")).equals("null")){
		//추가연장대여정보 추정치
		Hashtable fee_add2 = a_db.getContBcClsCase(rent_mng_id, rent_l_cd);
		
		
		fee_add.put("RENT_MNG_ID", 		rent_mng_id);
		fee_add.put("RENT_L_CD",   		rent_l_cd);
		fee_add.put("RENT_ST",     		"s");
		...
		fee_etc2.setRent_st		("s");
		flag3 = a_db.updateFeeEtcAdd(fee_etc2);
		
		fee_add = a_db.getContFeeAdd(rent_mng_id, rent_l_cd, "s");
	}
	...
	String s_rent_dt  = String.valueOf(fee_add.get("RENT_DT"));
	String s_start_dt = String.valueOf(fee_add.get("RENT_START_DT"));
	s_rent_dt = s_start_dt; //2009-07-06 영업효율산출기준일을 계약일에서 대여개시일로 변경
	...
	if(fee_size==1) fee_opt_amt = 0;
	else if(fee_size>1){
		ContFeeBean fees 		= a_db.getContFeeNew(rent_mng_id, rent_l_cd, Integer.toString(fee_size-1));
...
<!--
	...
	function getSecondhandCarAmt_h(){
		...
		if(fm2.fee_rent_st.value == '1') 		fm.rent_st.value 	= '1';//재리스
		else                                   	 	fm.rent_st.value 	= '2';//연장
	...
	function set_janga(){
		...
		var car_price 	= toInt(parseDigit(fm2.car_c_amt.value)) + toInt(parseDigit(fm2.opt_c_amt.value)) + toInt(parseDigit(fm2.col_c_amt.value)) + toInt(parseDigit(fm2.add_opt_amt.value));//
		var s_dc_amt = 0;
		
		if(toInt(fm2.s_dc1_amt.value) > 0 )		s_dc_amt 	= s_dc_amt + toInt(parseDigit(fm2.s_dc1_amt.value));
		if(toInt(fm2.s_dc2_amt.value) > 0 )		s_dc_amt 	= s_dc_amt + toInt(parseDigit(fm2.s_dc2_amt.value));
		if(toInt(fm2.s_dc3_amt.value) > 0 )		s_dc_amt 	= s_dc_amt + toInt(parseDigit(fm2.s_dc3_amt.value));
		
		if(fm2.s_dc1_re.value == '포인트DC')	s_dc_amt = s_dc_amt - toInt(parseDigit(fm2.s_dc1_amt.value));
		if(fm2.s_dc2_re.value == '포인트DC')	s_dc_amt = s_dc_amt - toInt(parseDigit(fm2.s_dc2_amt.value));
		if(fm2.s_dc3_re.value == '포인트DC')	s_dc_amt = s_dc_amt - toInt(parseDigit(fm2.s_dc3_amt.value));
		...
		<%if(fee_size>1){%>
			...
			fm.rent_dt.value = fm2.rent_start_dt.value;
			fm.fee_opt_amt.value = fm2.fee_opt_amt.value;
			fm.sh_amt.value = fm2.sh_amt.value;
		...
		if(fm2.car_gu.value == '0'){//재리스
		...
		}else if(fm2.car_gu.value == '2'){//중고차
						
			fm.rent_st.value = '3';			
			fm.car_mng_id.value = fm2.rent_l_cd.value;
			fm.rent_dt.value = fm2.rent_dt.value;						
			fm.a_b.value = fm2.con_mon.value;
			fm.today_dist.value = parseDigit(fm2.sh_km.value);
			fm.cust_sh_car_amt.value = fm2.sh_amt.value;
			fm.action='/acar/secondhand/getSecondhandJanga.jsp';
			fm.target='i_no';
			...
			var purc_gu 	= fm2.purc_gu.value;
			var car_st 		= fm2.car_st.value;
			var s_st 		= fm2.s_st.value;			
			...
			//--------------------------------------------------------------
				...
				fm2.o_1.value 		= car_price - s_dc_amt;			
				fm2.t_dc_amt.value 	= s_dc_amt;		
				fm2.action='getNewCarJanga.jsp';				
				fm2.target='i_no';				
		}		
		<%}%>
	...
	function getSecondhandCarDist(rent_dt, serv_dt){
		var fm = document.form1;
		rent_dt = fm.sh_day_bas_dt.value;
		serv_dt = fm.sh_km_bas_dt.value;
		var height = 300;
		window.open("search_todaydist.jsp?car_mng_id=<%=base.getCar_mng_id()%>&rent_dt="+rent_dt+"&serv_dt="+serv_dt, "VIEW_DIST", "left=0, top=0, width=650, height="+height+", scrollbars=yes");
	}			
//-->
</script> 
<style type="text/css">
    	...
    	<td colspan=2>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>영업지원 > 정산영업효율등록현황 > <span class=style5>정산영업효율등록</span></span></td>
      ...
      <td class='line'> 			
        <table border="0" cellspacing="1" cellpadding="0" width=100%>
          ...
          <tr>
			...
			원<font color="#666666">(부가세포함금액, 견적 반영분, LPG키트제외, 네비게이션 등)</font></td>
          </tr>
          <tr>
        ...
        <td class=line> 			  
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr>
                    ...
                    <td width="10%">&nbsp;
                      <input type='text' name='sh_ja' value='<%= sh_ja %>'size='4' class='defaultnum' readonly >
					  %</td>
	...
	<tr>
      <td class=line><table border="0" cellspacing="1" cellpadding="0" width=100%>
          <tr>
            ...
            <td width="25%">&nbsp;<%=AddUtil.parseDecimal(gins.getGi_amt())%>원</td>
            <td width="10%" class=title >보증보험료</td>
            <td width="27%">&nbsp;<%=AddUtil.parseDecimal(gins.getGi_fee())%>원</td>
          </tr>
      </table></td>
    </tr>
	<tr>
      ...
      <td class='line'> 
          ...
          <tr>
            ...
            <td width="15%">&nbsp;
                <input type='text' name="con_mon" value='<%=String.valueOf(fee_add.get("CON_MON"))%>' size="4" maxlength="2" class='whitetext' onChange='javascript:set_cont_date(this)'>
    			 개월
    			 <%if(e_bean.getRent_st().equals("s") && !e_bean.getEtc().equals("")){ %>
        ...
        <table border="0" cellspacing="1" cellpadding="0" width=100%>
          ...
          <tr>
            ...
            <td align="center"><input type='text' size='10' name='pp_s_amt' maxlength='11' class='num' value='<%=AddUtil.parseDecimal(fee.getPp_s_amt())%>' onBlur="javascript:set_fee_amt(this)" onKeyDown='javascript:enter_fee(this)'>
        	  원</td>
            <td align="center"><input type='text' size='10' name='pp_v_amt' maxlength='10' class='num' value='<%=AddUtil.parseDecimal(fee.getPp_v_amt())%>' onBlur="javascript:set_fee_amt(this)" onKeyDown='javascript:enter_fee(this)'>
        	  원</td>
            <td align='center'><input type='text' size='10' name='pp_amt' maxlength='11' class='num' value='<%=AddUtil.parseDecimal(fee.getPp_s_amt()+fee.getPp_v_amt())%>' onBlur="javascript:set_fee_amt(this)" onKeyDown='javascript:enter_fee(this)'>
        	  원</td>
            <td align='center'><%=a_db.getPpPaySt(rent_mng_id, rent_l_cd, fee.getRent_st(), "1")%></td>
            <td align="center">차가의
		    ...
		    <td class=line width="100%">
			  <table border="0" cellspacing="1" cellpadding='0' width=100%>
		        ...
		        <tr>
				  ...
				  <td align="center"><input type='text' size='12' name='bc_b_e1' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_B_E1")%><%=fee_etc.getBc_b_e1()%>'>&nbsp;</td>
		        ...
		        <tr>
				  ...
				  <td>경매장예상낙찰가격</td>
				  <td align="center"><input type='text' size='12' name='bc_b_e2' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_B_E2")%><%=fee_etc.getBc_b_e2()%>'>&nbsp;</td>
				</tr>	
		      </table>		
			</td>						
		  </tr>
		    ...
		    <td class=line width="100%">
			  <table border="0" cellspacing="1" cellpadding='0' width=100%>
		        ...
		        <tr>
				  ...
				  <td>10만원당월할부금</td>
				  <td align="center"><input type='text' size='12' name='bc_s_a' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_A")%><%=fee_etc.getBc_s_a()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				  <td rowspan='17' align="center">
					  ...
					  <span class="b"><a href="javascript:estimate_cmp('account', '')" onMouseOver="window.status=''; return true" title="영업효율견적">계산하기</a></span>
					  <%}%>
				  </td>				  
				</tr>
		        <tr>
				  ...
				  <td>계약기간</td>
				  <td align="center"><input type='text' size='12' name='bc_s_b' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_B")%><%=fee_etc.getBc_s_b()%>'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">c</td>
				  <td align="center">bc_s_c</td>				  
				  <td>기준대여료</td>
				  <td align="center"><input type='text' size='12' name='bc_s_c' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_C")%><%=fee_etc.getBc_s_c()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">d</td>
				  <td align="center">bc_s_d</td>				  
				  <td>목표마진율</td>
				  <td align="center"><input type='text' size='12' name='bc_s_d' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_D")%><%=fee_etc.getBc_s_d()%>'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">e</td>
				  <td align="center">bc_s_e</td>				  
				  <td>적용잔가율조정에따른대여료요금할인율</td>
				  <td align="center"><input type='text' size='12' name='bc_s_e' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_E")%><%=fee_etc.getBc_s_e()%>'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">f</td>
				  <td align="center">bc_s_f</td>				  
				  <td>자체출고요율</td>
				  <td align="center"><input type='text' size='12' name='bc_s_f' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_F")%><%=fee_etc.getBc_s_f()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">g</td>
				  <td align="center">bc_s_g</td>				  
				  <td>정상대여료</td>
				  <td align="center"><input type='text' size='12' name='bc_s_g' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_G")%><%=fee_etc.getBc_s_g()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">i</td>
				  <td align="center">bc_s_i</td>				  
				  <td>캐쉬백율</td>
				  <td align="center"><input type='text' size='12' name='bc_s_i' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_I")%><%=fee_etc.getBc_s_i()%>'>&nbsp;</td>
				</tr>
				<tr>
				  <td align="center">i2</td>
				  <td align="center">bc_s_i2</td>				  
				  <td>캐쉬백율</td>
				  <td align="center"><input type='text' size='12' name='bc_s_i' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_S_I")%><%=fee_etc.getBc_s_i2()%>'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">A</td>
				  <td align="center">bc_b_a</td>				  
				  <td>기본식관리비</td>
				  <td align="center"><input type='text' size='12' name='bc_b_a' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_B_A")%><%=fee_etc.getBc_b_a()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">B</td>
				  <td align="center">bc_b_b</td>				  
				  <td>일반식추가관리비</td>
				  <td align="center"><input type='text' size='12' name='bc_b_b' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_B_B")%><%=fee_etc.getBc_b_b()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">D</td>
				  <td align="center">bc_b_d</td>				  
				  <td>재리스초기영업비용</td>
				  <td align="center"><input type='text' size='12' name='bc_b_d' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_B_D")%><%=fee_etc.getBc_b_d()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">K</td>
				  <td align="center">bc_b_k</td>				  
				  <td>일반식최소관리비용</td>
				  <td align="center"><input type='text' size='12' name='bc_b_k' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_B_K")%><%=fee_etc.getBc_b_k()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">N</td>
				  <td align="center">bc_b_n</td>				  
				  <td>견적메이커탁송료</td>
				  <td align="center"><input type='text' size='12' name='bc_b_n' maxlength='10' class=fixnum value='<%//=fee_add.get("BC_B_N")%><%=fee_etc.getBc_b_n()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;</td>
				</tr>
		        <tr>
				  <td align="center">G</td>
				  <td align="center">bc_b_g</td>				  
				  <td>기타수익</td>
				  <td align="center"><input type='text' size='12' name='bc_b_g' maxlength='10' class='num' value='<%//=fee_add.get("BC_B_G")%><%=fee_etc.getBc_b_g()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;내용: <input type='text' size='20' name='bc_b_g_cont' maxlength='30' class=text value='<%//=fee_add.get("BC_B_G_CONT")%><%=fee_etc.getBc_b_g_cont()%>'></td>
				</tr>
		        <tr>
				  <td align="center">U</td>
				  <td align="center">bc_b_u</td>				  
				  <td>기타비용(수당추가지급등)</td>
				  <td align="center"><input type='text' size='12' name='bc_b_u' maxlength='10' class='num' value='<%//=fee_add.get("BC_B_U")%><%=fee_etc.getBc_b_u()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;내용: <input type='text' size='20' name='bc_b_u_cont' maxlength='30' class=text value='<%//=fee_add.get("BC_B_U_CONT")%><%=fee_etc.getBc_b_u_cont()%>'></td>
				</tr>
		        <tr>
				  <td align="center">AC</td>
				  <td align="center">bc_b_ac</td>				  
				  <td>기타 영업효율반영값</td>
				  <td align="center"><input type='text' size='12' name='bc_b_ac' maxlength='10' class='num' value='<%//=fee_add.get("BC_B_AC")%><%=fee_etc.getBc_b_ac()%>' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;내용: <input type='text' size='20' name='bc_b_ac_cont' maxlength='30' class=text value='<%//=fee_add.get("BC_B_AC_CONT")%><%=fee_etc.getBc_b_ac_cont()%>'></td>
				</tr>
		      </table>		
			</td>						
		  </tr>
