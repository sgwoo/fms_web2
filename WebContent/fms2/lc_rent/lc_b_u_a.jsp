...
<%
	...
	boolean flag9 = true;
	boolean flag12 = true;
	boolean flag13 = true;		// 첨단안전장치 수정 flag 값 2018.02.09
	boolean flag14 = true;		// 첨단안전장치 쿨메신저 알림 2018.02.12
...
<%
	...
	if(!o_udt_st.equals("") && !o_udt_st.equals(n_udt_st)){
		if(base.getUse_yn().equals("Y") && fee_size.equals("1") && cont_etc.getRent_suc_dt().equals("")){
			...
			String cont 		= "[ "+rent_l_cd+" "+firm_nm+" ] 장기계약의 차량인수지가 ("+c_db.getNameByIdCode("0035", "", o_udt_st)+" -> "+c_db.getNameByIdCode("0035", "", n_udt_st)+") 변동하였습니다. 확인바랍니다.";
			...
			if(!cs_bean.getUser_id().equals(""))	target_id = nm_db.getWorkAuthUser("차종관리자");
...
<%
	...
	if(ecar_pur_sub_amt_old == 0 || nm_db.getWorkAuthUser("전산팀",user_id)||nm_db.getWorkAuthUser("계약변경관리",user_id)||nm_db.getWorkAuthUser("차종관리자",user_id)){
	...
	ContFeeBean fee = a_db.getContFeeNew(rent_mng_id, rent_l_cd, fee_size);
		...
		if(!car_st.equals("2") && !car_st.equals("5")){
			...
			if(fee.getRent_st().equals("1") && !emp_id[0].equals("")){
				...
				if(nm_db.getWorkAuthUser("전산팀",user_id) && base.getUse_yn().equals("")){
		...
		if(car_gu.equals("1") && fee_size.equals("1")){
			...
			if(ecar_pur_sub_amt_old2 == 0 || nm_db.getWorkAuthUser("전산팀",user_id)||nm_db.getWorkAuthUser("계약변경관리",user_id)||nm_db.getWorkAuthUser("차종관리자",user_id)){
				car.setEcar_pur_sub_amt		(request.getParameter("ecar_pur_sub_amt")	==null? 0:AddUtil.parseDigit(request.getParameter("ecar_pur_sub_amt")));
				car.setEcar_pur_sub_st		(request.getParameter("ecar_pur_sub_st")==null?"":request.getParameter("ecar_pur_sub_st"));
			}
			//=====[car_etc] update=====
...
<%
	if(idx.equals("sanction")){
		...
		//연장/승계담당자 휴가시 업무대체자
		if(String.valueOf(begin.get("CLS_ST")).equals("계약승계") || String.valueOf(begin.get("CLS_ST")).equals("차종변경") || i_fee_size > 1){
			...
			if(!cs_bean.getUser_id().equals("")){
				if(cs_bean.getTitle().equals("오전반휴")){
					...
					if(AddUtil.getTimeAM().equals("오전")){
						target_id2 = nm_db.getWorkAuthUser("세금계산서담당자");	
					}								
		...
		if(now_stat.equals("계약승계") || now_stat.equals("차종변경")){
			//계약승계 조건변경으로 월대여료 변경 있으면 스케줄담당자에게 메세지 발송
			if(String.valueOf(begin.get("CLS_ST")).equals("계약승계") ){
				...
				if(cont_etc.getGrt_suc_o_amt()>cont_etc.getGrt_suc_r_amt() || cont_etc.getGrt_suc_o_amt()<cont_etc.getGrt_suc_r_amt()){
					cont = cont + " "+ "[계약승계]보증금 금액이 변경되었습니다.";
				}
				//피보험자 다른경우
				if(!cont_etc.getInsur_per().equals(a_cont_etc.getInsur_per())){
					cont = cont + " "+ "[계약승계]피보험자가 변경되었습니다.";
				}
				//이행보증보험 
	...
	if(idx.equals("sanction_req")){
		...
		int sanction_user_type = a_db.getSanctionUserType(base.getBus_id(),"");
  		...
  		if(sanction_user_type==1){
  			target_id = nm_db.getWorkAuthUser("5년이상계약점검자");	
  			cs_bean = csd.getCarScheTodayBean(target_id);
  			if(!cs_bean.getUser_id().equals("")){
  				target_id = nm_db.getWorkAuthUser("본사영업부팀장");	
  			}
  			sub 		= "장기계약 점검 요청";
  			cont 		= "["+firm_nm+"] 장기계약의 점검를 요청합니다.";
  		}
  		*/
...
<%
	if(idx.equals("sanction_req_delete")){//계약삭제요청하기
		...
		String cont 	= "["+firm_nm+" "+rent_l_cd+"] 장기계약의 삭제를 요청합니다. \n\n 요청사유 : "+sanction_req_delete_cont;
		String url 		= "/fms2/lc_rent/lc_b_s.jsp?rent_mng_id="+rent_mng_id+"|rent_l_cd="+rent_l_cd+"|now_stat="+now_stat;
		String target_id = nm_db.getWorkAuthUser("심사"); //월마감전산담당자->심사
		String m_url = "/fms2/lc_rent/lc_b_frame.jsp";
...
<%
	if(idx.equals("sanction_req_cancel")){//결재요청 취소하기
		...
		String cont 		= "["+firm_nm+" "+rent_l_cd+"] 장기계약의 결재요청이 취소되었습니다. \n\n 취소사유 : "+sanction_req_cancel_cont;
		String url 		= "/fms2/lc_rent/lc_b_s.jsp?rent_mng_id="+rent_mng_id+"|rent_l_cd="+rent_l_cd+"|now_stat="+now_stat;
		...
		if(target_bean.getDept_id().equals("1000")){
			target_bean2 	= umd.getUsersBean(nm_db.getWorkAuthUser("에이전트관리"));
			
			//에이전트는 문자로, 계약진행담당자가 있다면 계약진행담당자에게
		...
		if(target_bean.getDept_id().equals("1000")){
			...
			target_bean2 	= umd.getUsersBean(nm_db.getWorkAuthUser("에이전트관리2"));
...
<%
	if(idx.equals("sanction_cancel")){//결재 취소하기
		...
		if(fee_size.equals("1") || now_stat.equals("계약승계") || now_stat.equals("차종변경")){
			//=====[cont] update=====
...
<%
	if(idx.equals("delete")){
		...
		if(nm_db.getWorkAuthUser("전산팀",ck_acar_id) || nm_db.getWorkAuthUser("계약삭제",ck_acar_id)){
			...
			//계약승계, 차종변경 계약번호가 다른경우 새계약번호로 사고,과태료,정비 확인한다.
			...
			if(max_fee_tm == 0 && del_chk.equals("Y")){
				...
				//CMS 신청이 되어 있는 계약번호라면 CMS신청담당자에게 해지요청 메시지를 보낸다.
				...
				if(!cms.getCbit().equals("") || !cms.getApp_dt().equals("")){
					...
					String firm_nm		= request.getParameter("firm_nm")==null?"":request.getParameter("firm_nm");
					String sub 		= "장기계약 삭제 CMS 신청확인";
					String cont 		= "["+firm_nm+" "+rent_l_cd+"] 장기계약를 삭제합니다. CMS 신청 여부를 확인하여 취소하여 주십시오.";
					String url 		= "";
				...
				if(base.getCar_st().equals("4")){
					...
					if(!card_cms.getCbit().equals("") || !card_cms.getApp_dt().equals("")){
						...
						String firm_nm		= request.getParameter("firm_nm")==null?"":request.getParameter("firm_nm");
						String sub 		= "월렌트 삭제 카드자동출금 신청확인";
						String cont 		= "["+firm_nm+" "+rent_l_cd+"] 월렌트계약을 삭제합니다. 카드자동출금 신청 여부를 확인하여 취소하여 주십시오.";
						String url 		= "";
						String target_id = nm_db.getWorkAuthUser("신용카드자동출금");
				...
				if(now_stat.equals("계약승계") || now_stat.equals("차종변경")){
					...
					String sub 		= "계약승계 장기계약 삭제 보험확인";
					
					if(now_stat.equals("차종변경")){
						sub 		= "차종변경 장기계약 삭제 보험확인";
					}
...
<%
	if(idx.equals("delete_recar_reg")){
		...
		if(nm_db.getWorkAuthUser("전산팀",ck_acar_id) || nm_db.getWorkAuthUser("계약삭제",ck_acar_id)){
			...
			if(base.getUse_yn().equals("N")){
				...
				if(String.valueOf(cont_view_yes_car.get("CAR_MNG_ID")).equals("null")){
					...
					String new_rent_l_cd = as_db.getNextRent_l_cd(rent_l_cd.substring(0,7)+"S");		//신규계약코드
		...
		%>
<script language='javascript'>
<%		if(!flag1){	%>	alert('해지계약 보유차생성 에러입니다.\n\n확인하십시오');	<%		}	%>		
</script>
<%	}%>
...
<script language='javascript'>
<%		if(!flag1){  %>	alert('차량기본정보 등록 에러입니다.\n\n확인하십시오');	<%		}	%>
<%		if(!flag13){%>	alert('첨단안전장치 수정 에러입니다.\n\n확인하십시오');	<%		}	%>
<%		if(!flag14){%>	alert('쿨메신저 첨단안전장치 변경 이력 전송 실패입니다.\n총무팀 보험담당자에게 전달바랍니다.\n');	<%		}	%>
</script>
<%	}%>
...
<%
	if(idx.equals("ext_fee")){
		...
		//보증금
		ExtScdBean grt = ae_db.getAGrtScd(rent_mng_id, rent_l_cd, rent_st, "0", "1");//기존 등록 여부 조회
		int grt_gbn = 1;	//기존
		if(grt == null || grt.getRent_l_cd().equals("")){
			grt_gbn = 0;	//신규
			grt = new ExtScdBean();
		...
		//선납금
		ExtScdBean pp = ae_db.getAGrtScd(rent_mng_id, rent_l_cd, rent_st, "1", "1");//기존 등록 여부 조회
		int pp_gbn = 1;		//기존
		if(pp == null || pp.getRent_l_cd().equals("")){
			pp_gbn = 0;		//신규
			pp = new ExtScdBean();
		...
		//개시대여료
		ExtScdBean ifee = ae_db.getAGrtScd(rent_mng_id, rent_l_cd, rent_st, "2", "1");//기존 등록 여부 조회
		int ifee_gbn = 1;	//기존
		if(ifee == null || ifee.getRent_l_cd().equals("")){
			ifee_gbn = 0;	//신규
			ifee = new ExtScdBean();
...
<script language='javascript'>
<%		if(!flag1){	%>	alert('계약기본정보 등록 에러입니다.\n\n확인하십시오');	<%		}	%>		
<%		if(!flag2){	%>	alert('대여기타정보 등록 에러입니다.\n\n확인하십시오');	<%		}	%>		
<%		if(!flag3){	%>	alert('대여정보 등록 에러입니다.\n\n확인하십시오');		<%		}	%>		
<%		if(!flag6){	%>	alert('선수금스케줄 등록 에러입니다.\n\n확인하십시오');	<%		}	%>		
</script>
