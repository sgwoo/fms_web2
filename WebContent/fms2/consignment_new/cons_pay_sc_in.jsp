...
<%
	...
	Vector vt = cs_db.getConsignmentPayList(s_kd, t_wd, gubun1, st_dt, end_dt);
	int vt_size = vt.size();
		...
		<td class='line' width='70%'>
			<table border="0" cellspacing="1" cellpadding="0" width='100%'>
				<tr>
				  ...
				  <td width='7%' rowspan="3" class='title'>지출일자</td>
				  <td width='16%' rowspan="3" class='title'>기간</td>
				  <td colspan="2" class='title'>법인카드</td>
				  <td colspan="9" class='title'>청구금액</td>				  				  
			    </tr>
				<tr>
				  <td rowspan="2" width='6%' class='title'>탁송관리자</td>
			      <td rowspan="2" width='6%' class='title'>팀장</td>
			      <td rowspan="2" width='7%' class='title'>유류비</td>
			      <td rowspan="2" width='6%' class='title'>세차비</td>
			      <td rowspan="2" width='8%' class='title'>탁송료</td>			   
			      <td rowspan="2" width='5%' class='title'>유류비</td>
			      <td rowspan="2" width='5%' class='title'>세차비</td>
			      <td rowspan="2" width='5%' class='title'>세차<br>수수료</td>
			      <td colspan=4 class='title'>기타</td>
			      <td rowspan="2" width='9%' class='title'>소계</td>
			  </tr>
			   <tr>	
			      <td width='5%' class='title'>외부<br>탁송료</td>				  
...
<%		for(int i = 0 ; i < vt_size ; i++){
			Hashtable ht = (Hashtable)vt.elementAt(i);%>
				<tr>
					...
					<td  width='25%' align='center'><a href="javascript:parent.cons_doc_view('<%=ht.get("REQ_CODE")%>', '<%=ht.get("OFF_ID")%>', '<%=ht.get("REQ_DT")%>', '<%=ht.get("PAY_DT")%>', '<%=ht.get("BR_ID")%>', '', '<%=ht.get("DOC_NO")%>')" onMouseOver="window.status=''; return true"><%=AddUtil.ChangeDate2(String.valueOf(ht.get("REQ_DT")))%></a></td>					
...
<%			total_amt6 	= total_amt6 + Long.parseLong(String.valueOf(ht.get("CNT")));
		}%>
				<tr>
				  <td class='title'>&nbsp;</td>
				  <td class='title'>&nbsp;</td>				  
				  <td class='title'>&nbsp;</td>
				  <td class='title'>&nbsp;</td>
				  <td class='title'><%=total_amt6%>건</td>
				</tr>
			</table>
		</td>
		<td class='line' width='70%'>
...
<%		for(int i = 0 ; i < vt_size ; i++){
			Hashtable ht = (Hashtable)vt.elementAt(i);%>			
				<tr>											
					<td  width='6%' align='center'><%=ht.get("USER_NM1")%></td>
					<td  width='6%' align='center'><%=ht.get("USER_NM2")%></td>
					<td  width='7%' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("PAY_DT")))%></td>
					<td  width='16%' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("MIN_DT")))%>~<%=AddUtil.ChangeDate2(String.valueOf(ht.get("MAX_DT")))%></td>					
					<td  width='7%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("OIL_CARD_AMT")))%></td>
					<td  width='6%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("WASH_CARD_AMT")))%></td>
					<td  width='8%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("CONS_AMT")))%></td>							
					<td  width='5%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("OIL_AMT")))%></td>
