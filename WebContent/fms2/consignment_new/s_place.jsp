...
<%
	...
	String chk_client 		= request.getParameter("chk_client")==null?"":request.getParameter("chk_client");
	...
	//	System.out.println("chk_client >>>>" + chk_client);
		vt = cons_db.getPlaceSearch2("", s_kd, t_wd, chk_client);
	}
...
<!--
	...
	function Disp1(gubun, nm, addr, mng_off, tel){
		var fm = document.form1;
		<%if(go_url.equals("/fms2/consignment_new/cons_reg_step1_off_sh.jsp")){%>
		
			opener.form1.<%=st%>_place.value 	= nm;
			opener.form1.<%=st%>_comp.value 	= '(주)아마존카 '+mng_off;
			opener.form1.<%=st%>_tel.value 	= tel;		
			opener.form1.<%=st%>_title.value 	= '<%=user_bean.getUser_pos()%>';
			opener.form1.<%=st%>_man.value 	= '<%=user_bean.getUser_nm()%>';
			opener.form1.<%=st%>_m_tel.value 	= '<%=user_bean.getUser_m_tel()%>';		
		...
		<%}else{%>
			...
			<%//}%>
			if (nm == '조양골프연습장 주차장') {
				opener.form1.<%=st%>_tel[<%=idx%>].value 	= '070-8806-5035';
				opener.form1.<%=st%>_title[<%=idx%>].value 	= '';
				opener.form1.<%=st%>_man[<%=idx%>].value 	= '스마일TS';
				opener.form1.<%=st%>_m_tel[<%=idx%>].value 	= '010-6506-5035';				
			}
			if (nm == '웰메이드 주차장') {
				opener.form1.<%=st%>_tel[<%=idx%>].value 	= '051-851-0606';
			}
		<%}%>		
		self.close();
	...
	function Disp2(client_id, firm_nm, client_nm, gubun, tel, m_tel, addr, title){
		...
		<%if(go_url.equals("/fms2/car_pur/pur_doc_i.jsp")){%>
			opener.form1.rent_ext.value					= addr.substr(0,11);
		<%} else if(go_url.equals("/fms2/consignment_new/cons_reg_step1_off_sh.jsp")){%>
	...
	function Disp3(off_nm, own_nm, off_tel, off_addr){
		var fm = document.form1;
		<%if(go_url.equals("/fms2/consignment_new/cons_reg_step1_off_sh.jsp")){%>
			opener.form1.<%=st%>_place.value 	= off_addr;
			opener.form1.<%=st%>_comp.value 	=off_nm;
			opener.form1.<%=st%>_tel.value 	= off_tel;		
			opener.form1.<%=st%>_man.value 	= own_nm;
	...
	function Disp4(dp4_n){
		...
		var idx = <%=idx%>;
		if(idx=='0'){
			window.opener.$("#from_place").val(from_place);
			window.opener.$("#from_comp").val(from_comp);
			window.opener.$("#from_title").val(from_title);
			window.opener.$("#from_tel").val(from_tel);
			window.opener.$("#from_man").val("");
			window.opener.$("#from_m_tel").val("");
		}else{
        ...
        <td class=line>
    	    <table border="0" cellspacing="1" width=100%>
                ...
                <tr align="center">
                  ...
                  <td <%=td_color%>><%=ht.get("GUBUN")%></td>
                  <td <%=td_color%>><%=ht.get("TEL")%>/<%=ht.get("M_TEL")%></td>
                  <td <%=td_color%>><%=ht.get("ADDR")%></td>
                </tr>
                <%		}%>
            </table>
	    </td>
