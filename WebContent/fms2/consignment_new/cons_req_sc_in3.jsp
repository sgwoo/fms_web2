...
<form name='form1' method='post'>
  ...
  <input type='hidden' name='from_page' value='/fms2/consignment_new/cons_req_frame3.jsp'>
  <input type='hidden' name='cons_no' value=''>
  <input type='hidden' name='mode' value=''>    
  <input type='hidden' name='req_dt' value='<%=AddUtil.getDate()%>'>      
<table border="0" cellspacing="0" cellpadding="0" width='3300'>
  	<tr><td class=line2 colspan="2"></td></tr>
	<tr id='tr_title' style='position:relative;z-index:1'>
		<td class='line' width='26%' id='td_title' style='position:relative;'>
			<table border="0" cellspacing="1" cellpadding="0" width='100%'>
				<tr>
				  <td width='5%' class='title' style='height:80'>연번</td>
		          ...
		          <td width='17%' class='title'>탁송번호</td>
				  <td width='22%' class='title'>탁송업체</td>
		          <td width="10%" class='title'>탁송<br>구분</td>				  
		...
		<td class='line' width='74%'>
			<table border="0" cellspacing="1" cellpadding="0" width='100%'>
				<tr>
		          <td width="7%" rowspan="3" class='title'>차명</td>									
		          <td width="4%" rowspan="3" class='title'>청구일자</td>
				  <td colspan="2" class='title'>출발</td>
				  <td colspan="2" class='title'>도착</td>
				  <td width='3%' rowspan="3" class='title'>지불<br>구분</td>
				  <td width='4%' rowspan="3" class='title'>비용<br>구분</td>
				  <td width='3%' rowspan="3" class='title'>고객탁송료</td>				  
				  <td colspan="2" class='title'>법인카드</td>
				  <td colspan="9" class='title'>청구금액</td>
				  <td width='4%' rowspan="3" class='title'>운전자</td>
				  <td width='4%' rowspan="3" class='title'>의뢰자</td>
		          <td width="11%" rowspan="3" class='title'>확인일자</td>				  
				</tr>
				<tr>
				  <td width='6%' class='title'  rowspan="2">장소</td>
			      <td width='8%' class='title'  rowspan="2">시간</td>
			      <td width='6%' class='title'  rowspan="2">장소</td>
			      <td width='8%' class='title'  rowspan="2">시간</td>
			      <td width='4%' class='title'  rowspan="2">유류비</td>
			      <td width='3%' class='title'  rowspan="2">세차비</td>
			      <td width='5%' class='title'  rowspan="2">탁송료</td>			 
			      <td width='3%' class='title'  rowspan="2">유류비</td>
			      <td width='3%' class='title'  rowspan="2">세차비</td>
			      <td width='3%' class='title'  rowspan="2">세차<br>수수료</td>			
			      <td colspan=4 class='title'>기타</td>
			      <td width='5%' class='title'  rowspan="2">합계</td>
			  </tr>
			    <tr>	
			      <td width='3%' class='title'>외부<br>탁송료</td>				  
			      <td width='3%' class='title'>주차비</td>
			      <td width='3%' class='title'>보증수리<br/>대행</td>
			      <td width='3%' class='title'>검사대행</td>					    
			  </tr>	 
			</table>
		</td>
	</tr>
...
<%		for(int i = 0 ; i < vt_size ; i++){
			...
			%>
				<tr>
					...
					<td  width='17%' align='center'><a href="javascript:parent.view_cons('<%=ht.get("CONS_NO")%>')" onMouseOver="window.status=''; return true"><%=ht.get("CONS_NO")%>-<%=ht.get("SEQ")%></a></td>					
					<td  width='22%' align='center'><span title='<%=ht.get("OFF_NM")%>'><%=ht.get("OFF_NM")%></span></td>
					<td  width='10%' align='center'><%=ht.get("CONS_ST_NM")%></td>					
<%-- 					<td  width='15%' align='center'><span title='<%=ht.get("CAR_NO")%>'><%=Util.subData(String.valueOf(ht.get("CAR_NO")), 9)%></span></td> --%>
...
<%		for(int i = 0 ; i < vt_size ; i++){
			...
			int chk_amt = 0;
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("CONS_AMT")));
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("OIL_AMT")));
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("WASH_AMT")));
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("WASH_FEE")));
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("CONS_OTHER_AMT")));
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("OTHER_AMT")));
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("ETC1_AMT")));
			chk_amt = chk_amt + AddUtil.parseInt(String.valueOf(ht.get("ETC2_AMT")));
			chk_amt = chk_amt - AddUtil.parseInt(String.valueOf(ht.get("TOT_AMT")));
			%>
				<tr>			
					<td  width='7%' align='center'><span title='<%=ht.get("CAR_NM")%>'><%=Util.subData(String.valueOf(ht.get("CAR_NM")), 7)%></span></td>																	
					<td  width='4%' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("REQ_DT")))%></td>					
					<td  width='6%' align='center'><span title='<%=ht.get("FROM_PLACE")%>'><%=Util.subData(String.valueOf(ht.get("FROM_PLACE")), 5)%></span></td>
					<td  width='8%' align='center'><%=AddUtil.ChangeDate3(String.valueOf(ht.get("FROM_DT")))%></td>
					<td  width='6%' align='center'><span title='<%=ht.get("TO_PLACE")%>'><%=Util.subData(String.valueOf(ht.get("TO_PLACE")), 5)%></span></td>
					...
					<td  width='3%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("CUST_AMT")))%></td>
					<td  width='4%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("OIL_CARD_AMT")))%></td>
					<td  width='3%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("WASH_CARD_AMT")))%></td>
					<td  width='5%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("CONS_AMT")))%></td>		
					...
					<td  width='3%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("ETC1_AMT")))%></td>			
					<td  width='3%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("ETC2_AMT")))%></td>			
					<td  width='5%' align='right'>
						<%if(chk_amt > 0 || chk_amt < 0){ %>
						<font color=red>(이상분<%=chk_amt%>)</font>
						<%} %>
						<%=AddUtil.parseDecimal(String.valueOf(ht.get("TOT_AMT")))%></td>
					<td width='4%' align='center'><span title='<%=ht.get("DRIVER_NM")%>'><%=Util.subData(String.valueOf(ht.get("DRIVER_NM")), 3)%></span></td>
					<td width='4%' align='center'><%=ht.get("USER_NM1")%></td>
					<td  width='11%' align='center'><%=AddUtil.ChangeDate3(String.valueOf(ht.get("CONF_DT")))%></td>															
				</tr>
