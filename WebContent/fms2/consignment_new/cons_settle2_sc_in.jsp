		...
		<td class='line'>
			<table border="0" cellspacing="1" cellpadding="0" width='100%'>
				<tr>
				    ...
				    <td width='7%' class='title'>운전자</td>				  				  
				    <td width='14%' class='title'>출발요청일시</td>										
...
<%		for(int i = 0 ; i < vt_size ; i++){
			...
			%>
				<tr>
					...
					<td  width='10%' align='center'><a href="javascript:parent.view_cons('<%=ht.get("CONS_NO")%>')" onMouseOver="window.status=''; return true"><%=ht.get("CONS_NO")%>-<%=ht.get("SEQ")%></a></td>
					<td  width='10%' align='center'><span title='<%=ht.get("OFF_NM")%>'><%=Util.subData(String.valueOf(ht.get("OFF_NM")), 8)%></span></td>
					<td  width='5%' align='center'><%=ht.get("CONS_ST_NM")%></td>					
					<td  width='14%' align='center'><%=car_no%></td>
					<td  width='10%' align='center'><span title='<%=ht.get("CAR_NM")%>'><%=Util.subData(String.valueOf(ht.get("CAR_NM")), 8)%></span></td>															
					<td  width='10%' align='center'><%=ht.get("CONS_CAU_NM")%></td>
					<td  width='7%' align='center'><span title='<%=ht.get("DRIVER_NM")%>'><%=Util.subData(String.valueOf(ht.get("DRIVER_NM")), 3)%></span></td>
					<td  width='14%' align='center'><%=AddUtil.ChangeDate3(String.valueOf(ht.get("FROM_REQ_DT")))%></td>					
