...
<%
	...
	//Vector vt = cs_db.getConsignmentMngList(s_kd, t_wd, gubun1, st_dt, end_dt, "3"); --지급까지 끝난걸 완료로 
	Vector vt = cs_db.getConsignmentMngList(s_kd, t_wd, gubun1, st_dt, end_dt, "8");
	int vt_size = vt.size();
					...
					<div style="width: 500px;">
						<table class="inner_top_table left_fix" style="height: 80px;">
							<tr>
						       ...
						       <td width='22%' class='title title_border'>탁송번호</td>				  
							   <td width='22%' class='title title_border'>탁송업체</td>
						       <td width="10%" class='title title_border'>구분</td>
						       <td width="24%" class='title title_border'>차량번호</td>		
							</tr>
						</table>
					</div>
				</td>
			 ...
			 <%		for(int i = 0 ; i < vt_size ; i++){
						...
						%>
							<tr>
								...
								<td  width='22%' class='center content_border'><a href="javascript:parent.view_cons('<%=ht.get("STEP")%>','<%=ht.get("CONS_NO")%>','<%=ht.get("USER_ID1")%>')" onMouseOver="window.status=''; return true"><%=ht.get("CONS_NO")%>-<%=ht.get("SEQ")%></a></td>					
								...
								<td  width='24%' class='center content_border'><%=car_no%></td>
							</tr>
					<%		}%>
							<tr> 
						        <td  class='title content_border' colspan='5'>&nbsp;</td>
						    </tr>
				 <%} else  {%>  
						   	<tr>
