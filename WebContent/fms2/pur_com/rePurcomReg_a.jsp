...
<%
	...
	if(count == 0){
		...
		String cont 	= "[ "+rent_l_cd+" "+client.getFirm_nm()+" ] 신차취소현황에서 계약연동되어 계출관리 계약번호("+cpd_bean.getCom_con_no()+")가 등록되었습니다.";
		...
		if(cpd_bean.getDlv_st().equals("2")){
			...
			cont 	= "신차취소현황에서 계약연동 [ "+rent_l_cd+" "+client.getFirm_nm()+" ] ("+cpd_bean.getCom_con_no()+") 출고배정 ("+cpd_bean.getDlv_con_dt()+") ";
		}else{
			if(cpd_bean.getDlv_st().equals("1") && !cpd_bean.getDlv_est_dt().equals("")){
				sub 	= "계출관리 계약번호등록 & 자동차납품 출고예정";
				cont 	= "신차취소현황에서 계약연동 [ "+rent_l_cd+" "+client.getFirm_nm()+" ] ("+cpd_bean.getCom_con_no()+") 출고예정 ("+cpd_bean.getDlv_est_dt()+") ";
			}
		}
