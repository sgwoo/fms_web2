...
response.setHeader("Content-Disposition", "inline; filename=pur_con_sc_in_excel.xls");
%>
...
<%
	...
	Vector vt = umd.getPurComConList(s_kd, t_wd, sort, gubun1, gubun2, gubun3, gubun4, st_dt, end_dt);
	int vt_size = vt.size();
%>
...
<body>
<% int col_cnt = 22;%>
<table border="0" cellspacing="0" cellpadding="0" width='1965'>
    <tr>
	  <td colspan='<%=col_cnt%>' align='center' style="font-size : 20pt;" height="50">(주)아마존카 자동차납품 배정현황 리스트 (<%=AddUtil.getDate()%>)</td>
	</tr>
    <tr>
...
<table border="1" cellspacing="0" cellpadding="0" width='1965'>
                <tr> 
            ...
            <%	for(int i = 0 ; i < vt_size ; i++){
        	    ...
        	    <td width='200' align='center'><%=ht.get("OFF_NM")%><br><%=ht.get("DRIVER_NM")%>(<%=ht.get("DRIVER_SSN")%>)<br><%=ht.get("DRIVER_M_TEL")%></td>        		         		    	        		    
                </tr>
<%		}	%>
</table>
<script language='javascript'>
