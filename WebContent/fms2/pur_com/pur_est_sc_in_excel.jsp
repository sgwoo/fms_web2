...
response.setHeader("Content-Disposition", "inline; filename=pur_est_sc_in_excel.xls");
%>
...
<%
	...
	Vector vt = umd.getPurComEstDtList(s_kd, t_wd, sort, gubun1, gubun2, gubun3, gubun4, st_dt, end_dt);
	int vt_size = vt.size();
%>
...
<body>
<% int col_cnt = 25;%>
<table border="0" cellspacing="0" cellpadding="0" width='2215'>
    <tr>
	  <td colspan='<%=col_cnt%>' align='center' style="font-size : 20pt;" height="50">(주)아마존카 자동차납품 예정현황 리스트 (<%=AddUtil.getDate()%>)</td>
	</tr>
    <tr>
...
<table border="1" cellspacing="0" cellpadding="0" width='2215'>
                <tr> 
                    ...
                    <td width="80" class='title'>배정예정일</td>       		
        	    <td width="100" class='title'>변경구분</td>
                    <td width='120' class='title'>변경등록</td>
        	    ...
        	    <td width='100' class='title'>아마존카계약번호</td>
        	</tr>
            <%	for(int i = 0 ; i < vt_size ; i++){
                ...
                <tr> 
                    ...
                    <td width='75' align='center'><%=AddUtil.ChangeDate3(String.valueOf(ht.get("PUR_REQ_DT")))%></td>                        	    
                    ...
                    <td width='100' align='center'><%=ht.get("CNG_CONT")%></td>
                    <td width='120' align='center'><%=ht.get("CNG_REG_DT")%></td>
                    <td width='120' align='center'><%=ht.get("CNG_APP_DT")%>
                    	<%if(!String.valueOf(ht.get("CNG_REG_DT")).equals("") && String.valueOf(ht.get("CNG_APP_DT")).equals("")){%>
                    		<%if(String.valueOf(ht.get("CNG_CONT")).equals("신차취소현황으로 보내기") && String.valueOf(ht.get("SUC_YN")).equals("D")){%>
                    		<%}else{%>
        	    ...
        	    <td width='80' align='center'><%=ht.get("PURC_GU")%></td>
        	    <td width='80' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("R_CAR_C_AMT")))%></td>
        	    <td width='80' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("R_CAR_F_AMT")))%></td>        	    
        	    <td width='80' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("R_DC_AMT2")))%></td>        	    
        	    <td width='80' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("R_ADD_DC_AMT")))%></td>        	    
