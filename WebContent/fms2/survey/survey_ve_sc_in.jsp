...
<%
	...
	Vector vt = p_db.getRentCondAll(dt, ref_dt1, ref_dt2, gubun2, sort, s_kd, t_wd);
	int vt_size = vt.size();
%>
<html>
<head>
...
<%	if(vt_size > 0){%>
              ...
              <% for (int i = 0 ; i < vt_size ; i++){
		        ...
		        <%}%>                                                 
                            ...
                            <td align="center" width=50%><a href="javascript:parent.view_cont('<%=ht.get("RENT_MNG_ID")%>','<%=ht.get("RENT_L_CD")%>')"><%=ht.get("RENT_L_CD")%></a></td>
                          </tr>
              <%}%>
                        </table>
                   </td>            		            		
