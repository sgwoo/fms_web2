...
<%
	...
	Vector polls = p_db.getSurvey_One_view(poll_id, poll_seq);
...
<input type='hidden' name='poll_seq' value=''>
<input type='hidden' name='cmd' value=''>
<div class="navigation">
	<span class=style1>콜센터 ></span><span class=style5>콜항목 편집</span>
</div>
			...
			<table id="addTb2" border="0" cellspacing="1" cellpadding="0" width=100%>
				<thead>
					<tr>
						...
						<th align="center" height="35">문항 관리</th>
						<th></th>
					</tr>
				</thead>
				<%if(poll_size > 0){
				...
				<tbody>
					<tr class="tbltr"> 
						<td align="center"  width="5%">
							<%if(poll.get("A_SEQ").equals("0")){%>Q<%=poll.get("POLL_SEQ")%><%}else{%>A<%=poll.get("A_SEQ")%><%}%>
						</td>
						<td align="">
							<input type="text" id="answer" name="<%if(poll.get("A_SEQ").equals("0")){%>question<%}else{%>answer<%}%>" value="<%=poll.get("CONTENT")%>" style="width:95%;" />
						</td>	
						<td align="center" width="15%">
							<%if(!poll.get("A_SEQ").equals("0")){%>
							<input type='checkbox' name='answer_rem' id='answer_rem' value="Y" <%if(!poll.get("CHK").equals("")){%> checked <%}%> />별도설명허용
...
<input type="button" value="수정" class="button button4" onclick="update('<%=poll_id%>','<%=poll_seq%>')"/>
<input type="button" value="삭제" class="button button4" onclick="one_delete('<%=poll_id%>','<%=poll_seq%>')"/>
<input type="button" value="닫기" class="button button4" onclick="closeWin()"/>
