...
function ChangeDT(arg)
{
	...
	function cng_search(){
		var fm = document.RentCondSearchForm;
		if(fm.s_kd.options[fm.s_kd.selectedIndex].value == ""){ //전체
			fm.t_wd.style.display 	= 'none';
			fm.t_wd.value = "";
		}else{
			fm.t_wd.style.display 	= '';
			fm.t_wd.value = "";
		}
	}
//-->
...
<body>
<form action="./call_car_accident_sc.jsp" name="RentCondSearchForm" method="POST" target="c_foot">
<div class="navigation">
	<span class=style1>콜센터 ></span><span class=style5>사고콜등록현황</span>
</div>
...
<select name="s_kd" class="select" style="width:150px;" onchange="javascript:cng_search();">
	<option value=""  <%if(s_kd.equals("")){%> selected <%}%>>전체</option>
	<option value="1" <%if(s_kd.equals("1")){%> selected <%}%>>상호</option>
	<option value="2" <%if(s_kd.equals("2")){%> selected <%}%>>등록자</option>
</select>
&nbsp;
<input type="text" name="t_wd" size="21" value="<%=t_wd%>" class="text" onKeyDown="javasript:EnterDown()" style="display: none;">
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
