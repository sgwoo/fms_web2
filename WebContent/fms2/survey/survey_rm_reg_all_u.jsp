...
//			fm.target='nodisplay';
			fm.cmd.value = cmd;
			fm.target='c_foot';
			fm.action='survey_rm_reg_all_u_a.jsp';
			fm.submit();
		}
	}
...
<script>
//리스트 가기	
function list_go(st, from_page){
	var fm = document.form1;
	if(st== 'i' || from_page == 'rent_cond_rm_frame.jsp'){
		...
		parent.parent.location = "call_cond_rm_frame.jsp?auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&cont_st=<%=cont_st%>";
	}
}	
</script>
</head>
...
<form action='survey_rm_reg_all_u_a.jsp' name="form1" method='post' id="my_radio_box">
    <!--구분자-->
        ...
        <td class= colspan="3">
			...
			<%	if(vt_size > 0){%>
				<tr>
					...
					<td align="right"><input type="button" class="button button4" value="목록" onclick="list_go('u','<%=from_page%>');"/></td>
				</tr>
				<%}else{%>
				<tr>
					<td class=""><input type="button" class="button button4" value="저장" onclick="save('i');"/>&nbsp;<input type="button" class="button button4" value="설문거부" onclick="save('no');"/></td>
					<td align="right"><input type="button" class="button button4" value="목록" onclick="list_go('i','<%=from_page%>');"/></td>
				</tr>
				<%}%>
			</table>
		</td>
        ...
        <td class=line colspan="3">
			...
			%>
				...
				<tr> 
					...
					&nbsp;<label for="answer<%=i%><%=answ.get("A_SEQ")%>"><input type="radio" name="answer<%=i+1%>" id="answer<%=i%><%=answ.get("A_SEQ")%>" value="<%=answ.get("A_SEQ")%>" onclick="disable_textarea('answer_rem<%=i+1%><%=j+1%>')" ><%=answ.get("CONTENT")%></label><br/>
