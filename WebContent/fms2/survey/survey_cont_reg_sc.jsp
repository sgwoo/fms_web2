...
<frameset cols="50%, *" border=1> 
  <frame name="c_body" src="survey_cont_reg_hi_u.jsp?type=<%=type%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>&m_id=<%=m_id%>&l_cd=<%=l_cd%>&use_yn=<%=use_yn%>&poll_st=<%=poll_st%>" marginwidth=10 marginheight=10 scrolling='auto' >
  <frame name="c_foot" src="survey_cont_reg_all_u.jsp?type=<%=type%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>&m_id=<%=m_id%>&l_cd=<%=l_cd%>&poll_st=<%=poll_st%>&rent_dt=<%=rent_dt%>" marginwidth=10 marginheight=10 scrolling="auto">
