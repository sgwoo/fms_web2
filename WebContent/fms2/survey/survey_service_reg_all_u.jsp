...
<%
	...
	if(poll_st.equals("")){
		 vt		= p_db.getServicePollAll(m_id, l_cd, c_id, s_id);
 	}else{
		
		 vt		= p_db.getSurveyServicePollAll(m_id, l_cd, c_id, s_id);
	}
...
//			fm.target='nodisplay';
			fm.cmd.value = cmd;
			fm.target='c_foot';
			fm.action='survey_service_reg_all_u_a.jsp';
			fm.submit();
		}
	}
...
function list_go(st){
	var fm = document.form1;
		...
		parent.parent.location = "call_car_service_frame.jsp?auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&cont_st=<%=cont_st%>";
	}
}	
...
<body leftmargin="15">
    ...
    <form action='survey_service_reg_all_u_a.jsp' name="form1" method='post'>
    <!--구분자-->
			...
			<table border="0" cellspacing="1" cellpadding='0' width=100%>
				...
				<tr>
					<td class="">
						...
						&nbsp;<input type="button" class="button button4" value="계약확인요청" onclick="open_msg('service');"/>
					</td>
