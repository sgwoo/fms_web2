...
<% 
	...
	//1. 콜센터 정보-----------------------------------------------------------------------------------------------
    //신규 or 수정
    count  = p_db.checkServiceCall(m_id, l_cd, c_id, s_id);
    
    if (count > 0 ) {    
    	//if(!p_db.deleteServiceCall(m_id, l_cd, c_id, s_id)) flag1+=1;
    }	
	...
	Vector polls = p_db.getSurvey_Play_Q(poll_st, serv_dt);
	...
	for (int i = 0 ; i < poll_size ; i++){
		...
		base.setServ_id		(request.getParameter("s_id"));
		base.setP_gubun		(request.getParameter("p_gubun"));
		...
		if(cmd.equals("i")){
			...
			if(!p_db.insertServiceCall(base)) flag1+=1;
		} else if (cmd.equals("u")) {
			base.setAnswer		(a_value);
			base.setAnswer_rem	(request.getParameter("answer_rem"+ (i+1) + AddUtil.parseDigit(a_value) ));	 
			if (!p_db.updateServiceCall(base)) flag1+=1;
		...
		if(cmd.equals("i")){			
			if (base.getPoll_id() == poll_size) {
				if (base.getAnswer().equals("1")) {
					...
					String cont 	= "[대표와의 직접통화] 계약번호  "+l_cd+" 의 고객이 정비콜과 관련하여 대표와의 직접통화를 바랍니다.";
					String url = "/fms2/survey/call_car_service_frame.jsp";
					
					String xml_data = "";
					String xml_data2 = "";
					
					//if (poll_st.equals("순회정비")) {
					//고객지원팀장
					xml_data = "<COOLMSG>"+
										"<ALERTMSG>"+
						  				"    <BACKIMG>4</BACKIMG>"+
...
<%} else {%>
		alert("저장되었습니다");
		<%if(cmd.equals("no")){%>
		parent.parent.location = "car_service_s_frame.jsp?auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&cont_st=<%=cont_st%>";
		<%}else{%>
		parent.parent.location='survey_service_reg_frame.jsp?mode=2&m_id=<%=m_id%>&l_cd=<%=l_cd%>&c_id=<%=c_id%>&s_id=<%=s_id%>&p_gubun=<%=p_gubun%>&auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&use_yn=<%=use_yn%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>&poll_st=<%=poll_st%>&serv_dt=<%=serv_dt%>';
		<%}%>
<%}%>
</script>
</body>
