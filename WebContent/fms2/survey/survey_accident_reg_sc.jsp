...
<frameset cols="50%, *" border=1> 
  <frame name="c_body" src="survey_accident_reg_hi_u.jsp?dt=<%=dt%>&gubun2=<%=gubun2%>&ref_dt1=<%=ref_dt1%>&ref_dt2=<%=ref_dt2%>&type=<%=type%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>&m_id=<%=m_id%>&l_cd=<%=l_cd%>&use_yn=<%=use_yn%>&c_id=<%=c_id%>&accid_id=<%=accid_id%>&accid_st=<%=accid_st%>&p_gubun=<%=p_gubun%>&poll_st=<%=poll_st%>" marginwidth=10 marginheight=10 scrolling='auto' noresize>
  <frame name="c_foot" src="survey_accident_reg_all_u.jsp?dt=<%=dt%>&gubun2=<%=gubun2%>&ref_dt1=<%=ref_dt1%>&ref_dt2=<%=ref_dt2%>&type=<%=type%>&auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&s_kd=<%=s_kd%>&brch_id=<%=brch_id%>&s_bank=<%=s_bank%>&t_wd=<%=t_wd%>&cont_st=<%=cont_st%>&b_lst=<%=b_lst%>&m_id=<%=m_id%>&l_cd=<%=l_cd%>&c_id=<%=c_id%>&accid_id=<%=accid_id%>&accid_st=<%=accid_st%>&p_gubun=<%=p_gubun%>&poll_st=<%=poll_st%>&accid_dt=<%=accid_dt%>" marginwidth=10 marginheight=10 scrolling="auto">
