...
<!--
	...
	function toExcel(){	
		...
		var popup = window.open("about:blank", "Stat", "left=50, top=50, width=1050, height=800, scrollbars=yes, status=yes");	
		document.form1.action="master_car_excel_print.jsp";
		document.form1.method="post";
		document.form1.target="Stat";
		document.form1.submit();
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
	...
	<tr>
		<td align='right' width='100%'> 
			...
			<a href='javascript:toExcel()'>[엑셀]</a></a>
	...
	<tr>
		<td>
			<iframe src="master_car_sc_in.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&auth=<%=auth%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>" name="i_no" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 ></iframe>
		</td>
	</tr>
	<tr>
	</tr>
