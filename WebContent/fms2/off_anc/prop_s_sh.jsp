...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
			...
			<table border="0" cellspacing="1" cellpadding='0' width=100%>
				<tr>
					...
					<td width=40%>&nbsp;
						...
						&nbsp;
						<input type='text' name='t_wd' size='50' class='text' value='<%=t_wd%>' onKeyDown='javascript:enter()' style='IME-MODE: active'>
					</td>
				</tr>
				<tr>
					<td class=title width=10%>채택</td>
					<td>&nbsp;
						...
						<input type='radio' name="gubun2" value='6' <%if(gubun2.equals("6")){%>checked<%}%>>정보제공&nbsp;
						<input type='radio' name="gubun2" value='2' <%if(gubun2.equals("2")){%>checked<%}%>>불채택				
					</td>
					
					<td class=title width=10%>직군분류 
					</td>
					<td>&nbsp;
						<INPUT TYPE="radio" NAME="gubun3" value='0'  <%if(gubun3.equals("")){%>checked<%}%>>전체직원&nbsp;&nbsp;
						<INPUT TYPE="radio" NAME="gubun3" value='1'  <%if(gubun3.equals("1")){%>checked<%}%>>재직자&nbsp;&nbsp;
						<INPUT TYPE="radio" NAME="gubun3" value='2'  <%if(gubun3.equals("2")){%>checked<%}%>>퇴사자
					</td>
						
				</tr>
				<tr>					
					<td class=title width=10%>상태 
					</td>
					<td>&nbsp;
						<INPUT TYPE="radio" NAME="gubun4" value=''  <%if(gubun4.equals("")){%>checked<%}%>>전체&nbsp;&nbsp;
						<INPUT TYPE="radio" NAME="gubun4" value='1'  <%if(gubun4.equals("1")){%>checked<%}%>>완료&nbsp;&nbsp;
						<INPUT TYPE="radio" NAME="gubun4" value='4'  <%if(gubun4.equals("4")){%>checked<%}%>>이관&nbsp;&nbsp;
						<INPUT TYPE="radio" NAME="gubun4" value='3'  <%if(gubun4.equals("3")){%>checked<%}%>>처리&nbsp;&nbsp;
						<INPUT TYPE="radio" NAME="gubun4" value='2'  <%if(gubun4.equals("2")){%>checked<%}%>>보류
					</td>
	...
	<tr>
        <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>심사결과</span>
		&nbsp;
		  <select name="eval_dt">
			    <option value="">선택</option>
<%		for(int i = 0 ; i < vt_size2 ; i++){
			Hashtable ht = (Hashtable)vt2.elementAt(i);%>
                <option value="<%=ht.get("EVAL_DT")%>"><%=AddUtil.ChangeDate2(String.valueOf(ht.get("EVAL_DT")))%></option>
<%		}%>
        </select>	       
		&nbsp;<a href="javascript:popup_search();"><img src=/acar/images/center/button_search.gif border=0 align=absmiddle></a></td>
	</tr>
</table>
</form>
</body>
