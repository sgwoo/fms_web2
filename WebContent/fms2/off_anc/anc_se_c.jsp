...
<form action="./anc_se_u.jsp" name="AncDispForm" method="post">
    ...
    <tr>
	    <td align='right'>
	       ...
	       <%	if(acar_id.equals(a_bean.getUser_id()) || nm_db.getWorkAuthUser("전산팀",ck_acar_id) || user_id.equals("000096")){%>	
	    	<%    if ( user_id.equals("000029") || Integer.parseInt(AddUtil.getReplace_dt(a_bean.getExp_dt())) >=  Integer.parseInt(AddUtil.getReplace_dt(AddUtil.getDate()))) {%>
	...
	<td class='line'>
		 <table border="0" cellspacing="1" cellpadding="0" width="100%">
			...
			<tr>
				...
				<td colspan='3'>&nbsp;&nbsp;<%=car_comp_nm%>&nbsp;<%=car_nm%>&nbsp;<%if(a_bean.getEnd_st().equals("1")){%>차종단산<%}else if(a_bean.getEnd_st().equals("2")){%>색상단산<%}%></td>
			</tr>
			<%} %>			
			<tr>
			...
			<% content = AddUtil.replace(content,"<br/>","");  %>
			...
			<%if(!a_bean.getKeywords().equals("")){%>
			<tr>
				<td class="title" width=15%>키워드</td>
				<td colspan='3'>&nbsp;&nbsp;<%=a_bean.getKeywords()%></td>
			</tr>
			<%} %>
			<%if(attach_vt.size() > 0){%>
				...
				<tr> 
					 ...
					 <%}else{%>
					 <a href="https://fms3.amazoncar.co.kr/fms2/attach/download.jsp?SEQ=<%=ht.get("SEQ")%>" download><%=ht.get("FILE_NAME")%></a>
					 <%}%>
