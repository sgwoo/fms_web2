...
<%@ page import="java.net.*, java.util.*, acar.util.*, acar.off_anc.*, acar.user_mng.*" %>
...
<%
	...
	gubun_nm = URLEncoder.encode(gubun_nm, "UTF-8");
	int cnt = 2; //sc 출력라인수
	int height = AddUtil.parseInt(s_height)-emp_top_height-sh_height-(cnt*sc_line_height)-125;//현황 라인수만큼 제한 아이프레임 사이즈
	
	String valus = 	"?auth_rw="+auth_rw+"&user_id="+user_id+"&br_id="+br_id+
					"&gubun1="+gubun1+"&gubun_nm="+java.net.URLEncoder.encode(gubun_nm,"utf-8")+"&gubun="+gubun+
				   	"&sh_height="+height+"";
...
<!--
	function AncReg(){
		var SUBWIN="./anc_se_i.jsp?ck_acar_id=<%=ck_acar_id%>&from_page=/fms2/off_anc/upgrade_frame.jsp";	
		window.open(SUBWIN, "AncReg", "left=10, top=10, width=1024, height=750, scrollbars=no");
	}

	function AncDisp(bbs_id){
		var SUBWIN="./anc_se_c.jsp?ck_acar_id=<%=ck_acar_id%>&from_page=/fms2/off_anc/upgrade_frame.jsp&bbs_id=" + bbs_id;	
  ...
  <tr>
		<td colspan="2">
			<table border="0" cellspacing="0" cellpadding="0" width=100%>
				<tr>
					<td>
						<iframe src="upgrade_sc_in.jsp<%=valus%>&ck_acar_id=<%=ck_acar_id%>&s_width=<%=s_width%>&s_height=<%=s_height%>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
						</iframe>
					</td>
				</tr>
			</table>
