...
<%
	...
	String p_after 	= request.getParameter("p_after")==null?"":request.getParameter("p_after"); //사후제안여부
	...
	String title1 =  "[IT마케팅팀개발업무- 이관]"; 
	
%>
<%
	if(cmd.equals("i")){				//등록
		p_bean.setReg_id	(user_id);
		p_bean.setTitle		(p_after+title);  //사후제안 + 제목으로 
		...
		//이관인 경우 - fms업무협조에 등록
		if (prop_step.equals("9") ) {
		
		   int f_cnt = 0;
		   
		   f_cnt = cp_db.getPropCnt(prop_id);
		   ...
		   if (f_cnt  < 1  ) {
				...
				seq = cp_db.getCooperationSeqNext();
		   	   ...
		   	   cp_bean.setSeq		(Util.parseInt(seq));
				cp_bean.setIn_id		(user_id);
				cp_bean.setTitle		(title1+title);
				cp_bean.setContent		(AddUtil.ChangeDate2(reg_dt) + "  [제안건]  \r\r  [문제점] \r " + content1 + "\r\r [개선안]\r "+ content2);
				cp_bean.setSub_id		("");
				cp_bean.setOut_id		("000237");
				cp_bean.setReq_st		("1");
				cp_bean.setCom_id		(reg_id);

				cp_bean.setProp_id		(prop_id);
		   	   count1 = cp_db.insertCooperationIt(cp_bean);	   
		   }
