...
dhtmlXGridObject.prototype.toPDF = function(q, k, l, a, g, c) {
	function r(h) {
		...
		for (e = 2; e < b.hdr.rows.length; e++) {
			for (var c = 0, g = b.hdr.rows[e], j = "",
				f = 0; f < b._cCount; f++)
				if (b._srClmn && !b._srClmn[f] || b._hrrar[f] && (!b._fake || f >= b._fake.hdrLabels.length)) c++;
				else {
					var k = i[e][f],
						m = k[0] && k[0] > 1 ? ' colspan="' + k[0] + '" ' : "";
	...
	function m() {
		var h = [];
		if (g)
			for (var i = 0; i < g.length; i++) h.push(w(b.getRowIndex(g[i])));
		else
			for (i = 0; i < b.getRowsNum(); i++) h.push(w(i));
		return h.join("\n")
	}
		...
		for (var i = 1; i < b.ftr.rows.length; i++) {
			...
			for (var e = b.ftr.rows[i], f = 0; f < b._cCount; f++)
				if (!b._srClmn || b._srClmn[f]){
					var left = b._fake && !(f >= b._fake.hdrLabels.length);
					var leftrow = left ? b._fake.ftr.rows[i] : b.ftr.rows[i];
					if (!b._hrrar[f] || left) {
						for (var a = 0; a < leftrow.cells.length; a++) {
							var d = "",
								c = "";
							if (leftrow.cells[a]._cellIndexS == f) {
								d = _isIE ? leftrow.cells[a].innerText : leftrow.cells[a].textContent;
								d = d.replace(/[ \n\r\t\xA0]+/, " ");
								leftrow.cells[a].colSpan && leftrow.cells[a].colSpan != 1 && (c = " colspan='" + leftrow.cells[a].colSpan + "' ");
								leftrow.cells[a].rowSpan && leftrow.cells[a].rowSpan != 1 && (c = " rowspan='" + leftrow.cells[a].rowSpan + "' ");
								break
							}
						}
			...
			if ((!b._srClmn || b._srClmn[g]) && (!b._hrrar[g] || b._fake && g < b._fake.hdrLabels.length)) {
				...
				if (x) {
					var k = h(d.cell, "color"),
						...
						o = h(d.cell, "text-align") || h(d.cell,
							"textAlign"),
		...
		if (!this._srClmn || this._srClmn[a])
			if (!this._hrrar[a] || this._fake && !(a >= this._fake.hdrLabels.length)) {
				...
				g == "cus" && (g = this._customSorts[a].toString(), g = g.replace(/function[\ ]*/, "").replace(/\([^\f]*/,
					""));
				var c = q[1][a],
					r = (c[1] && c[1] > 1 ? ' rowspan="' + c[1] + '" ' : "") + (c[0] && c[0] > 1 ? ' colspan="' + c[0] + '" ' : "");
				...
				var m = this.combos[a] ?
					this.getCombo(a) : null;
				if (m)
					for (var o = 0; o < m.keys.length; o++) l += "<option value='" + k(m.keys[o]) + "'><![CDATA[" + m.values[o] + "]]\></option>";
				l += "</column>"
			}
	return l += "</head>"
};
