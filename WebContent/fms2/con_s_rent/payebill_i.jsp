...
<%@ page import="java.util.*, acar.util.*, acar.cont.*, acar.ext.*, acar.bill_mng.*, acar.client.*, tax.*, acar.res_search.*"%>
...
<body>
<form name='form1' action='/fms2/con_s_rent/payebill_i_a.jsp' method='post'>
        ...
        <td>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>재무회계 > 수금관리 > 기본식정비대차 관리 > <span class=style5>입금표발행</span></span></td>
			 		...
			 		<td class='line'>
			 			<table border="0" cellspacing="1" cellpadding="0" width=100%>
							...
							<tr>
								...
								<td>&nbsp;<input type='text' name='grt_s_amt' size='10' value='<%=Util.parseDecimal(sr_bean.getRent_s_amt())%>' class='whitenum' readonly>원</td>
								<td class='title'>부가세</td>
								<td>&nbsp;<input type='text' name='grt_v_amt' size='10' value='<%=Util.parseDecimal(sr_bean.getRent_v_amt())%>' class='whitenum' readonly>원</td>
							</tr>
							<tr>
							  <td class='title'>입금예정일</td>
							  <td>&nbsp;<%=sr_bean.getEst_dt()%></td>
							  <td class='title'>합계</td>
							  <td>&nbsp;<input type='text' name='grt_amt' size='10' value='<%=Util.parseDecimal(sr_bean.getRent_s_amt()+sr_bean.getRent_v_amt())%>' class='whitenum' readonly>원</td>
						  </tr>
							<tr>
							  <td class='title'>입금일자</td>
							  <td>
						      &nbsp;<input type='text' name='pay_dt' size='11' value='<%=sr_bean.getPay_dt()%>' class='whitetext' onBlur='javascript:this.value=ChangeDate(this.value);'></td>
							  <td class='title'>실입금액</td>
							  <td>
						      &nbsp;<input type='text' name='pay_amt' size='10' maxlength='10' value='<%=Util.parseDecimal(sr_bean.getPay_amt())%>' class='whitenum' onBlur='javascript:this.value=parseDecimal(this.value)'>원</td>
						  </tr>
						</table>
					</td>
				</tr>				
					...
					<td colspan='6' class='line'>
						<table border="0" cellspacing="1" cellpadding="0" width=100%>
							...
							<tr>
							  ...
							  <td>&nbsp;
								<textarea name="bigo" cols="45" class="text" rows="2">기본식정비대차 <%=reserv.get("CAR_NO")%></textarea>
                                </td>
						  </tr>
