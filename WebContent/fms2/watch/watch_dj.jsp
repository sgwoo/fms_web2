...
function SchReg(week, year, mon, day)
{
	...
	var SUBWIN="./watch_member_i.jsp?watch_type=4" + url;	//원본
		                ...
		                <table width=100% cellspacing=1 cellpadding=3 border=0>
			                <tr align="center" bgcolor=b0baec>
				                <td height=3 class=title1>
				                	<%if(myid.equals("000052") || nm_db.getWorkAuthUser("전산팀",myid)){%><input type="checkbox" name="ch_all" value="Y" onclick="javascript:AllSelect();">전체<%}%>
				                	</td>
			                ...
			                <tr align=center bgcolor=<%=tr_color%>>
								...
								<%if(myid.equals("000052") || nm_db.getWorkAuthUser("전산팀",myid)){%>							
								...
								<td align='center' width=20%> <!-- 광주 내근당직자 -->
								<% if( vt_size >0){	            
									for(int j=0; j<  vt_size ; j++){
										Hashtable ht = (Hashtable)vt.elementAt(j);
				                ...
				                <% if( vt_size >0){	            
									...
									<%}else {%>
										<%=ht.get("MEMBER_NM")%>
												...
												<%}%>&nbsp;&nbsp;&nbsp; 
									<%if(myid.equals("000052") || nm_db.getWorkAuthUser("전산팀",myid)){%><a href='javascript:SchDel( "<%=ht.get("MEMBER_ID")%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>")' border='0' class=copy><img src="/acar/images/center/button_in_cancel.gif" border="0" align="absmiddle"></a><%}%>
