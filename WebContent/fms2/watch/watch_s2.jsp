...
<%
	...
	Vector vt = wc_db.WatchScheAll(thisyear, thismonth, "5");
	int vt_size = vt.size();
...
function SchReg(week, year, mon, day, is_registered_watch)
{
	...
	var SUBWIN="./watch_member_i.jsp?watch_type=5" + url;	//원본
...
function WatchReg(id, year, mon, day)
{
	...
	var SUBWIN="./watch_in.jsp?cm=s2" + url;	
...
<form name='thefrm' method='post' action='./watch_s2.jsp'>
...
<table width=100% cellspacing=0 cellpadding=0 border=0 height=100% bgcolor=#FFFFFF>
		            ...
		            <td bgcolor=#7A9494 class=line>  
		                <table width=100% cellspacing=1 cellpadding=3 border=0>
			                <tr align="center" bgcolor=b0baec>
			                    ...
			                    <td height=3 class=title1>강남지점 주간당직자</td>
								<td height=3 class=title1>인천지점 주간당직자</td>
								<td height=3 class=title1>수원지점 주간당직자</td>
								<td height=3 class=title1>수도권남부 야간당직자</td>
			                </tr>
                			  <%	//요일별 색상
			                ...
			                <tr align=center bgcolor=<%=tr_color%>>
								...
								<td align='center' width=20%> <!-- 강남 당직자 -->
								<% if( vt_size >0){	            
								...
								<%if(day_of_week != 1 && day_of_week != 7){%>
									<a href='javascript:SchReg2("<%=day_of_week%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>", "5","5")'><img src=/acar/images/center/button_in_reg.gif border=0 align=absmiddle></a>			
								<%}%>
								<%}else{%>
									<%=ht.get("MEMBER_NM5")%>&nbsp;
									<a href='javascript:WatchReg3("<%=ht.get("MEMBER_ID5")%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>","5","5")' border='0' class=copy>
													<font color="blue"><b>[일지]</b></font></a>
									<a href='javascript:SchReg2("<%=day_of_week%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>", "5","5")'><img src=/acar/images/center/button_in_bg.gif border=0 align=absmiddle></a>
								<%}%>
								...
								<td align='center' width=20%> <!-- 인천 내근당직자 -->
								<% if( vt_size >0){	            
									for(int j=0; j<  vt_size ; j++){
										Hashtable ht = (Hashtable)vt.elementAt(j);
								...
								<%if(day_of_week != 1 && day_of_week != 7){%>
									<a href='javascript:SchReg2("<%=day_of_week%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>", "3","8")'><img src=/acar/images/center/button_in_reg.gif border=0 align=absmiddle></a>			
								<%}%>
								<%}else{%>
									<%=ht.get("MEMBER_NM3")%><!--(내선:<%=ht.get("IN_TEL3")%>)-->&nbsp;
									<a href='javascript:WatchReg3("<%=ht.get("MEMBER_ID3")%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>","3","5")' border='0' class=copy>
													<font color="blue"><b>[일지]</b></font></a>
									<a href='javascript:SchReg2("<%=day_of_week%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>", "3","8")'><img src=/acar/images/center/button_in_bg.gif border=0 align=absmiddle></a>
								<%}%>
								...
								<td align='center' width=20%> <!-- 수원 내근당직자 -->
								<% if( vt_size >0){	            
									for(int j=0; j<  vt_size ; j++){
										Hashtable ht = (Hashtable)vt.elementAt(j);
								...
								<%if(day_of_week != 1 && day_of_week != 7){%>
									<a href='javascript:SchReg2("<%=day_of_week%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>", "4","9")'><img src=/acar/images/center/button_in_reg.gif border=0 align=absmiddle></a>			
								<%}%>
								<%}else{%>
									<%=ht.get("MEMBER_NM4")%>&nbsp;
									<a href='javascript:WatchReg3("<%=ht.get("MEMBER_ID4")%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>","4","5")' border='0' class=copy>
													<font color="blue"><b>[일지]</b></font></a>
									<a href='javascript:SchReg2("<%=day_of_week%>", "<%=ht.get("START_YEAR")%>", "<%=ht.get("START_MON")%>", "<%=ht.get("START_DAY")%>", "4","9")'><img src=/acar/images/center/button_in_bg.gif border=0 align=absmiddle></a>
								<%}%>
				                ...
				                <td align='center' width=40%> 	<!-- 야간당직자 -->			                            
  ...
  <tr height='30'>
    <td align="center">
	  <%if(nm_db.getWorkAuthUser("수도권당직관리",myid) || nm_db.getWorkAuthUser("전산팀",myid)){%>
