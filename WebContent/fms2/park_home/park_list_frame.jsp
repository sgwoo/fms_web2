...
<FRAMESET rows="<%=height%>, *" border=0>
	<FRAME SRC="park_list_sh.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&gubun1=<%=gubun1%>&st_year=<%=st_year%>&st_mon=<%=st_mon%>" name="c_body" frameborder=0  marginwidth="10" marginheight="10" scrolling="no" noresize>
	<FRAME SRC="park_list_sc.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&gubun1=<%=gubun1%>&st_year=<%=st_year%>&st_mon=<%=st_mon%>" name="c_foot" frameborder=0  marginwidth="10" marginheight="10" scrolling="no" noresize>
</FRAMESET>
<NOFRAMES>
<BODY>
