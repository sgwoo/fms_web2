...
//검색하기
	...
	//계약선택
	function Disp(m_id, l_cd, c_id, c_no, c_nm, c_st, mng_id,  mng_nm, t_dist, firm_nm, init_reg_dt) {
        ...
        <table border="0" cellspacing="1" width=640>
          ...
          <tr align="center"> 
            ...
            <td><a href="javascript:Disp('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("CAR_MNG_ID")%>',
            							 '<%=ht.get("CAR_NO")%>','<%=ht.get("CAR_NM")%>' ,'<%=ht.get("CAR_ST")%>' ,'<%=ht.get("MNG_ID")%>','<%=ht.get("MNG_NM")%>', '<%=ht.get("T_DIST")%>', '<%=ht.get("FIRM_NM")%>', '<%=ht.get("INIT_REG_DT")%>')" onMouseOver="window.status=''; return true"><%=ht.get("RENT_L_CD")%></a></td>
