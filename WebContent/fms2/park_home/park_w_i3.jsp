...
//	String c_id = request.getParameter("c_id")==null?"":request.getParameter("c_id");//자동차관리번호
	...
	String rent_l_cd = request.getParameter("rent_l_cd")==null?"":request.getParameter("rent_l_cd");//검색어
	
	Vector vt = pk_db.getParkSearch(rent_l_cd, "", "", "", s_kd, "");
	int s_size = vt.size();
	Hashtable ht = (Hashtable)vt.elementAt(0);
	
	String rent_mng_id = (String)ht.get("RENT_MNG_ID");
	rent_l_cd = (String)ht.get("RENT_L_CD");
	String car_mng_id = (String)ht.get("CAR_MNG_ID");
	car_no = (String)ht.get("CAR_NO");
	car_nm = (String)ht.get("CAR_NM");
	car_st = (String)ht.get("CAR_ST");
	mng_id = (String)ht.get("MNG_ID");
	users_comp = (String)ht.get("MNG_NM");
	user_m_tel = (String)ht.get("USER_M_TEL");
	String firm_nm = (String)ht.get("FIRM_NM");
...
$(document).ready(function(){
	...
	fm.car_st.value = '<%=car_st%>';
	fm.mng_id.value = '<%=mng_id%>';
	fm.users_comp.value = '<%=users_comp%>';
					...
					<td style="padding-left:5px;">
						<select name="own_type">
                        	...
                        	<%for(int i = 0 ; i < own_types_size ; i++){
								...
								%>
							<option value='<%=own_type.getNm()%>' <%if(own_type.getNm().equals("재리스/월렌트")){%>selected<%}%>><%= own_type.getNm()%></option>
							<%}}%>
                        </select>
					</td>
