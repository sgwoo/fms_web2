		...
		for(int i=0 ; i < scd_size ; i++){
			...
			if(pay_amt != 0){
				...
				//연체료
					...
					paid_amt = f_bean.getPaid_amt();
					if ( f_bean.getPaid_amt2() > 0) {
						paid_amt = f_bean.getPaid_amt2();
					}
					
					if (pay_amt == paid_amt ) {//과태료금액과 틀리면 납입불가 - 가수금 처리 
					...
					if (pay_amt != paid_amt ) {
						ht14.put("CD_ACCT",  "25700");   //과태료 미납 가수금 						
					} else {
				  ...
				  //입금표발행요청관련  	  
					if ( cng_ins_ms.getSaleebill_yn().equals("1")  ) {
						   ...
						   String SeqId1 = "";
							...
							sb_bean1.setItemName1	("면책금 - " + cr_bean.getCar_no());
