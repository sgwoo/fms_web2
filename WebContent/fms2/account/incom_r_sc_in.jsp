    ...
    <td class='line' width='500' id='td_title' style='position:relative;'> 
	  <table border="0" cellspacing="1" cellpadding="0" width='100%'>
        <tr> 
          ...
          <td width='100' class='title'>은행(카드사)</td>
          <td width='170' class='title'>계좌(카드)번호</td>
          <td width='100' class='title'>거래일자</td>
        </tr>
      </table>
	</td>
	<td class='line' width='630'>
	  <table border="0" cellspacing="1" cellpadding="0" width='100%'>
		<tr>
		  ...
		  <td width="300" class='title'>적요</td>
		  <td width="130" class='title'>입금액</td>
		  <td width='100' class='title'>거래점</td>		  
	...
	<td class='line' width='630'>
	  <table border="0" cellspacing="1" cellpadding="0" width='100%'>
        <%	for(int i = 0 ; i < incom_size ; i++){
				Hashtable ht = (Hashtable)vt.elementAt(i);%>
		<tr>
		  <td width='100' align='center'><a href="javascript:parent.view_incom('<%=ht.get("INCOM_DT")%>', '<%=ht.get("INCOM_SEQ")%>', '<%=ht.get("P_GUBUN")%>', '<%=ht.get("P_GUBUN_NM")%>', '<%=ht.get("INCOM_AMT")%>', '<%=ht.get("IP_METHOD")%>' )" onMouseOver="window.status=''; return true" title="클릭하세요"><%=ht.get("P_GUBUN_NM")%></a></td>
