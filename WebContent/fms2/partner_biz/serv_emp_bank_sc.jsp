...
<!--
function view_detail(off_id, cpt_cd, mon_amt, save_dt){
	var url = "?auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&sort=<%=sort%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&br_id=<%=br_id%>";
	parent.location.href = "servemp_d_frame.jsp"+url+"&off_id="+off_id+"&cpt_cd="+cpt_cd+"&mon_amt="+mon_amt+"&save_dt="+save_dt;
}
...
<input type="hidden" name="sort" value="<%= sort %>">
    ...
    <tr> 
      <td><iframe src="serv_emp_bank_sc_in.jsp?auth_rw=<%=auth_rw%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&sort=<%=sort%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&br_id=<%=br_id%>&list_order=<%=list_order%>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
    </tr>
