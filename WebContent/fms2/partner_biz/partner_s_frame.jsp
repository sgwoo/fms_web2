...
<%
	...
	String s_kd = request.getParameter("s_kd")==null?"JMJP":request.getParameter("s_kd");
...
<frameset rows="<%=height%>, *, 0" border=0>
	<frame name="c_body"    src="./partner_s_sh.jsp?br_id=<%=br_id%>&user_id=<%=user_id%>&member_id=<%=member_id%>&client_id=<%=client_id%>&r_site=<%=r_site%>&auth_rw=<%=auth_rw%>&s_gubun1=<%=s_gubun1%>&s_gubun2=<%=s_gubun2%>&s_gubun3=<%=s_gubun3%>&s_gubun4=<%=s_gubun4%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&idx=<%=idx%>&sh_height=<%=height%>" marginwidth=10 marginheight=10 scrolling='no' noresize>
	<frame name="c_foot"    src="./partner_s_sc.jsp?br_id=<%=br_id%>&user_id=<%=user_id%>&member_id=<%=member_id%>&client_id=<%=client_id%>&r_site=<%=r_site%>&auth_rw=<%=auth_rw%>&s_gubun1=<%=s_gubun1%>&s_gubun2=<%=s_gubun2%>&s_gubun3=<%=s_gubun3%>&s_gubun4=<%=s_gubun4%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&idx=<%=idx%>&sh_height=<%=height%>" marginwidth=10 marginheight=10 scrolling="auto">
