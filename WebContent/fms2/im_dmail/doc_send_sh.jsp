      ...
      <td class=line>
        <table border="0" cellspacing="1" cellpadding='0' width=100%>
              <tr>
				...
				<td width="10%" class="title">메일구분</td>
                <td width="20%">&nbsp;
                  <select name='gubun5'>
                    <option value=""<%if(gubun5.equals(""))%>selected<%%>>선택</option>
					<option value="1" <%if(gubun5.equals("1"))%>selected<%%>>과태료</option>
					<option value="2" <%if(gubun5.equals("2"))%>selected<%%>>보험</option>
                    			<option value="3" <%if(gubun5.equals("3"))%>selected<%%>>채권</option>
                    			<option value="4" <%if(gubun5.equals("4"))%>selected<%%>>영업수당</option>
					<option value="5" <%if(gubun5.equals("5"))%>selected<%%>>FMS안내</option>
					<option value="6" <%if(gubun5.equals("6"))%>selected<%%>>차량관리</option>
					<option value="7" <%if(gubun5.equals("7"))%>selected<%%>>장기대여</option>
					<option value="8" <%if(gubun5.equals("8"))%>selected<%%>>해지정산</option>
					<option value="9" <%if(gubun5.equals("9"))%>selected<%%>>긴급출동업체</option>
					<option value="10" <%if(gubun5.equals("10"))%>selected<%%>>미납통행료</option>
					<option value="11" <%if(gubun5.equals("11"))%>selected<%%>>협력정비업체</option>					
					<option value="12" <%if(gubun5.equals("12"))%>selected<%%>>블랙박스</option>					
                  </select></td>
				  <%//}%>
              </tr>
              <tr>
