...
response.setHeader("Content-Disposition", "inline; filename=stat_rent_fuel_sc_y_prn.xls");
%>
    ...
    <tr> 
        <td class=line> 
		    ...
		    <%for (int j = f_year ; j <= AddUtil.getDate2(1) ; j++){%>
                    <td colspan="2" class=title><%=j%>년</td>
		    <%}%>
                </tr>
                <tr align="center"> 
		    <%for (int j = f_year ; j <= AddUtil.getDate2(1)+1 ; j++){%>
                    <td class=title width="60">대수</td>
                    <td class=title width="60">비율</td>
		    <%}%>
                </tr>                       
		...
		<%	for(int i = 0 ; i < 8 ; i++){%>
                <tr> 
                    <%if(i==0){%><td width="50" rowspan="9" align="center">재리스</td><%}%>                    
                    <td width="110" align="center" colspan="2"><%=s_st1_nm[i]%></td>					
		    <%		for (int j = (f_year-2000-1) ; j <= (AddUtil.getDate2(1)-2000) ; j++){%>	
                    <td align="right">
						<%if(i==0){%>		<%=cnt1[j]%>
		...
		<%	for(int i = 0 ; i < 8 ; i++){%>
                <tr> 
                    <%if(i==0){%><td width="50" rowspan="9" align="center">연장</td><%}%>                    
                    <td width="110" align="center" colspan="2"><%=s_st1_nm[i]%></td>					
		    <%		for (int j = (f_year-2000-1) ; j <= (AddUtil.getDate2(1)-2000) ; j++){%>	
                    <td align="right">
						<%if(i==0){%>		<%=cnt1[j]%>
		...
		<%	for(int i = 0 ; i < 8 ; i++){%>
                <tr> 
                    <%if(i==0){%><td width="50" rowspan="9" align="center">합계</td><%}%>                    
		...
		<%	}%>
		    ...
		    <%	for (int j = (f_year-2000-1) ; j <= (AddUtil.getDate2(1)-2000) ; j++){%>	
		    <td class=title style='text-align:right'><%=h_cnt1[j]+h_cnt2[j]+h_cnt3[j]+h_cnt4[j]+h_cnt5[j]+h_cnt6[j]+h_cnt7[j]%></td>
		    <td class=title style='text-align:right'>-</td>
		    <%	}%>					
                </tr>																																		
            </table>
