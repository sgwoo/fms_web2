...
<!--
	//세부리스트
	function view_stat(st, car_gu, rent_st, bus_st1){
		window.open('stat_bus_root_stat_list.jsp?s_yy=<%=s_yy%>&s_mm=<%=s_mm%>&days=<%=days%>&f_year=<%=f_year%>&gubun1=<%=gubun1%>&st='+st+'&car_gu='+car_gu+'&rent_st='+rent_st+'&bus_st1='+bus_st1, "STAT_LIST", "left=300, top=20, width=700, height=600, scrollbars=yes, status=yes, resize");
	}
//-->
</script>
</head>
        ...
        <td class=line> 
            <table width=100% border="0" cellspacing="1" cellpadding="0">
                ...
                <tr> 
					...
					<%	for (int j = 0 ; j < days+1 ; j++){%>	
                    <td align="right">
                    	<%if(i==2 && j==0){%><a href="javascript:view_stat('1','신차','신규','<%=bus_st1_nm[i]%>')"><%}%>
						<%if(i==0){%>		<%=cnt1[j]%>
						<%}else if(i==1){%>	<%=cnt2[j]%>
						<%}else if(i==2){%>	<%=cnt3[j]%>
						<%}else if(i==3){%>	<%=cnt4[j]%>
                ...
                <tr> 
					...
					<%	for (int j = 0 ; j < days+1 ; j++){%>	
                    <td align="right">
                    	<%if(i==2 && j==0){%><a href="javascript:view_stat('4','재리스','신규','<%=bus_st1_nm[i]%>')"><%}%>
						<%if(i==0){%>		<%=cnt1[j]%>
						<%}else if(i==1){%>	<%=cnt2[j]%>
						<%}else if(i==2){%>	<%=cnt3[j]%>
						<%}else if(i==3){%>	<%=cnt4[j]%>
