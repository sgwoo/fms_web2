...
<%
	...
	String ins_com_nm ="";
	 ...
	 for(int i=0;i < vid_size;i++){
		...
		ins_com_nm =  String.valueOf(ht_ch.get("VALUE14"));
		if(ins_com_nm.equals("0008")) ins_com_nm = "DB손해보험";
		else if(ins_com_nm.equals("0038")) ins_com_nm = "렌터카공제조합";
		
		result = inc_db.updateInsExcelComConfCng(reg_code, seq);
		%>
			<script>
				var result= '<%=result%>';
				if(result){
		...
		<%		
		info = inc_db.getinsConfCngList(rent_mng_id, rent_l_cd, car_mng_id, ins_st, reg_code, seq );
		info_size = info.size();
		%>
		    ...
		    <div class="content">
				<div id="wrap" style="width:100%;">
					...
					<div style="text-align:left;font-size:12pt;font-weight:normal;margin-left:90px;">
						...
						<div style="padding:3px;">2. 아래와 같이 당사 자동차 리스 계약자의 계약의 종료(해지) 되었으므로<br><span style="margin:10px;"></span> 당해 자동차보험의 피보험자 변경을 요청합니다.</div>
					...
					<div>
						<table border="0" cellspacing="0" cellpadding='0' width='100%' class="contents" style="table-layout:fixed; word-break:break-all;">
							<tr style="height:50;background-color:white;">
							...
							<%for(int j = 0 ; j < info_size ; j++){	
							 ...
							 <tr style="height:60;">
								<td><%=j+1%></td>				
								<td><%=ht.get("CAR_NO")%></td>				
								<td><%=ht.get("CAR_NM")%><br><%=ht.get("CAR_NAME")%></td>				
					...
					<div>
						<table border="0" cellspacing="0" cellpadding='0' width='100%' class="contents" style="table-layout:fixed; word-break:break-all;">
							...
							<tr style="height:50;">
								...
								<td width="200" class=""><input type="text" value="<%=ht.get("AFT_FIRM")%>" style="text-align:center;font-weight:bold"></td>				
								<td width="150" class=""><input type="text" value="<%=ht.get("AFT_SSN")%>" style="text-align:center;font-weight:bold"></td>				
							</tr>	
							<%} %>				
						</table>
					</div>
