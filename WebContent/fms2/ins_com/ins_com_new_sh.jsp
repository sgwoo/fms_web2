...
<script>
	function search(){
		...
		if(fm.gubun3.value == '기간' && fm.st_dt.value != '')		fm.st_dt.value = ChangeDate3(fm.st_dt.value);
		if(fm.gubun3.value == '기간' && fm.end_dt.value != '')	fm.end_dt.value = ChangeDate3(fm.end_dt.value);

		if(fm.gubun2.value =='완료' && fm.gubun3.value == '기간' && fm.st_dt.value == '' && fm.t_wd.value == ''){
			alert('완료는 기간이나 검색어로 조회하세요.'); return;	
		}
		fm.action = 'ins_com_new_sc.jsp';
		fm.target = 'c_foot';
		fm.submit();
	}
...
<div class="navigation">
	<span class=style1>보험관리 ></span><span class=style5>가입현황</span>
</div>
<div class="search-area">
	...
	<select id="gubun2" name="gubun2" class="select" style="width:70px;">
		...
		<option value='확인' <%if(gubun2.equals("확인")){%>selected<%}%>>확인</option>
	...
	<select id="gubun3" name="gubun3" class="select" style="width:70px;" onchange="skdChange(this.value)">
		...
		<option value='당월' <%if(gubun3.equals("당월")){%>selected<%}%>>당월</option>
		<option value='전일' <%if(gubun3.equals("전일")){%>selected<%}%>>전일</option>
		<option value='전월' <%if(gubun3.equals("전월")){%>selected<%}%>>전월</option>
  ...
  <input type="text" name="end_dt" size="10" value="<%=end_dt%>" class="text" >
	...
	<select id="s_kd" name="s_kd" class="select" style="width:100px;">
		...
		<option value='4' <%if(s_kd.equals("4")){%>selected<%}%>>고객</option>
	</select>
	&nbsp;
