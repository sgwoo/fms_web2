...
<!--
	...
	function select_ins_com(st){
		...
		var act_ment="";
		var chk_value="";
		for(var i=0 ; i<len ; i++){https://www.google.co.kr/search?newwindow=1&q=jquery.form.js+%EC%82%AC%EC%9A%A9%EB%B2%95&sa=X&ved=0ahUKEwjHr_DHwOjaAhXIqJQKHa2BCToQ1QIIowEoAA
		...
		if(st == '4'){
			var user_id = "<%=user_id%>";
			  ...
			  <%}%>
					...
					act_ment="반영";
					fm.action = "ins_com_act_new_a.jsp";
		}
		if(st == '5'){
	...
	function excel_download(){
		...
		if(user_id == "0007") user_id="000310";
		 ...
		 location.href = "https://fms.amazoncar.co.kr/off_web/ins_com/insuRegExcel3.do?"+
...
// 	 	location.href = "http://localhost:8077/off_web/ins_com/insuRegExcel3.do?"+
// 		"page=1&rows=1000&sidx=req_dt&sord=asc"+
// 		"&period="+period+"&startDate="+startDate+"&endDate="+endDate+"&gubun="+ encodeURI(gubun,"EUC-KR")+"&search_text="+encodeURI(search_text,"UTF-8")+"&search_gubun="+search_gubun+"&userId="+user_id+" "; 
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
  <tr>
	  ...
	  <%if(gubun1.equals("0007") && gubun2.equals("요청")){ %>
		  <input type="button" class="button btn-submit" value="확인처리" style="float:right;background: RGB(27,86,48);" onclick="next()"/>
		  <input type="button" class="button btn-submit" value="저장하기" style="float:right;background: RGB(118,189,29);" onclick="save()"/>
	  <%} %>
	  <%}%>	  
	  </td>
	</tr>
	<tr>
		<td>
			<table border="0" cellspacing="0" cellpadding="0" width=100%>
				<tr>
					<td>
						<iframe src="ins_com_new_sc_in.jsp<%=valus%>" name="inner" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
						</iframe>
					</td>
				</tr>
			</table>
...
<%-- 엑셀 전송용 폼 --%>
<form name='form2' id="excel-upload" action="https://fms.amazoncar.co.kr/off_web/ins_com/insuRegExcel3.do" method="post" enctype="multipart/form-data">
<!--  <form name='form2' id="excel-upload" action="http://localhost:8088/off_web/ins_com/insuRegExcel3.do" method="post" enctype="multipart/form-data">  -->
