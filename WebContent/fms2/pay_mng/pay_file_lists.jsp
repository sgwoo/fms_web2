      ...
      <td class=line> 
        <table border="0" cellspacing="1" cellpadding='0' width=100%>
          <tr>
            ...
            <td width="10%" class=title>증빙서류</td>			
          ...
          <tr>
            ...
            <td align="center">
		    	<a href="javascript:view_pay_ledger_doc('<%=pm.getReqseq()%>','<%=pm.getP_gubun()%>','<%=pm.getP_cd1()%>','<%=pm.getP_cd2()%>','<%=pm.getP_cd3()%>','<%=pm.getP_cd4()%>','<%=pm.getP_cd5()%>','<%=pm.getP_st1()%>','<%=pm.getP_st4()%>','')" onMouseOver="window.status=''; return true" title="클릭하세요">보기</a>
			</td>															
            ...
            <td align="center"><%=pm.getP_cont()%></td>						
          </tr>
	      <%}%>		
		</table>
	  </td>
