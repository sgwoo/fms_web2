...
<%@ page import="acar.pay_mng.*, acar.cont.*,acar.client.*, acar.car_register.*, acar.accid.*, acar.cus_reg.*, acar.user_mng.*"%>
...
<%	
	...
	//동일건중복체크
	int pay_chk_cnt  =  pm_db.getPayRegChk(pay);
%>
...
<!--
	...
	function list(){
		...
		if(fm.from_page.value == ''){
			fm.action = '/fms2/pay_mng/pay_dir_reg_step2.jsp';
		}else{
	...
	function cng_input_vat2()
	{
		...
		if(fm.vat_Rdio2.checked == true && inVat > 0)
		{
			...
			if(fm.buy_s_amt.value == fm.buy_amt.value){
				fm.buy_s_amt.value = 0;
				tot_buy_amt();
			}else{
				set_buy_v_amt();
				set_buy_amt();				
	...
	function save()
	{
		...
		var s_amt = toInt(parseDigit(fm.buy_s_amt.value));
		var r_amt = toInt(parseDigit(fm.r_labor.value))+toInt(parseDigit(fm.r_amt.value));
		
		if(fm.acct_code_nm.value == '선택')	fm.acct_code_nm.value = '';
		
		if(fm.buy_user_id.value == '' && fm.user_nm[0].value == '')	{	alert('비용담당자를 입력하십시오.'); 	fm.user_nm[0].focus(); 	return; }				
		...
		if(fm.acct_code.options[fm.acct_code.selectedIndex].value == '45600'){ 	//사고수리비
			if(fm.accid_dt.value == '' && fm.accid_yn.checked == false)		{	alert('사고를 연결하거나 약식등록을 해야 합니다.'); 	fm.accid_dt.focus(); 	return; }				
			if(fm.serv_dt.value == '' && fm.serv_yn.checked == false)		{	alert('정비를 연결하거나 약식등록을 해야 합니다.'); 	fm.serv_dt.focus(); 	return; }	
			
			if(fm.serv_yn.checked == true){
				if( r_amt == 0 ) 	{ alert('공입가, 부품가를 입력하십시오.'); 										fm.r_labor.focus();		return;	}
		...
		if(fm.acct_code.options[fm.acct_code.selectedIndex].value == '45700' && fm.acct_code_g[9].checked == true){ 	//차량정비비-재리스정비
			if(fm.serv_dt.value == '' && fm.serv_yn.checked == false)		{	alert('정비를 연결하거나 약식등록을 해야 합니다.'); 	fm.serv_dt.focus(); 	return; }				
		...
		if(fm.acct_code.options[fm.acct_code.selectedIndex].value == '45700' && fm.acct_code_g[10].checked == true){ 	//차량정비비-정기검사
		}
		...
		if(fm.serv_id.value  != '' && fm.serv_yn.checked  == true)	fm.serv_id.value = '';
		if(fm.accid_id.value != '' && fm.accid_yn.checked == true)	fm.accid_id.value = '';
		...
		if(confirm('<%if(item.getReqseq().equals("")){%>등록<%}else{%>수정<%}%>하시겠습니까?')){
			
			var link = document.getElementById("submitLink");
			var originFunc = link.getAttribute("href");
			link.setAttribute('href',"javascript:alert('처리 중입니다. 잠시만 기다려주세요');");
			
			fm.action = 'pay_dir_reg_step2_a.jsp';
			fm.target = 'i_no';
			fm.submit();		
...
<form action="" name="form1" method="POST">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<%if(pay_chk_cnt>1){%>  			
	<tr>
	  <td>&nbsp;<font color=red>지출처가 협력업체이면서 거래일자, 지출처명, 금액이 동일한 건이 이미 기등록되어 있습니다. 확인하십시오.</font></td>
	</tr>	
	<%}%>
	<tr>
	  <td><hr></td>
    ...
    <tr>
      <td class=line> 
        <table border="0" cellspacing="1" cellpadding='0' width=100%>		
		  ...
		  <%if(item.getI_seq()==0){
		   		item.setBuy_user_id	(user_id);
				
				UsersBean user_bean 	= umd.getUsersBean(user_id);
				if(user_bean.getLoan_st().equals(""))	item.setBuy_user_id("");
			}%>
          <tr>
            <td colspan="2" class=title>비용담당자</td>
            <td >&nbsp;			  
              <input name="user_nm" type="text" class="default" value="<%=c_db.getNameById(item.getBuy_user_id(), "USER")%>" size="10" style='IME-MODE: active' onKeyDown="javascript:enter('buy_user_id', '0')">
          ...
          <tr>
            ...
            <td>&nbsp;
                <input name='buy_s_amt' type='text' class='defaultnum' id="buy_s_amt" onBlur='javascript:this.value=parseDecimal(this.value); set_buy_v_amt();' value='<%=AddUtil.parseDecimalLong(item.getI_s_amt())%>' size='15' maxlength='15'>
                원&nbsp;
			</td>
          </tr>
          <tr>
            ...
            <td>&nbsp;
                <input name='buy_v_amt' type='text' class='defaultnum' id="buy_v_amt" onBlur='javascript:this.value=parseDecimal(this.value); set_buy_amt();' value='<%=AddUtil.parseDecimalLong(item.getI_v_amt())%>' size='15' maxlength='15'>
                원
				&nbsp;&nbsp;<font color="blue">※ 부가세 없음
              	<input type="checkbox" name="vat_Rdio2" value="0" onClick="javascript:cng_input_vat2()" <%if(item.getI_v_amt()==0 && item.getI_s_amt()>0)%>checked<%%>>
				</font>
			</td>
          </tr>
          <tr>
            ...
            <td>&nbsp;
              <input type='text' name='buy_amt' maxlength='15' value='<%=AddUtil.parseDecimalLong(item.getI_amt())%>' class='defaultnum' size='15' onBlur='javascript:this.value=parseDecimal(this.value); tot_buy_amt();'>
				원
				&nbsp;&nbsp;<font color="blue">※ 부가세 끝전조정
                <input type="radio" name="vat_Rdio" value="0" onClick="javascript:cng_input_vat()" >
				상향
				<input type="radio" name="vat_Rdio" value="1" onClick="javascript:cng_input_vat()">
				하향 &nbsp;</font></td>
          </tr>
          <tr>
    ...
    <tr id=tr_acct1 style='display:<%if(item.getAcct_code_g().equals("1")||item.getAcct_code_g().equals("2")||item.getAcct_code_g().equals("15")||item.getAcct_code_g().equals("3")){%>""<%}else{%>none<%}%>'>
    ...
    <tr id=tr_acct2 style='display:<%if(item.getAcct_code_g().equals("13")||item.getAcct_code_g().equals("4")||item.getAcct_code_g().equals("5")||item.getAcct_code_g().equals("27")){%>""<%}else{%>none<%}%>'>
    	<td colspan="2" class="line">
      		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<tr>
    ...
    <tr id=tr_acct3 style='display:<%if(item.getAcct_code_g().equals("6")||item.getAcct_code_g().equals("7")||item.getAcct_code_g().equals("8")||item.getAcct_code_g().equals("18")||item.getAcct_code_g().equals("21")||item.getAcct_code_g().equals("22")){%>""<%}else{%>none<%}%>'>
    	<td colspan="2" class="line">
    		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
	        	<tr>
    ...
    <tr id=tr_acct4 style='display:<%if(item.getAcct_code_g().equals("9")||item.getAcct_code_g().equals("10")){%>""<%}else{%>none<%}%>'>
    	<td colspan="2" class="line">
    		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<tr>
          			<td width="15%" class='title'>구분</td>
    ...
    <tr id=tr_acct6 style='display:<%if(item.getAcct_code_g().equals("11")||item.getAcct_code_g().equals("12")||item.getAcct_code_g().equals("14")){%>""<%}else{%>none<%}%>'>
    	<td colspan="2" class="line">
    		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<tr>
          			<td width="15%" class='title'>구분</td>
          			<td width="85%">&nbsp;
						...
						<input type="radio" name="acct_code_g" value="12" <%if(item.getAcct_code_g().equals("12")){%>checked<%}%>>기타
					</td>
        		</tr>
      		</table>
      	</td>
    ...
    <tr id=tr_acct7 style='display:<%if(item.getAcct_code_g().equals("16")||item.getAcct_code_g().equals("17")){%>""<%}else{%>none<%}%>'>
    	<td colspan="2" class="line">
    		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<tr>
          			<td width="15%" class='title'>구분</td>
    ...
    <tr id=tr_acct8 style='display:<%if(item.getAcct_code_g().equals("19")||item.getAcct_code_g().equals("23")){%>""<%}else{%>none<%}%>'>
    	<td colspan="2" class="line">
    		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<tr>
          			<td width="15%" class='title'>구분</td>
          			<td width="85%">&nbsp;
            			...
            			<input type="radio" name="acct_code_g" value="24" <%if(item.getAcct_code_g().equals("24")){%>checked<%}%>>차량자동차세
            			<input type="radio" name="acct_code_g" value="25" <%if(item.getAcct_code_g().equals("25")){%>checked<%}%>>차량환경개선부담금
            			<input type="radio" name="acct_code_g" value="26" <%if(item.getAcct_code_g().equals("26")){%>checked<%}%>>차량개별소비세																		
					</td>
        		</tr>
      		</table>
      	</td>
  ...
  <tr id=tr_acct3_1 style='display:<%if(item.getAcct_code().equals("45600")||item.getAcct_code().equals("45700")||item.getAcct_code().equals("45800")){%>""<%}else{%>none<%}%>'>
  ...
  <tr id=tr_acct3_4 style='display:<%if(item.getAcct_code().equals("45600")){%>""<%}else{%>none<%}%>'>
      <td colspan="2" class="line"><table border="0" cellspacing="1" cellpadding="0" width='100%'>
		...
		<%if(accid_dt.equals("") && item.getReqseq().equals("")){%>
        <tr>
          <td>&nbsp;
      ...
      <td colspan="2" class="line"><table border="0" cellspacing="1" cellpadding="0" width='100%'>
        ...
        <tr>
          <td>&nbsp;
            ...
            &nbsp;km /&nbsp;
			정비내용&nbsp;:&nbsp;
		    <input name="rep_cont" type="text"  class="text" size="25" value="" style='IME-MODE: active'>
			</td>
        </tr>     		
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <tr id=tr_acct3_6 style='display:<%if(item.getAcct_code().equals("45700") && (item.getAcct_code_g().equals("7") || item.getAcct_code_g().equals("8"))){%>""<%}else{%>none<%}%>'>
	...
	<tr>
	  <td align="right">
	    <input type="hidden" name="total_amt1" value="<%=total_amt1%>">  
		<input type="hidden" name="total_amt2" value="<%=total_amt2%>">  
		<%if( auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6")) {%>
	  <a id="submitLink" href="javascript:save()">
	  <%if(item.getReqseq().equals("")){%>
	  <%	if(total_amt1==0 || total_amt1 >total_amt2 || total_amt1 <total_amt2){%>
                        	...
                        	<tr>
								...
								<td align="center">
									<select name='dept_id' onChange="javascript:User_search2('user_case_id',<%=j+1%>);">
          								...
          								<option value='<%=dept.getCode()%>' <%if(String.valueOf(ht.get("DEPT_ID")).equals(dept.getCode())){%>selected<%}%>><%=dept.getNm()%></option>
          									<%}%>
        							</select><a href="javascript:User_search2('user_case_id',<%=j+1%>);">.</a>
        						</td>
								<td align="center">
								...
								<td align="center">
          								...
          								<option value='<%=dept.getCode()%>' <%if(String.valueOf(ht2.get("DEPT_ID")).equals(dept.getCode())){%>selected<%}%>><%=dept.getNm()%></option>
          									<%}%>
        							</select><a href="javascript:User_search2('user_case_id',<%=j+2%>);">.</a>
        						</td>
								<td align="center">
