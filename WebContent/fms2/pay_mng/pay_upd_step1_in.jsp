...
<!--
	...
	function save()
	{
		...
		<%if(item.getP_gubun().equals("99")){%>
		if(fm.user_nm[0].value == '')	{	alert('비용담당자를 입력하십시오.'); 	fm.buy_user_id.focus(); 	return; }				
		...
		if(confirm('수정하시겠습니까?')){
			...
			fm.action = 'pay_upd_step1_in_a.jsp';
			fm.target = 'i_no';
			fm.submit();		
          ...
          <tr>
            ...
            <td>&nbsp;
				...
				&nbsp;&nbsp;<font color="blue">※ 부가세 없음
              	<input type="checkbox" name="vat_Rdio2" value="0" onClick="javascript:cng_input_vat2()" <%if(item.getI_v_amt()==0){%>checked<%}%>>
				</font>
			</td>
          </tr>
          <tr>
            ...
            <td>&nbsp;
              <input type='text' name='buy_amt' maxlength='10' value='<%=AddUtil.parseDecimalLong(item.getI_amt())%>' class='defaultnum' size='15' onBlur='javascript:this.value=parseDecimal(this.value); tot_buy_amt();'>
				원
