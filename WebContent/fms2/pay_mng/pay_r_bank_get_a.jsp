...
<%
	...
	for(int i = 0 ; i < vt_size ; i++){
		...
		//act.setCommi		(AddUtil.parseDigit(String.valueOf(ht.get("TRAN_FEE")))); -> 월말 결산함.
		act.setAct_bit		("1");
		...
		//광주카드 담당자 문자발송
		/*
		if(act.getOff_nm().equals("광주카드선불(0679)차량대금")){
					UsersBean sender_bean 	= umd.getUsersBean(ck_acar_id);
					String sendphone 	= sender_bean.getUser_m_tel();
					String sendname 	= "(주)아마존카 "+sender_bean.getUser_nm();
					String destphone 	= "010-9699-8362";
					String destname 	= "광주카드 조순일";
					String msg_cont		= "(주)아마존카 "+act.getR_act_dt()+" 결재대금이 지급되었습니다. ("+act.getAmt()+") 확인바랍니다.-아마존카-";
					IssueDb.insertsendMail(sendphone, sendname, destphone, destname, "", "", msg_cont);
		}		
		*/
...
<%		}else{	%>		
			<%if(vt_size>0){	%>	alert('<%=vt_size%>건 수신처리하였습니다.');		<%}%>		
<%		}	%>		
