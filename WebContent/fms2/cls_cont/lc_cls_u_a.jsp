...
<%
	...
	cls.setDfee_v_amt(request.getParameter("dfee_v_amt")==null?0:		AddUtil.parseDigit(request.getParameter("dfee_v_amt")));   //과부족+미납대여료 당초금액	 -2022-04	
	...
	if(cls_st.equals("8")){//매입옵션인경우 
		...
		cls.setFdft_amt3(request.getParameter("fdft_amt3")==null?0:		AddUtil.parseDigit(request.getParameter("fdft_amt3")));//매입옵션고객납입금액
	}
	...
	cls.setB_tot_dist(request.getParameter("b_tot_dist")==null?0:AddUtil.parseDigit(request.getParameter("b_tot_dist")));   //추가 주행거리
...
//해지의뢰내역 추가 항목 - 20180907 cls_etc field가 너무 많아서 cls_etc_more에 추가 			
	...
	cc.setGuar_st(request.getParameter("guar_st")==null?"":		request.getParameter("guar_st"));  //보증인 유무
...
//	System.out.println("해지 보증인 수정=" + gu_size + ":" + rent_l_cd);
	...
	//중도매입옵션관련 추가 - 20161028
	 if ( AddUtil.parseDigit(request.getParameter("old_opt_amt")) > 0 ) {
		...
		//해지의뢰 추가내역 저장	 - 추후에 수정하는 부분 추가할것 - 20161111
