...
//	System.out.println("tax=" + base.getTax_type() + ": ven_type = " + ven_type + ":s_idno = " + s_idno + ":i_ven_code = " + i_ven_code + ":i_enp_no= " + i_enp_no + ":client_st = " + client_st );
...
/*		   	  				
	...
	// 부가세관련
		...
		if(ven_type.equals("1")){ //개인
			ht3_2.put("NO_COMPANY",	"8888888888"); //개인인 경우 사업자등록번호 8888888888
		} else {
...
/*		
	...
	// 부가세관련
		...
		if(ven_type.equals("1")){ //개인
			ht3_3.put("NO_COMPANY",	"8888888888"); //개인별 사업자등록번호 8888888888
		}   else {
...
//if ( !jung_st.equals("2") ) {		  	
  ...
  if ( AddUtil.parseDigit(request.getParameter("car_ja_amt_2")) > 0 ) {
		...
		//	ht5_1.put("CD_ACCT",  	"91800");  //면책금	- 차량수리정비비(45510) -20211116 회계계정변경 
		//	ht5_1.put("AMT",   String.valueOf( amt_10800) );
			
			ht5_1.put("TP_DRCR",   "1");   // 대변:2 , 차변:1	
			ht5_1.put("CD_ACCT",  	"45510");  //면책금	
 ...
 //대여료 세금 계산서 발행건 처리 
  if ( a4_s_amt > 0 ) {
	...
	// 부가세관련
		...
		if(ven_type.equals("1")){ //개인
			ht8_2.put("NO_COMPANY",	"8888888888"); //개인인 경우 사업자등록번호 88888888
		} else {
  ...
  if ( AddUtil.parseDigit(request.getParameter("dfee_amt_2"))  < 0 ) {
	 ...
	 System.out.println("amt_10800="+ amt_10800);
 ...
 /*
  if ( a6_s_amt > 0 ) {
		...
		// 부가세관련
			...
			if(ven_type.equals("1")){ //개인
				ht11_2.put("NO_COMPANY",	"8888888888"); //개인사업자등록번호
			} else {
 ...
 /*
  if ( a8_s_amt > 0 ) {
		...
		// 부가세관련
			...
			if(ven_type.equals("1")){ //개인
				ht15_2.put("NO_COMPANY",	"8888888888"); //개인사업자등록번호
			} else {
 ...
 // }	
  ...
  if ( a9_s_amt > 0 ) {
	 	...
	 	//  렌트(리스)사업매출인 경우는 부가세처리 
			...
			if ( AddUtil.parseDigit(request.getParameter("over_amt_2"))  < 1 ) { 	  //외상매출금 추가 (상계를  안하면 )
 ...
 if ( jung_st.equals("2")) {
  ...
  //고객에게 돌려줄 금액
	  if ( AddUtil.parseDigit(request.getParameter("fdft_amt2"))  < 0 ) {
	 		 ...
	 		 if (cls_st.equals("7") ) {
				 doc_cont = "출고전해지(신차) 고객환불" + "-" +  rent_l_cd + " " + firm_nm+ " " + re_bank_nm + "-" + re_acc_no + "-" + re_acc_nm;
			}  else {		 
				  doc_cont =  cls.getCls_st()  + " 고객환불" + "-" + car_no + " " + firm_nm + " " + re_bank_nm + "-" + re_acc_no + "-" + re_acc_nm;
			}	
			...
			if (cls_st.equals("8") ) {  //매입옵션인 경우 선수금으로
				ht17.put("CD_ACCT",   	"25900");  //선수금
				ht17.put("DT_START",  cls.getCls_dt()); 	//발생일자	
			} else {
				ht17.put("CD_ACCT",   	"25300");  //미지급금	
				ht17.put("DT_START",  "");  	//발생일자	
			}	
