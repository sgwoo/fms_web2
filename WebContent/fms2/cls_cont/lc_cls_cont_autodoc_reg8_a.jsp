...
//	fuel_cnt= ac_db.getFuelCnt(rent_l_cd, "Y");	
	...
	for(int i = 0; i<tax_size; i++){ 
		...
		if(tax_r_chk.equals("Y") && AddUtil.parseDigit(tax_r_supply[i]) != 0 ){  //금액이 0보다 크면???
...
*/	
  ...
  if ( AddUtil.parseDigit(request.getParameter("car_ja_amt_2")) > 0 ) {
		...
		//	ht5_1.put("CD_ACCT",  	"91800");  ////차량손해면책금	
		//	ht5_1.put("AMT",   String.valueOf( amt_10800) );	
	
			ht5_1.put("TP_DRCR",   "1");   // 대변:2 , 차변:1		
			ht5_1.put("CD_ACCT",  	"45510");  ////차량손해면책금	-> 차량수리정비비(45510) - 20211116 회계계정 변경 
 ...
 //대여료 세금 계산서 발행건 처리 
  ...
  if ( AddUtil.parseDigit(request.getParameter("etc3_amt_2"))  < 0 ) {	 
		...
		//		ht16_3.put("CD_PARTNER",	i_ven_code); //거래처    - A06 - 격락손해등 특수 case 
  ...
  if ( a8_s_amt > 0 ) {
	 	...
	 	//  렌트(리스)사업매출인 경우는 부가세처리 
			...
			if ( AddUtil.parseDigit(request.getParameter("etc4_amt_2"))  < 1 ) { 	  //외상매출금 추가
				...
				ht15_3.put("CD_ACCT",  "10800");   // 외상매출금 
		//		ht15_3.put("CD_ACCT",  "91800");   //면책금  
