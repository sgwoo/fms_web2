...
<%@ page import="acar.cont.*,acar.client.*, acar.car_mst.*, acar.car_register.*,  acar.receive.*, acar.accid.*"%>
...
<%
	...
	String add 	= request.getParameter("add")==null?"N":request.getParameter("add");	
...
//	System.out.println("lc_cls_u doc_bit = " + doc_bit);
	...
	AccidDatabase accid_db = AccidDatabase.getInstance();
	...
	if( ( cls.getDft_saction_id().equals("000028")  ||  cls.getDft_saction_id().equals("000005") ||  cls.getDft_saction_id().equals("000026")  ) && cls.getDft_saction_dt().equals("")  ) {	
		dft = "Y";
	}	
	...
	from_page = "/fms2/cls_cont/lc_cls_u.jsp";
   ...
   if (   fuel_cnt > 0   ) {     
	...
	vt_size8 = vts8.size(); 	
		...
		//중도매입옵션인경우
	if( clsa.getOld_opt_amt() > 0 )  {	
		if (  clsa.getAdd_saction_id().equals("000197")  && clsa.getAdd_saction_dt().equals("") ) {
			add = "Y";
		}	
	}
	...
	int  cls_cnt =   accid_db.getAccidSettleClsCnt(rent_mng_id, rent_l_cd);  //사전해지요청건인지 
	int  settle_cnt =  0;
	settle_cnt =   accid_db.getAccidSettleChk(rent_mng_id, rent_l_cd); 
...
//리스트
	...
	function sac_req(){
		...
		if ( <%=settle_cnt%>  >=  1 ){
			if ( <%=cls_cnt%> != <%=settle_cnt%>  ) {
				alert("해당차량과 관련하여 사고가 진행중입니다. 종결처리후 해지 하셔야 합니다.!!!!");
				return; 
			}			
	    }
		...
		if (  fm.dft_amt.value != fm.dft_amt_1.value ) {
		    if( get_length(Space_All(fm.dft_reason.value)) == 0 ) {
				alert("중도해지위약금 감액사유를 입력하셔야 합니다.!!");
				return;
			}
			
			if( fm.dft_cost_id.value == '' ) {
				alert("영업효율 귀속대상자를 선택하셔야 합니다.!!");
				return;
			}			
			
			if( fm.dft_saction_id.value == '000028'  ||  fm.dft_saction_id.value == '000005' ||  fm.dft_saction_id.value == '000026'   ) {
			
			} else { 
				alert("중도해지위약금 결재자를  영업팀장 또는 고객관리팀장 으로 지정하셔야 합니다. 사전에 협의하십시요.!!");
				return;
			}		
		}		
	    ...
	    if (  toInt(parseDigit(fm.old_opt_amt.value)) > 0 ) {
		    if ( fm.add_saction_id.value == '' ) {
			  ...
			  if ( fm.add_saction_dt.value == '' ) {
						alert("중도매입옵션 경우 결재가 되지 않았습니다. 결재 후 처리할 수 있습니다..!!");
						return;
			 }	 
		}		
		...
		if(fm.match[1].checked == true){  //만기매칭  유 선택시 위약금 없음 		 
				//중도해지 결재자 결재여부 체크 - 추가 - 임시로 막아놓음.
				if( fm.dft_saction_id.value == '000005'  ||  fm.dft_saction_id.value == '000028'  ||  fm.dft_saction_id.value == '000026'  ) {
				    if ( fm.dft_saction_dt.value == '' ) {
						alert("중도해지위약금 결재가 되지 않았습니다. 결재 후 처리할 수 있습니다..!!");
						return;
					}	
				}	
		}
		...
		// 기타손해배생금 발생시 증빙첨부 여부 또는 사전견적
		if ( toInt(parseDigit(fm.etc4_amt_1.value))  > 0 ) {			
			<%if ( attach2_vt_size < 1 ) {%>	
			
				 if ( toInt(parseDigit(fm.e_serv_amt.value)) == 0  ) {
				  } else { 	  
			     	 if ( ( toInt(parseDigit(fm.car_ja.value))+ 200000 )  >=  toInt(parseDigit(fm.e_serv_amt.value)) ) { 
							 alert(" 사전견적서 입력이 불가합니다. 견적서를 첨부하세요.!!");
							 return;  
					  }	
				 }			 
		...
		//cms 인출확인
		if ( toInt(parseDigit(fm.h7_amt.value))  > 0  || toInt(parseDigit(fm.fdft_amt2.value))  > 0 ) {		
		   ...
		   <% } else {%>
			if  (  fm.cms_chk.checked == false)  {  //cms 인출의뢰 미선택시
				if(confirm('cms 인출의뢰없이 진행하시겠습니까?')){											
					if(confirm('결재요청하시겠습니까?')){					
						fm.action='lc_cls_doc_sanction.jsp';		
						fm.target='i_no';
						fm.submit();
	...
	function save(){
		...
		if ( fm.cls_st.value == '8') {			
			if( toInt(parseDigit(fm.opt_amt.value)) == 0 ){ alert('매입옵션금액을 입력하십시오'); 		fm.opt_amt.focus(); 	return;	}	
		} else {	
	...
	function set_cls_amt2(obj){
	   ...
	   if ( fm.cls_st.value == '7'  ||  fm.cls_st.value == '10' ) {  //출고전해지. 개시전해지 - 계산시 부가세 틀린경우가 있음 	       
		...
		//각각 부가세 계산하여 더한다.	
		no_v_amt = toInt(parseDigit(fm.ex_di_v_amt_1.value)) + c_fee_v_amt + c_over_s_amt - c_pp_s_amt - c_rfee_s_amt ;
	  ...
	  //  fm.dfee_v_amt_1.value = toInt(parseDigit(fm.ex_di_v_amt_1.value)) + (toInt(parseDigit(fm.nfee_amt_1.value)) * 0.1); //확정 대여료 부가세 
		...
		fm.dfee_v_amt_1.value =parseDecimal( toInt(parseDigit(fm.ex_di_v_amt_1.value)) + c_fee_v_amt );  //당초 대여료 부가세      //확정 대여료 부가세 			  
	...
	//	}else if(obj == fm.over_amt_1){ //초과운행부담금 감액		 
		...
		if(obj == fm.m_over_amt){ //초과운행부담금 감액		 
			...
			//	fm.over_v_amt.value 	= parseDecimal( toInt(parseDigit(fm.j_over_amt.value)) * 0.1 );	 
	...
	//	fm.over_v_amt.value =  c_over_s_amt;  //당초 초과운행 부가세 
		fm.over_v_amt_1.value = parseDecimal( c_over_s_amt );  //확정 초과운행 부가세 
	...
	function doc_sanction(doc_bit){
		...
		if(fm.match[1].checked == true){  //만기매칭  무 선택시 위약금 없음 	
	...
	function sanction_req_delete(){
		var fm = document.form1;
		if(fm.sanction_req_delete_cont.value == ''){ alert('삭제 요청사유를 입력하십시오.'); return; }
	...
	function update(st){
		...
		if (st == 'cls_jung') {	
			...
			window.open("/fms2/cls_cont/updateTrCre.jsp<%=valus%>", "CHANGE_ITEM", "left= 50, top=100, width=1150, height=550, resizable=yes, scrollbars=yes, status=yes");
	...
	//위약금 등  결재
	function cls_etc_sanction(gubun) {
		var fm = document.form1;
		
		if ( gubun == 'req' ) {
			if(!confirm('삭제요청하시겠습니까?')){			return;		}
		}  else {
			if(!confirm('결재하시겠습니까?')){			return;		}
		}
				
		fm.action='lc_cls_etc_sanction.jsp?gubun='+gubun;		
		fm.target='i_no';
		fm.submit();
	...
	function clsConSanction(){
		...
		if(confirm('결재하시겠습니까?')){	
			fm.action='lc_cls_cont_sanction.jsp?doc_bit=2';	
			fm.target='i_no';
		//	fm.target='d_content';
			fm.submit();
		}		
	...
	function updDestSui()
	{
		...
		if(fm.des_addr.value == ""){ alert("서류배송주소를 입력하세요!!."); return;}
		if(fm.des_nm.value == ""){ alert("상호&수취인을 입력하세요!!."); return;}
									
		if(confirm('매입옵션 서류정보를 수정하시겠습니까?'))
		{		
			fm.target = 'i_no';
			fm.action='lc_cls_update_a.jsp?m_gubun=upd_des_addr';			
			fm.submit();
		} 		
	}
	...
	function print_view(mode)
	{
		...
		window.open("/fms2/con_fee/fee_scd_print.jsp?m_id="+m_id+"&l_cd="+l_cd+"&b_dt="+b_dt, "PRINT_VIEW", "left=50, top=50, width=770, height=640, scrollbars=yes");
	}
...
<input type='hidden' name='cls_msg' value=''>
...
<table border='0' cellspacing='0' cellpadding='0' width='100%'>
	...
	<% } %>	
	    ...
	    <%  if (doc.getUser_dt1().equals("") && ck_acar_id.equals("000029")){%>     
	 ...
	 <tr> 
 	 <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>해지내역&nbsp;
 	  <%  if (  nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("매입옵션관리자",user_id) || nm_db.getWorkAuthUser("채권관리팀",user_id)  || nm_db.getWorkAuthUser("보유차관리",user_id)  || nm_db.getWorkAuthUser("해지관리자",user_id) ){%>     
 		 <a href="javascript:update('cls_etc')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><!--<a href="javascript:sac_delete('1');"><img src=/acar/images/center/button_delete.gif align=absmiddle border=0></a>&nbsp; -->
 	 <% } %>			 
 		 </span></td>
  	  </tr> 
	<tr>
    ...
    <tr> 
      <td class='line'> 
       ...
       <% if (user_id.equals("000063")) {%>     
		...
		<% } else { %>
		  <input type='text' name='cls_dt' value='<%=AddUtil.ChangeDate2(cls.getCls_dt())%>' size='10' class='text' readonly onBlur='javascript: this.value = ChangeDate(this.value); '>
		<% }%>	  
			  </td> 
...
<%if( cls.getCls_st().equals("매입옵션") )  { %>
            ...
            <td  width=12%>&nbsp;
	            ...
	            <!--    <option value="000172" <% if(clsa.getAdd_saction_id().equals("000172")){%>selected<%}%>>정진식</option> 	   -->           
	             ...
	             <% if ( add.equals("Y") && clsa.getAdd_saction_dt().equals("") && ( user_id.equals(clsa.getAdd_saction_id() ) || user_id.equals("000277") ) ) { %>
			              &nbsp;<a href="javascript:cls_etc_sanction('add')"><img src="/acar/images/center/button_in_gj.gif" align="absmiddle" border="0"></a>
			      <% } %>			              
			      &nbsp;&nbsp;<%=clsa.getAdd_saction_dt()%><input type='hidden' name='add_saction_dt' value='<%=clsa.getAdd_saction_dt()%>'>   			              
				</td>			
...
<% } %>		    
           ...
           <tr>      
		            ...
		            <td>&nbsp;
					  <input type='text' readonly name='tot_dist' size='8' value='<%=AddUtil.parseDecimal(cls.getTot_dist())%>' class='num' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;km 
		            ...
		            <% if ( dashcnt > 0) { %><font color=red> ※  계기판 교환직전 주행거리 </font>
		             <input type='text'  readonly name='b_tot_dist' size='8' value='<%=AddUtil.parseDecimal(cls.getB_tot_dist())%>' class='num' onBlur='javascript:this.value=parseDecimal(this.value); '>&nbsp;km 
		           <% } %> 
		          &nbsp; 
		            <%if( cls.getCls_st().equals("매입옵션") ) { %>※ 매입옵션시 차량주행거리 <% } else { %> ※ 중도해지 및 만기시 차량주행거리 <%} %>
	        ...
	        <td class=line>
				...
				<script>
					function openDaumPostcode() {
	       		  ...
	       		  <tr>
					<td class=title width=13%>서류배송주소 <br>
					<!--   서류발송전이면 담당자도 수정가능 :cls_etc_more , sui 테이블 확인  -->
					 <%  if ( clsm.getEst_dt().equals("") ) { %> 		   		
					 &nbsp;<a href="javascript:updDestSui();"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a>&nbsp;    
					 <% } %>
					</td>
					<td  colspan="3">&nbsp; 
						<input type="text" name="des_zip"  id="des_zip" size="7" maxlength='7' value="<%=clsm.getDes_zip()%>">
						<input type="button" onclick="openDaumPostcode()" value="우편번호 찾기"><br>
						&nbsp;<input type="text" name="des_addr" id="des_addr" size="80" value="<%=clsm.getDes_addr()%>">
					</td>
			...
			<tr> 
	          ...
	          <!--        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;계약:&nbsp;<a href="javascript:search_grt_suc(2)" onMouseOver="window.status=''; return true" title="클릭하세요"><img src=/acar/images/center/button_in_search.gif align=absmiddle border=0></a> --></td>	                
		          ...
		          <tr>      
		            ...
		            <td>&nbsp;<input type='text' name='match_end_dt' size='15' value='<%=AddUtil.ChangeDate2(m_taecha.getCar_rent_et())%>' class='whitetext' ></td>
 	 	  ...
 	 	  <tr>
		    	...
		    	<table border="0" cellspacing="1" cellpadding="0" width=100%>
		          <tr> 		          	
		             ...
		             <td width="13%">&nbsp;<input type='text' name='opt_per' value='<%=f_opt_per%>' size='5' class='num' readonly  maxlength='4'>%</td>
		             <td class='title' width="13%">매입옵션가</td>
		             <td colspan=2 width="26%">&nbsp;<input type='text' name='opt_amt'size='15' class='num' <% if( nm_db.getWorkAuthUser("전산팀",user_id)  ){%> <% } else { %> readonly <%} %>  value="<%=AddUtil.parseDecimal(cls.getOpt_amt())%>" onBlur='javascript:this.value=parseDecimal(this.value); set_sui_c_amt();'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(VAT포함)</td> 
		          ...
		          <tr id=tr_opt3 style="display:<%if ( clsm.getSui_st().equals("Y") ){%>''<%}else{%>none<%}%>"> 
		             ...
		             <td colspan=2  >&nbsp;<input type='text' name='sui_d_amt' readonly  value='<%=AddUtil.parseDecimal(clsm.getSui_d_amt())%>' size='15' class='num' onBlur='javascript:this.value=parseDecimal(this.value);'>&nbsp;&nbsp; *천원절사</td> 
		          </tr>  
		       </table>
		      </td>        
         </tr> 
		            ...
		            <td  colspan="2" class='line'> 
		              <table border="0" cellspacing="1" cellpadding="0" width="100%">
		                ...
		                <tr> 
		                  ...
		                  <td align="center"> 
		                    <input type='text' name='pded_s_amt' readonly value='<%=AddUtil.parseDecimal(cls.getPded_s_amt())%>' size='15' class='num'   readonly onBlur='javascript:this.value=parseDecimal(this.value); '></td>
		                  <td>=선납금÷계약기간</td>
		                </tr>
		                <tr> 
        ...
        <td colspan="2" class='line'> 
          <table border="0" cellspacing="1" cellpadding="0" width=100%>
            ...
            <tr> 
              ...
              <td  align="center" class="title"> 
               ...
               <td align="center" class="title"> 
                ...
                <td class="title">&nbsp;<input type='checkbox' name='tax_chk0'  value='Y' <%if(cls.getTax_chk0().equals("Y")){%>checked<%}%>  disabled ><!--계산서발행의뢰-->
            ...
            <tr> 
              ...
              <td class='title' align="center" class="title"> 
                <input type='text' name='dly_amt_1'  value='<%=AddUtil.parseDecimal(cls.getDly_amt_1())%>' size='15' class='num' onBlur='javascript:this.value=parseDecimal(this.value); set_cls_amt(this);'></td>             
              <td class='title'>&nbsp; </td>
            </tr>
            <tr>  
              ...
              <td  align="center" class="title"> 
               ...
               <td  align="center" class="title"> 
                <input type='text' name='etc_amt_1' value='<%=AddUtil.parseDecimal(cls.getEtc_amt_1())%>' size='15' class='num' onBlur='javascript:this.value=parseDecimal(this.value); set_cls_amt(this);'></td>  
               ...
               <td class="title">&nbsp;<input type='checkbox' name='tax_chk1'  value='Y' <%if(cls.getTax_chk1().equals("Y")){%>checked<%}%>   disabled ><!--계산서발행의뢰--></td>
            </tr>
            <tr> 
              ...
              <td align="center" class="title"> 
                <input type='text' name='etc2_amt_1'  value='<%=AddUtil.parseDecimal(cls.getEtc2_amt_1())%>' size='15' class='num' onBlur='javascript:this.value=parseDecimal(this.value); set_cls_amt(this);'></td>  
               ...
               <td class="title">&nbsp;<input type='checkbox' name='tax_chk2'  value='Y' <%if(cls.getTax_chk2().equals("Y")){%>checked<%}%>   disabled ><!--계산서발행의뢰--></td>
            </tr>
            <tr> 
              ...
              <td align="center" class="title"> 
                <input type='text' name='etc3_amt_1' readonly  value='<%=AddUtil.parseDecimal(cls.getEtc3_amt_1())%>'size='15' class='num' onBlur='javascript:this.value=parseDecimal(this.value); set_cls_amt(this);'></td> 
             ...
             <!-- 기타손해배상금이 있는 경우만 --> 
              <%if( cls.getEtc4_amt_1() > 0 ) {%>
                 ...
                 %>
    			  ...
    			  <%}%>    		 		                
            <%} else { %>
            <input type='hidden' name='e_serv_amt' >
            <input type='hidden' name='e_serv_rem' >
            <% } %>
            </td>                                                        
            </tr>
           ...
           <!--    <input type='hidden' name='tax_supply' readonly  value='<%=AddUtil.parseDecimal(cls.getOver_amt_s())%>' size='15' class='num' > 
               <input type='hidden' name='tax_value' readonly  value='<%=AddUtil.parseDecimal(cls.getOver_amt_v())%>' size='15' class='num' >               
                 <input type='hidden' name='tax_g' size='20' class='text' value='초과운행대여료'>   초과운행 정산에 의해서만 금액 수정토록 함. --> 
...
<%  	   }  %>		     
	              ...
	              <td >&nbsp;</td> <!--출금원장에서-->
	              <td >&nbsp; <select name='pay_st'>
			                <option value="">--선택--</option>
			                <option value="1">계좌송금</option>
			                <option value="2">현금지급</option>	       
	               </select>	              
	               </td>
	              <td align="left" >	  
                 <%
            ...
            <table width=100% border=0 cellspacing=1 cellpadding=0>
                ...
                <tr>
                    ...
                    <td width=12%>&nbsp;
			              ...
			              <% if ( dft.equals("Y") && cls.getDft_saction_dt().equals("") && ( user_id.equals("000028")  || user_id.equals("000005") || user_id.equals("000026") || user_id.equals("000063") )  ) { %>
			              &nbsp;<a href="javascript:cls_etc_sanction('dft')"><img src="/acar/images/center/button_in_gj.gif" align="absmiddle" border="0"></a>
			              <% } %>
...
<%	}  %>	
    ...
    <tr>
	    <td align='center'>	
	     ...
	     <%	if(     nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("본사관리팀장",user_id) || nm_db.getWorkAuthUser("영업수당회계관리자",user_id) || nm_db.getWorkAuthUser("채권관리팀",user_id) || nm_db.getWorkAuthUser("매입옵션관리자",user_id) ) {%>
	    ...
	    <% } else { %> 
	     ...
	     <%}%> 
	      ...
	      <br><br>삭제요청사유 : <input type='text' name="sanction_req_delete" value='' size="100" class='text'>&nbsp;
	      <a href="javascript:cls_etc_sanction('req');" title='삭제 요청하기'>[삭제요청 메시지발송]</a><br><br>
	   <%}%> 
	    </td>
	</tr>			  
    <%}%>       
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr> 
                    ...
                    <td align="center"><%=c_db.getNameById(doc.getUser_id2(),"USER_PO")%><br><%=doc.getUser_dt2()%>
        			  ...
        			  <% if(user_id2.equals(user_id) || nm_db.getWorkAuthUser("전산팀",user_id)  || nm_db.getWorkAuthUser("본사영업기획팀장",user_id)  ){%>
        			  ...
        			  <%}%>        		     		  
        			   ...
        			   <!--결재 skip -->
       	 		    <% if ( doc.getUser_dt2().equals("")  &&  ( doc.getUser_id3().equals(user_id) || nm_db.getWorkAuthUser("전산팀",user_id)  || nm_db.getWorkAuthUser("해지관리자",user_id)   )    ){ %>
        			   <a href="javascript:doc_sanction('9');">[skip]</a>
        			<% } %>          			  
        			</td>  
