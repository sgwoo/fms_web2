...
//			til_bean.setItem_id(item_id);
	...
	//		til_bean.setReg_code(reg_code);
			...
			if ( tax_r_g[i].equals("해지 취소 대여료") ) {
	    			til_bean.setItem_dt1( String.valueOf(l_tax.get("ITEM_DT1"))  );   //사용기간
			...
			//		Hashtable br2 = c_db.getBranch(t_bean.getBranch_g());
				...
				if ( tax_r_g[i].equals("해지 개시대여료 환급")) {
					...
					t_bean.setTax_bigo	( tdt4+"(당초 세금계산서 작성일)");  
				}else if ( tax_r_g[i].equals("해지 선납금 환급")) {
	    			t_bean.setDoctype("02");   //공급가변동으로 처리
	    			t_bean.setTax_bigo	( tdt3+"(당초 세금계산서 작성일)");
	    		}else if ( tax_r_g[i].equals("해지 취소 대여료") ) {
	    		   //해지취소대여료만 있는 경우와 취소,미납대여료가 같이 있는 경우 처리는 틀림 :
	    		   if ( cls_fee == 1 ) {
	    			  t_bean.setDoctype("02");  //공급가변동으로 처리
	    			  t_bean.setTax_bigo	( String.valueOf(l_tax.get("TAX_DT"))+"(당초 세금계산서 작성일)");
	    		   } else if ( cls_fee == 2 ) {
  ...
  if ( AddUtil.parseDigit(request.getParameter("car_ja_amt_2")) > 0 ) {
 	  ...
 	  if (cls_st.equals("7") ) {
		...
		//	ht5_1.put("AMT",   String.valueOf( amt_10800) );		
			...
			ht5_1.put("CD_ACCT",  	"45510");  //면책금 - 차량수리정비비(45510)	- 20211116 회계계정 변경 
 ...
 //대여료 세금 계산서 발행건 처리 
  if ( a4_s_amt > 0 ) {
 	...
 	//  렌트(리스)사업매출인 경우는 부가세처리
		...
		if ( AddUtil.parseDigit(request.getParameter("dfee_amt_2"))  == 0 ) { 	 //외상매출금 추가 - 상계를 안하면 
...
//  }	
  ...
  if ( a9_s_amt > 0 ) {
  	  ...
  	  // 초과운행 계산서 미리 발행건일 수도 있음 - 이때는 외상매출금 
