...
<!--
	...
	function save(){
		...
		if ( fm.des_zip.value == '' || fm.des_addr.value == '' || fm.des_nm.value == '' || fm.des_tel.value == ''   ) { 
	  	...
	  	if(fm.cls_st.value == '8'){ //매입옵션 선택시 디스플레이	
			  ...
			  if  ( toInt(fm.fee_size.value) < 2 ) { //연장이 아니면 	 2개월					
			...
			// 중도매입옵션 결재
		 	if (  toInt(parseDigit(fm.old_opt_amt.value)) >   0  ) { 
				...
				if (  toInt(fm.r_mon.value)  <  12 ) {		
					 	alert("중도매입옵션 최소이용기간이 1년이상입니다..!! 기존(수기)방식으로 진행하세요.!!");
						return;					
				}
		   } 	
	...
	function cls_display(){
		...
		if(fm.cls_st.options[fm.cls_st.selectedIndex].value == '8'){ //매입옵션 선택시 디스플레이
			...
			//해지일이 계약만료일 2개월전 경우 매입옵션 행사 불가 - 중도매입옵션(대여개시후 1년 경과) 허용 변경
			if ( count > 61 ) {  
...
<input type='hidden' name='use_oe_dt' value='<%=base1.get("USE_OE_DT")%>'>  <!--  임의연장제외 마지막 스케쥴날짜  -->
...
<input type='hidden' name='today_dt' value='<%=AddUtil.getDate(4)%>'>
  ...
  <table border='0' cellspacing='0' cellpadding='0' width='100%'>
   ...
   <%} %>  
    ...
    <tr>
           	<td>&nbsp;<font color="#FF0000">***(중요)미리 기안하는 경우 연체이자가 계산될 수 있으니 반드시 대여료 스케쥴 확인하여 연체이자를 정리해주세요.!!! </font></td> 
    </tr>
 	 	  ...
 	 	  <tr>
		      ...
		      <td class='line'> 
		        <table border="0" cellspacing="1" cellpadding="0" width=100%>          	
		             ...
		             <td colspan=2 width="26%">&nbsp;<input type='text' name='opt_amt'size='13' class='num' readonly value="<%=AddUtil.parseDecimal(s_opt_amt)%>" onBlur='javascript:this.value=parseDecimal(this.value); set_sui_c_amt();'>&nbsp;(VAT포함)</td> 
          ...
          <table border="0" cellspacing="1" cellpadding="0" width=100%>
            ...
            <tr> 
              ...
              <td align="center" class="title"> 
                ...
                <input type='hidden' name='tax_g' size='20' class='text' value=''>
              <td class="title">&nbsp;
              <font color="#FF0000">*</font>보험가입면책금:&nbsp;<input type='text' class='num' name='car_ja' size='7' value='<%=AddUtil.parseDecimal(base.getCar_ja())%>' readonly ><input type='hidden' name='tax_chk3' value='N' ><input type='hidden' name='tax_chk3' value='N' ><!--계산서발행의뢰--></td>
            </tr>
               <tr> 
       		  ...
       		  <tr>
                    ...
                    <td width=12% >&nbsp;<input type='text' name='fdft_amt2'  size='15' class='num' readonly  ></td>
	                    ...
	                    <% } else { %>                  
	                 	...
	                 	<td>&nbsp;※ 미납입금액계 - 환불금액계</td>    
	                 	<td colspan=4 width=30%>&nbsp;<input type='checkbox' name='cms_after' value='Y' ><font color="red">CMS 확인후 처리</font></td>   	                    	
	                    <% } %>	  
              </tr>
