...
<!--	
	...
	function save(){
		...
		//출고전해지(신차),  개시전해지(재리스) 인경우 지점은 지점장 결재 득한후 처리가능		
		if( fm.cls_st.value == '7' || fm.cls_st.value == '10' ){				
			if( fm.br_id.value == 'B1'   ||   fm.br_id.value == 'D1'   ||   fm.br_id.value == 'G1'   ||   fm.br_id.value == 'J1'   ) {			
			       if ( fm.sb_saction_id.value == '' ) {
						alert("출고전해지(신차), 개시전해지(재리스)인 경우 지점은 지점장 결재필수입니다. 지점장결재자를 지정하셔야 합니다..!!");
						return;
			        }	
			}	
		}
...
//		}	
		...
		if ( fm.tax_reg_gu[1].checked == true ) {
			...
			if ( toInt(parseDigit(fm.rifee_s_amt.value))  > 0 ) {
				alert("개시대여료잔액이 있습니다. 항목별통합발행(1장)을 할 수 없습니다..!!");
				return;
		 	}
		 	
		 	if ( toInt(parseDigit(fm.rfee_s_amt.value))  > 0 ) {
				alert("선납금잔액이 있습니다. 항목별통합발행(1장)을 할 수 없습니다..!!");
				return;
		 	}		
		}
		...
		if ( toInt(parseDigit(fm.fdft_amt2.value))  < 0 ) {
		    ...
		    if ( fm.re_acc_nm.value  == 'null' ||   fm.re_acc_nm.value  == '') { 
		 	...
		 	if ( fm.refund_st[0].checked == false && fm.refund_st[1].checked == false  ){  //정산구분
					alert("환불구분을 선택하세요.!!");
					return;
			}		    	  
			...
			if ( fm.jung_st[0].checked == false && fm.jung_st[1].checked == false  ){  //정산구분
				alert("정산구분을 선택하세요.!!");
				return;
			}
...
//			fm.target='ii_no';
	...
	function cls_display(){
		...
		tr_ret.style.display		= 'block';	//차량회수	
		tr_gur.style.display		= 'block';	//채권관계
		tr_cre.style.display		= 'block';	//잔존채권처리
		...
		if ( fm.cls_st.value == '7' || fm.cls_st.value == '10' ) { //출고전해지(신차) , 개시전해지(재리스)
			...
			if (fm.cls_st.value == '7' ) {
				...
				tr_dae.style.display		= 'block';	//출고전대차	
			...
			if(fm.cancel_yn.options[fm.cancel_yn.selectedIndex].value == 'N'){
				...
				td_cancel_n.style.display 		= 'block';  //매출유지
...
//		}else if(obj == fm.over_amt_1){ //초과운행부담금 감액		 
	...
	//	}		
		...
		fm.cls_v_amt.value 			= parseDecimal( toInt(parseDigit(fm.no_v_amt_1.value)) );	
				...
				//돌려줄 금액이 있다면
		if ( toInt(parseDigit(fm.fdft_amt2.value)) < 0 ) {
			 tr_refund.style.display		= 'block';	//환불정보
			 tr_scd_ext.style.display		= 'block';	//잔존해지정산금
		} else {
			 tr_refund.style.display		= 'none';	//환불정보
	...
	function set_cls_s_amt(){
		...
		fm.d_amt.value 				= parseDecimal( toInt(parseDigit(fm.ex_di_amt.value)) + toInt(parseDigit(fm.nfee_amt.value)) + toInt(parseDigit(fm.dly_amt.value)));
	  	fm.d_amt_1.value 			= parseDecimal( toInt(parseDigit(fm.ex_di_amt_1.value)) + toInt(parseDigit(fm.nfee_amt_1.value)) + toInt(parseDigit(fm.dly_amt_1.value)));  //대여료관련
		...
		if ( toInt(parseDigit(fm.fdft_amt2.value)) < 0 ) {
			 tr_refund.style.display		= 'block';	//환불정보
			 tr_scd_ext.style.display		= 'block';	//잔존 해지정산금
		} else {
			 tr_refund.style.display		= 'none';	//환불정보
...
<input type='hidden' name='dly_e_dt' value='<%=base1.get("DLY_E_DT")%>'> <!-- 연체중 가장큰날짜 -->
...
<input type='hidden' name='df_e_dt' value='<%=base1.get("DF_E_DT")%>'> <!-- 해지일포함된 스케쥴 연체종료일 --> 
<input type='hidden' name='df_s_amt' value='<%=AddUtil.parseDecimal(AddUtil.parseInt((String)base1.get("DF_S_AMT")))%>'> <!-- 해지일포함된 스케쥴 연체금액 (잔액아님) --> 
...
<input type='hidden' name='taecha_st_dt' value='<%=taecha_st_dt%>'><!--만기매칭대차 시작일 -->
  ...
  <table border='0' cellspacing='0' cellpadding='0' width='100%'>
	  ...
	  <td class=line>
        <table border="0" cellspacing="1" cellpadding="0" width=100%>		  
          <tr>
            ...
            <td style="font-size : 8pt;" width="6%" class=title rowspan="2">계약기간</td>
          ...
          <tr> 
            ...
            <td colspan="5">&nbsp;
			  <textarea name="cls_cau" cols="100" class="text" style="IME-MODE: active" rows="3"></textarea> 
               ...
               <td width="12%" class=title ><font color=red>만기매칭<br>대차여부</font></td>
     	    <td>&nbsp; 
		  <select name="match" >
                <option value="">--선택--</option>
                <option value="Y">만기매칭</option>
              </select>
              </td>
          </tr>
          <tr>                                                      
           ...
           <td colspan="2" class='line'> 
		          <table border="0" cellspacing="1" cellpadding="0" width=100%>
		            <tr>
	                    ...
	                    <% } else { %>                  
	                  	<td class=title width=12% ><input type='checkbox' name='cms_chk' value='Y' >CMS인출의뢰</td>
	              ...
	              <tr> 
			            ...
			            <td colspan=5>&nbsp;<input type="checkbox" name="delay_st" value="Y" >정산후잔액환불 유보(선택)
	               ...
	               <tr> 			          
			            <td colspan=5>&nbsp;
	                      &nbsp;&nbsp;&nbsp;&nbsp;<font color=red><b>유보할 사유 구체적 내용: </b></font> <input type="text" name="delay_desc"  size='120' class='text'>
	                  </td>
	               </tr>     
    ...
    <tr>
        <td>&nbsp;<font color="#FF0000">***</font> 중도해지 위약금의 감액 관련해서는 사전에 김진좌팀장의 결재를 득해야 합니다. 이경우 중도해지위약금 감액 결재자 필수항목입니다.</td>
    </tr>
<!--
       <tr>
        <td>&nbsp;<font color="#FF0000">***</font> 선수금 후불처리 관련해서는 사전에 안보국팀장의 결재를 득해야 합니다.</td>
    </tr> -->
     ...
     <tr></tr><tr></tr><tr></tr>
      ...
      <!-- 초과운행부담금에 한함  block none-->
 	  ...
 	  <td> 
    	<table border="0" cellspacing="0" cellpadding="0" width=100%>
          <tr> 
 	 		 <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>초과운행추가대여료</span></td>
 	 	  </tr>
 	 	  <tr>
      		 <td class=line2></td>
   		  </tr>	
...
<!--
	...
	function set_init(){
 	...
 	// 	alert(fm.r_day.value);
		...
		//초과운행부담금	
		if ( <%= o_amt%> > 0 ) {
			tr_over.style.display 		= 'block';  //초과운행부담금
	  ...
	  //개시대여료가 없는 경우 		 	 
	    if(fm.ifee_s_amt.value == '0' ) {
			   ...
			   if ( toInt(fm.rent_end_dt.value) >= toInt(fm.use_s_dt.value)) { //만기이후 대여료 스케쥴이 생성이 안된 경우 
					 if ( toInt(fm.rent_end_dt.value) <= toInt(fm.dly_s_dt.value) ) { //만기후 해지시  미납분이 없는 경우
					//   	 alert(" 만기후 대여료스케쥴이 생성 안됨, 만기일 이전 미납분이 없는 경우");
					 ...
					 //  alert(" 만기후 대여료스케쥴이 생성 안됨, 만기일 이전 해지되는 경우");
				   	    	//미납잔액이 있는 경우      	
				   	   ...
				   	   // 	      alert(toInt(parseDigit(fm.ex_di_amt.value)));
					   	    	   if  ( fm.nnfee_s_amt.value  == '0' && fm.r_con_mon.value == '0') {
					   	    	        ...
					   	    	        //20120102
					   	    	         if  ( toInt(fm.df_e_dt.value) >= toInt(replaceString("-","",fm.cls_dt.value)) ) { 	//해지일포함된 스케쥴 연체종료일과 해지일이 같다면	
					   	    	               fm.nfee_mon.value      =   '0';
					   	    	               fm.nfee_day.value 	=   '0';	
			      ...
			      if ( toInt(fm.use_e_dt.value) <= toInt(fm.dly_s_dt.value) ) { //만기일 이전에 미납분이 없는 경우
			   	    	  ...
			   	    	  //     fm.nfee_mon.value 	= 	toInt(fm.s_mon.value) - 1;  // 잔액이 발생되었기에 1달 빼줌 
			   	    	 	 ...
			   	    	 	 if ( toInt(fm.nfee_mon.value) < 0 ) {
				   	    	     ...
				   	    	     if (  toInt(parseDigit(fm.ex_s_amt.value)) > 0 ){
			   	    	      		   ...
			   	    	      		   if  ( fm.nnfee_s_amt.value  == '0' ) { //마지막 잔액이 아닌 원래 스케쥴금액	
				   	    	      		            if ( fm.r_con_mon.value == '0') { //마지막 스케쥴이면서 잔액이 아닌경우
								   	    	    	    	     //20130104
								   	    	         if  ( toInt(fm.use_e_dt.value) == toInt(replaceString("-","",fm.cls_dt.value)) ) { 	//해지일포함된 스케쥴 연체종료일과 해지일이 같다면	
								   	    	               fm.nfee_mon.value      =   '0';
								   	    	               fm.nfee_day.value 	=   '0';				   	    	        
								   	    	         } else { 
									   	     ...
									   	     //               fm.nfee_mon.value 	= 	toInt(fm.r_mon.value) - toInt(fm.con_mon.value);  //경과일수 표시
												  ...
												  count1 = Math.floor(diff1_date/(24*60*60*1000));
					   	    	  	 	 	 ...
					   	    	  	 	 	 if ( fm.df_e_dt.value == '99999999' ) {  //20140626 수정 임의연장후 잔액이 있는 경우
 	...
 	//	alert("ccc"); 
	//	alert(toInt(parseDigit(fm.ex_di_amt.value)));   //과입금되는경우 체크 
//	   alert("개시 end mon =" + fm.nfee_mon.value );	
//	  alert("개시 end day=" + fm.nfee_day.value );	
		...
		//            대여료 부가세 계산					 
			 ...
			 //미납이 있고 , 1개월이상 미납이 또 생긴 경우  -  받을돈에서 받은돈   -- 20170914
	...
	//		alert("b = " + fm.nfee_amt.value );
		...
		//2015-03-09 추가 - 일부선납되면서 미납된 경우 다시 계산				
		if (  toInt(parseDigit(fm.di_amt.value)) > 0 && fm.nfee_mon.value == '0' ) {		
		   if ( toInt(parseDigit(fm.hs_mon.value)) < 1 ) {  //과부족이 보이면서 하단의 미납대여료도 중복으로 나오는 경우 - 20170324
				...
				//	 fm.ex_di_v_amt.value  =  0;	
				 //   fm.nfee_amt.value  = parseDecimal( toInt(parseDigit(fm.rr_s_amt.value))  -    toInt(parseDigit(fm.rc_s_amt.value)) );  //받을금액  - 받은금액
				 		//     alert("일부 선납이 있습니다. 미납금액을 다시 확인해주세요!!!." );		
			//		alert("cal rr_s_amt = " + fm.rr_s_amt.value );		
			//		alert("cal rc_s_amt = " + fm.rc_s_amt.value );		
			//	 	alert("c = " + fm.nfee_amt.value );	
				    alert(" 미납금액을 다시 확인해주세요!!!." );		  
				}    
		  } 
