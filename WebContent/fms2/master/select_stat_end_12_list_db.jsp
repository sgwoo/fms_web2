...
<%
	...
	Vector vt = ad_db.getSelectStatEndCont12ListDB(save_dt);
	int vt_size = vt.size();
	
	long sum_amt[] = new long[11];
	long h_sum_amt[] = new long[11];
...
<body>
<table border="1" cellspacing="0" cellpadding="0" width=1160>
	<tr>
	  <td colspan="14" align="center">○ 신규영업 중단하고 대여사업 계속시 현재가치</td>
	</tr>		
	<tr>
	  <td colspan="14" align="right">단위:백만원, 기준일자(<%=save_dt%>)</td>
	</tr>			
	<tr>
	  <td rowspan='2' colspan='3' class="title">구분</td>
	  <td colspan='3' class="title">차량대수</td>
	  <td colspan='3' class="title">부채</td>
	  <td colspan='4' class="title">자산</td>
	  <td rowspan='2' width="80" class="title">자산-부채</td>
	</tr>
	<tr>
	  <td width="80" class="title">상환중</td>
	  <td width="80" class="title">상환완료</td>
	  <td width="80" class="title">합계</td>
	  <td width="80" class="title">대출잔액</td>
	  <td width="80" class="title">보증금<br>현재가치</td>	  
	  <td width="80" class="title">합계</td>
	  <td width="80" class="title">만기시잔가<br>현재가치</td>
	  <td width="80" class="title">대여료<br>현재가치</td>
	  <td width="80" class="title">비용<br>현재가치</td>
	  <td width="80" class="title">합계</td>	  
	</tr>
	<%	int etc_count = 0;
			for(int i = 0 ; i < vt_size ; i++){
	...
	<%			
					...
					sum_amt[5] = sum_amt[5] + AddUtil.parseLong(String.valueOf(ht.get("D_AMT")));
					sum_amt[6] = sum_amt[6] + AddUtil.parseLong(String.valueOf(ht.get("A_CAR_AMT")));
					sum_amt[7] = sum_amt[7] + AddUtil.parseLong(String.valueOf(ht.get("A_FEE_AMT")));
					sum_amt[8] = sum_amt[8] + AddUtil.parseLong(String.valueOf(ht.get("A_PAY_AMT")));
					sum_amt[9] = sum_amt[9] + AddUtil.parseLong(String.valueOf(ht.get("A_AMT")));
					sum_amt[10] = sum_amt[10] + AddUtil.parseLong(String.valueOf(ht.get("C_AMT")));
				}
			}
	%>
	<tr>
