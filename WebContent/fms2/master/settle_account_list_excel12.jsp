...
response.setHeader("Content-Disposition", "inline; filename=settle_account_list_excel12.xls");
%>
...
<%
	...
	Vector vt = ad_db.getSettleAccount_list12(settle_year);
	int vt_size = vt.size();
...
<form name='form1' action='' method='post'>
  ...
  <table border="0" cellspacing="1" cellpadding="0" width=<%=table_width%>>
	<tr>
	  <td align="center"><%=AddUtil.parseInt(settle_year)%>년 계약 선납금 년도별 금액 (선수대여료스케줄)</td>
	</tr>		
	<tr>
	  <td>&nbsp;</td>	  
    </tr>		
		...
		<td class=line>
			<table border=0 cellspacing=1 width=100%>
			    <tr>
				  ...
				  <td colspan='3' class="title">약정선납금</td>
				  <td colspan='6' class="title">년도별 선납금(공급가)</td>
			    </tr>
			    <tr>
			    ...
			    <%	for(int i = 0 ; i < vt_size ; i++){
					...
					total_amt2	= total_amt2 + AddUtil.parseLong(String.valueOf(ht.get("PP_V_AMT")));
					total_amt3	= total_amt3 + AddUtil.parseLong(String.valueOf(ht.get("PP_AMT")));
