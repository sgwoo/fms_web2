...
response.setHeader("Content-Disposition", "inline; filename=settle_account_list_excel10.xls");
%>
...
<%
	...
	Vector vt = ad_db.getSettleAccount_list10(settle_year, "rent_l_cd");
	int vt_size = vt.size();
...
<form name='form1' action='' method='post'>
  ...
  <table border="0" cellspacing="1" cellpadding="0" width=<%=table_width%>>
	<tr>
	  <td align="center"><%=AddUtil.parseInt(settle_year)%>년 계약개시대여료리스트 (연도별 만기리스트)</td>
	</tr>		
	<tr>
	  <td>&nbsp;</td>	  
    </tr>		
		...
		<td class=line>
			<table border=0 cellspacing=1 width=100%>
			    <tr>
				  ...
				  <td width="110" class="title">공급가</td>
				  <td width="110" class="title">부가세포함액</td>
			    ...
			    <%	for(int i = 0 ; i < vt_size ; i++){
					...
					total_amt1	= total_amt1 + AddUtil.parseLong(String.valueOf(ht.get("IFEE_AMT")));
					...
					total_amt4	= total_amt4 + AddUtil.parseLong(String.valueOf(ht.get("IFEE_S_AMT")));
					
					for(int j = 0 ; j < 8 ; j++){
						if( AddUtil.parseInt(String.valueOf(ht.get("E_YEAR"))) - AddUtil.parseInt(settle_year) == j ){
							total_amt[j]	= total_amt[j] + AddUtil.parseLong(String.valueOf(ht.get("IFEE_S_AMT")));
						}
					}
					
					if( AddUtil.parseInt(String.valueOf(ht.get("E_YEAR"))) == 0 ){
							total_amt[9]	= total_amt[9] + AddUtil.parseLong(String.valueOf(ht.get("IFEE_S_AMT")));
					}
				%>
			    <tr>
		...
		<td class=line>
			<table border=0 cellspacing=1 width=100%>
			    ...
			    <tr>
				  ...
				  <td align="right"><%=Util.parseDecimal(total_amt4)%></td>				  
			    </tr>	
			</table>
		</td>
	</tr>			    
