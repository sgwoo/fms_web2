...
String[] c_id = request.getParameterValues("c_id[]");
	...
	if(c_id.length > 0){
		...
		int actn_num = ad_db.getAucNewSeq();
		...
		String file_name = "FMS 신차계약차량리스트 "+fm_start_dt+" ~ "+fm_end_dt;
	
		for(String car_mng_id : c_id){
			//car_mng_id 값 없으면 연번을 가져오게 되어있어서 5글자 이상되는 글자만 허용되게끔 변경
			if(car_mng_id.length() > 5){
			
				Hashtable ht = ad_db.getInsideReq08_3(car_mng_id);
				AucBean auc = new AucBean();
				
				if(ht.get("INIT_REG_DT") !=null){
					auc.setActn_dt(format.parse(String.valueOf(ht.get("INIT_REG_DT"))) );
				}
				auc.setActn_off_nm 	(String.valueOf(ht.get("ACTN_OFF_NM"))                  );
				if(ht.get("ACTN_CNT") !=null){
