...
<%
	...
	for(int i=start_row ; i <= value_line ; i++){
		...
		String mng_id2			= "";
		if(col_size>2){
			mng_id2 = value2[i]  ==null?"":AddUtil.replace(value2[i],"'","");
		}
		...
		if(mng2_bean.getUse_yn().equals("Y")){
			...
			ad_db.insertConvBusid2Case_New(rent_l_cd, mng_nm2, user_id);
		
			if(sms_ok.equals("Y")){
		
				if(mng_id2.equals("")){
					mng_id2 = mng2_bean.getUser_id();
				}
				
				out.println("mng_id2="+mng_id2+"");
		
				//계약 담당자 일괄변경시 문자발송.
				UsersBean target_bean 	= umd.getUsersBean(mng_id2);
				...
				String cont_sms 	= String.valueOf(sms.get("CAR_NO")) + " 차량 담당자는 " + target_bean.getUser_nm() + ", 연락처는 "+ target_bean.getUser_m_tel() + " 입니다. (주)아마존카"; 
				...
				if(!s_destphone.equals("") && !s_destphone.equals("null") ){
					...
					System.out.println("영업/관리 담당자배정="+cont_sms);	
				}
			}
		}
		out.println("<br>");
	...
	//담당자배정프로시저호출----------------------------------------------------------------------------
	String  d_flag1 =  ad_db.call_sp_conv_busidmngid_excel();
	System.out.println("영업/관리 담당자배정프로시저호출");
	//--------------------------------------------------------------------------------------------------	
