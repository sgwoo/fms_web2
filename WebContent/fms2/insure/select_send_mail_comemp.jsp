...
<% 
	...
	for(int i=0; i < vid.length; i++){
		...
		if(client_id.length()==6  && client.getCon_agnt_email().length() > 8 && client.getCon_agnt_email().indexOf(".")>0 && client.getCon_agnt_email().indexOf("@")>0 ){
			...
			//중복체크
			int overchk = ImEmailDb.getFmsInfoMailNotSendChkList(reg_code, "[아마존카] "+client.getFirm_nm()+" 님 임직원 전용 자동차 보험 가입 요청서 입니다.", "SSV:"+client.getCon_agnt_email());
			
			if(overchk == 0){
				DmailBean d_bean = new DmailBean();
				d_bean.setSubject			("[아마존카] "+client.getFirm_nm()+" 님 임직원 전용 자동차 보험 가입 요청서 입니다.");
				...
				d_bean.setMsgflag     		(0);
				d_bean.setContent			("http://fms1.amazoncar.co.kr/mailing/ins/ins_c_docs.jsp?client_id="+client_id);
				...
				String add_fileinfo 	= "업무용승용차 관련비용의 손비처리 안내문.pdf";
				String add_content 	= "https://fms3.amazoncar.co.kr/doc/amazoncar_comemp_1_doc1.pdf";
