...
<%
	...
	Vector vt = ie_db.getInsComempNotStatList(s_kd, t_wd, gubun1, gubun2,st_dt,end_dt,gubun3);
	int vt_size = vt.size();
%>
...
<!--
	...
	//메일수신하기
	function go_mail(rent_mng_id, rent_l_cd){			
		var SUBWIN="mail_input_comemp.jsp?rent_mng_id="+rent_mng_id+"&rent_l_cd="+rent_l_cd;
...
<table border="0" cellspacing="0" cellpadding="0" width='1600'>
	...
	<td class='line' width='320' id='td_con' style='position:relative;'>
		...
		<tr>
		    ...
		    <td width='40' align=center><input type="checkbox" name="ch_l_cd" value="<%=ht.get("CLIENT_ID")%>"></td>
	...
	<td class='line' width='1280'>
		...
		<tr>
		    ...
		    <td width='80' align='center'><span title='<%=ht.get("FIRM_EMP_NM")%>'>
		    ...
		    <td width='100' align='center'><a href='/fms2/lc_rent/newcar_doc_ins.jsp?client_id=<%=ht.get("CLIENT_ID")%>&user_id=<%=ht.get("BUS_ID2")%>&rent_l_cd=<%=ht.get("RENT_L_CD")%>' target="_blank"><img src=/acar/images/center/button_in_see.gif align=absmiddle border=0></a></td>
		    <td width='70' align='center'><a href="javascript:go_mail('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>')" title='개별 메일 발송'><img src=/acar/images/center/button_in_email.gif align=absmiddle border=0></a></td>
		</tr>
<%
		}
%>
