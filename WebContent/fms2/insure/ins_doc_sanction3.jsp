...
<%
	...
	if(doc_bit.equals("4")){
		...
		if( con_mon<4 ){
			...
			String cont 		= "["+car_no+" ,  "+cont_info.get("FIRM_NM")+"] 보험변경이 되어 정상요금 재계산을 요청합니다. 월대여료 "+Util.parseDecimal(d_bean.getO_fee_amt())+"원에서 "+Util.parseDecimal(d_bean.getN_fee_amt())+"원으로 변경 (월반영금액 "+Util.parseDecimal(d_bean.getD_fee_amt())+"원)";
