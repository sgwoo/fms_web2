...
// 	System.out.println(client_st);
	...
	String cool_url = "/fms2/insure/ins_doc_u.jsp?rent_mng_id="+rent_mng_id+"|rent_l_cd="+rent_l_cd+"|doc_no="+doc_no+"|c_id="+c_id+"|ins_st="+ins_st+"|ins_doc_no="+ins_doc_no;
		...
		if(doc_bit == 'amt_set'){
			...
			if(doc_bit == '1' && fm.scan_chk_yn.value=='Y' ){
				...
				<%if(!nm_db.getWorkAuthUser("전산팀",user_id)){%>		
				  if(fm.scan_yn.value == 'N'){	alert('보험변경요청서 스캔을 하십시오. 만약 스캔등록을 하였다면 새로고침을 한후 결재하시기 바랍니다.');	return;	}
				  <%}%>
				}
			<%}%>
...
//				fm.target='_blank';
	...
	function send_cool(send_id,target_id,cool_car_no,cool_url){
		window.open("/fms2/insure/send_coolmsg.jsp?send_id="+send_id+"&target_id="+target_id+"&cool_car_no="+cool_car_no+"&url="+cool_url, "", "left=100, top=10, width=720, height=800, scrollbars=yes");
	}
...
<table border='0' cellspacing='0' cellpadding='0' width='100%'>
    ...
    <tr> 
		...
		%>
			    ...
			    <option value="12" <%if(cha.getCh_item().equals("12")){%>selected<%}%>>자기신체사고가입금액(부상)</option>
                	    ...
                	    <option value="18" <%if(cha.getCh_item().equals("18")){%>selected<%}%>>견인고리</option>
        	        ...
        	        <input type="hidden" id="ch_item2" name="ch_item2" class="ch_item2" value="<%=cha.getCh_item()%>"/>			
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        <input type='hidden' size='40' name='ch_after2' value='<%=cha.getCh_after()%>'>
                    </td>
                </tr>		  
                <%}%>
            </table>
    ...
    <tr> 
        <td class='line'> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>            
                ...
                <tr> 
                    <td width='10%' class='title'>적용기간</td>
                    <td>&nbsp;<input type='text' name='cng_s_dt' size='11' value='<%=AddUtil.ChangeDate2(cng_doc.getCh_s_dt())%>' class='text' onBlur='javascript: this.value = ChangeDate(this.value);'> ~ <input type='text' name='cng_e_dt' size='11' value='<%=AddUtil.ChangeDate2(cng_doc.getCh_e_dt())%>' class='text' onBlur='javascript: this.value = ChangeDate(this.value);'> </td>
                </tr>                
                <tr> 
			...
			<a style="margin-left:100px;" href="javascript:view_scan()" onMouseOver="window.status=''; return true" title="클릭하세요"><img src=/acar/images/center/button_scan.gif align=absmiddle border=0></a>
			&nbsp;&nbsp;&nbsp;<a href="javascript:send_cool('<%=user_id%>','<%=doc.getUser_id1()%>','<%=cr_bean.getCar_no()%>','<%=cool_url%>')"><img src=/acar/images/center/button_in_msg.gif align=absmiddle></a>
		    </td>
                </tr>		
    ...
    <tr>
	<td>* 적용기간 종료 3일전에는 담당자(기안자)에게 종료예정임을 사내메신저로 통보할 예정입니다. 담당자는 변경내용의 연장 여부 등을 반드시 보험관리담당자에게 문서로 제출해야만 합니다.</td>
    </tr>		    
    <tr>
        <td>&nbsp;</td>
    </tr>
