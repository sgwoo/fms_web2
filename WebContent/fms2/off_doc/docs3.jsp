    	...
    	<td colspan=10>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>자료실 > <span class=style5>본사/지점 약도</span></span></td>
                ...
                <tr>
                	<td colspan="3 "align='right' >
                		...
                		<%//if(nm_db.getWorkAuthUser("임원",user_id) || nm_db.getWorkAuthUser("전산팀",user_id)  || user_id.equals("000057")){%>		
	  ...
	  <td class='line'>
		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
		  <tr>
			...
			<td class='title' width='700'>용도명</td>
			<td class='title' width='100'>종류</td>	
			<%if(nm_db.getWorkAuthUser("임원",user_id) || nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("자료실관리",user_id)  || nm_db.getWorkAuthUser("약도이미지관리",user_id)){%>			
		  ...
		  <%
       	...
       	for(int k=0;k<attach_vt_size;k++){
        	 ...
        	 ht_string = ht_string.substring(0,2);
      ...
      Iterator<String> iteratorKey = tm.descendingKeySet().iterator();
		  		  ...
		  		  String map_key = iteratorKey.next();
		  		 			 String[] map_result =  map.get(map_key).split(",");
		  		 					if(map_key.equals("여의")){%>
		  		 					 <tr>
		  		 					 		<td align="center" rowspan="<%=map_result.length%>">여의도본사</td>
														...
														sfile_type = sfile_type.substring(sfile_type.lastIndexOf(".")+1,sfile_type.length());%>
															<%if(ht.get("FILE_TYPE").equals("image/jpeg")||ht.get("FILE_TYPE").equals("image/pjpeg")||ht.get("FILE_TYPE").equals("application/pdf")){%>			
														 		<td align="center">
														 			...
														 			<a href="javascript:openPopF('<%=ht.get("FILE_TYPE")%>','<%=ht.get("SEQ")%>');" title='보기' ><img src="/acar/images/center/icon_<%=sfile_type%>.gif" align=absmiddle border="0"></a></td>
														 <%}else{%>
														 	<td align="center">
														 		<input type="checkbox" name="ch_l_cd" value="<%=ht.get("SEQ")%>" >
														 		<a href="https://fms3.amazoncar.co.kr/fms2/attach/download.jsp?SEQ=<%=ht.get("SEQ")%>" target='_blank'><img src="/acar/images/center/icon_<%=sfile_type%>.gif" align=absmiddle border="0"></a></td>
														 <%}%>
														 			<%if(nm_db.getWorkAuthUser("임원",user_id) || nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("자료실관리",user_id)  || nm_db.getWorkAuthUser("약도이미지관리",user_id)){%>		
														 <td align="center">
					             ...
					             <%
						           		...
						           		%>
		  		 					 <tr>
		  		 					 		<td align="center" rowspan="<%=map_result.length%>"><%=map_key%>지점</td>
		  		 							<%for(int k=0;k<map_result.length;k++){
		  		 								Hashtable ht = (Hashtable)attach_vt.elementAt(Integer.parseInt(map_result[k]));
             					   		if(k!=0){%>
             					   			<tr>
