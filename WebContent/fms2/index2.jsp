...
<%
	...
	*/
	...
	//접속IP가 허용된 IP인지 체크하여 카운트 
	int ipMatch_count = 0;
	for (int j = 0; j < access_ip.length; j++) {
		if (access_ip[j].contains(request.getRemoteAddr())) {
			ipMatch_count++;
			//System.out.println(access_ip[j]);
		}
	}
	//System.out.println("ipMatch_count >>> " + ipMatch_count);
		...
		if ((name.length() > 0) && (passwd.length() > 0)) {
			...
			if (result == 1) {
			    ...
			    if ( passwd.length() < 8  && !dept_id.equals("8888") && !dept_id.equals("1000") ) {
					...
					//세무소감사용 - 000203 :2022209
					if (nm_db.getWorkAuthUser("전산팀", user_id) || ipMatch_count > 0 || user_id.equals("000203") ) {
						...
						if (!user_id.equals("")) {
							...
							if (chk_count == 0) {
								...
								System.out.println("[로그인 FMS index2 ] DT:"+login_time+", ID:"+user_id+"("+user_nm+")");
							}
...
<div id="dialog">
	...
	접속 URL이 변경 되었습니다.<br>
    https://off1.amazoncar.co.kr:8443/off_web/index.jsp<br>
	또는<br>
    https://off2.amazoncar.co.kr:8443/off_web/index.jsp<br>
	으로 접속 부탁드립니다.
    </label>
</div>
...
<%if (login_yn.equals("no_access")) {%>
	alert("전산팀을 제외한 직원은 신규 로그인화면에서 로그인 해주세요.");
	location.replace('/fms2/index.jsp')
<%}%>

<% //if (ipMatch_count <= 0) {%>
//	location.replace('http://fms1.amazoncar.co.kr/fms2/index.jsp');
<%// }%>
</script>
</body>
</html>
