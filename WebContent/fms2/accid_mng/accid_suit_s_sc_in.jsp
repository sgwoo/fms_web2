...
<%
	...
	Vector accids = as_db.getAccidSuitList(br_id, gubun1, gubun2, gubun3, gubun4, gubun5, gubun6, brch_id, st_dt, end_dt, s_kd, t_wd, sort, asc, s_st);
					...
					<div>
						<table class="inner_top_table table_layout" style="height: 60px;">						  	
			       			<tr> 								
	                            ...
	                            <td width='130' class='title title_border'>사고일시</td>
		...
		<table class="tb">
			  ...
			  <%	if(accid_size > 0){%> 
			   <% 		for (int i = 0 ; i < accid_size ; i++){
		                 ...
		                 <tr style="height: 25px;">  
		                   ...
		                   <td <%if(accid.get("USE_YN").equals("N")){%>class='is center content_border'<%}%> width='40' class='center content_border'><a class=index1 href="javascript:parent.AccidentDisp('<%=accid.get("RENT_MNG_ID")%>', '<%=accid.get("RENT_L_CD")%>', '<%=accid.get("CAR_MNG_ID")%>', '<%=accid.get("ACCID_ID")%>', '<%=accid.get("ACCID_ST")%>', '<%=i%>', '14')"><img src="/acar/images/center/button_in_see.gif" align="absmiddle" border="0"></a></td>
		                   <td <%if(accid.get("USE_YN").equals("N")){%>class='is center content_border'<%}%> width='90' class='center content_border'><%=accid.get("SUIT_NM")%>
		                     <%if (!String.valueOf(accid.get("SUIT_NM")).equals("민사불가") && !String.valueOf(accid.get("MEAN_DT")).equals("")  ) { %>
		                  	   (종결)                         
		                   <%} %>      
		                   </td>
		                   <td <%if(accid.get("USE_YN").equals("N")){%>class='is center content_border'<%}%> width='70' class='center content_border'><%=accid.get("ACCID_ST_NM")%>                              
		                   </td>
			...
			%>
	                   <tr style="height: 25px;"> 
	                     ...
	                     <td <%if(accid.get("USE_YN").equals("N")){%>class='is center content_border'<%}%> width='130' class='center content_border'><%=AddUtil.ChangeDate3(String.valueOf(accid.get("ACCID_DT")))%></td>
			 ...
			 <%} else  {%>  
			       	<tr>
				       <td width="900" colspan="7" class='center content_border'>&nbsp;</td>
				    </tr>	              
		 	  <%}	%>
		 	   </table>
	  	 	 </div>
