...
/**********************************************************************************
    이부분에 로그파일 경로를 수정해주세요.*/
		...
		String log_path = request.getRealPath("/file/");
/**********************************************************************************/
		...
		try
		{
			...
			//아마존카 소스 추가----------------------------------------------------------------------------
			String req_resultmsg = "transSeq="+transSeq+"||goodsName="+goodsName+"||goodsAmt="+goodsAmt+
					"||buyerName="+buyerName+"||buyerPhoneNo="+buyerPhoneNo+
					"||pgCode="+pgCode+"||pgName="+pgName+
					"||payMethod="+payMethod+"||payMethodName="+payMethodName+
					"||status="+status+"||statusName="+statusName+"||pgResultMsg="+pgResultMsg+
				   	"||pgAppDate="+pgAppDate+"||pgAppTime="+pgAppTime+
				   	"||cardNo="+cardNo+"||approvalAmt="+approvalAmt+"||approvalNo="+approvalNo+"";
			
			System.out.println("[innopayPg_Noti.jsp]\n"+req_resultmsg);
			
			String var1 = request.getParameter("transSeq")==null?"":request.getParameter("transSeq");
			String var2 = request.getParameter("moid")==null?"":request.getParameter("moid");
			String var3 = request.getParameter("status")==null?"":request.getParameter("status");
			String var4 = request.getParameter("approvalNo")==null?"":request.getParameter("approvalNo");
			
			boolean flag1 = true;
			
			if(var3.equals("25") && !var2.equals("")){			
				//아마존카 완료처리 (완료처리, 영업담당자 메신저, 입금담당자 메신저)					
				flag1 = ar_db.updateArsCardReq(var2, req_resultmsg, var3, var1, var4, payMethod, buyerName, goodsName, goodsAmt, cardNo);
			}	
			//아마존카 소스 추가----------------------------------------------------------------------------			
