...
<!--
	...
	function save(){
		window.open("pur_pre_i.jsp<%=vlaus%>", "CHANGE_ITEM", "left=50, top=50, width=1050, height=600, scrollbars=yes, status=yes");
	}	
	...
	function go_setJgCode_pop(){
		window.open("pur_pre_pop.jsp?mode=jg_code", "PUR_PRE_POP", "left=100, top=20, width=600, height=500, scrollbars=auto");
 	}
    ...
    <tr> 
        ...
        <%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("출고관리자",user_id) || nm_db.getWorkAuthUser("고객사업자등록증변경",user_id)){%>
   		...
   		<%if(nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("영업팀내근직",user_id)){%>
    		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    		<input type="button" class="button" value="메세지수신 차종설정" onclick="javascript:go_setJgCode_pop();">
   		<%}%>
    	</td>
...
<script type="text/javascript">
	...
	myGrid.setHeader("연번,출고영업소,자체영업여부,계출번호,요청일시,엔진종류,차명,선택품목,외장색상,내장색상,가니쉬색상,소비자가,출고<br>예정일,1순위<br>예약자,진행구분,예약일,예약만료일,고객상호,고객<br>주소,연락처,메모,계약번호,2순위<br>예약자,출고일,사전계약<br>해지일,계약금,계약금<br>지급일");
	myGrid.setInitWidths("40,90,90,105,130,160,245,340,160,120,120,100,100,60,80,80,100,100,60,100,200,110,60,80,80,100,80");
	myGrid.setColTypes("ron,ro,ro,link,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");
	myGrid.attachHeader("#rspan,#select_filter,#select_filter,#text_filter,#rspan,#select_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#rspan,#rspan,#select_filter,#select_filter,#rspan,#rspan,#text_filter,#rspan,#rspan,#rspan,#text_filter,#select_filter,#rspan,#rspan,#rspan,#rspan");
	myGrid.attachHeader("#rspan,총 건수,#rspan,#cspan,{#stat_count}건,,,,,,,,,,,,,,,,,,,,",["text-align:center;",,"text-align:center;",,,,,,,,,,,,,,,,,,,,,]);
	myGrid.setColAlign("center,center,center,center,center,center,left,left,center,center,center,center,center,center,center,center,center,center,center,center,left,center,center,center,center,center,center");
	myGrid.enableTooltips("false,false,false,false,false,true,true,true,true,true,true,false,false,false,false,false,true,false,false,true,false,false,false,false,false,false,false");
    ...
    // 로드 완료 후 콜백 함수
 	myGrid.load(gridQString, function(){
			...
			var length1 = $(".odd_dhx_web").length;
			var length2 = $(".ev_dhx_web").length;
			
			for(var i=0; i<=length1; i++) {
				// 클래스 부여
				$(".ev_dhx_web").eq(i).children('td:eq(7)').addClass("ev"+i+"");
			    ...
			    // 해당 클래스 텍스트 존재 여부 검사
			    $(".ev"+i+":contains('"+search+"')").each(function () {
			        var regex = new RegExp(search,'gi');
			        var regex2 = new RegExp(search2,'gi');
			        var regex3 = new RegExp(search3,'gi');
			        var regex4 = new RegExp(search4,'gi');
			        var regex5 = new RegExp(search5,'gi');
			        var regex6 = new RegExp(search6,'gi');
			        var regex7 = new RegExp(search7,'gi');
					
					if(search && search2 && search3 && search4 && search5 && search6 && search7) {
						$(this).html( $(this).text().replaceAll(regex, "<span class='txt-hlight'>"+search+"</span>")
					    							.replaceAll(regex2, "<span class='txt-hlight'>"+search2+"</span>")
					    							.replaceAll(regex3, "<span class='txt-hlight'>"+search3+"</span>")
					    							.replaceAll(regex4, "<span class='txt-hlight'>"+search4+"</span>")
					    							.replaceAll(regex5, "<span class='txt-hlight'>"+search5+"</span>")
					    							.replaceAll(regex6, "<span class='txt-hlight'>"+search6+"</span>")
					    							.replaceAll(regex7, "<span class='txt-hlight'>"+search7+"</span>"));
					} else if (search && !search2 && !search3 && !search4 && !search5 && !search6 && !search7) {
						$(this).html( $(this).text().replaceAll(regex, "<span class='txt-hlight'>"+search+"</span>"));
					}  else if (search && search2 && !search3 && !search4 && !search5 && !search6 && !search7) {
						$(this).html( $(this).text().replaceAll(regex, "<span class='txt-hlight'>"+search+"</span>")
    												.replaceAll(regex2, "<span class='txt-hlight'>"+search2+"</span>"));
			...
			for(var i=0; i<=length2; i++) {
				$(".odd_dhx_web").eq(i).children('td:eq(7)').addClass("odd"+i+"");
			    ...
			    $(".odd"+i+":contains('"+search+"')").each(function () {
			        var regex = new RegExp(search,'gi');
			        var regex2 = new RegExp(search2,'gi');
			        var regex3 = new RegExp(search3,'gi');
			        var regex4 = new RegExp(search4,'gi');
