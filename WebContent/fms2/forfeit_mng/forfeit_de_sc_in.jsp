...
<%
	...
	String coll_yn 	= request.getParameter("coll_yn")==null?"":request.getParameter("coll_yn"); //수금건 포함
	...
	Vector fines = afm_db.getFineExpPreProxyList1(br_id, gubun1, gubun2, "2", "8", st_dt, end_dt, s_kd, t_wd, coll_yn, "6", asc, coll_yes);
	int fine_size = fines.size();
	
	String e_dem_dt = AddUtil.getDate(); //현재일
	    ...
	    <td class='line' width='75%'>			
            <table border="0" cellspacing="1" cellpadding="0" width='100%'>
                <tr> 
                    ...
                    <td width=6% class='title'>청구기관</td>
                    <td width=8% class='title'>위반일시</td>
                    <td width=8% class='title'>위반장소</td>
                    <td width=7% class='title'>위반내용</td>
                    <td width=9% class='title'>고지서번호</td>
                    <td width=4% class='title'>납부금액</td>
                    <td width=5% class='title'>납부기한</td>
                    <td width=5% class='title'>납부일자</td>
                    <td width=5% class='title'>청구일자</td>			
                    <td width=5% class='title'>수금일자</td>			
                    <td width=5% class='title'>발행일자</td>
					<td width=8% class='title'>수신일자</td>
    ...
    <tr>
	    <td class='line' width='25%' id='td_con' style='position:relative;'> 
          ...
          <%		for (int i = 0 ; i < fine_size ; i++){
						...
						if (!String.valueOf(fine.get("E_DEM_DT")).equals("") ) {
						    e_dem_dt = String.valueOf(fine.get("E_DEM_DT"));
			...
			%>
                <tr> 
            		<td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='5%' align='center'> 
              				<input type="checkbox" name="ch_l_cd" value="<%=fine.get("RENT_MNG_ID")%>^<%=fine.get("RENT_L_CD")%>^<%=fine.get("CAR_MNG_ID")%>^<%=fine.get("SEQ_NO")%>^<%=fine.get("CUST_NM")%>^<%=fine.get("DEM_DT")%>^<%=e_dem_dt%>^"  >   <!--  <% if (!String.valueOf(fine.get("E_DEM_DT")).equals("")  ) {%>disabled <% } %> -->
            		</td>
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='14%' align='center'><%=i+1%> 
                      <%if(fine.get("USE_YN").equals("N")){%>
                      (해약) 
                      <%}%>
                    </td>
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='11%' align='center'> 
                      <%if(gubun_chk.equals("-")){%>
                      - 
                      <%}else{%>
                       <% if ( !String.valueOf(fine.get("DEM_DT")).equals("")) {%>
                      		<a href="javascript:parent.view_mail('<%=fine.get("RENT_MNG_ID")%>','<%=fine.get("RENT_L_CD")%>','<%=fine.get("CAR_MNG_ID")%>','5','<%=fine.get("SEQ_NO")%>','','<%=fine.get("MNG_ID")%>', '<%=fine.get("DEM_DT")%>', '<%=fine.get("E_DEM_DT")%>', '<%=fine.get("SEQ_NO")%>')" onMouseOver="window.status=''; return true"><img src=/acar/images/center/button_in_see.gif border=0 align=absmiddle></a> 
                      <% } %>
                      <%}%>
                      <% if ( !String.valueOf(fine.get("DEM_DT")).equals("")) {%>
                      	<!--	<a href="javascript:parent.reg_re_mail('<%=fine.get("RENT_MNG_ID")%>','<%=fine.get("RENT_L_CD")%>','<%=fine.get("CAR_MNG_ID")%>','<%=fine.get("SEQ_NO")%>','<%=fine.get("CUST_NM")%>','<%=fine.get("DEM_DT")%>','<%=fine.get("E_DEM_DT")%>')" onMouseOver="window.status=''; return true">.</a> -->
                      <% } %>
                    </td>
        ...
        <!--			<span title='<%=fine.get("FIRM_NM")%>'><a href="javascript:parent.view_client('<%=fine.get("RENT_MNG_ID")%>','<%=fine.get("RENT_L_CD")%>','<%=fine.get("RENT_ST")%>')" onMouseOver="window.status=''; return true"><%=Util.subData(String.valueOf(fine.get("FIRM_NM")), 7)%></a></span>-->
          ...
          <%		}%>
                <tr>                    
                    <td class="title" colspan=2 align='center'></td>		    
                    <td class="title">&nbsp;</td>
                    <td class="title">&nbsp;</td>
	    ...
	    <td class='line' width='75%'>			
          ...
          <%		for (int i = 0 ; i < fine_size ; i++){
			Hashtable fine = (Hashtable)fines.elementAt(i);%>
                <tr> 
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=6% align='center'><span title='<%=fine.get("CAR_NM")%>'><%=Util.subData(String.valueOf(fine.get("CAR_NM")), 6)%></span></td>
                    ...
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=6% align='center'><span title='<%=fine.get("GOV_NM")%>'><%=Util.subData(String.valueOf(fine.get("GOV_NM")), 6)%></span></td>
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=8% align='center'><%=fine.get("VIO_DT")%></td>
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=8% align='center'><span title='<%=fine.get("VIO_PLA")%>'><%=Util.subData(String.valueOf(fine.get("VIO_PLA")), 8)%></span></td>
                    ...
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=9% align='center'><span title='<%=fine.get("PAID_NO")%>'><%=Util.subData(String.valueOf(fine.get("PAID_NO")), 16)%></span></td>
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=4% align='right'><%=Util.parseDecimal(String.valueOf(fine.get("PAID_AMT")))%> 
                    ...
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=5% align='center'><%=fine.get("E_DEM_DT")%></td>
					<td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width=8% align='center'><%if(!fine.get("OTIME").equals("00000000000000") && !fine.get("RD_TIME").equals("--")){%><%=fine.get("RD_TIME")%><%}else if(fine.get("RD_TIME").equals("0000-00-00") && fine.get("OTIME").equals("00000000000000")){%>미수신<%}else{%><%}%></td>
