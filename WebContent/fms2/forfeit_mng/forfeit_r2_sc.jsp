...
<!--
	...
	//	FineInMail(); //이메일까지 자동전송.
		...
		fm.target = "pop";
		fm.action = "forfeit_proxy_i.jsp?gubun=2";
		fm.submit();		
...
//메일발송 - 청구전 발송
	function FineInMail(){
		...
		if(!confirm('과태료 메일을 발행하시겠습니까?')){	return;	}
		fm.action = "forfeit_proxy_mail1_i_a.jsp";
		fm.target = "ii_no";
		fm.submit();
...
<% 	if(auth_rw.equals("2") || auth_rw.equals("4") || auth_rw.equals("6")){%>
  <tr> 
	...
	<!--
	<a href="javascript:FineInMail();"><img src=/acar/images/center/button_bh.gif border=0 align=absmiddle></a>
	-->
    </td>
   </tr>
<%	}%> 
        		...
        		<tr>
        		    <td width=100%>
        			    <table border="0" cellspacing="0" cellpadding="0" width=100%>
        			        <tr>
        				        <td align='center'>
        				        <iframe src="/fms2/forfeit_mng/forfeit_r2_sc_in.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&auth=<%=auth%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>" name="i_no" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 ></iframe>
        				        </td>
        			        </tr>
        			    </table>
        		    </td>
