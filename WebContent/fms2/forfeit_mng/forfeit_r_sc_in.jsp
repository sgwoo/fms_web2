...
<%
	...
	String s_kd = request.getParameter("s_kd")==null?"14":request.getParameter("s_kd");
	...
	Vector fines = afm_db.getFineExpPreList(br_id, gubun1, gubun2, gubun3, gubun4, gubun5, st_dt, end_dt, s_kd, t_wd, sort_gubun, asc);
    ...
    <tr>
	    <td class='line' width='27%' id='td_con' style='position:relative;'> 
          ...
          <%for (int i = 0 ; i < fine_size ; i++){
			...
			if (!String.valueOf(fine.get("E_DEM_DT")).equals("null") ) {
			...
			%>
                <tr> 
            		<td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='4%' align='center'> 
            		  	<input type="checkbox" name="ch_l_cd" value="<%=fine.get("RENT_MNG_ID")%>^<%=fine.get("RENT_L_CD")%>^<%=fine.get("CAR_MNG_ID")%>^<%=fine.get("SEQ_NO")%>^<%=fine.get("CUST_NM")%>^<%if(fine.get("DEM_DT").equals("")){%><%=fine.get("REG_DT")%><%}else{%><%=fine.get("DEM_DT")%><%}%>^<%if(fine.get("E_DEM_DT").equals("")){%><%=fine.get("REG_DT")%><%}else{%><%=fine.get("E_DEM_DT")%><%}%>^" <% if (!String.valueOf(fine.get("PROXY_DT")).equals("")  ) {%> <% } %> > 
            		</td>
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='12%' align='center'><%=i+1%> 
                      ...
                      - 
                      <%}else{%>
                      <a href="javascript:parent.view_mail('<%=fine.get("RENT_MNG_ID")%>','<%=fine.get("RENT_L_CD")%>','<%=fine.get("CAR_MNG_ID")%>','<%=fine.get("SEQ_NO")%>','<%=fine.get("E_DEM_DT")%>')" onMouseOver="window.status=''; return true"><img src=/acar/images/center/button_in_see.gif border=0 align=absmiddle></a> 
                      <%}%>
                    </td>
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='11%' align='center'> 
                      <%if(gubun_chk.equals("-")){%>
                    ...
                    <td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='26%' align='center'>
