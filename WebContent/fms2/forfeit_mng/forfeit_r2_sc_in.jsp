...
<%
	...
	Vector fines = afm_db.getFineExpPreListR2(br_id, gubun1, gubun2, gubun3, gubun4, st_dt, end_dt, s_kd, t_wd, sort_gubun, asc);
	int fine_size = fines.size();
	
	String e_dem_dt = AddUtil.getDate(); 
%>
    ...
    <tr>
	    <td class='line' width='25%' id='td_con' style='position:relative;'> 
          ...
          <%		for (int i = 0 ; i < fine_size ; i++){
			...
			if (!String.valueOf(fine.get("E_DEM_DT")).equals("null") ||!String.valueOf(fine.get("E_DEM_DT")).equals("")) {
				e_dem_dt = String.valueOf(fine.get("E_DEM_DT"));
			}
			%>
                <tr> 
            		<td <%if(fine.get("USE_YN").equals("N")){%>class='is'<%}%> width='4%' align='center'> 
					<input type="checkbox" name="ch_l_cd" value="<%=fine.get("RENT_MNG_ID")%>^<%=fine.get("RENT_L_CD")%>^<%=fine.get("CAR_MNG_ID")%>^<%=fine.get("SEQ_NO")%>^<%=fine.get("CUST_NM")%>^<%if(fine.get("DEM_DT").equals("")){%><%=fine.get("REG_DT")%><%}else{%><%=fine.get("DEM_DT")%><%}%>^<%if(fine.get("E_DEM_DT").equals("")){%><%=fine.get("REG_DT")%><%}else{%><%=fine.get("E_DEM_DT")%><%}%>^" <% if (!String.valueOf(fine.get("PROXY_DT")).equals("")  ) {%> <% } %> > <!-- disabled -->
            		</td>
