...
<!--
	...
	function view_memo(m_id, l_cd, c_id, tm_st, accid_id, serv_id, mng_id){
		...
		window.open("/fms2/con_fee/credit_memo_frame.jsp?auth_rw=<%=auth_rw%>&m_id="+m_id+"&l_cd="+l_cd+"&r_st=&fee_tm=&tm_st1=&bus_id2="+mng_id, "CREDIT_MEMO", "left=20, top=20, width=900, height=750");		
	}
	
	//면책금 세부내용 보기
	function view_ins_m(m_id, l_cd, c_id, accid_id, serv_id, idx){
		...
		fm.idx.value = idx;										
		fm.target = "d_content";
		fm.action = "/fms2/con_ins_m/ins_m_c.jsp";
		fm.submit();
	}
	...
	function  view_stat(){
		...
		document.form1.action="ins_m_sc_stat.jsp";
  ...
  <tr>
	<td>
      <table border="0" cellspacing="0" cellpadding="0" width=100%>
		<tr>
		  <td align='center'>
			<iframe src="/fms2/con_ins_m/ins_m_sc_in.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&auth=<%=auth%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&st_dt=<%=st_dt%>&end_dt=<%=end_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&asc=<%=asc%>#<%=idx%>" name="i_no" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" border="0" frameborder="0" noresize scrolling=yes, marginwidth=0, marginheight=0 >
			</iframe>
		  </td>
		</tr>
	  </table>
