...
<%
	...
	//doc_settle : 채권추심: doc_st:95,  최초 1회	
		
	DocSettleBean doc 		= d_db.getDocSettle(doc_no);
	
	if(doc.getDoc_id().equals("")){
		doc = d_db.getDocSettleCommi("95", rent_l_cd);
	...
	if(doc_no.equals("")){
		...
		sub 	= "채권추심의뢰  ";
		cont 	= "[계약번호:"+rent_l_cd+"]  채권추심의뢰 합니다.";  //진행요청 
		...
		doc.setUser_id4(user_id2);//팀장/지점장	
		...
		url 		= "/fms2/receive/receive_7_u.jsp?rent_mng_id="+rent_mng_id+"|rent_l_cd="+rent_l_cd+"|doc_no="+doc_no+"|doc_bit="+doc_bit+"|mode="+mode;
		...
		System.out.println("쿨메신저(채권추심의뢰요청)"+rent_l_cd+"---------------------"+target_bean.getUser_nm());
...
//		System.out.println(" doc_sanction doc_bit = " + doc_bit + ": doc_no = :" + doc_no +":user_id = " + user_id + ": rent_l_cd = " + rent_l_cd);
		...
		//총무팀장 결재이면 추심 종결처리료
		if(doc_bit.equals("4"))	doc_step = "3";  
			
		flag1 = d_db.updateDocSettle(doc_no, user_id, doc_bit, doc_step);
		//out.println("문서처리전 결재<br>");
		
		if (doc_bit.equals("3")  ) {	//추심종결일 저장	
			flag3 = re_db.updateSettleDt( rent_mng_id, rent_l_cd, settle_dt, user_id ) ; // 업무대여인  결재완료
		}
		
		//2. 쿨메신저 메세지 전송------------------------------------------------------------------------------------------
		if (doc_bit.equals("2")  ) {		
			sub 		= "채권추심의뢰 결재 완료 ";
			cont 	= "[ "+rent_l_cd +" ] "+ "채권추심의뢰 결재가 완료되었습니다. 채권추심을 진행해주십시요!!.";
			target_id = doc.getUser_id1();
		} else if (doc_bit.equals("3")  ) {		
			sub 		= "채권추심종결 결재 요청 ";
			cont 	= "[ "+rent_l_cd +" ] "+ "채권추심종결 결재를 의뢰합니다.!!";
			target_id = doc.getUser_id4();
		} else if (doc_bit.equals("4")  ) {		  //추심종결 결재
			sub 		= "채권추심종결 결재 완료 ";
			cont 	= "[ "+rent_l_cd +" ] "+ "채권추심종결 결재가 완료되었습니다.!!";
