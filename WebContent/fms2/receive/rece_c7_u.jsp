...
<%
	...
	//추심 입금정보
	ClsBandEtcBean band_etc = rc_db.getClsBandEtcInfo(rent_mng_id, rent_l_cd, seq );			
...
<!--	
	...
	//	if ( toInt(parseDigit(fm.draw_amt.value)) ==  toInt(parseDigit(fm.ip_amt.value))   ) {	
		//	fm.rate_amt.value 	= parseDecimal(        toInt(parseDigit(fm.draw_amt.value)) * toInt(fm.re_rate.value) / 100    );		 		
			fm.rate_amt.value 	= parseDecimal(        toInt(parseDigit(fm.draw_amt.value)) -  toInt(parseDigit(fm.ip_amt.value))    );		 		
	//	} else {
...
<input type='hidden' name='cmd' >
  ...
  <table border="0" cellspacing="0" cellpadding="0" width=100%>
        ...
        <table border="0" cellspacing="1" cellpadding="0" width='100%'>
          <tr> 
            <td class='title' width=14%>입금구분</td>
              <td>&nbsp; 
			  <select name="band_st">			  
			    <option value="1" <% if(band_etc.getBand_st().equals("1")){%>selected<%}%>>부분입금</option>
                <option value="2" <% if(band_etc.getBand_st().equals("2")){%>selected<%}%>>잔액</option>		      
              </select> </td>                    
          </tr>
	 
	 <tr> 
            <td class='title'>입금일</td>
            <td>&nbsp;&nbsp;<input type="text" name="band_ip_dt" value="<%=AddUtil.ChangeDate2(band_etc.getBand_ip_dt())%>" size="11" class='text' onBlur='javascript: this.value = ChangeDate(this.value); '></td> 
          </tr>
          <tr> 
            <td class='title'>회수금액</td>
            <td>&nbsp;&nbsp;<input type="text" name="draw_amt" value="<%=AddUtil.parseDecimal(band_etc.getDraw_amt())%>" size="15" class=num onBlur='javascript:this.value=parseDecimal(this.value); '></td>
          </tr>
           <tr> 
            <td class='title'>실입금액</td>
            <td>&nbsp;&nbsp;<input type="text" name="ip_amt" value="<%=AddUtil.parseDecimal(band_etc.getIp_amt())%>" size="15" class=num onBlur='javascript:this.value=parseDecimal(this.value); set_rate_amt();'></td>
          </tr>

	 <tr> 
            <td class='title'>수수료</td>
            <td>&nbsp;&nbsp;<input type="text" name="rate_amt" value="<%=AddUtil.parseDecimal(band_etc.getRate_amt())%>" size="15" class=num onBlur='javascript:this.value=parseDecimal(this.value); '>
            &nbsp;&nbsp;*수수료율: 회수금액의 <input type="text" name="re_rate" value="<%=cls_band.getRe_rate()%>" size="3" class=num>%</td>	             
 
          </tr>  
          <tr> 
            <td class='title'>수수료지급일</td>
            <td>&nbsp;&nbsp;<input type="text" name="rate_jp_dt" value="<%=AddUtil.ChangeDate2(band_etc.getRate_jp_dt())%>" size="11" class='text' onBlur='javascript: this.value = ChangeDate(this.value); '></td> 
          </tr>
    ...
    <tr>
	  ...
	  <a href='javascript:doc_reg()' onMouseOver="window.status=''; return true"><img src="/acar/images/center/button_modify.gif" align="absmiddle" border="0"></a>
	
	<!--  결재전 삭제 처리  -->
	<% if (band_etc.getUser_dt2().equals("") ) {%>
