...
<%		for(int i = 0 ; i < vt_size ; i++){
		        ...
		        <tr> 
        		    ...
        		    <td style="font-size:8pt" align='right'><%=Util.parseDecimal(String.valueOf(ht.get("PAY_AMT")))%></td>						
        		    ...
        		    <td style="font-size:8pt" align='center'><%=ht.get("PAY_DT")%></td>
  ...
  <%		}%>
	...
	<tr>
		<td align='right'>
		  <%if(nm_db.getWorkAuthUser("전산팀",ck_acar_id)){%><a href='view_precost_ins_chk_list2_a.jsp?cost_ym=<%=cost_ym%>&cost_st=<%=cost_st%>&car_use=<%=car_use%>&com_id=<%=com_id%>&chk_st=<%=chk_st%>' target='_blank'>[처리]</a>&nbsp;&nbsp;<%}%>
