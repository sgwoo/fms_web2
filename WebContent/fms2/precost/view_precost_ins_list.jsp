...
<%
	...
	//보험선급비용 미처리스트
	if(cost_st.equals("2")){
		vts = ai_db.getInsurPrecostYmList2(cost_ym, cost_st, car_use, com_id);
		vt_size = vts.size();
	}
	...
	<tr>
	    <td class=line>
		  ...
		  <tr valign="middle" align="center"> 
		    ...
		    <td width='70' rowspan="2" class=title>해지청구일</td>			
	      </tr>
		  <tr valign="middle" align="center">
...
<%		for(int i = 0 ; i < vt_size ; i++){
		        ...
		        <tr> 
        		    ...
        		    <td style="font-size:8pt" align='center'><%if(cost_ym.equals("200812")){%><%=ht.get("O_INS_EXP_DT")%><%}else{%><%=ht.get("O_INS_EXP_DT")%><%}%></td>		
        		    ...
        		    <td style="font-size:8pt" align='center'><%=ht.get("REQ_DT")%></td>
		        </tr>
...
//				sum6  = sum6  + Util.parseLong(String.valueOf(ht.get("BM_AMT")));
				...
				cnt2  = cnt2+1;
			}else if(String.valueOf(ht.get("INS_COM_ID")).equals("0038")){
...
//				sum14  = sum14  + Util.parseLong(String.valueOf(ht.get("BM_AMT")));
				...
				<tr>
		          <td class="title" colspan="4" style="font-size:8pt">합계</td>				
					<td class="title" align='center' colspan="3" style="font-size:8pt"><!--(<%=cost_ym%>가입분 : <%=Util.parseDecimal(total_amt4)%> )--></td>
					<!--<td class="title">&nbsp;</td>
					<td align='right' style="font-size:8pt" ><%//=Util.parseDecimal(total_amt2)%></td>-->
					<td>&nbsp;</td>
					<td align='right' style="font-size:8pt" ><%=Util.parseDecimal(total_amt)%></td>
					<td>&nbsp;</td>					
					<td align='right' style="font-size:8pt" ><%=Util.parseDecimal(total_amt3)%></td>
					<td>&nbsp;</td>					
				</tr>		  
	        </table>
	    </td>
	...
	<tr>
	    <td class=line>
	        <table border="0" cellspacing="1" cellpadding="0" width='100%'>
		        <tr valign="middle" align="center"> 
        		    <td style="font-size:8pt" width='100' class=title>보험사</td>
					<td style="font-size:8pt" width='100' class=title>건수</td>
        		    <td style="font-size:8pt" width='200' class=title>총보험료</td>
        		    <!--<td width='200' class=title >전월누계</td>-->
					<td style="font-size:8pt" width='300' class=title >당월산입</td>
        		    <td style="font-size:8pt" width='300' class=title>선급잔액</td>					
       		    </tr>
		        <tr valign="middle" align="center">
		          <td style="font-size:8pt" class=title>삼성화재</td>
		          <td style="font-size:8pt" align='right'><%=cnt1%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum1)%></td>
		          <!--<td align='right'><%//=Util.parseDecimal(sum2)%></td>-->
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum3)%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum4)%></td>
	          </tr>
		        <tr valign="middle" align="center">
		          <td style="font-size:8pt" class=title>동부화재</td>
		          <td style="font-size:8pt" align='right'><%=cnt2%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum5)%></td>
		          <!--<td align='right'><%//=Util.parseDecimal(sum6)%></td>-->
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum7)%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum8)%></td>
	          </tr>
		        <tr valign="middle" align="center">
		          <td style="font-size:8pt" class=title>렌트카조합</td>
		          <td style="font-size:8pt" align='right'><%=cnt3%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum9)%></td>
		          <!--<td align='right'><%//=Util.parseDecimal(sum10)%></td>-->
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum11)%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum12)%></td>
	          </tr>
		        <tr valign="middle" align="center">
		          <td style="font-size:8pt" class=title>기타</td>
		          <td style="font-size:8pt" align='right'><%=cnt4%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum13)%></td>
		          <!--<td align='right'><%//=Util.parseDecimal(sum14)%></td>-->
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum15)%></td>
		          <td style="font-size:8pt" align='right'><%=Util.parseDecimal(sum16)%></td>
	          </tr>
		        <tr valign="middle" align="center">
		          <td style="font-size:8pt" class="title" colspan="3">합계</td>
		          <!--<td align='right' class="title"><%//=Util.parseDecimal(total_amt2)%></td>-->
		          <td style="font-size:8pt" align='right' ><%=Util.parseDecimal(total_amt)%></td>
