...
<form name='form1' method='post'>
  ...
  <input type='hidden' name='from_page' value='/fms2/tint/tint_m_frame.jsp'>
...
<%		for(int i = 0 ; i < vt_size ; i++){
			Hashtable ht = (Hashtable)vt.elementAt(i);%>			
				<tr>
					...
					<td  width='150' align='center'><%=Util.subData(String.valueOf(ht.get("FIRM_NM")), 12)%><span title='<%=ht.get("FIRM_NM")%>'><a href="javascript:parent.view_client('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '1', '<%=ht.get("CAR_MNG_ID")%>')" onMouseOver="window.status=''; return true"></a></span></td>					
				</tr>
<%
		}
%>
