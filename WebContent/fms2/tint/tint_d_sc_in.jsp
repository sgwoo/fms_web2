...
<%
	...
	Vector vt = t_db.getTintReqDocList(s_kd, t_wd, gubun1, st_dt, end_dt);
	int vt_size = vt.size();
		...
		<td class='line' width='790'>
			<table border="0" cellspacing="1" cellpadding="0" width='100%'>
				<tr>
				  ...
				  <!--
				  <td colspan="3" class='title'>비용정산</td>
				  -->
			    </tr>
				<tr>
			      ...
			      <!--
			      <td width='8%' class='title'>아마존카</td>
			      <td width='8%' class='title'>영업사원</td>
			      <td width='8%' class='title'>고객</td>				  				  				  
			      -->
			  </tr>
			</table>
		</td>
...
<%		for(int i = 0 ; i < vt_size ; i++){
			Hashtable ht = (Hashtable)vt.elementAt(i);%>			
				<tr>											
					...
					<!--
					<td  width='80' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("A_AMT")))%></td>
					<td  width='80' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("E_AMT")))%></td>
					<td  width='80' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("C_AMT")))%></td>
					-->
				</tr>
<%			total_amt1 	= total_amt1 + AddUtil.parseLong(String.valueOf(ht.get("TINT_AMT")));
			...
			total_amt7 	= total_amt7 + AddUtil.parseLong(String.valueOf(ht.get("E_AMT")));
			total_amt8 	= total_amt8 + AddUtil.parseLong(String.valueOf(ht.get("C_AMT")));
			total_amt9 	= total_amt9 + AddUtil.parseLong(String.valueOf(ht.get("BLACKBOX_AMT")));
		}%>
				<tr>											
				    <td class='title'>&nbsp;</td>
				    <td class='title'>&nbsp;</td>
