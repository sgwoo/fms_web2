...
<%
	...
	Vector vt = t_db.getTintIngList(s_kd, t_wd, gubun1, gubun2, gubun3, st_dt, end_dt, sort);
	int vt_size = vt.size();
%>
...
<form name='form1' method='post'>
  ...
  <input type='hidden' name='from_page' value='/fms2/tint/tint_i_frame.jsp'>
  <input type='hidden' name='rent_mng_id' value=''>
  <input type='hidden' name='rent_l_cd' value=''>
  <input type='hidden' name='tint_no' value=''>  
  <input type='hidden' name='mode' value=''>    
<table border="0" cellspacing="0" cellpadding="0" width='1810'>
    <tr>
...
%>
				<tr>
					...
					<td  width='60' align='center'><%if(String.valueOf(ht.get("DEPT_ID")).equals("1000")){%><font color=orange><%}%><%=ht.get("BUS_NM")%><%if(String.valueOf(ht.get("DEPT_ID")).equals("1000")){%></font><%}%></td>					
				</tr>
<%
		}
%>
...
<%		for(int i = 0 ; i < vt_size ; i++){
			Hashtable ht = (Hashtable)vt.elementAt(i);%>			
				<tr>
					...
					<%}else{%>
					<td  width='150' align='center'><a href="javascript:parent.tint_action('<%=ht.get("TINT_NO")%>');"><span title='<%=ht.get("CAR_NM")%>'><%=Util.subData(String.valueOf(ht.get("CAR_NM")), 12)%></span></a></td>
