...
<%
	...
	Vector vt = ec_db.getDigitalKey2List(s_kd, t_wd, gubun1, gubun2, gubun3);
...
<form name='form1' method='post'>
  ...
  <input type='hidden' name='mode' value='com_reg'>
	    ...
	    <td class='line' width='1090'>
	        <table border="0" cellspacing="1" cellpadding="0" width='100%'>
        	    <tr>
        	          ...
        	          <td width='250' class='title'>트림</td>
        	          <td width='360' class='title'>선택사양</td>
        	          <!--
        	          <td width='200' class='title'>비고</td>        	           
        	          <td width='120' class='title'>사용기간</td>
        	          <td width='90' class='title'>신청일자</td>
        	          <td width='90' class='title'>등록일자</td>
        	          <td width='60' class='title'>스캔</td>
        	           -->
        	    </tr>
	        </table>
	    </td>
            ...
            <%	for(int i = 0 ; i < cont_size ; i++){
                ...
                <tr> 
                  ...
                  <td  width='30' align='center'>
		                  <%//if(!String.valueOf(ht.get("REG_DT")).equals("") && String.valueOf(ht.get("COM_REG_DT")).equals("")){%>
	    ...
	    <td class='line' width='1090'>
	        <table border="0" cellspacing="1" cellpadding="0" width='100%'>
        		<%	for(int i = 0 ; i < cont_size ; i++){
						...
						%>
        		<tr>        		          			
        		  <td <%=td_color%> width='90' align='center'><a href="javascript:parent.reg_Dkey('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>')" onMouseOver="window.status=''; return true"><%=ht.get("CAR_NO")%></a></td>
        		  ...
        		  <td <%=td_color%> width='150' align='center'><span title='<%=ht.get("CAR_NM")%>'><%=AddUtil.subData(String.valueOf(ht.get("CAR_NM")), 20)%></span></td>        		  
        		  <td <%=td_color%> width='250' align='center'><span title='<%=ht.get("CAR_NAME")%>'><%=AddUtil.subData(String.valueOf(ht.get("CAR_NAME")), 25)%></span></td>
        		  <td <%=td_color%> width='360' align='center'><span title='<%=ht.get("OPT")%>'><%=AddUtil.subData(String.valueOf(ht.get("OPT")), 35)%></span></td>
        		  <!-- 
        		  ...
        		  <td <%=td_color%> width='90' align='center'>
        		    <a href="javascript:parent.reg_Dkey('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>')" onMouseOver="window.status=''; return true">
        		    <%if(!String.valueOf(ht.get("CAR_NO")).equals("") && String.valueOf(ht.get("REG_DT")).equals("")){%>
        		  ...
        		  <td <%=td_color%> width='90' align='center'><%=AddUtil.ChangeDate2(String.valueOf(ht.get("COM_REG_DT")))%></td>
        		  <td <%=td_color%> width='60' align='center'><a href="javascript:parent.view_scan('<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>')" onMouseOver="window.status=''; return true" title='스캔관리'><img src=/acar/images/center/button_in_see.gif align=absmiddle border=0></a></td>        		  
        		  -->
        		</tr>
						<%	}	%>                
	        </table>
