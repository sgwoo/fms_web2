...
<!--
	...
	function save(){
		...
		<%if((cm_bean.getCar_comp_id().equals("0001")||cm_bean.getCar_comp_id().equals("0002")) && !String.valueOf(pur_com.get("RENT_L_CD")).equals("") && !String.valueOf(pur_com.get("RENT_L_CD")).equals("null") && !String.valueOf(pur_com.get("USE_YN_ST")).equals("해지")){%>
		if(fm.rpt_no.value != '<%=pur_com.get("COM_CON_NO")%>'){	alert('협력업체-계출관리 계약번호와 입력한 계출번호가 틀립니다. 확인하십시오.'); 		fm.rpt_no.focus(); 		return;		}
		<%}%>
...
<%if(coe_bean.getCar_off_id().equals("03900") && !String.valueOf(pur_com.get("RENT_L_CD")).equals("") && !String.valueOf(pur_com.get("RENT_L_CD")).equals("null") && !String.valueOf(pur_com.get("USE_YN_ST")).equals("해지")){%>
	<tr>
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>스캔   &nbsp;<a href="javascript:view_scan()" onMouseOver="window.status=''; return true" title="클릭하세요"><img src=/acar/images/center/button_scan.gif align=absmiddle border=0></a></span>
...
<%}else{%>    
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>매매주문서   &nbsp;<a href="javascript:view_scan()" onMouseOver="window.status=''; return true" title="클릭하세요"><img src=/acar/images/center/button_scan.gif align=absmiddle border=0></a></span>
    ...
    <tr>
	    ...
	    &nbsp;(협력업체-계출관리 계약번호:<%=pur_com.get("COM_CON_NO")%>)
	    <%}%>
...
<%}%>
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>	
                ...
                <tr>
                    ...
                    <td>&nbsp;<input type='text' size='12' name='dlv_est_dt' maxlength='12' class='default' value='<%=AddUtil.ChangeDate2(String.valueOf(est.get("DLV_EST_DT")))%>' <%if(pur.getDlv_est_dt().equals("")&&(!base.getDlv_dt().equals(""))){%>readonly<%}%> onBlur='javscript:this.value = ChangeDate(this.value);'>
        			  ...
        			  <%if(ck_acar_id.equals("000029") && pur.getDlv_est_dt().equals("") && (!base.getDlv_dt().equals(""))){%><span class="b"><a href="javascript:dlv_dt_chk()" onMouseOver="window.status=''; return true" title="클릭하세요">확인</a></span><%}%>
        			</td>
                ...
                <tr>
                    ...
                    <td>&nbsp;<input type='text' name='cons_amt2' maxlength='10' value='0' class='defaultnum' size='10' onBlur='javascript:this.value=parseDecimal(this.value); set_cons_amt();'>원</td>
                </tr>
                <tr>
	...
	<!--
	<tr>
	  <td><font color=#666666>&nbsp;※ 매출DC내용 간편입력 - 1:판매자정상조건, 2:장기재고 3:전시차 4:다량구매처우대 5:캠페인 </font></td>
	</tr>	-->
	<tr>
	    <td class=h></td>
	</tr>
...
<%--                       <%if(car.getNew_license_plate().equals("2")){%>&nbsp;신형번호판신청(대전/대구/광주/부산)<%}%> --%>
	...
	<tr>
	    <td> <span class=style2>블랙박스 </span>
            ...
            <%}%> --%>	    
            <%-- <%if(tint3.getReq_dt().equals("") && tint3.getConf_dt().equals("") && tint3.getPay_dt().equals("") && tint3.getDoc_code().equals("")){%> --%>
	            <%if(base.getReg_id().equals(user_id) || base.getBus_id().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id) || nm_db.getWorkAuthUser("계약결재",user_id) || nm_db.getWorkAuthUser("계약봉투점검자",user_id) || nm_db.getWorkAuthUser("전산팀",user_id) || nm_db.getWorkAuthUser("지점장",user_id)){%>
