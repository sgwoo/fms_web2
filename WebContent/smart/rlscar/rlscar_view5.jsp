...
<%@ include file="/smart/include/search_hidden.jsp" %>
	...
	<input type="hidden" name="lkas_yn" value='<%=secondhand.get("LKAS_YN")%>'>		<!-- 차선이탈 제어형 -->
	<input type="hidden" name="ldws_yn" value='<%=secondhand.get("LDWS_YN")%>'>	<!-- 차선이탈 경고형 -->
	<input type="hidden" name="aeb_yn" value='<%=secondhand.get("AEB_YN")%>'>		<!-- 긴급제동 제어형 -->
	<input type="hidden" name="fcw_yn" value='<%=secondhand.get("FCW_YN")%>'>		<!-- 긴급제동 경고형 -->
	<input type="hidden" name="hook_yn" value='<%=secondhand.get("HOOK_YN")%>'>	<!-- 견인고리 -->
	
<div id="wrap">
    <div id="header">
        <div id="gnb_box">
        	
			<div id="gnb_login"><%=car_no%> 편의사항</div>
