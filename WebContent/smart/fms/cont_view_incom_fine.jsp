...
<%
	...
	Vector vt = s_db.getFineList(gubun3, gubun4, "1", rent_l_cd, "");//(String gubun3, String gubun4, String s_kd, String t_wd)
	int vt_size = vt.size();
%> 
...
<body>
<form name='form1' method='post' action='cont_view_incom_fine.jsp'>
...
<!--//ui object -->
		...
		<br>
		 ...
		 <%for (int i = 0 ; i < vt_size ; i++){
			...
			<ul class="lst_type">
				<li>
					<span><h3>(<%=i+1%>) <%=ht.get("VIO_DT")%> 
					<%=Util.parseDecimal(String.valueOf(ht.get("PAID_AMT")))%>원 <%if(String.valueOf(ht.get("COLL_DT")).equals("")){%><em>미수금</em><%}%></h3></span>
					
					<br>
					| <%=ht.get("VIO_PLA")%>  <%=ht.get("VIO_CONT")%>  
					
					<br>					
					| 납부기한 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("PAID_END_DT")))%>					
					<br>					
					| 대납일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("PROXY_DT")))%>					
					<br>
					| 청구일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("DEM_DT")))%>
					
					<%if(!String.valueOf(ht.get("COLL_DT")).equals("")){%>
					<br>
					| 수금일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("COLL_DT")))%>
					<%}%>
