...
<%@ page import="java.util.*, acar.util.*, acar.cont.*, acar.insur.*"%>
...
<%
	...
	//자동차리스트
	Vector vt = a_db.getCarList(client_id, site_seq, s_kd, t_wd);
	int vt_size = vt.size();
%> 
...
<!--
	...
	function site_search(site_seq){
 	...
 	//계약서 내용 보기
	function view_cont(client_id, firm_nm, rent_mng_id, rent_l_cd, car_mng_id, car_no, car_nm){
		var fm = document.form1;
...
<body>
<form name='form1' method='post' action='cont_list.jsp'>
...
<div id="wrap">
    <div id="header">
        <div id="gnb_box">        	
            <div id="gnb_login">
				<%if(st.equals("4")){%>
				<%		if(site_seq.equals("00")){%>
							<span title='<%=firm_nm%>'>[본사]<%=AddUtil.subData(firm_nm, 7)%></span>
				<%		}else{%>
							<span title='<%=firm_nm%>'><%=AddUtil.subData(site_nm, 10)%></span>
				<%		}%>
				<%}else{%>
					<span title='<%=firm_nm%>'><%=AddUtil.subData(firm_nm, 10)%></span>
				<%}%>			
			</div>
            <div id="gnb_home">
		 ...
		 <%for (int i = 0 ; i < vt_size ; i++){
			...
			<ul class="lst_type">
				<li>
					<h3>
						[<%=i+1%>]				
						<a href="javascript:view_cont('<%=ht.get("CLIENT_ID")%>', '<%=ht.get("FIRM_NM")%>', '<%=ht.get("RENT_MNG_ID")%>', '<%=ht.get("RENT_L_CD")%>', '<%=ht.get("CAR_MNG_ID")%>', '<%=ht.get("CAR_NO")%>', '<%=ht.get("CAR_NM")%>')" onMouseOver="window.status=''; return true" title='계약상세내역'>
							<%=ht.get("CAR_NO")%> 
						</a>
					</h3> 
					<%if(!String.valueOf(ht.get("SITE_NM")).equals("")){%>
					| 지점/현장 : <%=ht.get("SITE_NM")%><br>
					<%}%>
					| <%=ht.get("CAR_NM")%>
					| <%=ht.get("USER_NM")%> | <font color="#5F00FF"><%=ins.getIns_com_nm()%></font><br> 
					<%if(String.valueOf(ht.get("CAR_NO")).equals("미등록")){%>
