...
/* UI Object */
.lst_type{width:auto;padding:20px; border-bottom:1px solid #c2c2c2;list-style:none; background-color:#ffffff; font-family:'나눔고딕',Dotum;font-size:15px;}
...
<%
	...
	Vector vt = s_db.getFeeList4(gubun3, gubun4, "1", rent_l_cd, "");//(String gubun3, String gubun4, String s_kd, String t_wd)
	int vt_size = vt.size();
%> 
...
<body>
<form name='form1' method='post' action='cont_view_incom_fee.jsp'>
...
<!--//ui object -->
		...
		<br>
		 ...
		 <%for (int i = 0 ; i < vt_size ; i++){
			...
			<ul class="lst_type">
				<li>
					<span><h3>(<%=i+1%>) <font color='#990000'><%=ht.get("FEE_TM")%>회차</font><%if(!ht.get("TM_ST1").equals("0"))%>(잔액)<%%> 
					<%=Util.parseDecimal(String.valueOf(ht.get("FEE_AMT")))%>원 <%if(String.valueOf(ht.get("RC_DT")).equals("")){%><em>미수금</em><%}%></h3></span>
					
					<br>
					| 예정일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("FEE_EST_DT")))%>
					
					
					<%if(!String.valueOf(ht.get("TAX_DT")).equals("")){%>
					<br>
					| 계산서일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("TAX_DT")))%>
					<%}%>
					
					<%if(!String.valueOf(ht.get("RC_DT")).equals("")){%>
					<br>
					| 수금일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("RC_DT")))%>
					<%}%>
