...
<%
	...
	if(!t_wd.equals("")){
		vt = rs_db.getServCarSearchList(s_kd, t_wd);
	}
...
<!--
	...
	function search(){
		...
		fm.action = 'search_serv_car_list.jsp';
		fm.target = '_self';
		fm.submit();
	}
	function enter() {
	...
	function setCode(rent_l_cd, car_mng_id, serv_id, off_nm, serv_dt){
...
<form name='form1' action='search_serv_car_list.jsp' method='post'>
			    ...
			    <div class="contents_box">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
              		<%	for (int i = 0 ; i < vt_size ; i++){
						...
						<tr>
							<th valign=top><a href="javascript:setCode('<%=ht.get("RENT_L_CD")%>', '<%=ht.get("CAR_MNG_ID")%>', '<%=ht.get("SERV_ID")%>', '<%=ht.get("OFF_NM")%>', '<%=ht.get("SERV_DT")%>');"><%=ht.get("CAR_NO")%></a><br><%=ht.get("CAR_NM")%></th>
							<td valign=top>
								<%=ht.get("FIRM_NM")%> 								
