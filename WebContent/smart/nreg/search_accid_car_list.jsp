...
<%
	...
	if(!t_wd.equals("")){
		vt = rs_db.getAccidCarSearchList(s_kd, t_wd);
	}
...
<!--
	...
	function search(){
		...
		fm.action = 'search_accid_car_list.jsp';
		fm.target = '_self';
		fm.submit();
	}
	function enter() {
	...
	function setCode(rent_l_cd, car_mng_id, accid_id, serv_id, off_nm, accid_car_no){
		var fm = opener.document.form1;				
		fm.sub_l_cd.value 	= rent_l_cd;		
		fm.sub_c_id.value 	= car_mng_id;		
		fm.accid_id.value 	= accid_id;				
		fm.serv_id.value 	= serv_id;						
		fm.off_nm.value 	= off_nm;
		fm.accid_car_no.value 	= accid_car_no;						
		self.close();
	}
//-->
</script>
...
<form name='form1' action='search_accid_car_list.jsp' method='post'>
			    ...
			    <div class="contents_box">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
              		<%	for (int i = 0 ; i < vt_size ; i++){
						...
						<tr>
							<th valign=top><a href="javascript:setCode('<%=ht.get("RENT_L_CD")%>', '<%=ht.get("CAR_MNG_ID")%>', '<%=ht.get("ACCID_ID")%>', '<%=ht.get("SERV_ID")%>', '<%=ht.get("OFF_NM")%>', '<%=ht.get("CAR_NO")%>');"><%=ht.get("CAR_NO")%></a><br><%=ht.get("CAR_NM")%></th>
							<td valign=top>
								<%=ht.get("FIRM_NM")%> 								
								<font color="#aeaeae"> &nbsp;|&nbsp; </font>
								  &nbsp;<%=ht.get("SERV_ST_NM")%>	
								<br> 
								<%=AddUtil.ChangeDate2(String.valueOf(ht.get("SERV_DT")))%> <%=ht.get("OFF_NM")%>
								  <br>&nbsp;						
