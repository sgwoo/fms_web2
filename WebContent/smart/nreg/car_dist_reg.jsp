...
<!--
	...
	function search_car(){
		...
		var SUBWIN="search_car_list.jsp?t_wd=";		
...
<div id="wrap">
    <div id="header">
        <div id="gnb_box">        	
			<div id="gnb_login">주행거리<%if(serv_id.equals("")){%>등록<%}else{%>수정<%}%></div>
			<div id="gnb_home">
	    	...
	    	<div class="roundedBox" id="type4" style="margin:0 auto;">
			    ...
			    <div class="contents_box">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
				    	...
				    	<tr>
				    		<th width="70">
							<%if(String.valueOf(ht.get("ST")).equals("정비")){%>
							<a href="javascript:view_dist('<%=ht.get("TOT_DIST")%>');"><%=ht.get("TOT_DT")%></a>
							<%}else{%>
							<%=ht.get("TOT_DT")%>
							<%}%>
							</th>
				    		<td><%=ht.get("ST")%> <%=AddUtil.parseDecimal(String.valueOf(ht.get("TOT_DIST")))%>km</td>
				    	</tr>
						<%	}%>
						<%	if(vt_size>3){%>
						<tr>
							<th></th>
							<td align="right"><a href="javascript:view_dist('');">[more]</a>
							</td>
						</tr>
						<%	}%>
						<%	if(vt_size==0){%>
	    	...
	    	<div class="roundedBox" id="type4" style="margin:0 auto;">
			    ...
			    <div class="contents_box">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
				    	<tr>
				    		<th width="70">기준일자</th>
				    		<td><input type="text" name="serv_dt" value='<%=si.getServ_dt()%>' size="11" class=text onBlur='javscript:this.value = ChangeDate(this.value);'>
								<%if(serv_id.equals("")){%>
						...
						<tr>
							<th>주행거리</th>
							<td><input type="text" name="tot_dist" class='text' size="10" value="<%=si.getTot_dist()%>">km</td>
						</tr>											
				    </table>
			    </div>
			    <div class="corner bottomLeft"></div>
	...
	<div id="cbtn"><a href="javascript:DistReg();"><%if(serv_id.equals("")){%><img src=/smart/images/btn_reg.gif align=absmiddle border=0><%}else{%><img src=/smart/images/btn_modify.gif align=absmiddle border=0><%}%></a></div>	
	<div id="footer"></div>  
</div>
</form>
<script language="JavaScript">
