...
<%
	...
	vt = rs_db.getRentMngList_New(s_kd, t_wd, self_st, user_id);
...
//		if(fm.t_wd.value == ''){ alert('검색어를 입력하십시오.'); fm.t_wd.focus(); return; }	
		fm.action = 'search_ret_car_list.jsp';
		fm.target = '_self';
		fm.submit();
	}
	function enter() {
	...
	function setCode(car_mng_id, rent_s_cd){
...
<form name='form1' action='search_ret_car_list.jsp' method='post'>
...
<div id="wrap">
    <div id="header">
        <div id="gnb_box">        	
			<div id="gnb_login">반차대기차량 조회</div>
			<div id="gnb_home"></div>
			    ...
			    <div class="contents_box">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
              		<%	for (int i = 0 ; i < vt_size ; i++){
						...
						<tr>
							<th valign=top><a href="javascript:setCode('<%=ht.get("CAR_MNG_ID")%>', '<%=ht.get("RENT_S_CD")%>');"><%=ht.get("CAR_NO")%></a><br><%=ht.get("CAR_NM")%></th>
							<td valign=top>
								<%=ht.get("CUST_NM")%> 
								<font color="#aeaeae"> &nbsp;|&nbsp; </font><%=ht.get("BUS_NM")%>
								<br> 
								<%=AddUtil.ChangeDate2(String.valueOf(ht.get("DELI_DT")))%>~<%=AddUtil.ChangeDate2(String.valueOf(ht.get("RET_PLAN_DT")))%> 
								<font color="#aeaeae"> &nbsp;|&nbsp; </font>
			                      <font color=#fd5f00><b><%=ht.get("RENT_ST")%></b></font>
								<br>&nbsp;   
							</td>
						</tr>						
              		<%	}%>
