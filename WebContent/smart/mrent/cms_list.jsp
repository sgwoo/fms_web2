...
<%
	...
	Vector vt = af_db.getACmsDateClientList(client_id, "", s_yy, s_mm);
	int vt_size = vt.size();
%> 
...
<body>
<form name='form1' method='post' action='cms_list.jsp'>
			...
			<ul class="lst_type">
				<li>
					<!--<a href="javascript:view_tax('<%=ht.get("TAX_NO")%>')" onMouseOver="window.status=''; return true" title='계산서상세내역'>-->
						<span><h3>(<%=i+1%>) <%=AddUtil.ChangeDate2(String.valueOf(ht.get("ADATE")))%> | <%=Util.parseDecimal(String.valueOf(ht.get("AOUTAMT")))%>원</h3></span>
					<!--</a>-->
					<br> 					
					| <%=ht.get("CAR_NO")%> 
					<br> 	
					| <%=ht.get("NM")%> | <%=ht.get("ABNO")%>					  
				</li>
			</ul>
          <%}%>
		  <%if(vt_size==0){%>
