...
<%
	...
	Vector vt = s_db.getInsurHList(gubun3, gubun4, "1", rent_l_cd, "");//(String gubun3, String gubun4, String s_kd, String t_wd)
	int vt_size = vt.size();
%> 
...
<body>
<form name='form1' method='post' action='cont_view_incom_accid.jsp'>
...
<!--//ui object -->
		...
		<br>
		 ...
		 <%for (int i = 0 ; i < vt_size ; i++){
			...
			<ul class="lst_type">
				<li>
					<span><h3>(<%=i+1%>) <%=ht.get("REQ_GU")%> <%=Util.parseDecimal(String.valueOf(ht.get("REQ_AMT")))%>원 <%if(String.valueOf(ht.get("PAY_DT")).equals("")){%><em>미수금</em><%}%></h3></span>
										
					<br>					
					
					| 보험회사 : <%=ht.get("OT_INS2")%>
					<br>					
					| 사고구분 : <%=ht.get("ACCID_ST")%>
					<br>
					| 사고일자 : <%=ht.get("ACCID_DT")%>
					<br>
					| 청구일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("REQ_DT")))%>					
					
					<%if(!String.valueOf(ht.get("PAY_GU")).equals("")){%>
					<br>
					| 입금구분 : <%=ht.get("PAY_GU")%>
					<%}%>
					...
					<br>
					| 실입금액 : <%=Util.parseDecimal(String.valueOf(ht.get("PAY_AMT")))%>원										
					<%}%>
