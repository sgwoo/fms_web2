...
<%
	...
	Vector vt = s_db.getGrtList2(gubun3, gubun4, "1", rent_l_cd, "");//(String gubun3, String gubun4, String s_kd, String t_wd)
	int vt_size = vt.size();
%> 
...
<body>
<form name='form1' method='post' action='cont_view_incom_pre.jsp'>
...
<!--//ui object -->
		...
		<br>
		 ...
		 <%for (int i = 0 ; i < vt_size ; i++){
			...
			<ul class="lst_type">
				<li>
					<span><h3>(<%=i+1%>) <%=ht.get("GUBUN")%> <%if(!String.valueOf(ht.get("EXT_TM")).equals("1")){%>잔액<%}%>
					<%=Util.parseDecimal(String.valueOf(ht.get("EXT_AMT")))%>원 <%if(String.valueOf(ht.get("EXT_PAY_DT")).equals("")){%><em>미수금</em><%}%></h3></span>
					
					<br>
					| 예정일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("EXT_EST_DT")))%>
					
					<br>
					<%if(String.valueOf(ht.get("GUBUN")).equals("승계수수료")){%>
					| 승계일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("RENT_SUC_DT")))%>
					<%}else{%>
					| 대여개시일 : <%=ht.get("RENT_START_DT")%>
					<%}%>
					
					<%if(!String.valueOf(ht.get("EXT_PAY_DT")).equals("")){%>
					<br>
					| 수금일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("EXT_PAY_DT")))%>
					<%}%>
