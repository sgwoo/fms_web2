...
<%
	...
	Vector vt = s_db.getInsurMList(gubun3, gubun4, "1", rent_l_cd, "");//(String gubun3, String gubun4, String s_kd, String t_wd)
	int vt_size = vt.size();
%> 
...
<body>
<form name='form1' method='post' action='cont_view_incom_serv.jsp'>
...
<!--//ui object -->
		...
		<br>
		 ...
		 <%for (int i = 0 ; i < vt_size ; i++){
			...
			<ul class="lst_type">
				<li>
					<span><h3>(<%=i+1%>) <%=ht.get("SERV_ST")%> <%=Util.parseDecimal(String.valueOf(ht.get("CUST_AMT")))%>원 <%if(String.valueOf(ht.get("CUST_PAY_DT")).equals("")){%><em>미수금</em><%}%></h3></span>
										
					<br>
					<%if( Util.parseInt(String.valueOf(ht.get("TOT_AMT")))==0 && String.valueOf(ht.get("OFF_NM")).equals("") ){
							...
							ht.put("OFF_NM", String.valueOf(s_ht.get("OFF_NM")));
							ht.put("TOT_AMT", String.valueOf(s_ht.get("TOT_AMT")));
							ht.put("SERV_DT", String.valueOf(s_ht.get("SERV_DT")));
							%>
					[선청구분]
					<br>
					<br>
					...
					| 정비업체 : <%=ht.get("OFF_NM")%>
					<br>
					| 정비금액 : <%=Util.parseDecimal(String.valueOf(ht.get("TOT_AMT")))%>원					
					<br>					
					| 정비일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("SERV_DT")))%>					
					<br>					
					| 입금예정일 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("CUST_PLAN_DT")))%>					
					<%if(!String.valueOf(ht.get("CUST_PAY_DT")).equals("")){%>
					<br>
					| 수금일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("CUST_PAY_DT")))%>
					<%}%>
