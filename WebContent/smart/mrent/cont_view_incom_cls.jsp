...
<%
	...
	Vector vt = s_db.getClsFeeScdList(gubun3, gubun4, "1", rent_l_cd, "");//(String gubun3, String gubun4, String s_kd, String t_wd)
	int vt_size = vt.size();
%> 
...
<body>
<form name='form1' method='post' action='cont_view_incom_cls.jsp'>
...
<!--//ui object -->
		...
		<br>
		 ...
		 <%for (int i = 0 ; i < vt_size ; i++){
			...
			<ul class="lst_type">
				<li>
					<span><h3> <%=AddUtil.ChangeDate2(String.valueOf(ht.get("CLS_DT")))%> 해지 <%if(!String.valueOf(ht.get("TM")).equals("1")){%>잔액<%}%>
					<%=Util.parseDecimal(String.valueOf(ht.get("AMT")))%>원 <%if(String.valueOf(ht.get("PAY_DT")).equals("")){%><em>미수금</em><%}%></h3></span>
					
					<br>
					| 예정일자 : <%=AddUtil.ChangeDate2(String.valueOf(ht.get("EST_DT")))%>
