...
<%	
	...
	if(actn_id.equals("000502") ){//글로비스(시화)
		actn_name="글로비스(시화)";	
	}else if(actn_id.equals("013011")){//글로비스(분당)
		actn_name="글로비스(분당)";	
	}else if(actn_id.equals("022846")){//롯데렌탈(주)
		actn_name="롯데렌탈(주)";	
	}else if(actn_id.equals("048691")){//kcar
		actn_name="Kcar";	
	}
    ...
    <div id="contents">
		<%if(idx.equals("1")){%>
				 <% for(int i = 0 ; i < actn_size ; i++){
					...
					if (ht.get("ACTN_ID").equals(actn_id)) {
					no += 1;
					%>
			<ul class="lst_type">
				<li>					
		...
		<%}else if(idx.equals("3")){%>
			...
			<ul class="lst_type">
				<li>					
					<span><h3>합계 <font color=#990000><%=AddUtil.parseDecimal(g_su2_amt)%>원</font></h3></span>					
				</li>
			</ul>
		<%}%>
	</div>     
