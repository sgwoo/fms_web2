...
<%	
	...
	Vector vt = vt = as_db.getAssetSaleList(s_yy, s_mm);
...
<body>
<form name='form1' method='post' action='asset_sale_list.jsp'>
...
<div id="wrap">
    <div id="header">
        <div id="gnb_box">        	
            <div id="gnb_login">자산매각현황</span></div>
            <div id="gnb_home">
		 ...
		 <%for(int i = 0 ; i < cont_size ; i++){
			...
			<ul class="lst_type">
				<li>					
					...
					<br>
					| <%if(ht.get("ASSCH_RMK").equals("매각")){%>매각일자<%}else{%> 해지일자<%}%> <%=AddUtil.ChangeDate2(String.valueOf(ht.get("ASSCH_DATE")))%> |
					<br>
					<%if(ht.get("ASSCH_RMK").equals("매각")){%>					
					| 경매장 <%=ht.get("FIRM_NM")%> | 
					<%}else{%>
					| 매입옵션 | 
					<%}%>
					<br> 
					| 매각액 <%=Util.parseDecimal(sup_amt)%>원 | 장부가액 <%=Util.parseDecimal(t1 + t2 - t6 - t8 - t12 )%>원 | 
					<br> 
					| 손익 <%=Util.parseDecimal(sup_amt - (t1 + t2 - t6 - t8 - t12) )%>원 | 
				</li>
			</ul>
		 <%}%>
		 <%if(cont_size==0){%>
