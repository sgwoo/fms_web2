...
//	String dept_id = request.getParameter("dept_id")==null?"":request.getParameter("dept_id");
	...
	//팀원 리스트
	Vector users = c_db.getUserList(dept_id, "", ""); 
	...
	//견적내기
	Vector etl = cs_db.getEstiUserHistory(thisyear, thismonth, thisday, "");
%>
...
<body>
<form name='form1' method='post' action='schedule_dept.jsp'>
...
<div id="wrap">
    <div id="header">
        <div id="gnb_box">        	
			<div id="gnb_login"><%=dept_nm%> 스케줄관리	
			</div> 
	...
	<div id="contents">
		<%	int l = nday;
			...
			if(AddUtil.getDate2(3) == l)	tr_color = "green"; 	//현재날짜
		%>
		<center>
			<!--이전날-->
		...
		<%	for(int i=0; i<users.size(); i++){
				...
				UsersBean s_user_bean = umd.getUsersBean(userid);%>
				
			<%	if(!nm_db.getWorkAuthUser("임원",userid)){%>
			
			<div>					
				<h3><font color='#990000'><b><%= c_db.getNameById(userid,"USER") %></b></font></h3><br />
			<%		cnt = 1;
					for(int j=0; j<cs_r.length; j++){
						cs_bean = cs_r[j];
						if(userid.equals(cs_bean.getUser_id())){
							cnt++;%>
							<font color="#7f7f7f"><b>[<%=cs_bean.getSch_chk()%>]</b></font> <%=cs_bean.getTitle()%><br />
							<%if(!cs_bean.getContent().equals("")){%>
							&nbsp;&nbsp;&nbsp;<font color="#666666"><%=cs_bean.getContent()%></font><br />
							<%}%>	
			<%			}
					}%>
			<%		if(s_user_bean.getLoan_st().equals("") && cnt==1){%>
			데이타가 없습니다.
			<%		}%>
			</div>
			
			<%		if(!s_user_bean.getLoan_st().equals("")){%>
			<div><font color="#7f7f7f"><b>[거래처방문]</b></font><br />
			<%			cnt = 1;
						for(int j=0; j<cvBns.length; j++){
							...
							if(c42_cvBn.getUpdate_id().equals(userid)){
								...
								<%= cnt++ %>.
								<tt><a href="/smart/fms/client_view_base.jsp?st=2&client_id=<%=c42_cvBn.getClient_id()%>&site_seq=00"><%=c_db.getNameById(c42_cvBn.getClient_id(),"CLIENT")%></a><br />	
			...
			<div><font color="#7f7f7f"><b>[자동차정비]</b></font><br />
			<%			cnt=1;
		...
		<div><font color="#7f7f7f"><b>[운행차검사]</b></font><br />
		<%				cnt=1;
		...
		<div><font color="#7f7f7f"><b>[견적업무]</b></font><br />
		<%				cnt=1;
						for(int j=0; j<etl.size(); j++){
							Hashtable ht = (Hashtable)etl.elementAt(j);
							String reg_id = (String)ht.get("REG_ID");
