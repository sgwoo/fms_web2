...
<%
	...
	Vector vt = kh_db.Know_how_AllList(gubun,  t_wd,  "");
	int vt_size = vt.size();
%>
...
<form name="form1" method="post"   action='knowhow_list.jsp'>
...
%> 
                 ...
                 <ul class="List">
	               	  <li><a href="knowhow_info.jsp?know_how_id=<%=ht.get("KNOW_HOW_ID")%>" title='지식내용'>
	               	 	<span>
		                 	<%if(ht.get("KNOW_HOW_ST").equals("1")){%>
							<font color="#990000">[지식Q&A]</font>
							<%}else if(ht.get("KNOW_HOW_ST").equals("2")){%>
							<font color="#990000">[오픈지식]</font>
							<%}%>
						<%=ht.get("TITLE")%>&nbsp;</span></a>				 							 
					 <span><font color='#3b44bb'><b>
					 <%=ht.get("USER_NM")%></b>&nbsp;|&nbsp;<%= AddUtil.ChangeDate2((String)ht.get("REG_DT")) %></font></span><br> 
					 <div style="height:7px;"></div>
		    		 <span><%=Util.subData(String.valueOf(ht.get("CONTENT")), 70)%>
		    			&nbsp;</span><br> 
