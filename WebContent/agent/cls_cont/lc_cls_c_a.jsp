...
//	cls.setSb_saction_id(request.getParameter("sb_saction_id")==null?"":	request.getParameter("sb_saction_id")); //출고전해지(신차), 개시전해지(재리스)인 경우 지점장 결재자
	...
	//전기차 출고전해지시 전기차 담당자에게 메세지
	if ( cls_st.equals("7") && elec_cnt > 0  ) {
	
		//2. 쿨메신저 메세지 전송------------------------------------------------------------------------------------------	
		UsersBean sender_bean 	= umd.getUsersBean(user_id);
		
		String sub 		= "전기차(수소차) 출고전해지 통보";
		String cont 	= " 전치가(수소차)가 출고전해지 되었습니다.  &lt;br&gt; &lt;br&gt; 상호명 : "+client.getFirm_nm()+" 계약번호 : "+rent_l_cd +"  &lt;br&gt; &lt;br&gt;  해지 업무에 참고 바랍니다.!!!";
		...
		System.out.println("쿨메신저[전기차(수소차) 출고전해지 ] "+rent_l_cd+"-----------------------"+target_bean.getUser_nm());		
	...
	if(!ec_bean.getRent_l_cd().equals("")){
		...
		String cont4 		= "[ "+ec_bean.getRent_l_cd()+" ] 해지처리 되었습니다. &lt;br&gt; &lt;br&gt;  전기차 충전기 신청내역이 있으니 확인하세요.";
