	...
	function save(){
		...
		if (  fm.dft_amt.value != fm.dft_amt_1.value ) {
			...
			if( fm.dft_saction_id.value == '000028'  ||  fm.dft_saction_id.value == '000005'   ) {
				...
				alert("중도해지위약금 결재자를  지정하셔야 합니다. 사전에 협의하십시요.!!");
				return;
			}	
...
<!--                <option value="10">개시전해지(재리스)</option>   -->
		    ...
		    <td >&nbsp;
		      ...
		      <input type='text' name='r_day' size='2' class='text' value='<%= base1.get("R_DAY")%>' >일&nbsp;</td>
        ...
        <td colspan="2" class='line'> 
          <table border="0" cellspacing="1" cellpadding="0" width=100%>
            ...
            <tr> 
              ...
              <td class='title' align="center" class="title"> 
                <input type='text' name='dly_amt_1' value='<%=AddUtil.parseDecimal(AddUtil.parseInt((String)base1.get("DLY_AMT")))%>' size='15' class='num' onBlur='javascript:this.value=parseDecimal(this.value); set_cls_amt(this);'> 원</td>
            ...
            <tr> 
              ...
              <td class='title_p'>=(D+E+F+G+H+I+J+K+L+M)&nbsp;&nbsp;
               <br>※ 계산서:&nbsp;             
               <input type="radio" name="tax_reg_gu" value="N" checked >항목별청구
               <input type="radio" name="tax_reg_gu" value="Y" >항목합산청구
    ...
    <tr>
        <td>&nbsp;<font color="#FF0000">***</font> 중도해지 위약금의 감액 관련해서는 사전에 정채달팀장의 결재를 득해야 합니다. 이경우 중도해지위약금 감액 결재자 필수항목입니다.</td>
    </tr>
        ...
        <td class=line>
            <table width=100% border=0 cellspacing=1 cellpadding=0>
       		  <tr>
                    ...
                    <td width=12% >&nbsp;<input type='text' name='fdft_amt3'  size='15' class='num' readonly  > 원</td>
 	 	  ...
 	 	  <tr>
		    	...
		    	<table border="0" cellspacing="1" cellpadding="0" width=100%>
		          <tr>
                    ...
                    <td width=12%>&nbsp;<input type='text' name='gi_amt' value='<%=AddUtil.parseDecimal(gins.getGi_amt())%>' size='15' class='num' readonly onBlur='javascript:this.value=parseDecimal(this.value);'> 원</td>
                    <td class=title width=12%>청구채권</td>
                    <td width=13%>&nbsp;<input type='text' name='gi_c_amt' size='15' class='num' onBlur='javascript:this.value=parseDecimal(this.value); set_gi_amt();'> 원</td>
                    <td class=title width=12%>잔존채권</td>
                    <td width=13%>&nbsp;<input type='text' name='gi_j_amt' size='15' class='num' readonly onBlur='javascript:this.value=parseDecimal(this.value);'> 원</td>
                </tr>
                <!-- 대표연대보증등 -->
<% if(gur_size > 0){
	...
	function set_init(){
 	...
 	// 	alert(fm.r_day.value);
	  ...
	  // 개시대여료 있는 경우에 한함. (해지일이 대여기간을 경과한 경우에 한함 )	 
	    ...
	    fm.dfee_v_amt.value =  toInt(parseDigit(fm.ex_di_v_amt.value)) + ( toInt(parseDigit(fm.nfee_amt.value)) * 0.1);  //당초 대여료 부가세 
	    fm.dfee_v_amt_1.value =  fm.dfee_v_amt.value; //확정 대여료 부가세 
