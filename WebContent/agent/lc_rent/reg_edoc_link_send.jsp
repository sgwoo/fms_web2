...
<% 
	...
	//mgr_email = "dev@amazoncar.co.kr";
	//mgr_m_tel = "010-2354-2348";

	if(!suc_mgr_email.equals("")){
		//suc_mgr_email = "dev@amazoncar.co.kr";
		//suc_mgr_m_tel = "010-2354-2348";
	}
	...
	//전자문서 전송데이터 처리		
	flag1 = ln_db.insertALink(doc_code, link_table, link_type, rent_l_cd, link_rent_st, link_im_seq, mgr_nm, mgr_email, mgr_m_tel, user_id, suc_mgr_nm, suc_mgr_email, suc_mgr_m_tel, link_com);
	
	
	//계약담당자 업데이트
	if(flag1 && mgr_cng.equals("Y")){
		//계약담당자
...
//		sendXML += "<?xml version='1.0' encoding='utf-8'?>";
		...
		sendXML += "<repre_ssn><%=ht.get("RENT_END_DT")%></repre_ssn> ";
		sendXML += "<repre_nm><%=ht.get("REPRE_SSN")%></repre_nm> ";
		sendXML += "<cms_dep_nm><%=ht.get("CMS_DEP_NM")%></cms_dep_nm> ";
		sendXML += "<cms_dep_ssn><%=ht.get("CMS_DEP_SSN")%></cms_dep_ssn> ";
		sendXML += "<cms_bank><%=ht.get("CMS_BANK")%></cms_bank> ";
		sendXML += "<cms_acc_no><%=ht.get("CMS_ACC_NO")%></cms_acc_no> ";
