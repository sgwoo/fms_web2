...
<!--
	...
	{
		window.open("/agent/client/client_s_frame.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/agent/lc_rent/lc_reg_step2.jsp", "CLIENT", "left=10, top=10, width=1100, height=600, scrollbars=yes, status=yes, resizable=yes");
	}		
	...
	function search_site()
	{
		...
		window.open("/agent/client/client_site_s_p.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/agent/lc_rent/lc_reg_step2.jsp&client_id="+fm.client_id.value, "CLIENT_SITE", "left=100, top=100, width=620, height=450");
	}			
	...
	function save(){
		...
		if(confirm('등록하시겠습니까?')){		
			...
			fm.cms_bank.value = fm.fee_bank.options[fm.fee_bank.selectedIndex].text;
...
//			fm.target='i_no';
	...
	function getSucCommi()
	{
  	...
  	}else if(fm.rent_suc_exem_cau.value == '5'){ //에이전트 승계수당과 상계
  		if(toInt(parseDigit(fm.rent_suc_commi.value)) <= 110000){
  			fm.rent_suc_commi.value = 0;	
  		}else{
  			fm.rent_suc_commi.value = parseDecimal(toInt(parseDigit(fm.rent_suc_commi.value)) - 110000);
  		}
  	}
 	}
...
<form action='' name="form1" method='post'>
  ...
  <input type='hidden' name="from_page" 	value="/agent/lc_rent/lc_cng_client_frame.jsp">
  ...
  <input type="hidden" name="legal_yn" id="legal_yn" value="<%=cont_etc.getLegal_yn()%>"><!-- 법률비용지원금 -->
  <input type="hidden" name="ev_yn" id="ev_yn" value="<%=cont_etc.getEv_yn()%>"><!-- 전기차 여부 -->
  <input type="hidden" name="top_cng_yn" id="top_cng_yn" value="<%=cont_etc.getTop_cng_yn()%>"><!-- 탑차(구조변경) -->
...
<table border='0' cellspacing='0' cellpadding='0' width='100%'>
	    ...
	    <td class=line>
            <table border="0" cellspacing="1" cellpadding="0" width=100%>		  
    		  ...
    		  <%for(int i=0; i<fee_size; i++){
    				...
    				fee_opt_amt = fees.getOpt_s_amt()+fees.getOpt_v_amt(); //마지막연장 재견적
    ...
    <%}%>
    <%if(settle_size>0 && r_settle_amt>0){ //휴대차료를 제외한 미납채권이 있을때%>	
	<tr>
    		  ...
    		  <%	} %>
                <tr> 
                    ...
                    <td colspan="7">&nbsp;
						...
						&nbsp;<input type="text" name="t_addr" id="t_addr1" size="65" value="">
					</td>
                </tr>
            </table>
        </td>
    ...
    <tr> 
		...
		<tr>
                ...
                <tr>
                    ...
                    <td width='20%'>&nbsp;
                    	<input type="hidden" name="ext_agnt" value="<%=user_id%>">
                ...
                <tr>
                    ...
                    <td colspan='3'>&nbsp;
                        <select name='rent_suc_exem_cau' onChange="javascript:getSucCommi();">
                            ...
                            <option value="5">에이전트 승계수당과 상계</option>
