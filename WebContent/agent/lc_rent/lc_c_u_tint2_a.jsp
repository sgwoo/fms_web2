	...
	if(cmd.equals("update_serial_no")){ // 일련번호 수정 전용
		...
		if(!tint.getTint_no().equals("")){
			...
			String tint_no = tint.getTint_no();
				...
				if(!tint.getDoc_code().equals("") && tint.getOff_id().equals("002849") && tint.getTint_yn().equals("Y") && !AddUtil.replace(n_sup_est_dt,"-","").equals(AddUtil.replace(o_sup_est_dt,"-",""))){
...
<script language='javascript'>
	...
	if('<%=from_page2%>' == 'pur_doc_i.jsp'){
		...
		if('<%=from_page%>' == '/agent/car_pur/pur_doc_u.jsp' || '<%=from_page%>' == '/agent/car_pur/pur_doc_frame.jsp'){
			fm.rent_st.value = '1';
