...
<%@ page import="acar.car_office.*, acar.user_mng.*, acar.insur.*"%>
...
<%
	...
	from_page = "/agent/lc_rent/lc_c_c_fee.jsp";
...
<!--
	...
	function update(st, rent_st){
		if(st == 'grt_amt' || st == 'pp_amt' || st == 'ifee_amt' || st == 'fee_amt' || st == 'inv_amt'){
			...
			var height = 600;
			window.open("/agent/lc_rent/lc_c_u.jsp<%=valus%>&cng_item="+st+"&taecha_no="+rent_st, "CHANGE_ITEM", "left=50, top=50, width=1050, height="+height+", scrollbars=yes");			
		}else{
			var height = 600;
	...
	function view_eval(){
		var fm = document.form1;
		window.open("/agent/lc_rent/view_eval.jsp?user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/agent/lc_rent/lc_c_c_fee.jsp&client_id="+fm.client_id.value+"&rent_mng_id="+fm.rent_mng_id.value+"&rent_l_cd="+fm.rent_l_cd.value, "VIEW_EVAL", "left=50, top=50, width=850, height=600");
	}
	...
	{
		var fm = document.form1;
		window.open("/fms2/con_fee/fee_scd_print.jsp?m_id=<%=rent_mng_id%>&l_cd=<%=rent_l_cd%>&b_dt=<%=Util.getDate()%>&cls_chk=&mode=", "PRINT_VIEW", "left=50, top=50, width=820, height=700, scrollbars=yes");
	}		
...
<table border='0' cellspacing='0' cellpadding='0' width='100%'>
  <%if(!base.getCar_st().equals("2")){%>
	...
	%>	
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2><%if(i >1){%><%=i-1%>차 연장 <%}%>대여요금 </span>
	    	<%if(i==1){%>
	    	&nbsp;&nbsp;&nbsp;&nbsp;
	    	<a href="javascript:print_view('');" title='대여료수금관리' onMouseOver="window.status=''; return true">[대여료 수금 스케쥴]</a>
	    	<%}%>
	    	</td>
    </tr>
    <tr>
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr>
                    ...
                    <td width="20%">&nbsp;<%=AddUtil.ChangeDate2(fees.getRent_start_dt())%>
					  <%if(i == 1 && fees.getRent_start_dt().equals("") && user_id.equals(base.getBus_id())){%>
                ...
                <tr>
                    ...
                    <td colspan="3">&nbsp;<%if(client.getClient_st().equals("1")){%>대표:<%if(cont_etc.getClient_guar_st().equals("1")){%>가입<%}else if(cont_etc.getClient_guar_st().equals("2")){%>면제<%}%><%}%><%if(gur_size>0){%>/연대보증:<%=gur_size%>명<%}%></td>
                </tr>
    		  <%}%>
            </table>
        </td>
        ...
        <td class=line>
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr>
                    ...
                    <td align="center">-</td>
                    <td align="center">&nbsp;월보험료(공급가) = 년간보험료<br>
                ...
                <tr>
                    ...
                    <td align="center" >
                    	<input type='text' size='9' name='driver_add_amt' maxlength='10' class='whitenum' value='<%=AddUtil.parseDecimal(fee_etcs.getDriver_add_amt())%>' > 원 
                    </td>
                    <td align="center" >
                    	<input type='text' size='9' name='driver_add_v_amt'  maxlength='9' class='whitenum' value='<%=AddUtil.parseDecimal(fee_etcs.getDriver_add_v_amt())%>' onBlur="javascript:set_fee_amt(this)" onKeyDown='javascript:enter_fee(this)'> 원
                    </td>
                    <td align='center' >
                    	<input type='text' size='9' maxlength='10'  name='driver_add_total_amt' class='whitenum' value='<%=AddUtil.parseDecimal(fee_etcs.getDriver_add_amt() + fee_etcs.getDriver_add_v_amt())%>' onBlur="javascript:set_fee_amt(this)" onKeyDown='javascript:enter_fee(this)'> 원
                    </td>
                    <td align="center">-</td>
                    <td align='center'>-</td>
                    <td align='center'>-</td>
                ...
                <tr>
                    ...
                    <td align="center" >
                    	<input type='text' size='9' name='tinv_s_amt' maxlength='11' class='whitenum' value='<%=AddUtil.parseDecimal(fees.getInv_s_amt() + fees.getIns_s_amt() + fee_etcs.getDriver_add_amt())%>'> 원 
                    </td>
                    <td align="center" >
                       	<input type='text' size='9' name='tinv_v_amt'  maxlength='9' class='whitenum' value='<%=AddUtil.parseDecimal(fees.getInv_v_amt() + fees.getIns_v_amt() + fee_etcs.getDriver_add_v_amt())%>' onBlur="javascript:set_fee_amt(this)" onKeyDown='javascript:enter_fee(this)'> 원
                    </td>
                    <td align='center' >
                    	<input type='text' size='9' maxlength='10'  name='tinv_amt' class='whitenum' value='<%=AddUtil.parseDecimal(fees.getInv_s_amt() + fees.getInv_v_amt() + fees.getIns_s_amt() + fees.getIns_v_amt() + fee_etcs.getDriver_add_amt() + fee_etcs.getDriver_add_v_amt())%>' onBlur="javascript:set_fee_amt(this)" onKeyDown='javascript:enter_fee(this)'> 원
                    </td>
                    <td align="center">-</td>
                    <td align='center'>-</td>
                    <td align='center'>-</td>
	...
	<tr id=tr_fee3 style="display:<%if(!base.getCar_st().equals("2")){%>''<%}else{%>none<%}%>">
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>납입방법 <%if(user_id.equals("000000") || base.getBus_id().equals(user_id) || base.getBus_id2().equals(user_id)){%><a href="javascript:update('pay_way','')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><%}%><font color="#CCCCCC"> ( 회계업무권한 : 납입방법 수정 )</font></span></td>
    </tr>
    <tr>
        <td class=line2></td>
    </tr>					
        ...
        <td class='line'> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr>
                    ...
                    <td width="20%">&nbsp;<%String fee_pay_st = fee.getFee_pay_st();%><%if(fee_pay_st.equals("1")){%>자동이체<%}else if(fee_pay_st.equals("2")){%>무통장입금<%}else if(fee_pay_st.equals("3")){%>지로<%}else if(fee_pay_st.equals("4")){%>수금<%}else if(fee_pay_st.equals("5")){%>기타<%}%></td>
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>세금계산서 <%if(base.getBus_id().equals(user_id)){%><a href="javascript:update('tax','')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><%}%><font color="#CCCCCC"> ( 회계업무권한 : 청구서발행정보 수정 )</font></span></td>
    </tr>
    <tr>
        <td class=line2></td>
    </tr>
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>출고전대차 <%if(user_id.equals("000000") || base.getBus_id().equals(user_id)){%><a href="javascript:update('taecha_info','')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><%}%><font color="#CCCCCC"> ( 회계업무권한 : 지연대차 수정 )</font></span></td>
	</tr>
    ...
    <tr>
	    <td align='right'>        
	        <%if(user_id.equals("000000") || base.getBus_id().equals(user_id)){%><a href="javascript:update('taecha','<%=ht.get("NO")+""%>')"><img src=/acar/images/center/button_modify_ss.gif align=absmiddle border=0></a><%}%></span></td>
	</tr> 
    <tr> 
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>				
