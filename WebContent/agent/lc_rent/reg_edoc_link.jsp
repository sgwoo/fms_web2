...
<%@ page import="java.util.*, acar.util.*, acar.cont.*, acar.client.*, acar.car_register.*, acar.car_mst.*, acar.alink.*"%>
...
<!--
	...
	function link_send(){		
		...
		<%if(link_table.equals("rm_rent_link")){%>
		if(fm.link_com[0].checked == false && fm.link_com[1].checked == false){		
			alert('전송구분을 선택하십시오.'); return;
		}
		if(fm.link_com[1].checked == true){		
			//alert('온라인은 아직 사용하지 않습니다.'); return;
		}
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
    ...
    <tr> 
        <td colspan="2" class='line'> 
 		...
 		%>                
			...
			<!--<Script language='JavaScript'>CallInsertTypeE('<%=ht.get("REGNO")%>','<%=ht.get("RENT_L_CD")%>','<%=ht.get("RENT_ST")%>','<%=ht.get("IM_SEQ")%>');</Script>-->

                        <%=ht.get("DOC_STAT")%>
                        
                        <%if(!String.valueOf(ht.get("DOC_STAT")).equals("완료") && !String.valueOf(ht.get("DOC_STAT")).equals("승인거절") && !String.valueOf(ht.get("DOC_STAT")).equals("기간종료") && !String.valueOf(ht.get("DOC_STAT")).equals("계약파기")){%>
                            &nbsp;
 		            <a href="javascript:link_resend('1', '<%=ht.get("DOC_CODE")%>')" onMouseOver="window.status=''; return true">[재전송]</a>&nbsp;
                        <%}%>
                        <%if(!String.valueOf(ht.get("DOC_STAT")).equals("완료") && !String.valueOf(ht.get("DOC_STAT")).equals("계약파기")){%>
                            &nbsp;
        ...
        <td colspan="2" class='line'> 
 		...
 		%>                
                ...
                <tr>
                    ...
                    <td align="center" <%if(String.valueOf(ht.get("DOC_TYPE")).equals("2")){%>rowspan='2'<%}%>>                                            
                        <%if(!String.valueOf(ht.get("DOC_YN")).equals("D")){// && !String.valueOf(ht.get("DOC_STAT")).equals("완료")%>
                            &nbsp;
    ...
    <!--대기없이 바로 전송 가능-->
    <%if(!base.getUse_yn().equals("N") && wait_yn.equals("N")){%>
    <%    if(link_type.equals("2")){%>
	<tr>
    ...
    <tr> 
        <td><font color=red><b>※ 인터넷옵션-보안-인터넷-사용자지정수준-[도메인간의 데이터 원본 액세스]을 사용으로 되어 있어야 정상 처리됩니다. </b></font></td>
    </tr>    
<!--     <tr>  -->
