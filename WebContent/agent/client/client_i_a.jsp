...
<%@ page import="acar.client.*, acar.bill_mng.*"%>
...
<%	if(count == 0){
		if(client != null){%>
			alert('등록되었습니다');
			parent.location='/agent/client/client_s_frame.jsp?auth_rw=<%=auth_rw%>&br_id=<%=br_id%>&user_id=<%=user_id%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&asc=<%=asc%>&from_page=<%=from_page%>';
<%		}else{%>
			alert('등록되지 않았습니다');
<%		}
	}else{%>
