...
<!--
	...
	function update_client_etc(){
		var fm = document.form1;
		window.open('client_u_mini.jsp?client_id=<%=client_id%>&firm_nm=<%=client.getFirm_nm()%>&client_nm=<%=client.getClient_nm()%>&br_id=<%=br_id%>&user_id=<%=user_id%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&asc=<%=asc%>&from_page=<%=from_page%>', "CLIENT_ETC", "left=100, top=100, width=850, height=475, resizable=yes, scrollbars=yes, status=yes");
	}

	function mgr_modify(m_id, l_cd, mgr_st, car_no){
		var fm = document.form1;
		window.open('/acar/mng_client2/mgr_addr.jsp?m_id='+m_id+'&l_cd='+l_cd+'&mgr_st='+mgr_st+'&car_no='+car_no+'&client_id=<%=client_id%>&firm_nm=<%=client.getFirm_nm()%>&client_nm=<%=client.getClient_nm()%>&br_id=<%=br_id%>&user_id=<%=user_id%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&asc=<%=asc%>&from_page=<%=from_page%>', "MGR_ADDR"+l_cd+mgr_st, "left=100, top=100, width=650, height=275, resizable=yes, scrollbars=yes, status=yes");
	}
	
	function post_modify(m_id, l_cd, mgr_st, car_no){
		var fm = document.form1;
		window.open('/acar/mng_client2/post_addr.jsp?m_id='+m_id+'&l_cd='+l_cd+'&mgr_st='+mgr_st+'&car_no='+car_no+'&client_id=<%=client_id%>&firm_nm=<%=client.getFirm_nm()%>&client_nm=<%=client.getClient_nm()%>&br_id=<%=br_id%>&user_id=<%=user_id%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&asc=<%=asc%>&from_page=<%=from_page%>', "MGR_ADDR"+l_cd+mgr_st, "left=100, top=100, width=650, height=185, resizable=yes, scrollbars=yes, status=yes");
	}		
...
<table border="0" cellspacing="0" cellpadding="0" width=100%>
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>영업지원 > 고객관리 > <span class=style5>고객 일괄변경</span></span></td>
	...
	<!--
	<tr>
		<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/arrow_sh.gif align=absmiddle>&nbsp;
	    ...
	    <td align='right' class="line">
	        <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr>
                    ...
                    <td width='22%' class='title'> 우편물주소 <a href="javascript:select_post_reg('');"><img src="/acar/images/center/button_in_modify_ig.gif" align="absmiddle" border="0"></a></td>			
                    <td width='15%' class='title'> 운전면허번호<a href="javascript:select_post_reg('');"><img src="/acar/images/center/button_in_modify_ig.gif" align="absmiddle" border="0"></a></td>		
		...
		for(int i = 0 ; i < car_size ; i++){
			Hashtable car = (Hashtable)cars.elementAt(i);%>		
                <tr>
					...
					<td <%if(String.valueOf(car.get("USE_YN")).equals("N")){%>class=is<%}%> align="center"><%=car.get("CAR_NO")%></td>
					<td <%if(String.valueOf(car.get("USE_YN")).equals("N")){%>class=is<%}%> align="center">[<%=car.get("TAX_TYPE")%>]<a href="javascript:post_modify('<%=car.get("RENT_MNG_ID")%>', '<%=car.get("RENT_L_CD")%>', '', '<%=car.get("CAR_NO")%>')" onMouseOver="window.status=''; return true"><%=car.get("P_ADDR")%></a><br><br>&nbsp;<%=car.get("TAX_AGNT")%></td>
					<td <%if(String.valueOf(car.get("USE_YN")).equals("N")){%>class=is<%}%> align="center"><a href="javascript:post_modify('<%=car.get("RENT_MNG_ID")%>', '<%=car.get("RENT_L_CD")%>', '', '<%=car.get("CAR_NO")%>')" onMouseOver="window.status=''; return true"><%=car.get("LIC_NO")%></a></td><!-- 운전면허번호추가(20180921) -->
                    <td>
