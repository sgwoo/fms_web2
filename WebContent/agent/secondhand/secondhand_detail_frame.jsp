...
<frameset rows="135,*,1" border=0 cols="*"> 
  <frame name="detail_head" src="secondhand_detail_h.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&car_mng_id=<%=car_mng_id%>&rent_mng_id=<%=rent_mng_id%>&rent_l_cd=<%=rent_l_cd%>&jg_code=<%=jg_code%>&est_st=<%=est_st%>&fee_opt_amt=<%=fee_opt_amt%>&fee_rent_st=<%=fee_rent_st%>&gubun=<%=gubun%>&gubun2=<%=gubun2%>&gubun_nm=<%=gubun_nm%>&sort_gubun=<%=sort_gubun%>&brch_id=<%=brch_id%>&res_yn=<%=res_yn%>&res_mon_yn=<%=res_mon_yn%>&est_id=<%=est_id%>&spe_seq=<%=spe_seq%>&est_table=<%=est_table%>&from_page=<%=from_page%>&list_from_page=<%=list_from_page%>" marginwidth=10 marginheight=10 scrolling='no' noresize>
  <frame name="detail_body" src="secondhand_price_20090901.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&car_mng_id=<%=car_mng_id%>&rent_mng_id=<%=rent_mng_id%>&rent_l_cd=<%=rent_l_cd%>&jg_code=<%=jg_code%>&est_st=<%=est_st%>&fee_opt_amt=<%=fee_opt_amt%>&fee_rent_st=<%=fee_rent_st%>&gubun=<%=gubun%>&gubun2=<%=gubun2%>&gubun_nm=<%=gubun_nm%>&sort_gubun=<%=sort_gubun%>&brch_id=<%=brch_id%>&res_yn=<%=res_yn%>&res_mon_yn=<%=res_mon_yn%>&est_id=<%=est_id%>&spe_seq=<%=spe_seq%>&est_table=<%=est_table%>&from_page=<%=from_page%>&list_from_page=<%=list_from_page%>" marginwidth=10 marginheight=10 scrolling="auto" noresize>
