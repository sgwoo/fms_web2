...
// 											fm.rg_8_amt.value = parseDecimal(toInt(parseDigit(fm.o_1.value)) * toInt(fm.rg_8.value)/100);	
	...
	function EstiReg(){
		...
		if (Number(fm.agree_dist.value.replace(/,/g,"")) < 5000 || Number(fm.agree_dist.value.replace(/,/g,"")) > 50000) {
    		alert("연간 약정운행거리 최대값을 초과하여 입력하였습니다.\n\n* 차종별 연간 약정운행거리 최대값\n- 스타렉스 벤 및 트럭 : 5,000 ~ 60,000kmkm/년 이하\n- 그외차종 : 5,000 ~ 50,000km/년 이하");
    		return;
    	}
		<%}%>
  ...
  <form action="" name="form1" method="POST" >
        ...
        <td colspan=2>
            <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>재리스 > 견적내기 > <span class=style5><%=rent_l_cd%> 조정견적</span></span></td>
    ...
    <tr> 
        <td class=line colspan="2"> 
			...
			<!--<option value="3" <%if(ins_dj.equals("3"))%>selected<%%>>5억원</option>-->
                      </select> </td>
                </tr>
                <tr> 
                    ...
                    <td><select name="ins_age">
                        ...
                        <option value="5" <%if(ins_age.equals("5"))%>selected<%%>>만30세이상</option>
                        <option value="6" <%if(ins_age.equals("6"))%>selected<%%>>만35세이상</option>
                        <option value="7" <%if(ins_age.equals("7"))%>selected<%%>>만43세이상</option>
                        <option value="8" <%if(ins_age.equals("8"))%>selected<%%>>만48세이상</option>
                        <option value="9" <%if(ins_age.equals("9"))%>selected<%%>>만22세이상</option>
                        <option value="10" <%if(ins_age.equals("10"))%>selected<%%>>만28세이상</option>
                        <option value="11" <%if(ins_age.equals("11"))%>selected<%%>>만35세이상~만49세이하</option>
                      </select></td>
                </tr>
                <tr> 
                ...
                <tr> 
                    ...
                    <td><select name="insurant">
                            <option value="1" <%if(insurant.equals("1")||ins_per.equals(""))%>selected<%%>>아마존카</option>
                            <%if(a_a.equals("12")){%>
                ...
                <tr> 
                    ...
                    <td><select name="com_emp_yn">
                            ...
                            <option value="Y" <%if(com_emp_yn.equals("Y"))%>selected<%%>>가입</option>
                          </select></td>
                </tr>               
                <%}else{%>  
