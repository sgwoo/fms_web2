...
<%
	...
	for(int i=0; i<a_r.length; i++){
        a_bean = a_r[i];
		chk_view = a_bean.getP_view().equals("A") || a_bean.getP_view().equals("J");
		if(a_bean.getBbs_st().equals("8") || (a_bean.getBbs_st().equals("1") && chk_view) || (a_bean.getBbs_st().equals("4") && chk_view) ||
			(a_bean.getBbs_st().equals("5") && chk_view) || (a_bean.getBbs_st().equals("6") && chk_view) || 
			(a_bean.getBbs_st().equals("2") && chk_view)){
			anc_count++;
		}
		if(a_bean.getBbs_st().equals("3") && (a_bean.getP_view().equals("A") || a_bean.getP_view().equals("J"))){
			pan_count++;
		}
	}
	...
	int div_height = 100+count_gong*39;
	
	//에이전트 견적만인 사용자중 공지사항 미공개
	String main_yn="Y";
	
	if(acar_id.equals("000348")){
		main_yn="N";
	}
		...
		<div id = "anc_div" style="width:1024px; height:<%=div_height%>px; margin:0 auto;">
			...
			<div id = "anc_div2" style="float:left;  width:510px; height:<%=div_height%>px; border:1px solid #dcdcdc;">
				...
				<div style="float:left;  width:47px; height:52px; ">
					<a class="page-link" href="/agent/agent_bbs/agent_bbs_frame.jsp" param="FMS운영관리:공지사항:공지사항">
			...
			<!--판매조건-->
			<div id = "anc_div3" style="float:left; width:510px; border:1px solid #dcdcdc; border-left: 0px; height:<%=div_height%>px;">
				...
				<div style="float:left;  width:47px; height:52px; ">
					<a class="page-link" href="/agent/agent_bbs/agent_bbs_frame.jsp" param="FMS운영관리:공지사항:판매사항"><span class="icon-plus panPlus"></span></a>
				</div>
				<div style="clear:both;  width:490px; height:52px; border-top:1px solid #dcdcdc;padding:10px;">
					<table id="bigPan" style="line-height:18px; width:490px;">
						<%
							int pan_in_count = 0;
							AncBean a_r2 [] = oad.getAncAll(acar_id);
							String[] latestObj2 = new String[4];
						...
						<tr>
							<td style="font-size:15px;">
								<a href="javascript:Anc_Open('<%=a_bean.getBbs_id()%>','<%=acar_id%>');"
						    		><span class="ancTableTitle" style="font-size:15px;"><%=Util.subData(String.valueOf(a_bean.getTitle()),35)%></span>
						    			<%if(a_bean.getRead_yn().equals("Y")){%><!-- 필독처리 -->
						    				<span style="font-size:15px; background-color:rgba(238, 34, 0, 0.81);border-radius:2px; color:white; vertical-align:middle;">
								  			<span style="font-size:11px;vertical-align:text-top;margin-bottom:5px;">&nbsp;필독&nbsp;</span></span>
						    			<%}%>
