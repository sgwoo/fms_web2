...
function view_detail(off_id, off_nm){
	<%if(from_page.equals("/agent/consignment/cons_i_c.jsp")){%>
		...
		parent.self.close();
        <%}else if(from_page.equals("/agent/car_pur/reg_cons.jsp") || from_page.equals("/agent/car_pur/pur_doc_i.jsp")){%>		
		parent.opener.form1.off_id.value = off_id;
		parent.opener.form1.off_nm.value = off_nm;
  		<%if(from_page.equals("/agent/car_pur/reg_cons.jsp")){%>				
...
<form name="form1" action="" method="post">
    <tr> 
      <td><iframe src="cus0602_sc_in.jsp?s_kd=<%=s_kd%>&t_wd=<%=t_wd%>&sort_gubun=<%=sort_gubun%>&sort=<%=sort%>&from_page=<%=from_page%>&use_yn=<%=use_yn %>" name="serv_offList" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
    </tr>
