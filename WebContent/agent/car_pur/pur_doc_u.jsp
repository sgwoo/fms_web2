...
<%@ page import="acar.car_office.*, acar.user_mng.*, acar.tint.*, acar.doc_settle.*, card.*, acar.car_sche.*, acar.estimate_mng.*, acar.forfeit_mng.*"%>
...
<!--
	...
	function view_emp(emp_id){
		var fm = document.form1;
		window.open("/agent/car_office/car_office_p_s.jsp?user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/fms2/lc_rent/lc_c_c_fee.jsp&cmd=view&emp_id="+emp_id, "VIEW_EMP", "left=50, top=50, width=850, height=700, resizable=yes, scrollbars=yes, status=yes");
	}
	
	//영업소보기
	function view_car_office(car_off_id){
		var fm = document.form1;
		window.open("view_car_office.jsp?user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/fms2/car_pur/pur_doc_u.jsp&cmd=view&car_off_id="+car_off_id, "VIEW_CAR_OFF", "left=50, top=50, width=850, height=200, resizable=yes, scrollbars=yes, status=yes");
	}	
	...
	{
		var fm = document.form1;	
		window.open("s_grt_suc.jsp?from_page=/agent/car_pur/pur_doc_u.jsp&t_wd=<%=base.getClient_id()%>&rent_l_cd=<%=rent_l_cd%>&rent_dt=<%=base.getRent_dt()%>", "SERV_GRT_OFF", "left=10, top=10, width=800, height=500, scrollbars=yes, status=yes, resizable=yes");
	}		
	...
	function update_car_amt(){
		var st = 'car_amt';
		var height = 400;
		window.open("/agent/lc_rent/lc_c_u.jsp<%=valus%>&cng_item="+st, "CHANGE_ITEM", "left=50, top=50, width=850, height="+height+", resizable=yes, scrollbars=yes, status=yes");
	}
	...
	function cng_com_tint(){
		...
		if(fm.com_tint.value ==''){									//없음
			...
			fm.cleaner_st.value	 = '2';//있음[0].checked	=true;
			<%}%>
	...
	function tint_update(st){
		if(st == '1')	window.open("/agent/lc_rent/lc_c_u_tint1.jsp<%=valus%>", "CHANGE_TINT", "left=100, top=100, width=950, height=650");
		else		window.open("/agent/lc_rent/lc_c_u_tint2.jsp<%=valus%>&st="+st, "CHANGE_TINT", "left=100, top=100, width=950, height=650");	
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
	    <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>문서스캔 &nbsp;
	    <%if(!doc.getDoc_step().equals("3")){ %><a href="javascript:view_scan()" onMouseOver="window.status=''; return true" title="클릭하세요"><img src=/acar/images/center/button_scan.gif align=absmiddle border=0></a><%} %></span></td>
	</tr>  			
    <tr>
        <td class=line2></td>
    </tr>  
            ...
            <table border="0" cellspacing="1" cellpadding='0' width=100%>	
    		  <%if(!cr_bean.getCar_no().equals("")){%>
                ...
                <tr>
                    ...
                    <td>&nbsp;<input type='text' size='11' name='udt_est_dt' maxlength='12' class='default' value='<%=AddUtil.ChangeDate2(pur.getUdt_est_dt())%>' onBlur='javscript:this.value = ChangeDate(this.value);'></td>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
				...
				<tr>                                
                    ...
                    <%if(fdcd.getAmt4() >0){%>
                    <font color=red>※ 카드할부 승인신청 차량입니다. 승인일자는 <%=AddUtil.ChangeDate2(fdcd.getVio_dt())%> 입니다. 
                    <%	if(AddUtil.parseInt(AddUtil.replace(pur.getCon_est_dt(),"-","")) < AddUtil.parseInt(AddUtil.replace(fdcd.getVio_dt(),"-",""))){ %>
                    승인일자보다 선처리는 안됩니다. 
                    <%	}%>                    
                    지급요청일 변경시 출고/대출담당자에게 메시지를 발송합니다. 출고담당자는 유의하여 주십시오.</font>
                    <%}%>
    ...
    <tr>
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr>
                    <td align="center">
        			  <select name="trf_st<%=j+1%>" class='default' readonly>
                        <option value="">==선택==</option>
                    ...
                    <td align="center"><input type='text' size='10' maxlength='15' name='trf_amt<%=j+1%>' readonly class='defaultnum' value='<%=AddUtil.parseDecimal(trf_amt)%>' onBlur='javascript:this.value=parseDecimal(this.value); set_trf_amt();'>
                      원</td>
                    <td align="center">
        			  <select name="card_kind<%=j+1%>" class='default' readonly>
                  	    <option value=''>선택</option>
                  		<%	if(ck_size > 0){
        						for (int i = 0 ; i < ck_size ; i++){
        							Hashtable ht = (Hashtable)card_kinds.elementAt(i);%>
                    ...
                    <td align="center">
        			  <select name="acc_st<%=j+1%>" class='default' readonly>
                        <option value="">==선택==</option>
                    ...
                    <td align="center">
        			  <input type='text' size='22' maxlength='100' name='cardno<%=j+1%>' readonly class='default' value='<%=cardno%>'>
        			</td>
                    <td align="center"><input type='text' size='25' maxlength='100' name='trf_cont<%=j+1%>' readonly class='default' value='<%=trf_cont%>'>
					</td>
                    <td align="center">
                ...
                <tr>
                    <td class=title>합계</td>
                    <td align="center"><input type='text' size='10' maxlength='10' name='tot_trf_amt' readonly class='defaultnum' value='<%=AddUtil.parseDecimal(pur.getTrf_amt1()+pur.getTrf_amt2()+pur.getTrf_amt3()+pur.getTrf_amt4())%>'>
                      원</td>
                    <td class=title>처리예정일</td>
                    <td>&nbsp;
                      <input type='text' size='11' name='pur_est_dt' maxlength='12' class='default' readonly value='<%=AddUtil.ChangeDate2(pur.getPur_est_dt())%>' onBlur='javscript:this.value = ChangeDate(this.value);'>					  
					  </td>
...
<%--                       <%if(car.getNew_license_plate().equals("2")){%>&nbsp;신형번호판신청(대전/대구/광주/부산)<%}%> --%>
	...
	<tr>
	    <td> <span class=style2>블랙박스 </span>
            	...
            	<%if(!doc.getDoc_step().equals("3") && (base.getReg_id().equals(user_id) || base.getBus_id().equals(user_id) || cont_etc.getBus_agnt_id().equals(user_id))){%>
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr> 
                    ...
                    <td class=title width=10%>완결예정일</td>
                    <td width=15%>&nbsp;<input type='text' size='11' name='guar_est_dt' maxlength='12' class='<%if(cont_etc.getGuar_end_st().equals("2")){%>white<%}%>text' value='<%=AddUtil.ChangeDate2(cont_etc.getGuar_est_dt())%>' onBlur='javscript:this.value = ChangeDate(this.value);'></td>
        	    </tr>	
        		  <%}%>		
    		</table>
	    </td>
