...
<%
	...
	Vector commis = ac_db.getCommis(emp_id, ck_acar_id);
...
<!--
	...
	function commi_doc(rent_mng_id, rent_l_cd, doc_no){
		var fm = document.form1;
		if(doc_no == ''){
			window.open("/agent/commi/commi_doc_i.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/fms2/lc_rent/lc_c_c_fee.jsp&rent_mng_id="+rent_mng_id+"&rent_l_cd="+rent_l_cd+"&doc_no="+doc_no, "COMMI_DOC", "left=50, top=50, width=850, height=600, scrollbars=yes");
		}else{
			window.open("/agent/commi/commi_doc_u.jsp?auth_rw=<%=auth_rw%>&user_id=<%=user_id%>&br_id=<%=br_id%>&from_page=/fms2/lc_rent/lc_c_c_fee.jsp&rent_mng_id="+rent_mng_id+"&rent_l_cd="+rent_l_cd+"&doc_no="+doc_no, "COMMI_DOC", "left=50, top=50, width=850, height=600, scrollbars=yes");		
		}
	}	
//-->
</script>
	    ...
	    <td class='line' width='60%'>
		    <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <tr>
					...
					<!--
                    <td width=10% class=title>처리</td>
                    <td width=10% class=title>취소</td>			
					-->
		        </tr>
		    </table>
	    </td>
          ...
          <%for(int i = 0 ; i < commi_size ; i++){
			CommiBean commi = (CommiBean)commis.elementAt(i);%>
                <tr> 
                    ...
                    <td width=17% align='center'><span title='<%=commi.getFirm_nm()%>'><%=Util.subData(commi.getFirm_nm(), 5)%></span></td>
