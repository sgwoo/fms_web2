...
<!--
	...
	<%if(go_url.equals("/agent/pay_mng/pay_dir_reg.jsp")){%>
	...
	function Disp1(user_pos, user_id, user_nm, user_h_tel, user_m_tel){
		var fm = document.form1;
		if('<%=go_url%>' == '/agent/consignment/cons_reg_step3.jsp' && '<%=size%>' == '1'){
			if(fm.value.value == '1'){
		...
		}else if('<%=go_url%>' == '/agent/consignment/cons_reg_step2.jsp' && '<%=size%>' == '1'){
			if(fm.value.value == '1'){
				opener.form1.<%=st%>_tel.value 		= user_h_tel;
				opener.form1.<%=st%>_title.value 	= user_pos;
				opener.form1.<%=st%>_man.value 		= user_nm;
        ...
        <td class=line>
    	    <table border="0" cellspacing="1" width=100%>
                ...
                <%for (int i = 0 ; i < vt_size ; i++){
    				...
    				if(!m_tel.contains("01035580026")  &&  !m_tel.contains("01090905323") &&  !m_tel.contains("01049451922") &&
        					!m_tel.contains("01040140911")	
        				){		//맹영철(휴대폰번호 바뀜)(2017.09.25) , 정만복, 정형석, 유명훈 추가	
    			%>
	                <tr align="center">
	                    <td><%=i+1%></td>
