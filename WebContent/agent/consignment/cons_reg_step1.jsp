...
//		if(fm.cons_su.value == "0")	{ 	alert("차량대수를 입력하십시오."); 		return;	}
		...
		if(fm.off_id.value == '002740' || fm.off_id.value == '003158' || fm.off_id.value == '009217'  || fm.off_id.value == '010255' ){  //전국탁송, 아마존카, 아마존탁송 인 경우 필수
		...
		//		alert(fm.cmp_app[0].value);
				...
				if(fm.cmp_app[1].value == "" || fm.cmp_app[1].value == "S00" || fm.cmp_app[1].value == "D00" || fm.cmp_app[1].value == "B00" || fm.cmp_app[1].value == "C00" || fm.cmp_app[1].value == "E00"  || fm.cmp_app[1].value == "F00"  || fm.cmp_app[1].value == "T00" || fm.cmp_app[1].value == "W00"){	alert('탁송구간2을 선택하십시오.');	return;}
			}
		}
		...
		for(i=0; i<size ; i++){
			if(fm.car_no[i].value == "") 		{ 	alert((i+1)+"번 탁송 : 선택된 차량이 없습니다. 차량번호가 없으면 차대번호를 직접 입력하세요");	return;	}
		...
		cut_length2("[탁송]출발일시:"+fm.from_req_dt[0].value+" "+fm.from_req_h[0].value+"시/출발지:"+fm.from_place[0].value+"/도착지:"+fm.to_place[0].value,90);		
		...
		if( fm.off_id.value == '009217') {	
			...
			if (fm.cons_st.value == "2") {
				//탁송1
				if (fm.cmp_app[0].value >= "T41") {  //영남주차장과 신엠제이 외에만 사용 ( 지방으로 탁송은 상관없음)				   
					if (fm.from_comp[0].value.indexOf("(주)신엠제이모터스") !=-1 || fm.from_comp[0].value.indexOf("(주)아마존카 본사") !=-1 || fm.to_comp[0].value.indexOf("(주)신엠제이모터스") !=-1 || fm.to_comp[0].value.indexOf("(주)아마존카 본사") !=-1) {
						alert("1번 탁송 구간을 (영남,신엠제이)구간으로 다시 선택하세요!!!"); 	
						return;						
					}				
				}
		
				if (fm.cmp_app[0].value <= "T08" || fm.cmp_app[0].value == "T40" ) {  //영남주차장과 신엠제이 이외는 기존 거 사용 불가 ( 지방으로 탁송은 상관없음)				   
					if (fm.from_comp[0].value.indexOf("(주)신엠제이모터스")!=-1 || fm.from_comp[0].value.indexOf("(주)아마존카 본사")!=-1 || fm.to_comp[0].value.indexOf("(주)신엠제이모터스")!=-1 || fm.to_comp[0].value.indexOf("(주)아마존카 본사")!=-1) {
					} else {	
						alert("1번 탁송 구간을(영남,신엠제이 제외)구간으로 다시 선택하세요!!!"); 	
						return;						
					}					
				}
								
				//탁송2
				if (fm.cmp_app[1].value >= "T41") {  //영남주차장과 신엠제이 외에만 사용 ( 지방으로 탁송은 상관없음)				   
					if (fm.from_comp[1].value.indexOf("(주)신엠제이모터스")!=-1 || fm.from_comp[1].value.indexOf("(주)아마존카 본사")!=-1 || fm.to_comp[1].value.indexOf("(주)신엠제이모터스")!=-1 || fm.to_comp[1].value.indexOf("(주)아마존카 본사")!=-1) {
						alert("2번 탁송 구간을 (영남,신엠제이)구간으로 다시 선택하세요!!!"); 	
						return;						
					}					
				}
				
				if (fm.cmp_app[1].value <= "T08" || fm.cmp_app[1].value == "T40" ) {  //영남주차장과 신엠제이 이외는 기존 거 사용 불가 ( 지방으로 탁송은 상관없음)				   
					if (fm.from_comp[1].value.indexOf("(주)신엠제이모터스")!=-1 || fm.from_comp[1].value.indexOf("(주)아마존카 본사")!=-1 || fm.to_comp[1].value.indexOf("(주)신엠제이모터스")!=-1 || fm.to_comp[1].value.indexOf("(주)아마존카 본사")!=-1) {
					} else {	
						alert("2번 탁송 구간을 (영남,신엠제이 제외)구간으로 다시 선택하세요!!!"); 	
						return;						
					}						
				}
			...
			if (fm.cons_st.value == "2") {
				...
				if (fm.cmp_app[0].value >= "E01" && fm.cmp_app[0].value <= "E47") {  //영남주차장만 사용 (지방으로 탁송은 상관없음)
					...
					if ( fm.from_comp[0].value.indexOf("오토크린") !=-1 || fm.to_comp[0].value.indexOf("오토크린") !=-1|| fm.from_comp[0].value.indexOf("아마존모터스") !=-1 || fm.to_comp[0].value.indexOf("아마존모터스") !=-1)   {		
						...
						alert("1번 탁송 구간을 (영남및 그외정비소  출발/도착)구간으로 다시 선택하세요!!!"); 	
						return;						
					}						
				}
				...
				if (fm.cmp_app[1].value >= "E01" && fm.cmp_app[1].value <= "E47") {  //영남주차장만 사용 (지방으로 탁송은 상관없음)
					...
					if ( fm.from_comp[1].value.indexOf("오토크린") !=-1 || fm.to_comp[1].value.indexOf("오토크린") !=-1|| fm.from_comp[1].value.indexOf("아마존모터스") !=-1 || fm.to_comp[1].value.indexOf("아마존모터스") !=-1)   {		
						...
						alert("2번 탁송 구간을 (영남및 그외정비소  출발/도착)구간으로 다시 선택하세요!!!"); 	
						return;						
					}						
				}
    	...
    	<td colspan=10>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>Agent > 탁송관리 > <span class=style5>탁송의뢰등록</span></span></td>
...
<%				}%>				
	...
	<tr>
	    <td><font color="green">* 사전전화요청에 대해서는 2010-06-08 [탁송등록에관한 간편처리방안] 이란 제목으로 업그레이드정보에 공지되어 있습니다.</font></td>
	</tr>	
	<tr>
	    <td align="right" style='height:1; background-color:#e5e5e5;'></td>
	</tr>	
