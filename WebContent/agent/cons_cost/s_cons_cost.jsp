        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                ...
                <tr> 
                    ...
                    <td colspan='3'>&nbsp;<%if(udt_st.equals("1")){%>서울본사<%}%><%if(udt_st.equals("2")){%>부산지점<%}%><%if(udt_st.equals("3")){%>대전지점<%}%><%if(udt_st.equals("5")){%>대구지점<%}%><%if(udt_st.equals("6")){%>광주지점<%}%></td>
    		    </tr>
    		</table>
	    </td>
	</tr> 
        ...
        <td class=line> 
    		...
    		%>
                <tr align="center">
                    ...
                    <td><%if(udt_st.equals("1")){%><a href="javascript:Disp1(<%=ht.get("TO_PLACE1")%>)" onMouseOver="window.status=''; return true"><%=AddUtil.parseDecimal(ht.get("TO_PLACE1"))%></a><%}else{%><%=AddUtil.parseDecimal(ht.get("TO_PLACE1"))%><%}%></td>
                    <td><%if(udt_st.equals("2")){%><a href="javascript:Disp1(<%=ht.get("TO_PLACE2")%>)" onMouseOver="window.status=''; return true"><%=AddUtil.parseDecimal(ht.get("TO_PLACE2"))%></a><%}else{%><%=AddUtil.parseDecimal(ht.get("TO_PLACE2"))%><%}%></td>
                    <td><%if(udt_st.equals("3")){%><a href="javascript:Disp1(<%=ht.get("TO_PLACE3")%>)" onMouseOver="window.status=''; return true"><%=AddUtil.parseDecimal(ht.get("TO_PLACE3"))%></a><%}else{%><%=AddUtil.parseDecimal(ht.get("TO_PLACE3"))%><%}%></td>		  
                    <td><%if(udt_st.equals("5")){%><a href="javascript:Disp1(<%=ht.get("TO_PLACE4")%>)" onMouseOver="window.status=''; return true"><%=AddUtil.parseDecimal(ht.get("TO_PLACE4"))%></a><%}else{%><%=AddUtil.parseDecimal(ht.get("TO_PLACE4"))%><%}%></td>
                    <td><%if(udt_st.equals("6")){%><a href="javascript:Disp1(<%=ht.get("TO_PLACE5")%>)" onMouseOver="window.status=''; return true"><%=AddUtil.parseDecimal(ht.get("TO_PLACE5"))%></a><%}else{%><%=AddUtil.parseDecimal(ht.get("TO_PLACE5"))%><%}%></td>		  
                </tr>
            <%		}%>
            </table>
	    </td>
