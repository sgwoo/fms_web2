...
<!--
	...
	function save(){
		...
		/* 20140828 에이전트 요청 용품은 별도 의뢰해서 처리하기로 함.
		if(fm.tint_end_yn.value == 'N'){
			alert('용품이 정산되지 않았습니다. 정산후 기안하세요'); 
			return;	
		}
		*/
        ...
        <td class=line> 
            <table border="0" cellspacing="1" cellpadding='0' width=100%>
                <tr> 
                    ...
                    <td width=15%>&nbsp;<input type='text' size='12' name='pp_est_dt' maxlength='12' class="<%if(pp_st.equals("완결")||pp_st.equals("면제"))%>white<%%>text" value='<%=AddUtil.ChangeDate2(fee.getPp_est_dt())%>' onBlur='javscript:this.value = ChangeDate(this.value);'></td>
                ...
                <tr> 
                    ...
                    <td class=title width=10%>완결예정일</td>
                    <td width=15%>&nbsp;<input type='text' size='11' name='gi_est_dt' maxlength='10' class="<%if(gi_st.equals("완결")||gi_st.equals("면제")){%>white<%}%>text" value='<%=AddUtil.ChangeDate2(gins.getGi_est_dt())%>' onBlur='javscript:this.value = ChangeDate(this.value);'></td>
                    <td class=title width=10%>미결사유</td>
    	    ...
    	    <table border="0" cellspacing="1" cellpadding="0" width=100%>
                ...
                <tr> 
                    ...
                    <td colspan='3'>&nbsp;
                        ...
                        <font color='red'>※ 블랙박스 일련번호가 없거나 7자리 미만으로 정상번호가 아닙니다. 계약관리-용품에서 먼저 입력하십시오. 미처리시 영업수당 기안등록이 안됩니다.</font>
                        <%} %>
                    </td>
                </tr>  
                <tr> 
            ...
            <td colspan='5'>&nbsp;
              <%if(String.valueOf(alink_lc_rent.get("RENT_L_CD")).equals(rent_l_cd)){%>[전자계약서]<%}%>
                ...
                <tr> 
                    ...
                    <td>&nbsp;<%String pur_bus_st = pur.getPur_bus_st();%><%if(pur_bus_st.equals("1")){%>자체영업<%}else if(pur_bus_st.equals("2")){%>영업사원영업<%}else if(pur_bus_st.equals("3")){%>실적이관<%}%></td>
