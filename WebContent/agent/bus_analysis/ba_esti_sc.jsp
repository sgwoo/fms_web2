...
<!--
	...
	function EstiDisp(est_type, est_id, set_code){	
		...
		fm.target = 'd_content';
		fm.action = '/agent/estimate_mng/esti_mng_atype_u.jsp';		
		fm.submit();
	}
//-->
</script>
...
<form name='form1' action='' method='post'>
  ...
  <input type='hidden' name='from_page2' value='/agent/bus_analysis/ba_esti_frame.jsp'>
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
	...
	<td>
		...
		<tr>
		    <td colspan=2><iframe src="ba_esti_sc_in.jsp?auth_rw=<%=auth_rw%>&br_id=<%=br_id%>&user_id=<%=user_id%>&gubun1=<%=gubun1%>&gubun2=<%=gubun2%>&gubun3=<%=gubun3%>&gubun4=<%=gubun4%>&gubun5=<%=gubun5%>&gubun6=<%=gubun6%>&s_dt=<%=s_dt%>&e_dt=<%=e_dt%>&s_kd=<%=s_kd%>&t_wd=<%=t_wd%>" name="EstiList" width="100%" height="<%=height%>" cellpadding="0" cellspacing="0" topmargin=0 marginwidth="0" border="0" frameborder="0"></iframe></td>
		</tr>								
	    </table>
        </td>
    </tr>
