...
<!--
	...
	function setCode(id, seq, nm, amt, s_st, jg_code, jg_f, jg_g, jg_3, jg_w, jg_h, jg_i, jg_b, jg_opt_st, jg_g_7, end_dt, jg_tuix_st, lkas_yn, ldws_yn, aeb_yn, fcw_yn, car_etc, car_etc2, jg_opt_yn, garnish_yn, hook_yn, jg_g_15) {
		//alert('->' + lkas_yn + '|' + ldws_yn + '|' + aeb_yn + '|' + fcw_yn);//del
		...
		if (jg_g_7 == "3") {
			...
			if (car_comp_id == "0056" || car_comp_id == '0056') {				
				//용품
		...
		if (car_comp_id == "0056") {
			...
			//$(opener.document).find("#sel_a_b_1 option[value='60']").prop("disabled", true);
			...
			//$(opener.document).find("#sel_a_b_2 option[value='60']").prop("disabled", true);
			...
			//$(opener.document).find("#sel_a_b_3 option[value='60']").prop("disabled", true);
			...
			//$(opener.document).find("#sel_a_b_4 option[value='60']").prop("disabled", true);
		...
		fm.lkas_yn_opt_st.value = lkas_yn;		// 차선이탈 제어형 옵션 설정 여부
		fm.ldws_yn_opt_st.value = ldws_yn		// 차선이탈 경고형 옵션 설정 여부
		fm.aeb_yn_opt_st.value = aeb_yn		// 긴급제동 제어형 옵션 설정 여부
		fm.fcw_yn_opt_st.value = fcw_yn 		// 긴급제동 경고형 옵션 설정 여부
	...
	function save(){
		...
		<%if(idx.equals("2") || idx.equals("3")){%>
			<%if( (idx.equals("2") && car_comp_id.equals("0005") && (car_cd.equals("69") || car_cd.equals("70")))  || idx.equals("3") ){%>
				if(fm.dir_nm.value == ''){
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
    	...
    	<td>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1><span class=style5><%if(idx.equals("1")){%>차종<%}else if(idx.equals("2")){%>옵션<%}else if(idx.equals("3")){%>색상<%}else if(idx.equals("5")){%>연비<%}else{%>제조사DC<%}%></span></span></td>
    ...
    <tr>
    	<td style="text-align:right;">
    		※ <span style="font-weight:bold;color:red;">연비 미선택 가능</span>: 연비 선택시에만 견적서상에 연비관련 라인이 추가됩니다</br>
    		※ 타이어 <span style="font-weight:bold;color:red;">휠 사이즈</span>에 따라 연비가 차이남<span style="font-weight:bold;color:red;">(옵션선택시 주의 요망)</span>
    	</td>
    </tr>
    <%} %>
    <tr> 
    ...
    <tr> 
        <td class=line>
		    ...
		    <td class="title" width=25%>비고</td>	
   					...
   					%>
		   				<%if (String.valueOf(var.get("JG_TUIX_ST")).equals("Y")) {%>
		   					<tr class="hide" style="display: none;">
			                    ...
			                    <td>&nbsp;
			                        <a href="javascript:setCode('<%=var.get("ID")%>', '<%=var.get("SEQ")%>', '<%=var.get("NM")%>', '<%=var.get("AMT")%>', '<%=var.get("S_ST")%>', '<%=var.get("JG_CODE")%>', '<%=var.get("JG_F")%>', '<%=var.get("JG_G")%>', '<%=var.get("JG_3")%>', '<%=var.get("JG_W")%>', '<%=var.get("JG_H")%>', '<%=var.get("JG_I")%>', '<%=var.get("JG_B")%>', '<%=var.get("JG_OPT_ST")%>', '<%=var.get("JG_G_7")%>', '<%=var.get("END_DT")%>', '<%=var.get("JG_TUIX_ST")%>', '<%=var.get("LKAS_YN")%>', '<%=var.get("LDWS_YN")%>', '<%=var.get("AEB_YN")%>', '<%=var.get("FCW_YN")%>', '<%=var.get("ETC")%>', '<%=var.get("ETC2")%>', '<%=var.get("JG_OPT_YN")%>', '<%=var.get("GARNISH_YN")%>', '<%=var.get("HOOK_YN")%>');"><%=var.get("NM")%></a>    
			                    </td>
			                    <td align="right"><%=AddUtil.parseDecimal(String.valueOf(var.get("AMT")))%>원</td>			                    
					    		<td>&nbsp;<%=var.get("ETC")%>
                ...
                <%} else {%>
	              <%for(int i = 0 ; i < size ; i++){
	                ...
	                <tr> 
	                    <td align="center">
	                        <%if(idx.equals("2")){%>
	                            <input type="checkbox" name="car_s_seq" onClick="javascript:setOptSum();" value='<%=var.get("ID")%>||<%=var.get("NM")%>||<%=var.get("AMT")%>||<%=var.get("JG_OPT_ST")%>||<%=var.get("JG_TUIX_ST")%>||<%=var.get("LKAS_YN")%>||<%=var.get("LDWS_YN")%>||<%=var.get("AEB_YN")%>||<%=var.get("FCW_YN")%>||<%=var.get("HOOK_YN")%>'>
	                        <%}else if(idx.equals("3")){%>
	                    ...
	                    <td>&nbsp;
	                        ...
	                        <%}else if(idx.equals("1")){%>
	                        	<table style="margin-top: -15px;">
	                        		<tr>
	                        			<td width="90%"><a href="javascript:setCode('<%=var.get("ID")%>', '<%=var.get("SEQ")%>', '<%=var.get("NM")%>', '<%=var.get("AMT")%>', '<%=var.get("S_ST")%>', '<%=var.get("JG_CODE")%>', '<%=var.get("JG_F")%>', '<%=var.get("JG_G")%>', '<%=var.get("JG_3")%>', '<%=var.get("JG_W")%>', '<%=var.get("JG_H")%>', '<%=var.get("JG_I")%>', '<%=var.get("JG_B")%>', '<%=var.get("JG_OPT_ST")%>', '<%=var.get("JG_G_7")%>', '<%=var.get("END_DT")%>', '<%=var.get("JG_TUIX_ST")%>', '<%=var.get("LKAS_YN")%>', '<%=var.get("LDWS_YN")%>', '<%=var.get("AEB_YN")%>', '<%=var.get("FCW_YN")%>', '<%=var.get("ETC")%>', '<%=var.get("ETC2")%>', '', '<%=var.get("GARNISH_YN")%>', '<%=var.get("HOOK_YN")%>', '<%= var.get("JG_G_15")%>');"><%=var.get("NM")%></a></td>
	                        			<td><input type="button" class="button" value="사양" onclick="javascript:view_spec('<%=var.get("ID")%>', '<%=var.get("SEQ")%>');"></td>
	                        		</tr>
	                        	</table>
	                        <%}else{%>
	                            <a href="javascript:setCode('<%=var.get("ID")%>', '<%=var.get("SEQ")%>', '<%=var.get("NM")%>', '<%=var.get("AMT")%>', '<%=var.get("S_ST")%>', '<%=var.get("JG_CODE")%>', '<%=var.get("JG_F")%>', '<%=var.get("JG_G")%>', '<%=var.get("JG_3")%>', '<%=var.get("JG_W")%>', '<%=var.get("JG_H")%>', '<%=var.get("JG_I")%>', '<%=var.get("JG_B")%>', '<%=var.get("JG_OPT_ST")%>', '<%=var.get("JG_G_7")%>', '<%=var.get("END_DT")%>', '<%=var.get("JG_TUIX_ST")%>', '<%=var.get("LKAS_YN")%>', '<%=var.get("LDWS_YN")%>', '<%=var.get("AEB_YN")%>', '<%=var.get("FCW_YN")%>', '<%=var.get("ETC")%>', '<%=var.get("ETC2")%>', '', '<%=var.get("GARNISH_YN")%>', '<%=var.get("HOOK_YN")%>');"><%=var.get("NM")%></a>
	                        <%}%>    
	                    </td>                                        			
                        <%if(idx.equals("1")){%>
              ...
              <%if(idx.equals("2") || idx.equals("3")){%>
              	<%if( (idx.equals("2") && car_comp_id.equals("0005") && (car_cd.equals("69") || car_cd.equals("70")))  
              			|| (idx.equals("3") && Integer.parseInt(car_comp_id) > 5) ){%>
                <tr> 
                    <td class="title" style="height: 40px;" width=5%>-</td>
                    <td width=40%>&nbsp;                
                            <%if(idx.equals("2")){%>
		    ...
		    <td align=right width=30%><input type="text" name="dir_amt" value="" size="10" class=num>원</td>
		    <td width=25%>&nbsp; </td>
                </tr>
                <%}%>
                <%}%>
...
%>
	<table width=100% border=0 cellpadding=0 cellspacing=0>
        <tr>
            <td width=7><img src=/acar/images/center/menu_bar_1.gif width=7 height=33></td>
            <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class="style1 style5">해당 차종 참고사항</span></td>
            <td width=7><img src=/acar/images/center/menu_bar_2.gif width=7 height=33></td>
        </tr>
    </table>
<%	    
...
%>
	<table width=100% border=0 cellpadding=0 cellspacing=0>
        <tr>
            <td width=7><img src=/acar/images/center/menu_bar_1.gif width=7 height=33></td>
            <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class="style1 style5">연비 참고사항</span></td>
            <td width=7><img src=/acar/images/center/menu_bar_2.gif width=7 height=33></td>
        </tr>
    </table>
    <br/>
    ...
    <%if(!String.valueOf(ht4.get("CAR_B")).equals("null")){%>
    	<span class="parent-title"><%=(String)ht4.get("CAR_NAME")%> 사양 포함</span>
    	<p style="margin-left:20px;">
    		<%=car_b4%>
    	</p>
    <%} %>
    <%if(!String.valueOf(ht3.get("CAR_B")).equals("null")){%>
    	<span class="parent-title"><%=(String)ht3.get("CAR_NAME")%> 사양 포함</span>
    	<p style="margin-left:20px;">
    		<%=car_b3%>
    	</p>
    <%} %>
    <%if(!String.valueOf(ht2.get("CAR_B")).equals("null")){%>
    	<span class="parent-title"><%=(String)ht2.get("CAR_NAME")%> 사양 포함</span>
    	<p style="margin-left:20px;">
    		<%=car_b2%>
    	</p>
    <%} %>
    <%if(!String.valueOf(car_b.get("CAR_B")).equals("null")){%>
    <span class="parent-title" style="color:#434343;">기본 사양</span>
...
<!--
	...
	<%if(idx.equals("2")){%>
		...
		ofm.hook_yn.value 	= '<%=tmp_hook_yn%>';//견인고리
	<%}%>
