...
<%
	...
	EstimateBean [] e_r = e_db.getEstimateShList(gubun1, gubun2, gubun3, gubun4, gubun5, gubun6, s_dt, e_dt, s_kd, t_wd, esti_m, esti_m_dt, esti_m_s_dt, esti_m_e_dt);
	int size = e_r.length;
%>
<html>
<head>
...
<!--
	...
	function EstiDisp(est_id, set_code){	
		...
		fm.target = 'd_content';
		fm.action = 'esti_sh_mng_u.jsp';
		fm.submit();
	}
	
	function EstiMemo(est_id, user_id){
		var SUBWIN="./esti_memo_i.jsp?est_id="+est_id+"&user_id="+user_id+"&from_page=esti_sh_sc_in.jsp";	
...
<table border=0 cellspacing=0 cellpadding=0 width=1790>
    ...
    <tr id='tr_title' style='position:relative;z-index:1'> 
        <td width=720 class='line' id='td_title' style='position:relative;'> 
    ...
    <tr> 
        <td width=720 class='line' id='td_con' style='position:relative;'> 
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
            <%for(int i=0; i<size; i++){
                ...
                <tr> 
                    ...
                    <td width=100 align=center><span title='<%=bean.getEst_nm()%>'><%=AddUtil.substringbdot(bean.getEst_nm(), 14)%></span></td>
                </tr>
                  <%}%>
                  <% if(size == 0) { %>
                <tr> 
                    <td width=720 align=center colspan="8">등록된 데이타가 없습니다.</td>
                </tr>
       			<%}%>
            </table>
        </td>
        <td width=1070 class='line'>
            <table border="0" cellspacing="1" cellpadding="0" width=100%>
                <%for(int i=0; i<size; i++){
    					bean = e_r[i];%>
                <tr> 
                    <td width=100 align="center"><span title='<%=bean.getEst_tel()%>'><%=AddUtil.substringbdot(bean.getEst_tel(), 13)%></span></td>
                    <td width=200>&nbsp;<span title='<%="["+bean.getMgr_nm()+"] "+ bean.getCar_nm()+" "+ bean.getCar_name()%>'><%=AddUtil.substringbdot("["+bean.getMgr_nm()+"] "+ bean.getCar_nm()+" "+ bean.getCar_name(), 28)%></span></td>
                    ...
                    <td width=80 align="right"><%=Util.parseDecimal(bean.getFee_s_amt()+bean.getFee_v_amt())%>원</td>
                    <td width=80 align="right"><%=Util.parseDecimal(bean.getGtr_amt())%>원</td>
                    <td width=70 align="right"><%=Util.parseDecimal(bean.getPp_s_amt()+bean.getPp_v_amt())%>원</td>
                    <td width=70 align="right"><%=Util.parseDecimal(bean.getIfee_s_amt()+bean.getIfee_v_amt())%>원</td>												
                    <td width=60 align=center><%=c_db.getNameById(bean.getReg_id(), "USER")%></td>
