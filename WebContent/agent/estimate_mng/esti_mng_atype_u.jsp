...
<%
  ...
  ej_bean = e_db.getEstiJgVarCase(cm_bean.getJg_code(), "");
	...
	if(e_bean1.getPrint_type().equals("2") || e_bean1.getPrint_type().equals("3") || e_bean1.getPrint_type().equals("4") || e_bean1.getPrint_type().equals("5")){
...
<!--
	...
	function go_list(){
		...
		<%if(from_page2.equals("/agent/bus_analysis/ba_esti_frame.jsp")){%>
		fm.action = '/agent/bus_analysis/ba_esti_frame.jsp';
		<%}else if(from_page2.equals("/agentbus_analysis/ba_stat_frame.jsp")){%>
		fm.action = '/agent/bus_analysis/ba_stat_frame.jsp';
		<%}else{%>
	...
	function DelEsti(est_id){
		...
		if(!confirm('삭제하시겠습니까? 삭제되면 복구되지 않습니다.')){	return; }	
	...
	function EstiClsPerCng(idx, est_id){
		...
		if(!confirm('적용위약율를 수정하시겠습니까?')){	return; }	
		fm.action = 'upd_esti_cust_a.jsp';		
		fm.target = 'i_no';					
		fm.submit();
	}	
	...
	function all_delete(){
		...
		if(!confirm('일괄 삭제하시겠습니까? 삭제되면 복구되지 않습니다.')){	return; }			
		fm.action = 'esti_mng_d_a.jsp';		
		fm.target = 'i_no';					
		fm.submit();				
	}			
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
  <form action="" name="form1" method="POST" >
    	...
    	<td colspan=2>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>영업지원 > 견적시스템 > <span class=style5>신차다중견적내기</span></span></td>
        ...
        <td class=line colspan="2"> 
            <table border="0" cellpadding=0 cellspacing="1" width=100%>
                <tr> 
                    ...
                    <td >&nbsp;<input type="text" name="est_email" value="<%=e_bean1.getEst_email()%>" size="30" class=text style='IME-MODE: inactive'>
                      </td>
                </tr>
                <tr>				
                ...
                <tr>
                    ...
                    <td class=title>비용비교</td>
                    <td>&nbsp;<input type="radio" name="compare_yn" value="N" <% if(e_bean1.getCompare_yn().equals("N")||e_bean1.getCompare_yn().equals("")) out.print("checked"); %>>
                      없음
                ...
                <tr>
                    ...
                    <td colspan="3">&nbsp;<input type="radio" name="vali_type" value="0" <% if(e_bean1.getVali_type().equals("0")||e_bean1.getVali_type().equals("")) out.print("checked"); %>>
                ...
                <tr>
                    ...
                    <td>&nbsp;<input type="radio" name="pp_ment_yn" value="Y" <%if(e_bean1.getPp_ment_yn().equals("Y"))out.print("checked");%>>
                ...
                <tr>
                    ...
                    <td colspan="2">&nbsp;
                      <input type="hidden" name="damdang_id" value="<%=e_bean1.getReg_id()%>">
        		          <input type="radio" name="caroff_emp_yn" value="1" <%if(e_bean1.getCaroff_emp_yn().equals("1") || e_bean1.getCaroff_emp_yn().equals("")){%>checked<%}%>>기본설정값 표기(<%=c_db.getNameById(e_bean1.getReg_id(), "USER")%>)
        		          <input type="radio" name="caroff_emp_yn" value="4" <%if(e_bean1.getCaroff_emp_yn().equals("4")){%>checked<%}%>>직접입력 ( 이름: <input type="text" name="damdang_nm" value="<%=e_bean1.getDamdang_nm()%>" size="10" class=text maxlength='7'> 휴대폰: <input type="text" name="damdang_m_tel" value="<%=e_bean1.getDamdang_m_tel()%>" size="15" class=text maxlength='15'> )
        		          <input type="radio" name="caroff_emp_yn" value="3" <%if(e_bean1.getCaroff_emp_yn().equals("3")){%>checked<%}%>>담당자 미표기
        		          
        		          <!-- 20201020 개소세환원문구 모든차종에대해 표기로 변경됨에 따라 아래 견적서 개소세환원문구 표기여부 임시 주석처리 -->
			            <input type="hidden" name="info_st" id="info_st" value="">        			  
                	</td>
                	<td>
                		<%if(e_bean1.getCar_comp_id().equals("0001") || e_bean1.getCar_comp_id().equals("0002")){%><input type="button" class="button" value="자체탁송료 조회" onclick="javascript:search_cons_cost();"><%}%>
                	</td>
            ...
            <table border="0" cellspacing="1" width=100%>
                <tr> 
                    ...
                    <td colspan="2"> 
						...
						&nbsp;&nbsp;<input type="text" name="etc" value="<%=etc%>" size="140" class="whitetext" readonly>
                    </td>
                </tr>
                <tr> 
                    <td class=title>차명</td>
                    <td> 
                      &nbsp;<%=cm_bean.getCar_nm()%> (차종코드:<%=cm_bean.getJg_code()%>)</td>
    ...
    <tr> 
		...
		(견적일자 : <%=AddUtil.ChangeDate3(e_bean1.getReg_dt())%> <%=c_db.getNameById(e_bean1.getReg_id(),"USER")%>)
		</td>
    </tr>	
    <tr>
        <td class=line2 colspan=2></td>
        ...
        <td class=line colspan="2"> 
            <table border="0" cellspacing="1" width=100%>
                ...
                <tr> 
                    ...
                    <td class=title width='7%'>표준 최대잔가</td>
                ...
                <tr> 
                    <td class=title width='7%'>표준 약정운행거리</td>
                ...
                <tr> 
                    <td class=title width='7%'>적용 약정운행거리</td>
                ...
                <tr> 
                    <td class=title width='7%'>조정 최대잔가</td>
                ...
                <tr id=tr_com_emp_yn style="display:<%if(e_bean1.getCom_emp_yn().equals("")&&e_bean2.getCom_emp_yn().equals("")&&e_bean3.getCom_emp_yn().equals("")&&e_bean4.getCom_emp_yn().equals("")){%>'none'<%}else{%>''<%}%>">
                  <td colspan='2' class=title>임직원 한정운전특약</td>
...
<%--                     <%if(e_bean4.getNew_license_plate().equals("1")){%>신형번호판신청(수도권)<%} else if (e_bean4.getNew_license_plate().equals("2")){%>신형번호판신청(대전/대구/광주/부산)<%}%> --%>
        ...
        <td class=line colspan="2"> 
            <table border="0" cellspacing="1" width=100%>
                ...
                <tr> 
                    ...
                    <td colspan="3">&nbsp;<input type="text" name="cls_per" size="4" value='<%=e_bean1.getCls_per()%>' class=num>%&nbsp;<a href="javascript:EstiClsPerCng(0, '<%=e_bean1.getEst_id()%>')"><img src=/acar/images/center/button_in_jgj.gif align="absmiddle" border="0" alt="조정"></a></td>
                    <td colspan="3">&nbsp;<input type="text" name="cls_per" size="4" value='<%=e_bean2.getCls_per()%>' class=num>%&nbsp;<a href="javascript:EstiClsPerCng(1, '<%=e_bean2.getEst_id()%>')"><img src=/acar/images/center/button_in_jgj.gif align="absmiddle" border="0" alt="조정"></a></td>
                    <td colspan="3">&nbsp;<input type="text" name="cls_per" size="4" value='<%=e_bean3.getCls_per()%>' class=num>%&nbsp;<a href="javascript:EstiClsPerCng(2, '<%=e_bean3.getEst_id()%>')"><img src=/acar/images/center/button_in_jgj.gif align="absmiddle" border="0" alt="조정"></a></td>
                    <td colspan="3">&nbsp;<input type="text" name="cls_per" size="4" value='<%=e_bean4.getCls_per()%>' class=num>%&nbsp;<a href="javascript:EstiClsPerCng(3, '<%=e_bean4.getEst_id()%>')"><img src=/acar/images/center/button_in_jgj.gif align="absmiddle" border="0" alt="조정"></a></td>
                </tr>
                ...
                <tr> 
                    ...
                    <td colspan="3">&nbsp;<%if(!e_bean2.getEst_id().equals("") && !e_bean2.getPrint_type().equals("6")){%><a href="javascript:ReEsti('<%=e_bean2.getEst_id()%>');" title='비교견적'><img src=/acar/images/center/button_in_esti.gif align=absmiddle border=0></a><%}%></td>
                    <td colspan="3">&nbsp;<%if(!e_bean3.getEst_id().equals("") && !e_bean3.getPrint_type().equals("6")){%><a href="javascript:ReEsti('<%=e_bean3.getEst_id()%>');" title='비교견적'><img src=/acar/images/center/button_in_esti.gif align=absmiddle border=0></a><%}%></td>
                    <td colspan="3">&nbsp;<%if(!e_bean4.getEst_id().equals("") && !e_bean4.getPrint_type().equals("6")){%><a href="javascript:ReEsti('<%=e_bean4.getEst_id()%>');" title='비교견적'><img src=/acar/images/center/button_in_esti.gif align=absmiddle border=0></a><%}%></td>
                </tr>
            </table>
        </td>
    </tr>	
	...
	<%if(size>0){%>
		<%if(!e_bean1.getPrint_type().equals("6")){%>
            ...
            <%//if(!ck_acar_id.equals("000215") && !ck_acar_id.equals("000216")){%>      
            ...
            <table border="0" cellspacing="1" width=100%>
				<tr>
					...
					<td>&nbsp;
						...
						<%-- <input type="text" name="est_m_tel" value="<%=e_bean1.getEst_tel()%>" size="20" class=text> --%>
					</td>
				</tr>			
                <tr>
    ...
    <tr> 
        <td></td>
        <td align="right"><a href="javascript:esti_result_sms();" title='문자보내기'><img src=/acar/images/center/button_send_smsgo.gif align=absmiddle border=0></a></td>
    </tr>      	    
    <tr>
        <td class=h></td>
    </tr>         
...
// 						String rent_way = e_bean1.getA_a().substring(1);
                ...
                <tr>
                  	<td width="10%" class=title>
                  		<input type="checkbox" name="ch_mms_id" value="<%=e_bean1.getEst_id()%>" checked>견적 알림톡1</td>
                  	<td width="90%">&nbsp;
...
// 						String a_a				= e_bean3.getA_a().substring(0,1);
// 						String rent_way 	= e_bean3.getA_a().substring(1);
						String a_a = "";
						String rent_way = "";
						...
						String s_a_a				= "";
						String s_rent_way 	= "";
						if(a_a.equals("1")) s_a_a="리스플러스"; else s_a_a="장기렌트";
						if(rent_way.equals("1")) s_rent_way="일반식(정비포함)"; else s_rent_way="기본식";
						//조정대여료가 있다면
