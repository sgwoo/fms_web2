...
<%@ page import="java.util.*, acar.util.*, acar.common.*, acar.estimate_mng.*, acar.car_office.*, acar.car_mst.*" %>
...
<%	
	...
	if (e_bean.getPrint_type().equals("6")) {
		...
		for (int i = 0 ; i < size ; i++) {
			...
			if(i==2) e_bean3 = e_db.getEstimateCase("");
			if(i==3) e_bean4 = e_db.getEstimateCase("");
		}	
...
<!--
	...
	function GetCarCode(){
		...
		var num_car_comp_id = parseInt($('#car_comp_id option:selected').val());
	...
	function sub_list(idx){
		...
		if(idx != '1' && fm.car_name.value == ''){ alert('차종을 선택하십시오.'); return;}		
		//파라미터 jg_g_7 추가
	    var jg_g_7_val = "";
	    if (idx != 1) {
		...
		var SUBWIN="./search_sub_list.jsp?idx="+idx+"&a_a=&car_comp_id="+fm.car_comp_id.value+"&car_cd="+fm.code.value+"&car_id="+fm.car_id.value+"&car_seq="+fm.car_seq.value+"&car_nm="+fm.code.options[fm.code.selectedIndex].text+"&jg_g_7="+jg_g_7_val+"&garnish_yn_opt_st="+garnish_yn_opt_st+"&hook_yn_opt_st="+hook_yn_opt_st;
...
// 			fm.ro_13_amt[idx].value = parseDecimal( toInt(parseDigit(fm.o_1.value)) * toFloat(fm.ro_13[idx].value) / 100 );	
			fm.ro_13_amt[idx].value = parseDecimal( getCutRoundNumber(toInt(parseDigit(fm.o_1.value)) * toFloat(fm.ro_13[idx].value) / 100, 1000 )); 
			
			if(fm.a_a[idx].value == '11' || fm.a_a[idx].value == '12'){
				if(fm.ls_yn.value == 'Y'){
// 					fm.ro_13_amt[idx].value = parseDecimal( toInt(parseDigit(fm.o_12.value)) * toFloat(fm.ro_13[idx].value) / 100 );	
...
//상품선택시 매입옵션 구분 셋팅
  function SelectA_a(idx){
	...
	function set_amt(){
		...
		if(fm.a_a[idx].value == '11' || fm.a_a[idx].value == '12'){
			if(fm.ls_yn.value == 'Y'){
				fm.o_12.value = parseDecimal(toInt(parseDigit(fm.car_amt.value)) + toInt(parseDigit(fm.opt_amt.value)) + toInt(parseDigit(fm.col_amt.value)) - toInt(parseDigit(fm.dc_amt2.value)));		
			}
		}	
	}
...
<%-- 			if(fm.tint_s_yn[<%=j%>].checked ==true) fm.r_tint_s_yn[<%=j%>].value = 'Y'; --%>
			...
			if(fm.tint_ps_yn[<%=j%>].checked ==true) {	
				...
				fm.r_tint_ps_st[<%=j%>].value = fm.setTint_ps_sel[<%=j%>].value;	// 고급썬팅 선택값		// 고급썬팅 추가금액(공급가)
			}
...
<%-- 			if(fm.tint_s_yn[<%=j%>].checked ==false) fm.r_tint_s_yn[<%=j%>].value = 'N';		// 전면썬팅 체크 해제 시 값 변경 추가 2017.12.27 --%>
			if(fm.tint_sn_yn[<%=j%>].checked ==false) fm.r_tint_sn_yn[<%=j%>].value = 'N';		// 전면썬팅 미시공 할인 체크 해제 시 값 변경 추가
			...
			if(fm.tint_bn_yn[<%=j%>].checked ==false) fm.r_tint_bn_yn[<%=j%>].value = 'N';		// 거치형 내비게이션 체크 해제 시 값 변경 추가 2017.12.27
			if(fm.tint_cons_yn[<%=j%>].checked ==false) fm.r_tint_cons_yn[<%=j%>].value = 'N';		// 추가탁송료 체크 해제 시 값 변경 추가 2020.06.15
			if(fm.tint_eb_yn[<%=j%>].checked ==false) fm.r_tint_eb_yn[<%=j%>].value = 'N';		// 블랙박스 미제공 할인 (빌트인캠,고객장착..) 추가 20200422
	    	...
	    	<%-- fm.r_eco_e_tag[<%=j%>].value = fm.eco_e_tag[<%=j%>].value; --%>// 맑은서울스티커 발급
		...
		<%-- if(fm.jg_g_7.value == '3' || fm.jg_g_7.value == '4'){		// 전기차, 수소차 일 경우
			...
			if(rg_8_amt_replace > deposit){
				...
				fm.rg_8_amt[<%=j%>].value = numberWithCommas(deposit);
				return;
			}
    ...
    //개인사업자일 때 선택으로 되어있으면 가입 또는 미가입으로 선택하도록 수정
      if (doc_type_value == "2") {
		...
		if(toInt(parseDigit(fm.car_comp_id.value)) > 5){		
			...
			if(toInt(parseDigit(fm.dc_amt.value)) > 0){ 		alert('수입차는 제조사DC금액이 추가되면 견적할 수 없습니다.'); 			return; }
		}
   ...
   // 기본보증금 계산식 수정 2021.07.09.
      ...
      if(toInt(parseDigit(fm.car_comp_id.value)) > 5){
		...
		<%}%>
			...
			if(fm.badcust_chk.value == ''){ 
				//alert('불량고객 확인을 하십시오.'); 	return;		
				//window.open('/acar/bad_cust/stat_badcust_list.jsp?badcust_chk_from=esti_mng_atype_i.jsp&est_nm='+fm.est_nm.value+'&est_tel='+fm.est_tel.value+'&est_mail='+fm.est_mail.value+'&est_fax='+fm.est_fax.value, "BADCUST", "left=10, top=10, width=1200, height=560, resizable=yes, scrollbars=yes, status=yes");
				//return;
			}
	...
	function search_cons_cost(){
		...
		if (car_comp_id == '0001' || car_comp_id == '0002') {   
			...
			fm.action = '/acar/estimate_mng/search_cons_cost.jsp';		
			fm.submit();	
		}
	}
	...
	function setEst_yn(print_type){
		...
		//상품별, 견적별종합, 전기차 수소차 인수 반납 선택형 에서만 복사버튼 노출되게
	    if (print_type==1 || print_type==5 || print_type==6) {
	    	$("#btn_copy2, #btn_copy3, #btn_copy4").css("display","block");
	    } else {
...
// 	      fm.rg_8[0].value = toInt(fm.rg_8[0].value)-10;
          ...
          if(Number($("#car_comp_id option:selected").val()) > 5){ // 수입차
       	...
       	// 친환경차 구분상 전기차면서 차량가격 합계 금액이 8천만원 초과 시 +10%, 6500만원 넘으면 +5%, 6500만원 이하면 그대로.
       	// 친환경차 구분상 전기차거나 수소차면서 차량가격 7500만원 이하면 -5%  2022.05.06.
          if (fm.jg_g_7.value == '3'){
...
<%--       	fm.tint_s_yn[<%=j%>].disabled = true; --%>
		...
		fm.tint_cons_amt[<%=j%>].disabled = true;
      	...
      	<%-- fm.eco_e_tag[<%=j%>].disabled = true; --%>// 맑은서울스티커 발급
...
<%--       	fm.tint_s_yn[<%=j%>].disabled = false; --%>
		...
		fm.tint_cons_amt[<%=j%>].disabled = false;
      	...
      	<%-- fm.eco_e_tag[<%=j%>].disabled = false; --%>// 맑은서울스티커 발급
...
function setLoc_st(num,val){
	...
	/* if ($("#car_comp_id option:selected").val() == "0056") {
		...
		if (val == '13') LocStSet();	// 전기차 고객주소지 보조금 없는 견적 선택 시 차량인도지역은 본인 지점을 디폴트 값으로. 2021.11.02.
	/* } */
}
...
// 		if($("#tint_s_yn_1"	).is(":checked")==true){$("#tint_s_yn_" +num).prop("checked",true);	}else{$("#tint_s_yn_" +num).prop("checked",false);}	//전면썬팅
		if($("#tint_sn_yn_1"	).is(":checked")==true){$("#tint_sn_yn_" +num).prop("checked",true);	}else{$("#tint_sn_yn_" +num).prop("checked",false);}	//전면썬팅 미시공할인
		...
		if($("#tint_cons_yn_1"	).is(":checked")==true){$("#tint_cons_yn_" +num).prop("checked",true);	}else{$("#tint_cons_yn_" +num).prop("checked",false);}	//추가탁송료등
		$("#tint_cons_amt_"	+num).val($("#tint_cons_amt_1").val());	//추가탁송료등
...
//신규_전기차 수소차 인수반납 선책형 및 반납형 견적일 경우 1번견적 최대잔가율 조회시 2번견적또한 같이 조회
function set_time() {
	var fm = document.form1;
    var jg_g_7 = fm.jg_g_7.value;
    var jg_code = fm.jg_code.value;
...
<form action="/agent/estimate_mng/get_carcd_null.jsp" name="form2" method="post">
...
<form action='/agent/estimate_mng/get_o13_20110101.jsp' name="sh_form" method='post'>
...
<form action="./esti_mng_atype_i_a.jsp" name="form1" method="POST" >
	...
	<!-- <input type="hidden" name="r_eco_e_tag" value=""> --><!-- 맑은서울스티커 발급 -->
    	...
    	<td colspan=2>
    	    <table width=100% border=0 cellpadding=0 cellspacing=0>
                <tr>
                    ...
                    <td class=bar>&nbsp;&nbsp;&nbsp;<img src=/acar/images/center/menu_bar_dot.gif width=4 height=5 align=absmiddle>&nbsp;<span class=style1>Agent > 견적시스템 > <span class=style5>신차견적내기</span></span></td>
        ...
        <td class=line colspan="2"> 
            <table border="0" cellpadding=0 cellspacing="1" width=100%>
                <tr> 
                    ...
                    <td>&nbsp;<input type="text" name="est_email" value="<%=e_bean.getEst_email()%>" size="50" class=text style='IME-MODE: inactive'>                
                      </td>
                </tr>                
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="3">
                    	<div style="float: left;">&nbsp;<label><input type="radio" name="doc_type" value="1" onClick="javascript:SetComEmpYn()" <% if(e_bean.getDoc_type().equals("1")||e_bean.getDoc_type().equals("")) out.print("checked"); %>>
		                      법인고객</label>
		            		<label><input type="radio" name="doc_type" value="2" onClick="javascript:SetComEmpYn()" <% if(e_bean.getDoc_type().equals("2")) out.print("checked"); %>>
		                      개인사업자</label> 
		            		<label><input type="radio" name="doc_type" value="3" onClick="javascript:SetComEmpYn()" <% if(e_bean.getDoc_type().equals("3")) out.print("checked"); %>>
	           		  	...
	           		  	<div style="float: left;" id="doc_type_check_div">
	           		  		&nbsp;&nbsp;&nbsp;
	           		  		<input type="checkbox" id="dir_pur_commi_yn" name="dir_pur_commi_yn" value="Y" onclick="javascript:dir_pur_change()" <% if(e_bean.getDir_pur_commi_yn().equals("Y")) out.print("checked"); %>><label for="dir_pur_commi_yn">특판출고(실적이관가능)</label>
	           		  	</div>
                    </td>
                </tr>
                <tr>
                    <td class=title>신용도</td>
                    <td>&nbsp;<input type="radio" name="spr_yn" value="2" <% if(e_bean.getSpr_yn().equals("2")||e_bean.getSpr_yn().equals("")) out.print("checked"); %>>
                      초우량기업
                      ...
                      일반기업 
					            <input type="radio" name="spr_yn" value="3" <% if(e_bean.getSpr_yn().equals("3")) out.print("checked"); %>>
                      신설법인
                      </td>
                    <td width=10% class=title>비용비교</td>
                    <td>&nbsp;<input type="radio" name="compare_yn" value="N" <% if(e_bean.getCompare_yn().equals("N")||e_bean.getCompare_yn().equals("")) out.print("checked"); %>>
                      없음
                      <input type="radio" name="compare_yn" value="Y" <% if(e_bean.getCompare_yn().equals("Y")) out.print("checked"); %>>
                      있음 
                      </td>
                </tr>
                <tr>
                    <td class=title>견적유효기간</td>
                    <td colspan="3">&nbsp;<input type="radio" name="vali_type" value="0" <% if(e_bean.getVali_type().equals("0")||e_bean.getVali_type().equals("")) out.print("checked"); %>>
                ...
                <tr>
                    ...
                    <td>&nbsp;<input type="radio" name="pp_ment_yn" value="Y" <%if(e_bean.getPp_ment_yn().equals("Y"))out.print("checked");%>>
                      표기(초기납입금은 고객님의 신용도에 따라 심사과정에서 조정될 수 있습니다.)
                      <input type="radio" name="pp_ment_yn" value="N" <% if(e_bean.getPp_ment_yn().equals("N")||e_bean.getPp_ment_yn().equals("")) out.print("checked");%>>
                      미표기
					</td>
					<td width=10% class=title>보증보험료산출 등급</td>
                    <td>&nbsp;
                ...
                <tr>
                    <td class=title>견적담당자 표기여부</td>
                    <td colspan="2">&nbsp;
                    	<input type="hidden" name="damdang_id" value="<%=ck_acar_id%>">
        		        <input type="radio" name="caroff_emp_yn" value="1" <%if(e_bean.getCaroff_emp_yn().equals("1") || e_bean.getCaroff_emp_yn().equals("")){%>checked<%}%>>기본설정값 표기
        		        <input type="radio" name="caroff_emp_yn" value="4" <%if(e_bean.getCaroff_emp_yn().equals("4")){%>checked<%}%>>직접입력 ( 이름: <input type="text" name="damdang_nm" value="<%=e_bean.getDamdang_nm()%>" size="10" class=text maxlength='7'> 휴대폰: <input type="text" name="damdang_m_tel" value="<%=e_bean.getDamdang_m_tel()%>" size="15" class=text maxlength='15'> )
        		        <input type="radio" name="caroff_emp_yn" value="3" <%if(e_bean.getCaroff_emp_yn().equals("3")){%>checked<%}%>>담당자 미표기
        ...
        <td class=line colspan="2">
			...
			<%for (int i = 0; i < off_pre_vt_size; i++) {
				...
				<tr> 
                    ...
                    <td align='center'><%if( !acar_de.equals("1000") || ( acar_de.equals("1000") && String.valueOf(pur_ht.get("BUS_NM")).equals(session_user_nm) ) ){ %><%=pur_ht.get("FIRM_NM")%><%} else{%>***<%}%></td>
                    <td align='center'><%if( !acar_de.equals("1000") || ( acar_de.equals("1000") && String.valueOf(pur_ht.get("BUS_NM")).equals(session_user_nm) ) ){ %><%=pur_ht.get("CUST_TEL")%><%} else{%>***<%}%></td>
                    <td align='center'><%if( !acar_de.equals("1000") || ( acar_de.equals("1000") && String.valueOf(pur_ht.get("BUS_NM")).equals(session_user_nm) ) ){ %><%=pur_ht.get("ADDR")%><%} else{%>***<%}%></td>
...
<%}%>
            ...
            <table border="0" cellspacing="1" width=100%>
                <tr>
                    ...
                    <td colspan="2">
                      ...
                      <textarea id="etc" name="etc" cols="100" rows="1" style="overflow: hidden; border: 0;" readonly><%=etc%></textarea>
                  </td>
                </tr>
                <tr> 
                ...
                <tr>
                	...
                	<td>
                		...
                		<input type="text" name="conti_rat" value="<%=e_bean.getConti_rat()%>" size="35" class=whitetext readonly>
                		<input type="hidden" name="conti_rat_seq"/>
                	</td>
                	<td></td>
                </tr>
                <tr> 
    ...
    <tr>
        <td><font class="num_weight" color="red">※ 차량가격/색상/대여기간/약정운행거리가이 변경되었을 경우 최대잔가율을 꼭 재계산하십시오.</font></td>
    </tr>
    <tr>
...
<!--             <table border="0" cellspacing="1" width=100%> -->
            <table border="0" cellspacing="0" width=100% id='esti_condition'>
                <tr>
                  ...
                  <%-- <td colspan='4'>&nbsp;<input type="radio" name="print_type" value="1" onClick="javascript:setEst_yn(1);" <% if(e_bean.getPrint_type().equals("1")||e_bean.getPrint_type().equals("5")||e_bean.getPrint_type().equals("")) out.print("checked"); %>>
                      상품별
                      <input type="radio" name="print_type" value="5" onClick="javascript:setEst_yn(5);">
                      견적별종합 
					            <input type="radio" name="print_type" value="2" onClick="javascript:setEst_yn(2);" <% if(e_bean.getPrint_type().equals("2")) out.print("checked"); %>>
                      렌트 
					            <input type="radio" name="print_type" value="3" onClick="javascript:setEst_yn(3);" <% if(e_bean.getPrint_type().equals("3")) out.print("checked"); %>>
                      리스 		
					            <input type="radio" name="print_type" value="4" onClick="javascript:setEst_yn(4);" <% if(e_bean.getPrint_type().equals("4")) out.print("checked"); %>>
                      종합
                  </td> --%>
                  <td colspan='4'>&nbsp;
			            <label><input type="radio" name="print_type" value="1" onClick="javascript:setEst_yn(1);" <% if(e_bean.getPrint_type().equals("1")||e_bean.getPrint_type().equals("5")||e_bean.getPrint_type().equals("")) out.print("checked"); %> <% if(e_bean.getPrint_type().equals("6")) { %>disabled<% } %>>
			                      상품별</label>
			            <label><input type="radio" name="print_type" value="5" onClick="javascript:setEst_yn(5);" <% if(e_bean.getPrint_type().equals("6")) { %>disabled<% } %>>
			                      견적별종합</label> 
			            <label><input type="radio" name="print_type" value="2" onClick="javascript:setEst_yn(2);" <% if(e_bean.getPrint_type().equals("2")) out.print("checked"); %> <% if(e_bean.getPrint_type().equals("6")) { %>disabled<% } %>>
			                      렌트</label> 
			            <label><input type="radio" name="print_type" value="3" onClick="javascript:setEst_yn(3);" <% if(e_bean.getPrint_type().equals("3")) out.print("checked"); %> <% if(e_bean.getPrint_type().equals("6")) { %>disabled<% } %>>
			                      리스</label>    
			            <label><input type="radio" name="print_type" value="4" onClick="javascript:setEst_yn(4);" <% if(e_bean.getPrint_type().equals("4")) out.print("checked"); %> <% if(e_bean.getPrint_type().equals("6")) { %>disabled<% } %>>
			                      종합</label>
			            <label <%if ((e_bean.getPrint_type().equals("6") || e_bean2.getPrint_type().equals("6") || e_bean3.getPrint_type().equals("6") || e_bean4.getPrint_type().equals("6"))) {%>style="display: ;"<%} else {%>style="display: none;"<%}%>>
			            	<input type="radio" name="print_type" value="6" onClick="javascript:setEst_yn(6);" <% if(e_bean.getPrint_type().equals("6")) out.print("checked"); %> <% if(!e_bean.getPrint_type().equals("6")) { %>disabled<% } %>>
                ...
                <tr>
                  ...
                  <td width="22%" class=title><input type="checkbox" name="est_yn" id="est_yn_1" value="Y" checked <%if(e_bean.getPrint_type().equals("6")){%>disabled<%}else{%><%}%> onClick="javascript:setO11();">견적1</td>
                  <td width="21%" class=title>
                  	<table width="100%">
                  		<tr><td width="95%" class=title style="border: none !important;"><input type="checkbox" name="est_yn" id="est_yn_2" value="Y" <%if(est_table.equals("esti_spe") && !a_a[1].equals("")){%>checked<%}%> <%if(e_bean.getPrint_type().equals("6")){%>disabled<%}else{%><%}%> onClick="javascript:setO11();">견적2</td>
                  		<td width="5%" class=title style="border: none !important;"><input type="button" class="button" id="btn_copy2" value="복사" style="margin-left: 20px;" onclick="javascript:copy_esti_1('2');"></td></tr>
                  	</table>
                  </td>
                  <td width="21%" class='title esti_target'>
                  	<table width="100%">
                  		<tr><td width="95%" class=title style="border: none !important;"><input type="checkbox" name="est_yn" id="est_yn_3" value="Y" <%if(est_table.equals("esti_spe") && !a_a[2].equals("")){%>checked<%}%> onClick="javascript:setO11();eh_chk('3');">견적3</td>
                  		<td width="5%" class=title style="border: none !important;"><input type="button" class="button" id="btn_copy3" value="복사" style="margin-left: 20px;" onclick="javascript:copy_esti_1('3');"></td></tr>
                  	</table>
                  </td>
                  <td width="21%" class='title esti_target'>
                  	<table width="100%">
                  		<tr><td width="95%" class=title style="border: none !important;"><input type="checkbox" name="est_yn" id="est_yn_4" value="Y" <%if(est_table.equals("esti_spe") && !a_a[3].equals("")){%>checked<%}%> onClick="javascript:setO11();eh_chk('4');">견적4</td>
                  		<td width="5%" class=title style="border: none !important;"><input type="button" class="button" id="btn_copy4" value="복사" style="margin-left: 20px;" onclick="javascript:copy_esti_1('4');"></td></tr>
                  	</table>
                  </td>
                ...
                <!-- 신규_printtype_TODO -->
                <tr id=tr_ecar_return style="display:<%if(e_bean.getPrint_type().equals("6")){%>''<%}else{%>none<%}%>">
                ...
                <tr> 
                    ...
                    <td>&nbsp;
                    	<select id="sel_a_b_1" name='sel_a_b' onchange="javascript:setA_b('1',this.value);set_return2_change(); set_other_esti_value(this);">
							...
							<option value="directInput"<%if(!e_bean.getA_b().equals("24")&&!e_bean.getA_b().equals("36")&&!e_bean.getA_b().equals("48")&&!e_bean.getA_b().equals("60")){%> selected<%}%> >직접입력</option></select><!-- <%if (e_bean.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
                    ...
                    <td>&nbsp;
						...
						<%}%>	
							<option value="directInput"<%if(!e_bean.getA_b().equals("24")&&!e_bean2.getA_b().equals("36")&&!e_bean2.getA_b().equals("48")&&!e_bean2.getA_b().equals("60")){%> selected<%}%> >직접입력</option></select><!-- <%if (e_bean2.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
                    ...
                    <td class='esti_target'>&nbsp;
						...
						<%}%>	
							<option value="directInput"<%if(!e_bean.getA_b().equals("24")&&!e_bean3.getA_b().equals("36")&&!e_bean3.getA_b().equals("48")&&!e_bean3.getA_b().equals("60")){%> selected<%}%> >직접입력</option></select><!-- <%if (e_bean3.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
                    ...
                    <td class='esti_target'>&nbsp;
						...
						<%}%>	
							<option value="directInput"<%if(!e_bean.getA_b().equals("24")&&!e_bean4.getA_b().equals("36")&&!e_bean4.getA_b().equals("48")&&!e_bean4.getA_b().equals("60")){%> selected<%}%>>직접입력</option></select><!--  <%if (e_bean4.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
...
<%-- 							<%} %> --%>
							...
							<option value="directInput"<%if(e_bean.getAgree_dist()!=10000 && e_bean.getAgree_dist()!=15000 && e_bean.getAgree_dist()!=20000 && e_bean.getAgree_dist()!=23000 && e_bean.getAgree_dist()!=25000 && e_bean.getAgree_dist()!=28000
														 && e_bean.getAgree_dist()!=30000 && e_bean.getAgree_dist()!=35000 && e_bean.getAgree_dist()!=40000 && e_bean.getAgree_dist()!=45000
													  	 && e_bean.getAgree_dist()!=50000){%> selected<%}%> >직접입력</option></select> <!-- <%if (e_bean.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
...
<%-- 							<%} %> --%>
							...
							<option value="directInput"<%if(e_bean2.getAgree_dist()!=10000 && e_bean2.getAgree_dist()!=15000 && e_bean2.getAgree_dist()!=20000 && e_bean2.getAgree_dist()!=23000 && e_bean2.getAgree_dist()!=25000 && e_bean2.getAgree_dist()!=28000
														 && e_bean2.getAgree_dist()!=30000 && e_bean2.getAgree_dist()!=35000 && e_bean2.getAgree_dist()!=40000 && e_bean2.getAgree_dist()!=45000
													  	 && e_bean2.getAgree_dist()!=50000){%> selected<%}%> >직접입력</option></select> <!-- <%if (e_bean2.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
...
<%-- 							<%} %> --%>
							...
							<option value="directInput"<%if(e_bean3.getAgree_dist()!=10000 && e_bean3.getAgree_dist()!=15000 && e_bean3.getAgree_dist()!=20000 && e_bean3.getAgree_dist()!=23000 && e_bean3.getAgree_dist()!=25000 && e_bean3.getAgree_dist()!=28000
														 && e_bean3.getAgree_dist()!=30000 && e_bean3.getAgree_dist()!=35000 && e_bean3.getAgree_dist()!=40000 && e_bean3.getAgree_dist()!=45000
													  	 && e_bean3.getAgree_dist()!=50000){%> selected<%}%> >직접입력</option></select> <!-- <%if (e_bean3.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
...
<%-- 							<%} %> --%>
							...
							<option value="directInput"<%if(e_bean4.getAgree_dist()!=10000 && e_bean4.getAgree_dist()!=15000 && e_bean4.getAgree_dist()!=20000 && e_bean4.getAgree_dist()!=23000 && e_bean4.getAgree_dist()!=25000 && e_bean4.getAgree_dist()!=28000
														 && e_bean4.getAgree_dist()!=30000 && e_bean4.getAgree_dist()!=35000 && e_bean4.getAgree_dist()!=40000 && e_bean4.getAgree_dist()!=45000
													  	 && e_bean4.getAgree_dist()!=50000){%> selected<%}%> >직접입력</option></select> <!-- <%if (e_bean4.getCar_comp_id().equals("0056")) {%>disabled<%}%> -->
                ...
                <tr> 
                    ...
                    <td>&nbsp;
                        ...
                        &nbsp;&nbsp;&nbsp;&nbsp;
	                        <select name='rtn_run_amt_yn' id='rtn_run_amt_yn_1'>
                    ...
                    <td>&nbsp;
                        ...
                        <%-- <%if ((nm_db.getWorkAuthUser("전산팀", user_id) || nm_db.getWorkAuthUser("본사영업부팀장", user_id) || nm_db.getWorkAuthUser("영업팀내근직", user_id) || user_id.equals("000057"))) {%>
                        &nbsp;&nbsp;&nbsp;&nbsp;
	                        <select name='rtn_run_amt_yn' id='rtn_run_amt_yn_2'>
	                        	<option value='0'>환급대여료 적용</option>
	                        	<option value='1'>미적용</option>
	                        </select>
                        <%} %> --%>
                    ...
                    <td class='esti_target'>&nbsp;
                        ...
                        <%-- <%if ((nm_db.getWorkAuthUser("전산팀", user_id) || nm_db.getWorkAuthUser("본사영업부팀장", user_id) || nm_db.getWorkAuthUser("영업팀내근직", user_id) || user_id.equals("000057"))) {%>
                        &nbsp;&nbsp;&nbsp;&nbsp;
	                        <select name='rtn_run_amt_yn' id='rtn_run_amt_yn_3'>
	                        	<option value='0'>환급대여료 적용</option>
	                        	<option value='1'>미적용</option>
	                        </select>
                        <%} %> --%>
                    ...
                    <td class='esti_target'>&nbsp;
                        ...
                        <%-- <%if ((nm_db.getWorkAuthUser("전산팀", user_id) || nm_db.getWorkAuthUser("본사영업부팀장", user_id) || nm_db.getWorkAuthUser("영업팀내근직", user_id) || user_id.equals("000057"))) {%>
                        &nbsp;&nbsp;&nbsp;&nbsp;
	                        <select name='rtn_run_amt_yn' id='rtn_run_amt_yn_4'>
	                        	<option value='0'>환급대여료 적용</option>
	                        	<option value='1'>미적용</option>
	                        </select>
                        <%} %> --%>
                ...
                <tr>                     
                    ...
                    <td>&nbsp;
                        ...
                        <span class="num_weight">%</span>&nbsp;&nbsp;&nbsp;<a onclick="set_time()" href="javascript:searchO13(0);"><img src=/acar/images/center/button_in_max.gif align=absmiddle border=0></a>
                    </td>
                    <td>&nbsp;
                        <input type="text" name="o_13" id="o_13_2" size="4" value='<%=e_bean2.getO_13()%>' class="whitenum num_weight" readonly  onblur="javascript:compare(1, this)">
      					<span class="num_weight">%</span>&nbsp;&nbsp;&nbsp;<a href="javascript:searchO13(1);"><img src=/acar/images/center/button_in_max.gif align=absmiddle border=0></a></td>
                    <td class='esti_target'>&nbsp;
                        <input type="text" name="o_13" id="o_13_3" size="4" value='<%=e_bean3.getO_13()%>' class="whitenum num_weight" readonly  onblur="javascript:compare(2, this)">
      					<span class="num_weight">%</span>&nbsp;&nbsp;&nbsp;<a href="javascript:searchO13(2);"><img src=/acar/images/center/button_in_max.gif align=absmiddle border=0></a></td>
                    <td class='esti_target'>&nbsp;
                        <input type="text" name="o_13" id="o_13_4" size="4" value='<%=e_bean4.getO_13()%>' class="whitenum num_weight" readonly  onblur="javascript:compare(3, this)">
      					<span class="num_weight">%</span>&nbsp;&nbsp;&nbsp;<a href="javascript:searchO13(3);"><img src=/acar/images/center/button_in_max.gif align=absmiddle border=0></a></td>
                </tr>  
                <tr> 
                ...
                <tr> 
                    ...
                    <td>&nbsp;<select name="opt_chk" id="opt_chk_1">
                    ...
                    <td>&nbsp;<select name="opt_chk" id="opt_chk_2">
                    ...
                    <td class='esti_target'>&nbsp;<select name="opt_chk" id="opt_chk_3">
                    ...
                    <td class='esti_target'>&nbsp;<select name="opt_chk" id="opt_chk_4">
      ...
      원 </td>
                ...
                <tr> 
                    ...
                    <td>&nbsp;<font color="#666666"> 
                      ...
                      개월치</font></td>
                    <td class='esti_target2'>&nbsp;<font color="#666666">
                      <input type="text" name="g_10" id="g_10_2" class=num size="2" value="<%=e_bean2.getG_10()%>">
            개월치</font></td>
                    <td class='esti_target esti_target2'>&nbsp;<font color="#666666">
            ...
            개월치</font></td>
                ...
                <tr> 
                  ...
                  <td >&nbsp;<select name="ins_per" id="ins_per_1" onchange="javascript:set_disable_value();">
                    ...
                    <td class='esti_target3'>&nbsp;<select name="ins_per" id="ins_per_2">
                    ...
                    <td class='esti_target3'>&nbsp;<select name="ins_per" id="ins_per_3">
                          <option value="1" <%if(e_bean.getIns_per().equals("1") || e_bean.getIns_per().equals("")){%>selected<%}%>>아마존카(보험포함)</option>
                          <%-- <option value="2" <%if(e_bean.getIns_per().equals("2")){%>selected<%}%> disabled>고객(보험미포함)</option> --%>
                      </select></td>
                    <td class='esti_target3'>&nbsp;<select name="ins_per" id="ins_per_4">
                          <option value="1" <%if(e_bean.getIns_per().equals("1") || e_bean.getIns_per().equals("")){%>selected<%}%>>아마존카(보험포함)</option>
                          <%-- <option value="2" <%if(e_bean.getIns_per().equals("2")){%>selected<%}%> disabled>고객(보험미포함)</option> --%>
                      </select></td>
                </tr>                
                <tr>
                  ...
                  <td >&nbsp;<select name="ins_dj" id="ins_dj_1" onchange="javascript:set_disable_value();">
                  ...
                  <td class='esti_target2'>&nbsp;<select name="ins_dj" id="ins_dj_2">
                        <%-- <option value="1" <%if(e_bean.getIns_dj().equals("1")){%>selected<%}%>>5천만원/5천만원</option> --%>
                  ...
                  <td class='esti_target esti_target2'>&nbsp;<select name="ins_dj" id="ins_dj_3">
                        <%-- <option value="1" <%if(e_bean.getIns_dj().equals("1")){%>selected<%}%>>5천만원/5천만원</option> --%>
                  ...
                  <td class='esti_target esti_target2'>&nbsp;<select name="ins_dj" id="ins_dj_4">
                        <%-- <option value="1" <%if(e_bean.getIns_dj().equals("1")){%>selected<%}%>>5천만원/5천만원</option> --%>
                        <option value="2" <%if(e_bean.getIns_dj().equals("2") || e_bean.getIns_dj().equals("")){%>selected<%}%>>1억원/1억원</option>
                        <option value="4" <%if(e_bean.getIns_dj().equals("4")){%>selected<%}%>>2억원/1억원</option>
                        <option value="8" <%if(e_bean.getIns_dj().equals("8")){%>selected<%}%>>3억원/1억원</option>
                ...
                <tr>
                  <td class=title>운전자연령</td>
                    <td>&nbsp;<select name="ins_age" id="ins_age_1" onchange="javascript:set_disable_value();">
                    ...
                    <td class='esti_target2'>&nbsp;<select name="ins_age" id="ins_age_2">
                    ...
                    <td class='esti_target esti_target2'>&nbsp;<select name="ins_age" id="ins_age_3">
                          <option value="1" <%if(e_bean.getIns_age().equals("1") || e_bean.getIns_age().equals("")){%>selected<%}%>>만26세이상</option>
                          <option value="3" <%if(e_bean.getIns_age().equals("3")){%>selected<%}%>>만24세이상</option>
                          <option value="2" <%if(e_bean.getIns_age().equals("2")){%>selected<%}%>>만21세이상</option>
                        </select>
                    </td>
                    <td class='esti_target esti_target2'>&nbsp;<select name="ins_age" id="ins_age_4">
                          <option value="1" <%if(e_bean.getIns_age().equals("1") || e_bean.getIns_age().equals("")){%>selected<%}%>>만26세이상</option>
                          <option value="3" <%if(e_bean.getIns_age().equals("3")){%>selected<%}%>>만24세이상</option>
                          <option value="2" <%if(e_bean.getIns_age().equals("2")){%>selected<%}%>>만21세이상</option>
                        </select>
                ...
                <tr> 
                    ...
                    <td>&nbsp;<input type="text" id="car_ja0"  name="car_ja" class=num size="10" value="<%=AddUtil.parseDecimal(e_bean.getCar_ja())%>" onBlur='javascript:this.value=parseDecimal(this.value);set_disable_value();'>
		          원 </td>
		                    <td class='esti_target2'>&nbsp;<input type="text" id="car_ja1" name="car_ja" class=num size="10" value="<%=AddUtil.parseDecimal(e_bean2.getCar_ja())%>" onBlur='javascript:this.value=parseDecimal(this.value);'>
		            원 </td>
		                    <td class='esti_target esti_target2'>&nbsp;<input type="text" id="car_ja2" name="car_ja" class=num size="10" value="<%=AddUtil.parseDecimal(e_bean.getCar_ja())%>" onBlur='javascript:this.value=parseDecimal(this.value);'>
		            원 </td>
		                    <td class='esti_target esti_target2'>&nbsp;<input type="text" id="car_ja3" name="car_ja" class=num size="10" value="<%=AddUtil.parseDecimal(e_bean.getCar_ja())%>" onBlur='javascript:this.value=parseDecimal(this.value);'>
		            원 </td>
                </tr>
                   
                <tr> 
                    <td class=title>보증보험<br>
                    </td>
                    <td>&nbsp;
      ...
      원 </td>
                 ...
                 <%
                	// 경차, 승합/화물차는 임직원한정운전특약 라인 안 보이도록 처리.
//                 	int sh_code_num = 0;
...
//             		if(sh_code_num == 0 || (sh_code_num>1999 && sh_code_num<7000) || (sh_code_num>1999999 && sh_code_num<7000000)) flag1 = true;
                ...
                <tr id=tr_com_emp_yn style="display:<%if(e_bean.getCom_emp_yn().equals("N") || !flag1 ){%>none<%}else{%>''<%}%>">
                  <td colspan='2' class=title><!-- 법인 -->임직원 한정운전특약</td>
                    <td>&nbsp;<select name="com_emp_yn" id="com_emp_yn_1" onchange="javascript:set_disable_value();">
                  ...
                  <option value="Y" <%if(e_bean.getCom_emp_yn().equals("Y")){%>selected<%}%>>가입</option>
                   <%}%>
                    ...
                    <td class='esti_target2'>&nbsp;<select name="com_emp_yn" id="com_emp_yn_2">
                    <%-- <%if (e_bean.getDoc_type().equals("2")) {%> --%>
                    <%if (e_bean.getDoc_type().equals("2") && flag1 ) {%>
                  ...
                  <option value="Y" <%if(e_bean2.getCom_emp_yn().equals("Y")){%>selected<%}%>>가입</option>
                   <%}%>
                    ...
                    <td class='esti_target esti_target2'>&nbsp;<select name="com_emp_yn" id="com_emp_yn_3">
                    <%-- <%if (e_bean.getDoc_type().equals("2")) {%> --%>
                    <%if (e_bean.getDoc_type().equals("2") && flag1 ) {%>
	                 	<option value="" <%if (e_bean.getCom_emp_yn().equals("")) {%>selected<%}%>>선택</option>
                   <%}%>
                  ...
                  <option value="Y" <%if(e_bean.getCom_emp_yn().equals("Y")){%>selected<%}%>>가입</option>
                   <%}%>
                    ...
                    <td class='esti_target esti_target2'>&nbsp;<select name="com_emp_yn" id="com_emp_yn_4">
                    <%-- <%if (e_bean.getDoc_type().equals("2")) {%> --%>
                    <%if (e_bean.getDoc_type().equals("2") && flag1 ) {%>
	                 	<option value="" <%if (e_bean.getCom_emp_yn().equals("")) {%>selected<%}%>>선택</option>
                   <%}%>
                ...
                <tr id="tr_tint_s_yn" <%if (e_bean.getCar_comp_id().equals("0056") || e_bean.getCar_comp_id().equals("0057") || (jg_code_num > 9017300 && jg_code_num < 9018200 ) ) {%>style="display:none;"<%}%>> 
                    ...
                    <td><%-- &nbsp;
                      ...
                      <select name="setTint_ps_sel" id="setTint_ps_sel_1" onchange="javascript:setTint_ps('0', this.value);set_disable_value();">
                    ...
                    <td class='esti_target2'><%-- &nbsp;
                     ...
                     <span id="tint_sn_yn_div_2" <%if(car_comp_id_num > 5) {%>style="display:none;"<%} %>>
                      ...
                      <span id="tint_bn_yn_div_2" <%if(car_comp_id_num > 5) {%>style="display:none;"<%} %>>
	                      	<br>
	                      	&nbsp;
	                      <label><input type="checkbox" name="tint_bn_yn" id="tint_bn_yn_2" value="Y" <%if(e_bean2.getTint_bn_yn().equals("Y")){%>checked<%}%>> 블랙박스 미제공할인(빌트인캠,고객장착)</label>
                      </span>
                      <br>
                      &nbsp;
...
<!--                       		<option value="2">대전/대구/광주/부산</option> -->
                    ...
                    <td class='esti_target esti_target2'><%-- &nbsp;
                      ...
                      <span id="tint_bn_yn_div_3" <%if(car_comp_id_num > 5) {%>style="display:none;"<%} %>>
                      	<br>
                      	&nbsp;
	                      <label><input type="checkbox" name="tint_bn_yn" id="tint_bn_yn_3" value="Y" <%if(e_bean.getTint_bn_yn().equals("Y")){%>checked<%}%>> 블랙박스 미제공할인(빌트인캠,고객장착)</label>
                      </span>
	                      <br>
	                      &nbsp;
...
<!--                      		<option value="2">대전/대구/광주/부산</option> -->
                    ...
                    <td class='esti_target esti_target2'><%-- &nbsp;
                      ...
                      <span id="tint_bn_yn_div_4" <%if(car_comp_id_num > 5) {%>style="display:none;"<%} %>>
	                      	<br>
	                      	&nbsp;
	                      <label><input type="checkbox" name="tint_bn_yn" id="tint_bn_yn_4" value="Y" <%if(e_bean.getTint_bn_yn().equals("Y")){%>checked<%}%>> 블랙박스 미제공할인(빌트인캠,고객장착)</label>
                      </span>
                      <br>
                      &nbsp;
...
<%-- 				<%if (!(ej_bean.getSh_code().equals("9133") || ej_bean.getSh_code().equals("9237") || ej_bean.getSh_code().equals("3871") || ej_bean.getSh_code().equals("9015435") || ej_bean.getSh_code().equals("9025435") || ej_bean.getSh_code().equals("9015436") || ej_bean.getSh_code().equals("9015437") || ej_bean.getSh_code().equals("9025439") || ej_bean.getSh_code().equals("9025440") )) { // 전기화물차 외 전기차%> --%>
                    	...
                    	<select name="ecar_loc_st" id="ecar_loc_st_2" onChange="javascript:setLoc_st('1', this.value);">
							<option value="" <%if (!(e_bean2.getEcar_loc_st().equals("4") || e_bean2.getEcar_loc_st().equals("5"))) {%>selected<%}%>>선택</option>
                   	  	<%for (int i = 0; i < code34_size; i++) {
                           	CodeBean code = code34[i];%>
                        	<option value='<%= code.getNm_cd()%>'
                        		<%if ( code.getNm_cd().equals(e_bean2.getEcar_loc_st()) ) {%>selected<%}%>  
...
<%--                         		<%if ( !( code.getNm_cd().equals("1") || code.getNm_cd().equals("4") || code.getNm_cd().equals("5")) ) {%>disabled style="background-color: #E5E5E5;"<%}%> --%>
                ...
                <tr id=tr_hcar_loc style="display:<%if(ej_bean.getJg_g_7().equals("4")){%>''<%}else{%>none<%}%>"> 
                    ...
                    <td>&nbsp;<select name="hcar_loc_st" id="hcar_loc_st_1" onChange="javascript:setLoc_st('0',this.value);  set_other_esti_value(this);">
                    ...
                    <td class='esti_target2'>&nbsp;<select name="hcar_loc_st" id="hcar_loc_st_2" onChange="javascript:setLoc_st('1',this.value);">
                    ...
                    <td class='esti_target esti_target2'>&nbsp;<select name="hcar_loc_st" id="hcar_loc_st_3" onChange="javascript:setLoc_st('2',this.value);">
                    	  <option value="" <%if(e_bean.getHcar_loc_st().equals(""))%>selected<%%>>선택</option>
                    	  <%for(int i = 0 ; i < code37_size ; i++){
                            CodeBean code = code37[i];%>
                        <option value='<%= code.getNm_cd()%>' <%if(e_bean.getHcar_loc_st().equals(code.getNm_cd())){%>selected<%}%>><%= code.getNm()%></option>
                    ...
                    <td class='esti_target esti_target2'>&nbsp;<select name="hcar_loc_st" id="hcar_loc_st_4" onChange="javascript:setLoc_st('3',this.value);">
                    	  <option value="" <%if(e_bean.getHcar_loc_st().equals(""))%>selected<%%>>선택</option>
                    	  <%for(int i = 0 ; i < code37_size ; i++){
                            CodeBean code = code37[i];%>
                        <option value='<%= code.getNm_cd()%>' <%if(e_bean.getHcar_loc_st().equals(code.getNm_cd())){%>selected<%}%>><%= code.getNm()%></option>
                ...
                <%-- <tr id=tr_eco_etag style="display:<%if(ej_bean.getJg_g_7().equals("1")||ej_bean.getJg_g_7().equals("2")||ej_bean.getJg_g_7().equals("5")){%>''<%}else{%>none<%}%>">
		        	<td colspan='2' class=title>맑은서울스티커 발급<br>(남산터널 이용 전자태그)</td>
                    <td>
                    	&nbsp;
                    	...
                    	<%if (ej_bean.getJg_g_7().equals("3") || ej_bean.getJg_g_7().equals("4")) {%>
                   		...
                   		<%}%>		
                      	</select> <textarea class="white_sizeFix" cols="26" rows="2">하이브리드차량 맑은서울&#13;&#10;발급시 서울등록/대여료상승</textarea></td>
                    <td>
                    	&nbsp;
                ...
                <tr> 
                    ...
                    <td>&nbsp;<select name="loc_st" id="loc_st_1" onchange="javascript:set_disable_value();" <%if (e_bean.getCar_comp_id().equals("0056")) {%>disabled<%} else {%><%}%>>
                        ...
                        <%}%>
                      </select> <%-- <span id="loc_st_cmt_1" style="display:  <%if (e_bean.getCar_comp_id().equals("0056")) {%>''<%} else {%>none<%}%>; font-size: 10px; letter-spacing: 0px;">서울강서서비스센터에서 교육 후 인도</span> --%></td>
                    <td class='esti_target2'>&nbsp;<select name="loc_st" id="loc_st_2" <%if (e_bean.getCar_comp_id().equals("0056")) {%>disabled<%} else {%><%}%>>
                    	  <%for(int i = 0 ; i < code33_size ; i++){
                            CodeBean code = code33[i];%>
                        <option value='<%= code.getNm_cd()%>' <%if(e_bean2.getLoc_st().equals(code.getNm_cd())){%>selected<%}%>><%= code.getNm()%></option>
                        <%}%>
                      </select> <%-- <span id="loc_st_cmt_2" style="display:  <%if (e_bean.getCar_comp_id().equals("0056")) {%>''<%} else {%>none<%}%>; font-size: 10px; letter-spacing: 0px;">서울강서서비스센터에서 교육 후 인도</span> --%></td>
                    <td class='esti_target esti_target2'>&nbsp;<select name="loc_st" id="loc_st_3" <%if (e_bean.getCar_comp_id().equals("0056")) {%>disabled<%} else {%><%}%>>
                    	  <%for(int i = 0 ; i < code33_size ; i++){
                            CodeBean code = code33[i];%>
                        <option value='<%= code.getNm_cd()%>' <%if(e_bean.getLoc_st().equals(code.getNm_cd())){%>selected<%}%>><%= code.getNm()%></option>
                        <%}%>
                      </select> <%-- <span id="loc_st_cmt_3" style="display:  <%if (e_bean.getCar_comp_id().equals("0056")) {%>''<%} else {%>none<%}%>; font-size: 10px; letter-spacing: 0px;">서울강서서비스센터에서 교육 후 인도</span> --%></td>
                    <td class='esti_target esti_target2'>&nbsp;<select name="loc_st" id="loc_st_4" <%if (e_bean.getCar_comp_id().equals("0056")) {%>disabled<%} else {%><%}%>>
                    	  <%for(int i = 0 ; i < code33_size ; i++){
                            CodeBean code = code33[i];%>
                        <option value='<%= code.getNm_cd()%>' <%if(e_bean.getLoc_st().equals(code.getNm_cd())){%>selected<%}%>><%= code.getNm()%></option>
                        <%}%>
                      </select> <%-- <span id="loc_st_cmt_4" style="display:  <%if (e_bean.getCar_comp_id().equals("0056")) {%>''<%} else {%>none<%}%>; font-size: 10px; letter-spacing: 0px;">서울강서서비스센터에서 교육 후 인도</span> --%></td>
                </tr> 
                ...
                <tr> 
			        ...
			        <td>&nbsp;
                    	<select id="sel_o_11_1" onchange="javascript:setO_11('1',this.value);set_return2_change();dir_pur_change();">
							...
							<option value="directInput"<%if(e_bean.getO_11()!=0 && e_bean.getO_11()!=1.0 && e_bean.getO_11()!=2.0 && e_bean.getO_11()!=3.0){%> selected<%}%>>직접입력</option></select>
                    	<input type="text" name="o_11" class="num" maxlength="3" size="3" id="o_11_1" value="<%=e_bean.getO_11()%>" onBlur='javascript:set_return2_change();dir_pur_change();'>&nbsp;%<br>&nbsp;
                    </td>
                    <td class='esti_target2'>&nbsp;
                    	<select id="sel_o_11_2" onchange="javascript:setO_11('2',this.value);dir_pur_change();">
							...
							<option value="directInput"<%if(e_bean2.getO_11()!=0 && e_bean2.getO_11()!=1.0 && e_bean2.getO_11()!=2.0 && e_bean2.getO_11()!=3.0){%> selected<%}%>>직접입력</option></select>
                    	<input type="text" name="o_11" class="num" maxlength="3" size="3" id="o_11_2" value="<%=e_bean2.getO_11()%>" onBlur='javascript:dir_pur_change();'>&nbsp;%<br>&nbsp;
                    </td>
                    <td class='esti_target esti_target2'>&nbsp;
                    	<select id="sel_o_11_3" onchange="javascript:setO_11('3',this.value);dir_pur_change();">
							...
							<option value="directInput"<%if(e_bean3.getO_11()!=0 && e_bean3.getO_11()!=1.0 && e_bean3.getO_11()!=2.0 && e_bean3.getO_11()!=3.0){%> selected<%}%>>직접입력</option></select>
                    	<input type="text" name="o_11" class="num" maxlength="3" size="3" id="o_11_3" value="<%=e_bean3.getO_11()%>" onBlur='javascript:dir_pur_change();'>&nbsp;%<br>&nbsp;
                    </td>
                    <td class='esti_target esti_target2'>&nbsp;
                    	<select id="sel_o_11_4" onchange="javascript:setO_11('4',this.value);dir_pur_change();">
							...
							<option value="directInput"<%if(e_bean4.getO_11()!=0 && e_bean4.getO_11()!=1.0 && e_bean4.getO_11()!=2.0 && e_bean4.getO_11()!=3.0){%> selected<%}%>>직접입력</option></select>
                    	<input type="text" name="o_11" class="num" maxlength="3" size="3" id="o_11_4" value="<%=e_bean4.getO_11()%>" onBlur='javascript:dir_pur_change();'>&nbsp;%<br>&nbsp;
                    </td>
                </tr> 											
                <tr> 
    ...
    <tr> 
        <td colspan="2"><font color="#666666">* 영업수당 : 3% 이내에서 가감 가능</font> </td>
    </tr>	
    <tr>
        <td class=h></td>
    </tr>
...
<!--
	...
	<%if(e_bean.getPrint_type().equals("2") || e_bean.getPrint_type().equals("3") || e_bean.getPrint_type().equals("4") || e_bean.getPrint_type().equals("5") || e_bean.getPrint_type().equals("6")){%>
