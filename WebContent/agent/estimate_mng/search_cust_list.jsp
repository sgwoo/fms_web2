...
<%
	...
	if(!t_wd.equals("")){
		vars = e_db.getCustSubList(t_wd, ck_acar_id);
	}
...
<!--
	...
	function setCode(st, nm, ssn, tel, fax, reg_dt, doc_type, est_email, spr_kd, spr_kd_rent_dt){	
		...
		<%if(from_page.equals("/agent/secondhand/secondhand_price_20090901.jsp")){%>			
			fm.cust_nm.value 	= nm;			
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
    ...
    <tr> 
        <td><img src=/acar/images/center/icon_arrow.gif align=absmiddle> <span class=style2>신차견적고객 (최대10라인)</span></td>
    </tr>
    <tr> 
        <td class=line2></td>
    </tr>
	...
	<%	if(!t_wd.equals("")){
				...
				if(from_page.equals("/agent/secondhand/secondhand_price_20090901.jsp")){
					//재리스고객
					vars = e_db.getLcCustSprSubList("0", t_wd, ck_acar_id);
				}else{
					//신차고객
					vars = e_db.getLcCustSprSubList("1", t_wd, ck_acar_id);
				}
		        */
				vars = e_db.getLcCustSubList(t_wd, ck_acar_id);
			}
			size = vars.size();%>
    		    <%	if(size >5) size = 5; %>	
    <tr> 
