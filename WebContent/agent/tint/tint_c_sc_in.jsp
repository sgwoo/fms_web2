...
<form name='form1' method='post'>
  ...
  <input type='hidden' name='from_page' value='/agent/tint/tint_c_frame.jsp'>
	...
	<tr id='tr_title' style='position:relative;z-index:1'>
	  <td class='line' width='20%' id='td_title' style='position:relative;'>
		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
		  <tr>
		    <td width='14%' class='title' style='height:45'>연번</td>			
	  ...
	  <td class='line' width='80%'>
		<table border="0" cellspacing="1" cellpadding="0" width='100%'>
		  <tr>
			...
			<td colspan="6" class='title'>용품청구</td>
			<td rowspan="2" width="13%" class='title'>고객</td>
		  </tr>
		  <tr>
			<td width='13%' class='title'>차명</td>				  
			<td width='14%' class='title'>차대번호</td>				  				  			  				  
			<td width='8%' class='title'>색상</td>	
			<td width='12%' class='title'>작업마감일시</td>				
			<td width='8%' class='title'>썬팅비</td>	
			<td width='8%' class='title'>청소용품비</td>					  
			<td width='8%' class='title'>네비게이션</td>					  
			<td width='8%' class='title'>기타금액</td>			
			<td width='8%' class='title'>청구금액</td>							  
		  </tr>
		</table>
	  </td>
	</tr>
...
<%		for(int i = 0 ; i < vt_size ; i++){
			Hashtable ht = (Hashtable)vt.elementAt(i);%>			
				<tr>
					...
					<%}%>
					<td  width='12%' align='center'><font color=red><%=AddUtil.ChangeDate3(String.valueOf(ht.get("SUP_DT")))%></font></td>
					<td  width='8%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("TINT_AMT")))%></td>							
					<td  width='8%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("CLEANER_AMT")))%></td>
					<td  width='8%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("NAVI_AMT")))%></td>
					<td  width='8%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("OTHER_AMT")))%></td>
					<td  width='8%' align='right'><%=AddUtil.parseDecimal(String.valueOf(ht.get("TOT_AMT")))%></td>
					<td  width='13%' align='center'><span title='<%=ht.get("FIRM_NM")%>'><%=Util.subData(String.valueOf(ht.get("FIRM_NM")), 9)%></span></td>
				</tr>
<%
			total_amt1 	= total_amt1 + Long.parseLong(String.valueOf(ht.get("TINT_AMT")));
			total_amt2 	= total_amt2 + Long.parseLong(String.valueOf(ht.get("CLEANER_AMT")));
			total_amt3 	= total_amt3 + Long.parseLong(String.valueOf(ht.get("NAVI_AMT")));
