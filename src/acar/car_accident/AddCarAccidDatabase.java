...
public class AddCarAccidDatabase{
    ...
    */    
    ...
    public AccidentBean [] getAccidentAll(String br_id, String gubun1, String gubun2, String st_dt, String end_dt, String s_kd, String t_wd) throws DatabaseException, DataSourceEmptyException{
		...
		if(s_kd.equals("1"))		subQuery += " and nvl(b.firm_nm, '') like '%"+t_wd+"%'\n";			
		else if(s_kd.equals("2"))	subQuery += " and upper(b.rent_l_cd) like upper('%"+t_wd+"%')\n";
		else if(s_kd.equals("3"))	subQuery += " and nvl(substr(a.accid_dt,1,8), '') like '%"+t_wd+"%'\n";
		else if(s_kd.equals("4"))	subQuery += " and nvl(a.our_car_nm, '') like '%"+t_wd+"%'\n";
		else if(s_kd.equals("5"))	subQuery += " and nvl(c.car_no, '') like '%"+t_wd+"%'\n";
		else if(s_kd.equals("6"))	subQuery += " and nvl(a.our_driver, '') like '%"+t_wd+"%'\n";
		else if(s_kd.equals("7"))	subQuery += " and nvl(a.ot_car_no, '') like '%"+t_wd+"%'\n";
		else if(s_kd.equals("8"))	subQuery += " and nvl(a.ot_driver, '') like '%"+t_wd+"%'\n";
		else if(s_kd.equals("9"))	subQuery += " and b.mng_id = '"+t_wd+"'\n";
		else if(s_kd.equals("10"))	subQuery += " and nvl(a.our_ins, '') like '%"+t_wd+"%'\n";
    ...
    public AccidentBean [] getCarRentListAll( String gubun, String rent_l_cd, String firm_nm, String car_no ) throws DatabaseException, DataSourceEmptyException{
        ...
        ResultSet rs = null;
       	String query =	" select /*+  merge(a) */ "+
						...
						" from   cont_n_view a , car_reg c,  car_etc g, car_nm cn , cls_cont cc \n"+  
						...
						"        and c.car_no like '%" + car_no + "%' and a.firm_nm like '%" + firm_nm + "%'";
	...
	public RentListBean getCont_View(String m_id, String l_cd) throws DatabaseException, DataSourceEmptyException{
		...
		RentListBean bean = new RentListBean();
       	...
       	String query =  " select /*+  merge(a) */ "+
						...
						"        a.firm_nm, a.client_nm, c.car_no, c.init_reg_dt, a.mng_id, a.mng_id2, a.r_site, c.car_nm, cn.car_name"+
    ...
    public AccidentBean getAccidentBean(String car_mng_id, String accid_id) throws DatabaseException, DataSourceEmptyException{
        ...
        query = " select"+
				...
				" from accident a\n"+//, my_accid b
				" where "+//a.car_mng_id=b.car_mng_id(+) and a.accid_id=b.accid_id(+) and
    ...
    public AccidentBean [] getCarAccidList(String car_mng_id) throws DatabaseException, DataSourceEmptyException{
        ...
        ResultSet rs = null;
       	String query =	" select /*+  merge(a) */ "+
						...
						"        b.our_driver, b.accid_dt, b.accid_addr, b.accid_cont, b.sub_etc, b.accid_cont2,"+
    ...
    public AccidentBean getCarAccidCase(String car_mng_id, String accid_id) throws DatabaseException, DataSourceEmptyException{
        ...
        PreparedStatement pstmt = null;
        ResultSet rs = null;
       	String query =	" select decode(accid_st, '1','피해', '2','가해', '3','쌍방', '8','단독','6','수해', ' ') accid_st,"+
    ...
    public int updateAccident(AccidentBean bean) throws DatabaseException, DataSourceEmptyException{
        ...
        query = " update accident set"+
				...
				" IMP_FAULT_SUB=?, OUR_FAULT_PER=?, OT_POL_ST=?, OT_POL_NUM=?, OT_POL_FAX=?, MEMO=? "+//83
				" where car_mng_id=? and accid_id=?\n";
       ...
       try{
			...
			//추가end		
          	pstmt.setString(84, bean.getCar_mng_id().trim());
			pstmt.setString(85, bean.getAccid_id().trim());
	...
	/**
	*	면책금/휴차료/대차료 마지막 메모 조회
	*/
	public String getMaxMemo(String m_id, String l_cd, String tm_st, String accid_id, String serv_id) throws DatabaseException, DataSourceEmptyException{
		...
		if(!accid_id.equals("")) where = " and accid_id='"+accid_id+"'";
		if(!serv_id.equals("")) where = " and serv_id='"+serv_id+"'";

		query = " select substr(a.reg_dt,1,4)||'-'||substr(a.reg_dt,5,2)||'-'||substr(a.reg_dt,7,2)||'  ['||b.user_nm||' --> '||a.speaker|| ']  '||a.content"+
				" from tel_mm a, users b"+
				" where rent_mng_id='"+m_id+"' and rent_l_cd='"+l_cd+"' and tm_st='"+tm_st+"'"+where+
				" and a.reg_dt||a.reg_dt_time = (select max(reg_dt||reg_dt_time) max_dt from tel_mm where rent_mng_id='"+m_id+"' and rent_l_cd='"+l_cd+"' and tm_st='"+tm_st+"'"+where+")"+
		...
		try{
			...
			rs = pstmt.executeQuery();
			while(rs.next()){
				memo = rs.getString(1)==null?"":rs.getString(1);
			}
			rs.close();
            pstmt.close();
		}catch(SQLException e){
			try{
				System.out.println("[AddCarAccidDatabase:getMaxMemo]"+e);
				e.printStackTrace();			
				con.rollback();
			}catch(SQLException _ignored){}
              throw new DatabaseException("exception");
	...
	*/
	...
	/**	
	 *	면책금/휴차료/대차료 메모 deleteInsMemo
	 */
	public boolean deleteInsMemo(String m_id, String l_cd, String c_id, String tm_st, String seq) throws DatabaseException, DataSourceEmptyException{
        Connection con = connMgr.getConnection(DATA_SOURCE);
        if(con == null)
            throw new DataSourceEmptyException("Can't get Connection !!");
		...
		String query =  " delete from tel_mm where RENT_MNG_ID = '"+m_id+"' and RENT_L_CD = '"+l_cd+"' and  CAR_MNG_ID ='"+c_id+"' and TM_ST = '"+tm_st+"' and SEQ = '"+seq+"' ";
		...
		try {
            ...
            try{
				...
				System.out.println("[AddCarAccidDatabase:deleteInsMemo]\n"+e);
				System.out.println("[AddCarAccidDatabase:deleteInsMemo]\n"+query);
                con.rollback();
            }catch(SQLException _ignored){}
              throw new DatabaseException("exception");
