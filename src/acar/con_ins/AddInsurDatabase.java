...
public class AddInsurDatabase
{
	...
	public Vector getInsPayList(String f_list, String br_id, String gubun1, String gubun2, String gubun3, String gubun4, String st_dt, String end_dt, String s_kd, String t_wd, String sort_gubun, String asc, String gubun)
	{
		getConnection();
		...
		if(f_list.equals("now")){		
				...
				query = " select "+
					"        a.car_mng_id, a.ins_st, a.ins_sts, a.car_use, b.rent_mng_id, b.rent_l_cd, b.client_id, b.firm_nm, c.car_no, c.first_car_no, c.car_nm, cn.car_name, b.use_yn,"+
					"        (decode(a.air_ds_yn,'Y',1,0)+decode(a.air_as_yn,'Y',1,0)) as air,"+
					"        decode(a.age_scp,'1','21세이상','2','26세이상','3','모든운전자') as age_scp,"+
					"        a.ins_com_id, d.ins_com_nm, e.ins_tm, decode(e.ins_tm2,'2',-e.pay_amt,e.pay_amt) pay_amt, e.pay_yn,"+
					...
					"        nvl(h.exp_dt,a.ins_exp_dt) exp_dt, h.req_dt as cls_req_dt, a.ins_con_no "+
					" from   insur a, cont_n_view b, ins_com d, scd_ins e, ins_cls h, car_reg c, car_etc g, car_nm cn \n"+
					" where  a.car_mng_id=e.car_mng_id and a.ins_st=e.ins_st"+
					"        and a.ins_com_id=d.ins_com_id"+
					"        and a.car_mng_id=b.car_mng_id and b.reg_dt||b.rent_l_cd=(select max(reg_dt||rent_l_cd) from cont where rent_l_cd not like 'RM%' and car_mng_id=a.car_mng_id)"+
		...
		//만료된 보험 갱신 대상 리스트(스케줄 내용 필요없다)
			...
			query = " select  "+
					"        DISTINCT a.car_mng_id, a.ins_st, a.ins_sts, a.car_use, b.rent_mng_id, b.rent_l_cd, b.client_id, b.firm_nm, c.car_no, c.first_car_no, c.car_nm, cn.car_name, b.use_yn,"+
					"        (decode(a.air_ds_yn,'Y',1,0)+decode(a.air_as_yn,'Y',1,0)) as air,"+
					"        decode(a.age_scp,'1','21세이상','2','26세이상','3','모든운전자') as age_scp,"+
					...
					"        DECODE(a.ins_exp_dt,'','',SUBSTR(a.ins_exp_dt,1,4)||'-'||SUBSTR(a.ins_exp_dt,5,2)||'-'||SUBSTR(a.ins_exp_dt,7,2)) as ins_exp_dt,"+
					"        nvl(h.exp_dt,a.ins_exp_dt) exp_dt, h.req_dt as cls_req_dt, a.ins_con_no "+
					" from   insur a, cont_n_view b, ins_com d, scd_ins e, "+
					"        (select car_mng_id, max(ins_st) ins_st from insur group by car_mng_id) g, ins_cls h ,   car_reg c,  car_etc ce, car_nm cn \n"+
					" where  a.car_mng_id=e.car_mng_id(+) and a.ins_st=e.ins_st(+)"+
					"        and a.ins_com_id=d.ins_com_id"+
					"        and a.car_mng_id=b.car_mng_id and b.reg_dt||b.rent_l_cd=(select max(reg_dt||rent_l_cd) from cont where rent_l_cd not like 'RM%' and car_mng_id=a.car_mng_id)"+
					"        and nvl(a.enable_renew,'Y') = 'Y'"+
					"        and a.car_mng_id=g.car_mng_id and a.ins_st=g.ins_st"+
					"        and a.car_mng_id=h.car_mng_id(+) and a.ins_st=h.ins_st(+) "+
					"	     and a.car_mng_id = c.car_mng_id and b.rent_mng_id = ce.rent_mng_id and b.rent_l_cd = ce.rent_l_cd \n"+
			...
			query = " select  "+
					"        DISTINCT a.car_mng_id, a.ins_st, a.ins_sts, a.car_use, b.rent_mng_id, b.rent_l_cd, b.client_id, b.firm_nm, c.car_no, c.first_car_no, cn.car_name as car_nm, b.use_yn,"+
					"        (decode(a.air_ds_yn,'Y',1,0)+decode(a.air_as_yn,'Y',1,0)) as air,"+
					"        decode(a.age_scp,'1','21세이상','2','26세이상','3','모든운전자') as age_scp,"+
					"        a.ins_com_id, d.ins_com_nm, '' as ins_tm, 0 as pay_amt, '' as pay_yn, '' as ins_est_dt, '' as pay_dt,"+
					"        DECODE(a.ins_start_dt,'','',SUBSTR(a.ins_start_dt,1,4)||'-'||SUBSTR(a.ins_start_dt,5,2)||'-'||SUBSTR(a.ins_start_dt,7,2)) as ins_start_dt,"+
		...
		else if(s_kd.equals("14"))	query += " and a.car_use='1'\n";
		else if(s_kd.equals("15"))	query += " and a.car_use<>'1'\n";
		...
		else if(sort_gubun.equals("6"))	query += " order by b.use_yn desc, a.ins_exp_dt "+sort+", b.firm_nm";		
	...
	public Vector getInsPayStat(String f_list, String br_id, String gubun1, String gubun2, String gubun3, String gubun4, String st_dt, String end_dt, String s_kd, String t_wd)
	{
		...
		if(f_list.equals("now")){		
			...
			sub_query = " select /*+  merge(b) */ "+
						"        a.car_mng_id, a.ins_st, b.rent_mng_id, b.rent_l_cd, b.client_id, b.firm_nm, c.car_no, c.car_nm, b.use_yn,"+
						"        (decode(a.air_ds_yn,'Y',1,0)+decode(a.air_as_yn,'Y',1,0)) as air,"+
						"        decode(a.age_scp,'1','21세이상','2','26세이상','3','모든운전자') as age_scp,"+
	...
	public InsurBean getIns(String c_id, String ins_st)
	{
		...
		query = " select /*+  merge(C) */ \n"+
				...
				" where  I.car_mng_id = C.car_mng_id and I.car_mng_id = cr.car_mng_id and \n"+
...
//			pstmt.setString(41, ins.getUse_yn());
			...
			pstmt.setString(70, ins.getCar_mng_id());
			pstmt.setString(71, ins.getIns_st());
	...
	public Vector getRentHistoryList(String c_id)
	{
		...
		query = " select /*+  merge(a) */ "+
				"        a.rent_l_cd, decode(a.use_yn,'Y','대여','해지') use_yn,"+
				...
				" where  a.car_mng_id='"+c_id+"' and a.rent_mng_id=b.rent_mng_id and a.rent_l_cd=b.rent_l_cd and b.rent_st='1'"+
	...
	public Vector getInsScds(String c_id, String ins_st)
	{
		...
		try {
			...
			while(rs.next())
			{
				...
				scd.setIncom_dt(rs.getString("INCOM_DT")==null?"":rs.getString("INCOM_DT"));
				scd.setIncom_seq(rs.getInt("INCOM_SEQ"));
	...
	public InsurBean getInsPay(String c_id, String ins_st, String ins_tm)
	{
		...
		query = " select /*+  merge(b) */ "+
				"        a.car_mng_id, a.ins_st, b.rent_l_cd, b.firm_nm, c.pay_amt, c.ins_est_dt, c.r_ins_est_dt"+
				" from   insur a, cont_n_view b, scd_ins c"+
				" where  a.car_mng_id=? and a.ins_st=? and c.ins_tm=? "+
				"        and a.car_mng_id=c.car_mng_id and a.ins_st=c.ins_st and a.car_mng_id=b.car_mng_id"+
				" ";
	...
	public boolean updateInsScd(InsurScdBean scd)
	{
		...
		try 
		{
			...
			pstmt.setString(5, scd.getIncom_dt());
			pstmt.setInt(6, scd.getIncom_seq());	
			
			pstmt.setString(7, scd.getCar_mng_id());
			pstmt.setString(8, scd.getIns_st());
			pstmt.setString(9, scd.getIns_tm());
	...
	//보험사 조회
	public Hashtable getInsCom(String ins_com_id, String ins_com_nm)
	{
		getConnection();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Hashtable ht = new Hashtable();
		String query = "";
		query = " select * from ins_com where 1=1 ";
		
		if(!ins_com_id.equals("") && ins_com_nm.equals(""))  query += " and ins_com_id='"+ins_com_id+"'";
		if(ins_com_id.equals("") && !ins_com_nm.equals(""))  query += " and ins_com_nm='"+ins_com_nm+"'";
		if(!ins_com_id.equals("") && !ins_com_nm.equals("")) query += " and (ins_com_id='"+ins_com_id+"' or ins_com_nm='"+ins_com_nm+"')";
		...
		try {
			...
			System.out.println("[AddInsurDatabase:getInsCom]"+ e);
	  		e.printStackTrace();
		} finally {
			try{
                if(rs != null )		rs.close();
	...
	public boolean insertInsCom(InsurComBean ins)
	{
		...
		String query = "insert into ins_com (INS_COM_ID, INS_COM_NM, CAR_RATE, INS_RATE, EXT_DATE,"+
						" AGNT_TEL, AGNT_FAX, AGNT_IMGN_TEL, ACC_TEL, ZIP, ADDR, INS_COM_F_NM, ZIP1, ADDR1)"+//
						" values("+
		...
		try 
		{
			...
			pstmt.setString(12, ins.getZip1			());
			pstmt.setString(13,ins.getAddr1			());
			pstmt.executeUpdate();
			pstmt.close();
	...
	public boolean updateInsCom(InsurComBean ins)
	{
		...
		try 
		{
			...
			pstmt.setString(12, ins.getZip1());
			pstmt.setString(13,ins.getAddr1());
			pstmt.setString(14,ins.getIns_com_id());
		    pstmt.executeUpdate();
			pstmt.close();
		   	conn.commit();
	...
	public Vector getInsStatList()
	{
		...
		query = " select /*+  merge(a) */ "+ 
				"        a.rent_l_cd, a.firm_nm, a.client_nm, cr.car_no, cr.car_nm, "+
				"        decode(b.ins_st,'0','신규','갱신') ins_st, d.ins_com_nm, "+
				"        b.ins_start_dt, b.ins_exp_dt, nvl(a.bus_id2,' ') bus_id2, nvl(a.mng_id,' ') mng_id"+ 
				...
				"        and b.ins_com_id=d.ins_com_id and a.car_mng_id = cr.car_mng_id \n"+
				" order by a.firm_nm";
	...
	//보험 납부 리스트 조회 : con_ins_sc_in.jsp
	public Vector getInsIncomPayList(String f_list, String br_id, String gubun1, String gubun2, String gubun3, String gubun4, String st_dt, String end_dt, String s_kd, String t_wd, String sort_gubun, String asc, String gubun)
	{
		getConnection();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		//만료되지 않은 현재 진행 보험(납부리스트)  - 
		if(f_list.equals("now")){	//입금원장에서 해지보험료 환급기 간혹 납부보험료를 포함해도 환불이 있는 경우 incom에서만 입금처리해주고 네오염은 별도 입력 -

			query = " select /*+  merge(b) */ "+
					"        a.car_mng_id, a.ins_st, a.ins_sts, b.rent_mng_id, b.rent_l_cd, b.client_id, b.firm_nm, c.car_no, c.first_car_no, c.car_nm, cn.car_name, b.use_yn,"+
					"        (decode(a.air_ds_yn,'Y',1,0)+decode(a.air_as_yn,'Y',1,0)) as air, a.car_use, "+
					"        decode(a.age_scp,'1','21세이상','2','26세이상','3','모든운전자') as age_scp,"+
					"        a.ins_com_id, d.ins_com_nm, e.ins_tm,  e.pay_amt, e.pay_yn, e.ins_tm2, decode(e.ins_tm2, '2', '해지', '1', '변경') ins_tm2_nm, "+
					"        DECODE(a.ins_start_dt,'','',SUBSTR(a.ins_start_dt,1,4)||'-'||SUBSTR(a.ins_start_dt,5,2)||'-'||SUBSTR(a.ins_start_dt,7,2)) as ins_start_dt,"+
					"        DECODE(a.ins_exp_dt,'','',SUBSTR(a.ins_exp_dt,1,4)||'-'||SUBSTR(a.ins_exp_dt,5,2)||'-'||SUBSTR(a.ins_exp_dt,7,2)) as ins_exp_dt,"+
					"        DECODE(e.ins_est_dt,'','',SUBSTR(e.ins_est_dt,1,4)||'-'||SUBSTR(e.ins_est_dt,5,2)||'-'||SUBSTR(e.ins_est_dt,7,2)) as ins_est_dt,"+
					"        DECODE(e.r_ins_est_dt,'','',SUBSTR(e.r_ins_est_dt,1,4)||'-'||SUBSTR(e.r_ins_est_dt,5,2)||'-'||SUBSTR(e.r_ins_est_dt,7,2)) as r_ins_est_dt,"+
		...
		//만료된 보험 갱신 대상 리스트(스케줄 내용 필요없다)
			...
			query = " select /*+  merge(b) */ "+
					"        DISTINCT a.car_mng_id, a.ins_st, a.ins_sts, b.rent_mng_id, b.rent_l_cd, b.client_id, b.firm_nm, c.car_no, c.first_car_no, c.car_nm, cn.car_name, b.use_yn,"+
					"        (decode(a.air_ds_yn,'Y',1,0)+decode(a.air_as_yn,'Y',1,0)) as air, a.car_use, "+
					"        decode(a.age_scp,'1','21세이상','2','26세이상','3','모든운전자') as age_scp,"+
					"        a.ins_com_id, d.ins_com_nm, '' as ins_tm, 0 as pay_amt, '' as pay_yn, '' as ins_est_dt, '' as pay_dt,"+
					"        DECODE(a.ins_start_dt,'','',SUBSTR(a.ins_start_dt,1,4)||'-'||SUBSTR(a.ins_start_dt,5,2)||'-'||SUBSTR(a.ins_start_dt,7,2)) as ins_start_dt,"+
					"        DECODE(a.ins_exp_dt,'','',SUBSTR(a.ins_exp_dt,1,4)||'-'||SUBSTR(a.ins_exp_dt,5,2)||'-'||SUBSTR(a.ins_exp_dt,7,2)) as ins_exp_dt"+
					" from   insur a, cont_n_view b, ins_com d, scd_ins e,  car_reg c,  car_etc ce, car_nm cn, \n"+
					"        (select max(rent_l_cd) rent_l_cd from cont where car_st<>'4' group by car_mng_id) f,"+
			...
			query = " select /*+  merge(b) */ "+
					"        DISTINCT a.car_mng_id, a.ins_st, a.ins_sts, b.rent_mng_id, b.rent_l_cd, b.client_id, b.firm_nm, c.car_no, c.first_car_no, cn.car_name as car_nm, b.use_yn,"+
					"        (decode(a.air_ds_yn,'Y',1,0)+decode(a.air_as_yn,'Y',1,0)) as air,"+
					"        decode(a.age_scp,'1','21세이상','2','26세이상','3','모든운전자') as age_scp,"+
					"        a.ins_com_id, d.ins_com_nm, '' as ins_tm, 0 as pay_amt, '' as pay_yn, '' as ins_est_dt, '' as pay_dt,"+
					"        DECODE(a.ins_start_dt,'','',SUBSTR(a.ins_start_dt,1,4)||'-'||SUBSTR(a.ins_start_dt,5,2)||'-'||SUBSTR(a.ins_start_dt,7,2)) as ins_start_dt,"+
					"        DECODE(a.ins_exp_dt,'','',SUBSTR(a.ins_exp_dt,1,4)||'-'||SUBSTR(a.ins_exp_dt,5,2)||'-'||SUBSTR(a.ins_exp_dt,7,2)) as ins_exp_dt"+
					" from   insur a, cont_n_view b, ins_com d, scd_ins e, (select max(rent_l_cd) rent_l_cd from cont where car_st<>'4' group by car_mng_id) f,"+
					"        (select car_mng_id, max(ins_st) ins_st from insur group by car_mng_id) g ,  car_reg c,  car_etc ce, car_nm cn \n"+
					" where  a.car_mng_id=e.car_mng_id(+) and a.ins_st=e.ins_st(+)"+
					"        and a.ins_com_id=d.ins_com_id"+
		...
		else if(s_kd.equals("4"))	query += " and (nvl(c.car_no, '') like '%"+t_wd+"%' or nvl(b.first_car_no, '') like '%"+t_wd+"%')\n";
		else if(s_kd.equals("6"))	query += " and upper(b.brch_id) like upper('%"+t_wd+"%')\n";
		else if(s_kd.equals("9"))	query += " and c.car_nm||cn.car_name like '%"+t_wd+"%'\n";
		else if(s_kd.equals("10"))	query += " and nvl(a.ins_start_dt, '') like '%"+t_wd+"%'\n";
		else if(s_kd.equals("11"))	query += " and nvl(a.ins_exp_dt, '') like '%"+t_wd+"%'\n";
		...
		query += " and nvl(e.ins_tm2,'0') in ('2', '1')";
