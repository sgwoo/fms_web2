...
public class InsurDatabase
{
	...
	public boolean insertInsScd(InsurScdBean scd)
	{
		getConnection();
		boolean flag = true;
		String query = "insert into scd_ins (CAR_MNG_ID, INS_TM, INS_ST, INS_EST_DT, PAY_AMT, PAY_YN, PAY_DT, CH_TM) values"+
						" (?, ?, ?, replace(?, '-', ''), ?, ?, replace(?, '-', ''), ?)";
		...
		try 
		{
		    ...
		    conn.commit();
	  		...
	  		System.out.println("[InsurDatabase:insertInsScd]"+e);
	  		e.printStackTrace();
	  		flag = false;
	  		conn.rollback();
		} finally {
	...
	public boolean dropInsScd(String car_mng_id, String ins_st, String ins_tm)
	{
		...
		try 
		{
			...
			conn.commit();
	  		...
	  		System.out.println("[InsurDatabase:dropInsScd]"+e);
	  		e.printStackTrace();
	  		flag = false;
			conn.rollback();
		} finally {
	...
	public boolean updateInsScd(InsurScdBean scd)
	{
		...
		try 
		{
			...
			pstmt.setString(5, scd.getCh_tm());
			pstmt.setString(6, scd.getCar_mng_id());
			pstmt.setString(7, scd.getIns_st());
			pstmt.setString(8, scd.getIns_tm());
		    pstmt.executeUpdate();
			pstmt.close();
		    conn.commit();
		    
	  	} catch (Exception e) {
	  		System.out.println("[InsurDatabase:updateInsScd]"+e);
	  		e.printStackTrace();
	  		flag = false;
	  		conn.rollback();
		} finally {
	...
	public InsurScdBean getInsScd(String car_mng_id, String ins_st, String ins_tm)
	{
		...
		try {
			...
			System.out.println("[InsurDatabase:getInsScd]"+e);
	  		e.printStackTrace();
		} finally {
			try{
		                if(rs != null )		rs.close();
	...
	public Vector getInsScds(String car_mng_id, String ins_st)
	{
		...
		try {
			...
			System.out.println("[InsurDatabase:getInsScds]"+e);
	  		e.printStackTrace();
		} finally {
			try{
		                if(rs != null )		rs.close();
	...
	public boolean insertIns(InsurBean ins)
	{
		...
		try 
		{
		    ...
		    conn.commit();
	  		...
	  		System.out.println("[InsurDatabase:insertIns]"+e);
	  		e.printStackTrace();
	  		flag = false;
	  		conn.rollback();
		} finally {
...
//			pstmt.setString(41, ins.getUse_yn());
		    ...
		    conn.commit();
	  		...
	  		System.out.println("[InsurDatabase:updateIns]"+e);
	  		e.printStackTrace();
	  		flag = false;
	  		conn.rollback();
		} finally {
	...
	public InsurBean getIns(String car_mng_id, String ins_st)
	{
		...
		query = "select /*+  merge(C) */ \n"+
				...
				" C.FIRM_NM, C.CLIENT_NM, cr.car_num, N.CAR_NAME,\n"+
				" C.RENT_L_CD, C.car_ja, cr.car_no, cr.car_nm, M.ins_com_nm\n"+
				" from insur I, cont_n_view C, car_reg cr, car_etc g,  INS_COM M, car_nm N, (select max(rent_l_cd) rent_l_cd from cont where car_st<>'4' group by car_mng_id) E\n"+
				...
				" and C.rent_mng_id = g.rent_mng_id(+)  and C.rent_l_cd = g.rent_l_cd(+)  \n"+
                       		" and g.car_id=N.car_id(+)  and    g.car_seq=N.car_seq(+) \n"+
		...
		try{
			...
			System.out.println("[InsurDatabase:getIns]"+e);
	  		e.printStackTrace();
		} finally {
			try{
		                if(rs != null )		rs.close();
	...
	public Vector getInsList(String gubun, String s_kd, String t_wd, String term)
	{
		...
		try {
			...
			System.out.println("[InsurDatabase:getInsList]"+e);
	  		e.printStackTrace();
		} finally {
			try{
		                if(rs != null )		rs.close();
...
*/
	public Vector getInsConStat(String s_kd, String t_wd)
	{
		getConnection();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			System.out.println("[InsurDatabase:getInsConStat]"+e);
	  		e.printStackTrace();
		} finally {
			try{
		                if(rs != null )		rs.close();
